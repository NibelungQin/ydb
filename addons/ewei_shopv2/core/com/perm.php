...
class Perm_EweiShopV2ComModel extends ComModel
{
    ...
    public function allPerms()
    {
        $redis = redis();
        $key = "allPerms";
        $allPerms_json = $redis->get($key);
        if(!empty($allPerms_json)){
            $allPerms = $perms = json_decode($allPerms_json,true);
            self::$allPerms = $perms;
        }
        if (empty($allPerms)) {
            $perms = array(
            ...
            self::$allPerms = $perms;
            $expire_time_week = 7*24*60*60;
            $redis->set($key,json_encode($perms),$expire_time_week);
        }
    ...
    protected function perm_app()
    {
        return $this->isopen("app") && $this->is_perm_plugin("app") ? array(
            ...
            "tabbar" => array("text" => "底部导航", "main" => "保存-log", "xxx" => array("submit" => "main")),
            "newrelease" => array("text" => "新版发布", "main" => "提交-log", "xxx" => array("audit" => "main")),
            "release" => array("text" => "旧版发布", "main" => "提交-log", "xxx" => array("audit" => "main")),
            "tmessage" => array(
                "text" => "模板消息",
                "main" => "查看",
                "add" => "添加-log",
    ...
    protected function perm_order()
    {
        return array(
            ...
            "batchsend" => array("text" => "批量发货", "main" => "批量发货-log", "xxx" => array("import" => "main")),
            "list" => array(
                "text" => "订单管理",
                "main" => "浏览全部订单",
                "status_1" => "浏览关闭订单",
    ...
    public function perm_dividend()
    {
        return $this->isopen("dividend") && $this->is_perm_plugin("dividend") ? array(
            ...
            "statistics" => array("order" => array("text" => "分红订单", "main" => "查看列表")),
            "apply" => array(
                "text" => "分红审核",
                "view1" => "待审核浏览",
                "view2" => "待打款浏览",
    ...
    public function perm_membercard()
    {
        return $this->isopen("membercard") && $this->is_perm_plugin("membercard") ? array(
            ...
            "statistics" => array("text" => "领取记录", "main" => "查看")
        ) : array();
    }
    ...
    private function check($permtype = "")
    {
        ...
        if ($_W["role"] == "vice_founder") {
            $vice_founder = pdo_fetchcolumn("SELECT COUNT(id)FROM " . tablename("uni_account_users") . "WHERE uid=:uid AND role=:role AND uniacid=:uniacid",
                array(":uid" => $uid, ":role" => "vice_founder", ":uniacid" => intval($_W["uniacid"])));
        ...
        $redis = Redis();
        $key = "ewei_shop_perm_user_".$uid."_".$_W["uniacid"];
        ...
        if(empty($user)){
            ...
            $expire_time_day = 24*60*60;
            $redis->set($key,json_encode($user),$expire_time_day);
        }
                                    ...
                                    foreach ($v as $kk => $vv) {
                                        if (!is_array($vv)) {
                                            if ($all) {
                                                if ($ke == "text") {
                                                    $text = str_replace("-log", "",
                                                        $value["text"] . "-" . $val["text"] . "-" . $v["text"]);
                                                } else {
                                                    $text = str_replace("-log", "",
                                                        $value["text"] . "-" . $val["text"] . "-" . $v["text"] . "-" . $vv);
                                                }
                                                $array[] = array(
                                                    "text" => $text,
