...
class Sms_EweiShopV2ComModel extends ComModel
{
	...
	public function send($mobile, $tplid, $data, $replace = true)
	{
		...
		if ($template['type'] == 'juhe') {
			...
			if (!empty($emayResult)) {
                ...
                else if ($template['type'] == 'tencent') {
                    include_once EWEI_SHOPV2_VENDOR . 'tencent/SmsSingleSender.php';
                    $ssender = new SmsSingleSender($smsset['tencent_key'], $smsset['tencent_secret']);
                    $result = $ssender->sendWithParam("86", $mobile,$template['smstplid'],$params,$template['smssign'], "", "");
                    $rsp = json_decode($result);
                    if (isset($rsp->result) && $rsp->result>0) {
                            return array('status' => 0, 'message' => '短信发送失败(错误信息: ' . $rsp->errmsg. ')');
                    }
		}
		else {
	...
	protected function sms_verify($tplid, $smsset)
	{
		...
		if ($template['type'] == 'juhe') {
			...
			if (empty($smsset['tencent'])) {
				return array('status' => 0, 'message' => '未开启腾讯云短信!');
			}

			if (empty($smsset['tencent_secret'])) {
				return array('status' => 0, 'message' => '未填写腾讯云短信SDK AppID!');
			}

			if (empty($smsset['tencent_key'])) {
				return array('status' => 0, 'message' => '未填写腾讯云短信keySecret!');
			}
			...
			if (empty($template['smssign'])) {
				return array('status' => 0, 'message' => '未填写腾讯云短信签名!');
			}
		}
		else {
