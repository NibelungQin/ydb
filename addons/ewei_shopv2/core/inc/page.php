...
class Page extends WeModuleSite
{
    ...
    public function template($filename = "", $type = TEMPLATE_INCLUDEPATH, $account = false)
    {
        ...
        if ((DevelopmentUtil::isProductionEnvironment() && ($_GPC["do"] == "web" || defined("IN_SYS")))
            || (DevelopmentUtil::isTestingEnvironment() && $_GET['__test_web__'])) {
        ...
        if ((DevelopmentUtil::isProductionEnvironment() && defined("IN_SYS"))
            || (DevelopmentUtil::isTestingEnvironment() && $_GET['__test_web__'])) {
            if (!$isv3) {
        ...
        if (!is_file($source)) {
            ExceptionUtil::exit("Error: template source '" . $filename . "' is not exist!", true);
        }
