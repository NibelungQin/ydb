...
class MobilePage extends Page 
{
	...
	public function footerMenus($diymenuid = NULL, $ismerch = false, $texts = array()) 
	{
		...
		else if ($controller == 'packagegoods')
		{
			include $this->template('packagegoods/_packagegoods_footer');
		}
		else if ($controller == 'advertisement')
		{
			include $this->template('advertisement/_advertisement_footer');
		}
	...
	public function diyPage($type) {
		...
		if (!(empty($diypagedata))) {
			...
			if (!(empty($diypageid))) {
				$page = p('diypage')->getFrontPage($diypageid, true);
