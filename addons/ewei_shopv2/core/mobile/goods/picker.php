...
class Goods_Picker_EweiShopV2MobilePage extends MobilePage
{
    public function main()
    {
        ...
        $goods = pdo_fetch('select id,thumb,title,marketprice,total,maxbuy,minbuy,unit,hasoption,showtotal,
       				diyformid,diyformtype,diyfields,isdiscount,presellprice,isdiscount_time,isdiscount_discounts,
       				hascommission,nocommission,commission,commission1_rate,marketprice,commission1_pay,needfollow,
       				followtip, followurl, `type`, isverify, maxprice, minprice, merchsale,ispresell,preselltimeend,
       				unite_total,threen,preselltimestart,presellovertime,presellover,islive,liveprice,minliveprice,
       				maxliveprice
        ...
        if (is_weixin()) {
            ...
            if (!empty($goods['needfollow']) && !$follow) {
                $followtip = empty($goods['followtip'])
                    ? '如果您想要购买此商品，需要您关注我们的公众号，点击【确定】关注后再来购买吧~' : $goods['followtip'];
        ...
        if (!empty($goods) && $goods['hasoption']) {
            ...
            foreach ($specs as &$spec) {
                $spec['items'] = pdo_fetchall('select * from ' . GoodsSpecItem::TABLE_NAME
                    . ' where specid=:specid and `show`=1 order by displayorder asc',
                    array(':specid' => $spec['id']));
            }

            unset($spec);
            $options = pdo_fetchall('select * from ' . GoodsOption::TABLE_NAME
                . ' where goodsid=:goodsid and uniacid=:uniacid order by displayorder asc',
                array(':goodsid' => $id, ':uniacid' => $_W['uniacid']));
        }
        ...
        } elseif ($goods['isdiscount'] && time() <= $goods['isdiscount_time']) {
        ...
        if (p('seckill') && !p('seckill')->getSeckill($goods['id'])) {
            if ($goods['nocommission'] == 1) {
                $seecommission = 0;
            } elseif ($goods['hascommission'] == 1 && $goods['nocommission'] == 0) {
                ...
                if ($clevel == 'false') {
                    ...
                    $goods_commission = !empty($goods['commission']) ? json_decode($goods['commission'],
                        true) : array();

                    if ($goods_commission['type'] == 0) {
                        $seecommission = (1 <= $set['level'])
                        ...
                        if (is_array($options) && !empty($options)) {
                            foreach ($options as $k => $v) {
                                $seecommission = 1 <= $set['level']
                                    ? (0 < $goods['commission1_rate']
                                        ? $goods['commission1_rate'] * $v['marketprice'] / 100
                                        : $v['commission1_pay'])
                                    : 0;
                            ...
                            foreach ($options as $k => $v) {
                                if ('option' . $v['id'] == $key) {
                                    if (strexists($value[0], '%')) {
                                        $options[$k]['seecommission'] =
                                            (float)(str_replace('%', '', $value[0]) / 100)
                                            * $v['marketprice'];
                                        continue;
                                    }
                ...
                foreach ($options as $ke => $vl) {
                    if ($clevel != 'false' && $clevel) {
                        $options[$ke]['seecommission'] = 1 <= $set['level']
                            ? round($clevel['commission1'] * $vl['marketprice'] / 100, 2)
                            : 0;
                    } else {
                        $options[$ke]['seecommission'] = 1 <= $set['level']
                            ? round($set['commission1'] * $vl['marketprice'] / 100, 2)
                            : 0;
                    }
                }
            } elseif ($clevel != 'false' && $clevel) {
                $seecommission = 1 <= $set['level'] ? round($clevel['commission1'] * $goods['marketprice'] / 100,
                    2) : 0;
            } else {
                $seecommission = 1 <= $set['level'] ? round($set['commission1'] * $goods['marketprice'] / 100,
                    2) : 0;
            }
        }
        ...
        if ($sale_plugin) {
            ...
            $gifts = pdo_fetchall('select id,goodsid,giftgoodsid,thumb,title
                ...
                . ' where uniacid = ' . $_W['uniacid'] . ' and activity = 2 and status = 1
                ...
                foreach ($gid as $ke => $val) {
                    if ($val == $id) {
                        ...
                        foreach ($giftgoods as $k => $val) {
                            $giftdata = pdo_fetch('select id,title,thumb,marketprice,total
									from ' . Goods::TABLE_NAME
                                . ' where uniacid = ' . $_W['uniacid'] . ' and deleted = 0 and total > 0
								 		and status = 2 and id = ' . $val . ' ');
        ...
        $set['texts'] = array(
            ...
            'commission_detail' => empty($set['texts']['commission_detail'])
                ...
                : ($set['texts']['commission_detail'] == '佣金明细'
                    ? '提现明细' : $set['texts']['commission_detail']),
