...
use Ydb\Entity\Manual\MemberFavorite;
...
class Member_Cart_EweiShopV2MobilePage extends MobileLoginPage
{
    ...
    public function get_list()
    {
        ...
        $sql = 'SELECT f.id,f.total,f.goodsid,g.total as stock,g.preselltimeend,g.presellprice as gpprice,
                    g.hasoption, o.stock as optionstock,g.presellprice,g.ispresell, g.maxbuy,g.title,g.thumb,
                    ifnull(o.marketprice, g.marketprice) as marketprice, g.productprice,o.title as optiontitle,
                    o.presellprice,f.optionid,o.specs,g.minbuy,g.maxbuy,g.unit,g.merchid,g.checked,
                    g.isdiscount_discounts,g.isdiscount,g.isdiscount_time,g.isnodiscount,g.discounts,g.merchsale,
                    f.selected,g.type,g.intervalfloor,g.intervalprice
                FROM ' . MemberCart::TABLE_NAME . ' f '
            . ' left join ' . Goods::TABLE_NAME . ' g on f.goodsid = g.id '
            . ' left join ' . GoodsOption::TABLE_NAME . ' o on f.optionid = o.id '
        ...
        foreach ($list as &$g) {
            ...
            if (0 < $g['ispresell'] && ($g['preselltimeend'] == 0 || time() < $g['preselltimeend'])) {
                $g['marketprice'] = 0 < (int)$g['hasoption'] ? $g['presellprice'] : $g['gpprice'];
            }
            ...
            if (!empty($g['optionid'])) {
                ...
                $optionData = pdo_fetch('SELECT * FROM ' . GoodsOption::TABLE_NAME
                    . ' WHERE uniacid = ' . (int)$_W['uniacid'] . ' AND id = ' . (int)$g['optionid']);
        ...
        if (empty($list)) {
            ...
            foreach ($list as $g) {
                ...
                $merch_user[$goodsmerchid]['goods_count'] = (int)$merch_user[$goodsmerchid]['goods_count'];
                $merch_user[$goodsmerchid]['goods_selected'] = (int)$merch_user[$goodsmerchid]['goods_selected'];
    ...
    public function select()
    {
        ...
        global $_GPC;
        $id = (int)$_GPC['id'];
        $select = (int)$_GPC['select'];

        if (!empty($id)) {
            $data = pdo_fetch('select id,goodsid,optionid, total from ' . MemberCart::TABLE_NAME
                . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1 ',
                array(':id' => $id, ':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));

            if (!empty($data)) {
                pdo_update('ewei_shop_member_cart', array('selected' => $select),
    ...
    public function update()
    {
        ...
        $goods = pdo_fetch('select id,maxbuy,minbuy,total,unit from ' . Goods::TABLE_NAME
            . ' where id=:id and uniacid=:uniacid and status=1 and deleted=0',
        ...
        pdo_update('ewei_shop_member_cart', array('total' => $goodstotal, 'optionid' => $optionid),
            array('id' => $id, 'uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
        $seckillinfo = plugin_run('seckill::getSeckill', $data['goodsid'], $data['optionid'],
            true, $_W['openid']);
        if ($seckillinfo && $seckillinfo['status'] == 0) {
            $g = array();
            $g['seckillmaxbuy'] = $seckillinfo['maxbuy'];
    ...
    public function add()
    {
        ...
        $total <= 0 && ($total = 1);
        $optionid = (int)$_GPC['optionid'];
        $goods = pdo_fetch('select id,marketprice,diyformid,diyformtype,diyfields, isverify, `type`,merchid,
                    cannotrefund,hasoption
        ...
        if (!empty($_W['shopset']['wap']['open']) && !empty($_W['shopset']['wap']['mustbind'])
        ...
        $gift = pdo_fetch('select * from ' . Gift::TABLE_NAME
            . ' where uniacid = ' . $_W['uniacid'] . ' and id = ' . $giftid . ' and starttime >= ' . time()
            . ' and endtime <= ' . time() . ' and status = 1 ');
        ...
        $data = pdo_fetch('select id,total,diyformid from ' . MemberCart::TABLE_NAME
            . ' where goodsid=:id and openid=:openid and   optionid=:optionid  and deleted=0
                    and  uniacid=:uniacid   limit 1',
            array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid'], ':optionid' => $optionid, ':id' => $id));
        ...
        $cartcount = pdo_fetchcolumn('select sum(total) from ' . MemberCart::TABLE_NAME
            . ' where openid=:openid and deleted=0 and uniacid=:uniacid limit 1',
            array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
    ...
    public function addwholesale()
    {
        ...
        if (!empty($_W['shopset']['wap']['open']) && !empty($_W['shopset']['wap']['mustbind'])
            && empty($member['mobileverify'])) {
            show_json(0,
                array('message' => '请先绑定手机', 'url' => mobileUrl('member/bind', null, true)));
        }
    ...
    private function _validateCartOverLimit()
    {
        global $_W;
        global $_GPC;
        $listCount = pdo_fetch('select count(*) sum from ' . MemberCart::TABLE_NAME
            . ' where uniacid = :uniacid and openid = :openid and deleted = 0',
            array('uniacid' => $_W['uniacid'], 'openid' => $_W['openid']));
    ...
    public function remove()
    {
        ...
        $sql = 'update ' . tablename('ewei_shop_member_cart')
            . ' set deleted=1 where uniacid=:uniacid and openid=:openid and id in (' . implode(',', $ids) . ')';
    ...
    public function tofavorite()
    {
        ...
        foreach ($ids as $id) {
            $goodsid = pdo_fetchcolumn('select goodsid from ' . MemberCart::TABLE_NAME
                . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1 ',
                array(':id' => $id, ':uniacid' => $uniacid, ':openid' => $openid));

            if (!empty($goodsid)) {
                $fav = pdo_fetchcolumn('select count(*) from ' . MemberFavorite::TABLE_NAME
                    . ' where goodsid=:goodsid and uniacid=:uniacid and openid=:openid and deleted=0 limit 1 ',
                    array(':goodsid' => $goodsid, ':uniacid' => $uniacid, ':openid' => $openid));
    ...
    public function submit()
    {
        ...
        $params = array(':uniacid' => $uniacid, ':openid' => $openid);
        $sql = 'SELECT f.id,f.total,f.goodsid,g.total as stock, o.stock as optionstock, g.hasoption,g.maxbuy,
                    g.title,g.thumb,ifnull(o.marketprice, g.marketprice) as marketprice, g.productprice,
                    o.title as optiontitle,f.optionid,o.specs,g.minbuy,g.maxbuy,g.usermaxbuy,g.unit,g.merchid,
                    g.checked,g.isdiscount_discounts,g.isdiscount,g.isdiscount_time,g.isnodiscount,g.discounts,
                    g.merchsale,f.selected,g.status,g.deleted as goodsdeleted,g.type,g.intervalfloor,g.intervalprice
                FROM ' . MemberCart::TABLE_NAME . ' f '
            . ' left join ' . Goods::TABLE_NAME . ' g on f.goodsid = g.id '
            . ' left join ' . GoodsOption::TABLE_NAME . ' o on f.optionid = o.id '
            . ' where 1 ' . $condition . ' ORDER BY `id` DESC ';
        ...
        $array = pdo_fetchall('select og.optionid  from ' . OrderGoods::TABLE_NAME . ' og '
            . ' left join ' . Order::TABLE_NAME . ' o on og.orderid=o.id '
            . ' where o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ',
        ...
        foreach ($list as &$g) {
            ...
            if ($seckillinfo && $seckillinfo['status'] == 0) {
                ...
                if (0 < $g['usermaxbuy']) {
                    $cartGoodsCount = $this->countSameGoodsInCart($list, $g);
                    if ($g['usermaxbuy'] < $cartGoodsCount || $g['usermaxbuy'] < $g['allt']) {
                    ...
                    $total_buy = $order_goodscount + $cartGoodsCount;
                ...
                if (!empty($optionid)) {
                    $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual`,weight
                ...
                } elseif (($g['stock'] != -1) && empty($g['stock'])) {
    ...
    /**
     * 计算购物车中的同类商品个数
     * @param $list array 购物车商品列表
     * @param $g array 需要统计个数的商品
     * @return int 同类商品的个数
     */
    private function countSameGoodsInCart($list, $g): int
    {
        $goodsList = array_filter($list, static function ($goods) use ($g) {
            return $goods['goodsid'] === $g['goodsid'];
        });
        return array_reduce($goodsList, static function ($count, $goods) {
            return $count + $goods['total'];
        }, 0);
    }
}
