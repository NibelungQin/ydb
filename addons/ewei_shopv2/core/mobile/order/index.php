...
class Order_Index_EweiShopV2MobilePage extends MobileLoginPage
{
    protected function merchData()
    {
        ...
        return array($is_openmerch, $merch_plugin, $merch_data);
    }
    ...
    public function get_list()
    {
        ...
        if ($show_status != '') {
            ...
            switch ($show_status) {
                ...
                case 5:
                    ...
                    break;
                default:
                    $condition .= ' and status=' . (int)$show_status;
            }
            if ($show_status != 5) {
        ...
        $list = pdo_fetchall(
            'select id,addressid,ordersn,price,dispatchprice,status,iscomment,isverify,verifyendtime,
					verified,verifycode,verifytype,iscomment,refundid,expresscom,express,expresssn,finishtime,
					`virtual`,sendtype,paytype,expresssn,refundstate,dispatchtype,verifyinfo,merchid,isparent,
					iscycelbuy,cycelbuy_periodic,userdeleted
 					from ' . Order::TABLE_NAME . (' where 1 ' . $condition . ' order by createtime desc
 					LIMIT ') . ($pindex - 1) * $psize . ',' . $psize,
            $params
        );
        $total = pdo_fetchcolumn(
            'select count(*) from ' . Order::TABLE_NAME . (' where 1 ' . $condition),
            $params
        );
        $refunddays = (int)$_W['shopset']['trade']['refunddays'];
        if ($is_openmerch == 1) {
        ...
        foreach ($list as &$row) {
            ...
            $sql = 'SELECT og.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,og.price,og.title as gtitle,
       				og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,og.seckill_taskid,
                	og.sendtype,og.expresscom,og.expresssn,og.express,og.sendtime,og.finishtime,og.remarksend,
       				og.single_refundid,og.single_refundstate
            ...
            if ($com_verify) {
                        ...
                        if ($row['verifytype'] == 0 || $row['verifytype'] == 1 || $row['verifytype'] == 3) {
                            ...
                            $verifyinfo = array(
                                array(
                                    ...
                                    'verified' => $row['verifytype'] == 0 ? $row['verified']
                                        : $row['goods'][0]['goods']['total'] <= count($vs)
                                )
                            );
                            if ($row['verifytype'] == 0 || $row['verifytype'] == 3) {
                                $canverify = empty($row['verified']) && $showverify;
                            } elseif ($row['verifytype'] == 1) {
                ...
                } elseif (!empty($row['dispatchtype'])) {
                    $canverify = $row['status'] == 1 && $showverify;
                }
            }
            $row['canverify'] = $canverify;
            if ($is_openmerch == 1) {
                $row['merchname'] = $merch_user[$row['merchid']]['merchname']
                    ? $merch_user[$row['merchid']]['merchname'] : $_W['shopset']['shop']['name'];
            }
            if ($row['isonlyverifygoods']) {
                $row['canverify'] = false;
                $verifygood = pdo_fetch(
                    'select * from ' . VerifyGoods::TABLE_NAME . ' where orderid=:orderid limit 1',
                    array(':orderid' => $row['id'])
                );
                if (!empty($verifygood)) {
                    $row['verifygoods_id'] = $verifygood['id'];
                    $verifynum = pdo_fetchcolumn(
                        'select sum(verifynum) from ' . VerifygoodsLog::TABLE_NAME . ' where verifygoodsid =:id  ',
                        array(':id' => $verifygood['id'])
                    );
                    if (empty($verifygood['limittype'])) {
                        $limitdate = (int)$verifygood['starttime'] + (int)$verifygood['limitdays'] * 86400;
                    } else {
                        $limitdate = (int)$verifygood['limitdate'];
                    }
    ...
    public function detail()
    {
        ...
        $order = pdo_fetch(
            'select * from ' . Order::TABLE_NAME . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1',
        ...
        if ($order['refundid'] != 0) {
            $refund = pdo_fetch(
                'SELECT *  FROM ' . OrderRefund::TABLE_NAME
                . ' WHERE orderid = :orderid and uniacid=:uniacid order by id desc',
                array(':orderid' => $order['id'], ':uniacid' => $_W['uniacid'])
            );
        }
        ...
        $goodsid_array = array();
        $goods = pdo_fetchall(
            'select og.id,og.single_refundstate,og.sendtime,og.goodsid,og.price,g.title,og.title as gtitle,g.thumb,
            		g.status, g.cannotrefund, og.total,g.credit,og.optionid,og.optionname as optiontitle,g.isverify,
            		g.storeids,og.seckill,g.isfullback,og.seckill_taskid' . $diyformfields
            . $condition1 . ',og.prohibitrefund  from ' . tablename('ewei_shop_order_goods') . ' og '
            . ' left join ' . Goods::TABLE_NAME . ' g on g.id=og.goodsid '
            . (' where ' . $scondition . ' and og.uniacid=:uniacid '),
            $param
        );
        ...
        foreach ($goods as &$g) {
            if ($g['isfullback']) {
                $isfullback = true;
                $fullbackgoods = pdo_fetch(
                    'SELECT * FROM ' . FullbackGoods::TABLE_NAME
                    . ' WHERE goodsid = :goodsid and uniacid = :uniacid  limit 1 ',
                    array(':goodsid' => $g['goodsid'], ':uniacid' => $uniacid)
                );
                if ($g['optionid']) {
                    $option = pdo_fetch(
                        'select `day`,allfullbackprice,fullbackprice,allfullbackratio,fullbackratio,isfullback
                      from ' . GoodsOption::TABLE_NAME . ' where id = :id and uniacid = :uniacid ',
                        array(':id' => $g['optionid'], ':uniacid' => $uniacid)
                    );
        ...
        if (!empty($order['addressid'])) {
            ...
            if (!is_array($address)) {
                $address = pdo_fetch(
                    'select * from  ' . MemberAddress::TABLE_NAME . ' where id=:id limit 1',
        ...
        if (!empty($order['storeid'])) {
            if (0 < $merchid) {
                $store = pdo_fetch(
                    'select * from  ' . MerchStore::TABLE_NAME . ' where id=:id limit 1',
                    array(':id' => $order['storeid'])
                );
            } else {
                $store = pdo_fetch(
                    'select * from  ' . Store::TABLE_NAME . ' where id=:id limit 1',
                    array(':id' => $order['storeid'])
                );
            }
        }
        ...
        if (com('verify')) {
            ...
            if ($order['isverify']) {
                ...
                if (empty($storeids)) {
                    if (0 < $merchid) {
                        $stores = pdo_fetchall(
                            'select * from ' . MerchStore::TABLE_NAME
                            . ' where  uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)',
                            array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid)
                        );
                    } else {
                        $stores = pdo_fetchall(
                            'select * from ' . Store::TABLE_NAME . ' where  uniacid=:uniacid and status=1 and type in(2,3)',
                    ...
                    $stores = pdo_fetchall(
                        ...
                        . ' where id in (' . implode(',', $storeids) . ')
						 and uniacid=:uniacid and merchid=:merchid and status=1 and type in(2,3)',
                        array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid)
                    );
                } else {
                    $stores = pdo_fetchall(
                        'select * from ' . Store::TABLE_NAME . ' where id in (' . implode(
                            ',',
                            $storeids
                        ) . ') and uniacid=:uniacid and status=1 and type in(2,3)',
                        array(':uniacid' => $_W['uniacid'])
                    );
                }
                if ($order['verifytype'] == 0 || $order['verifytype'] == 1 || $order['verifytype'] == 3) {
                    ...
                    $verifyinfo = array(
                        array(
                            ...
                            'verified' => $order['verifytype'] == 0 || $order['verifytype'] == 3 ? $order['verified']
                                : $goods[0]['total'] <= count($vs)
                        )
                    );
            ...
            } elseif (!empty($order['dispatchtype'])) {
                $verifyinfo = array(
...
', '<br/>', $order['virtual_str']);
        ...
        if ($order['status'] == 1 || $order['status'] == 2) {
            ...
            if ($order['isverify'] != 1 && empty($order['virtual'])) {
                if (0 < $order['refundstate']) {
                    ...
                    $refunddays = (int)$tradeset['refunddays'];
                    if (0 < $refunddays) {
                        $days = (int)((time() - $order['finishtime']) / 3600 / 24);
                        if ($days <= $refunddays) {
                            $canrefund = true;
                        }
                    }
        ...
        if (p('ccard')) {
            ...
            if (!empty($comdata['become_goodsid']) && !empty($goodsid_array)
                && in_array($comdata['become_goodsid'], $goodsid_array)) {
                $canrefund = false;
            }
        }
        ...
        if (0 < $order['sendtype'] && 1 <= $order['status']) {
            $order_goods = pdo_fetchall('select orderid,goodsid,sendtype,expresscom,expresssn,express,sendtime
					from ' . OrderGoods::TABLE_NAME . '
            	where orderid = ' . $orderid . ' and uniacid = ' . $uniacid . ' and sendtype > 0
            	group by sendtype order by max(sendtime) asc ');
            $expresslist = m('util')->getExpressList($order['express'], $order['expresssn']);
            if (0 < count($expresslist)) {
                $express = $expresslist[0];
            }
            ...
            if (!$isonlyverifygoods) {
                ...
                if (strlen($verifycode) == 8) {
                    ...
                    if (strlen($verifycode) == 9) {
                        $verifycode = substr($verifycode, 0, 3) . ' '
                            . substr($verifycode, 3, 3) . ' '
                            . substr($verifycode, 6, 3);
                    }
                }
            }
        }
        ...
        if (!empty($order['virtual']) && !empty($order['virtual_str'])) {
            ...
            $virtualtemp = pdo_fetch(
                'SELECT linktext, linkurl,description FROM ' . VirtualType::TABLE_NAME
        ...
        if ((empty($order['ispackage']) && empty($ispeerpay) && empty($isfullback) && !empty($tradeset['single_refund']))
            || $order['refundstate'] == 3) {
    ...
    public function express()
    {
        ...
        if (0 < $order['sendtype'] && $sendtype == 0) {
            $i = 1;
            while ($i <= (int)$order['sendtype']) {
                $bundlelist[$i]['sendtype'] = $i;
                $bundlelist[$i]['orderid'] = $orderid;
                $bundlelist[$i]['goods'] = pdo_fetchall(
                    'select g.title,g.thumb,og.total,og.optionname as optiontitle,og.expresssn,og.express,
                    	og.sendtype,og.expresscom,og.sendtime from ' . OrderGoods::TABLE_NAME . ' og '
                    . ' left join ' . Goods::TABLE_NAME . ' g on g.id=og.goodsid '
                    . ' where og.orderid=:orderid and og.sendtype = ' . $i . ' and og.uniacid=:uniacid ',
        ...
        $goods = pdo_fetchall(
            'select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,og.optionname as optiontitle,
            		g.isverify,og.expresssn,og.express,og.sendtype,og.expresscom,og.sendtime,g.storeids'
            . $diyformfields . '
            	from ' . OrderGoods::TABLE_NAME . ' og '
            . ' left join ' . Goods::TABLE_NAME . ' g on g.id=og.goodsid '
            . ' where og.orderid=:orderid and og.single_refundtime=0 ' . $condition . ' and og.uniacid=:uniacid ',
            array(':uniacid' => $uniacid, ':orderid' => $orderid)
        );
        if (0 < $sendtype) {
