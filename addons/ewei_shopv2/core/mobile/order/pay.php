...
use Ydb\Entity\Manual\NewstoreGoodsOption;
...
class Order_Pay_EweiShopV2MobilePage extends MobileLoginPage
{
    public function main()
    {
        ...
        if (!empty($ispeerpay)) {
            if (pdo_fetchcolumn('SELECT COUNT(*) FROM ' . OrderPeerpayPayinfo::TABLE_NAME . ' WHERE openid = :openid AND pid = :pid',
                array(':openid' => $_W['openid'], ':pid' => $ispeerpay['id']))) {
                $this->message('每人只能代付一次');
            }
            $peerpayMessage = trim($_GPC['peerpaymessage']);
            $peerpay_info = (double)pdo_fetchcolumn('select SUM(price) from ' . OrderPeerpayPayinfo::TABLE_NAME . ' where pid=:pid limit 1',
                array(':pid' => $ispeerpay['id']));
            $peerprice = (float)$_GPC['peerprice'];
            if (empty($peerprice) || $peerprice <= 0) {
                header('Location:' . mobileUrl('order/pay/peerpayshare', array('id' => $orderid)));
                exit();
            }
            $openid = pdo_fetchcolumn('SELECT openid FROM ' . Order::TABLE_NAME . ' WHERE id = :id AND uniacid = :uniacid LIMIT 1',
                array(':id' => $orderid, ':uniacid' => $_W['uniacid']));
        }
        $order = pdo_fetch('select * from ' . Order::TABLE_NAME . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1',
        ...
        } elseif ($order['status'] == 1 && $order['tradestatus'] == 1) {
        ...
        $log = pdo_fetch('SELECT * FROM ' . CorePaylog::TABLE_NAME . ' WHERE `uniacid`=:uniacid AND `module`=:module AND `tid`=:tid limit 1',
            array(':uniacid' => $uniacid, ':module' => 'ewei_shopv2', ':tid' => $order['ordersn']));
        if (!empty($ispeerpay)) {
        } elseif (!empty($log) && $log['status'] != '0') {
            if (empty($order['istrade'])) {
                header('location: ' . mobileUrl('order/detail', array('id' => $order['id'])));
            } else {
                header('location: ' . mobileUrl('newstore/norder/detail', array('id' => $order['id'])));
        ...
        $seckill_goods = pdo_fetchall('select goodsid,optionid,seckill from  ' . OrderGoods::TABLE_NAME
        ...
        if (empty($order['price']) && !$credit['success']) {
            header('location: ' . mobileUrl('order/pay/complete',
                    array('id' => $order['id'], 'type' => 'credit', 'ordersn' => $order['ordersn'])));
            exit();
        }
        load()->model('payment');
        ...
        if (is_weixin()) {
            ...
            if (!empty($order['ordersn2'])) {
                ...
                if (100 <= $var) {
                    $var = $var + 1;
                    pdo_update('ewei_shop_order', array('ordersn2' => $var),
                        array('id' => $orderid, 'uniacid' => $uniacid, 'openid' => $openid));
                }
                $params['tid'] .= 'GJ' . $var;
            }
        ...
        if (empty($seckill_goods) && empty($ispeerpay)) {
            if (isset($set['pay']) && $set['pay']['alipay'] == 1 && is_array($setting['payment']['alipay'])
                && ($setting['payment']['alipay']['switch'] || $setting['payment']['alipay']['pay_switch'])) {
                $params = array();
                $params['tid'] = trim($log['tid']);
    ...
    public function complete()
    {
        ...
        $uniacid = $_W['uniacid'];
        $openid = $_W['openid'];
        $gpc_ordersn = (empty($_GPC['ordersn']) ? $_GPC['ordersn'] : str_replace(array('_borrow', '_B'), '',
            $_GPC['ordersn']));
        $ispeerpay = m('order')->checkpeerpay($orderid);
        if (!empty($ispeerpay)) {
            $_SESSION['peerpay'] = $ispeerpay['id'];
            $peerpay = $_GPC['peerpay'];
            $peerpay = (float)str_replace(',', '', $peerpay);
            ...
            } elseif ($ispeerpay['peerpay_type'] == 1 && !empty($ispeerpay['peerpay_selfpay'])
                && $ispeerpay['peerpay_selfpay'] < $peerpay && 0 < (float)$ispeerpay['peerpay_selfpay']) {
                if ($_W['ispost']) {
                    show_json(0, '参数错误');
                } else {
                    $this->message('参数错误', mobileUrl('order'));
            ...
            $openid = pdo_fetchcolumn('select openid from ' . Order::TABLE_NAME . ' where id=:orderid and uniacid=:uniacid limit 1',
        ...
        if (is_h5app() && empty($orderid)) {
            ...
            $orderid = pdo_fetchcolumn('select id from ' . Order::TABLE_NAME . ' where ordersn=:ordersn and uniacid=:uniacid and openid=:openid limit 1',
                array(':ordersn' => $ordersn, ':uniacid' => $uniacid, ':openid' => $openid));
        }
        if (empty($orderid)) {
            if ($_W['ispost']) {
                show_json(0, '参数错误');
        ...
        if ($go_flag == 1) {   // 微信回调在一道宝公众号查询之前到达，跳转支付成功页面
            $pay_result = true;
            if ($_W['ispost']) {
                $_SESSION[EWEI_SHOPV2_PREFIX . '_order_pay_complete'] = 1;
                show_json(1, array('result' => $pay_result));
            } else {
                header('location:' . mobileUrl('order/pay/success',
                        array('id' => $order['id'], 'result' => $pay_result)));
            }
            return;
        }
        if (0 < $order['ispackage']) {
            $package = pdo_fetch('SELECT * FROM ' . Package::TABLE_NAME . ' WHERE uniacid = ' . $uniacid . ' and id = ' . $order['packageid'] . ' ');
            if (empty($package)) {
                show_json(0, '未找到套餐！');
            }
        ...
        if ((empty($order['isnewstore']) || empty($order['storeid'])) && empty($order['istrade'])) {
            $order_goods = pdo_fetchall('select og.id,g.title, og.goodsid,og.optionid,g.total as stock,
                        ...
                        g.buylevels,g.buygroups,g.totalcnf,og.seckill
            ...
            foreach ($order_goods as $data) {
                ...
                if (($seckillinfo && $seckillinfo['status'] == 0) || !empty($ispeerpay)) {
                    ...
                    if (0 < $data['minbuy'] && $data['buycount'] < $data['minbuy']) {
                        if ($_W['ispost']) {
                            ...
                            $this->message($data['title'] . '<br/> ' . $data['min'] . $unit . '起售!',
                                mobileUrl('order'));
                        }
                    }
                    if (0 < $data['maxbuy'] && $data['maxbuy'] < $data['buycount']) {
                        if ($_W['ispost']) {
                            ...
                            $this->message($data['title'] . '<br/> 一次限购 ' . $data['maxbuy'] . $unit . '!',
                                mobileUrl('order'));
                        }
                    }
                    if (0 < $data['usermaxbuy']) {
                        ...
                        if ($data['usermaxbuy'] <= $order_goodscount) {
                            if ($_W['ispost']) {
                                ...
                                $this->message($data['title'] . '<br/> 最多限购 ' . $data['usermaxbuy'] . $unit,
                                    mobileUrl('order'));
                            }
                        }
                    }
                    ...
                    if ($data['buylevels'] != '') {
                        ...
                        if (!in_array($member["level"], $buylevels, true)) {
                            if ($_W['ispost']) {
                    ...
                    if ($data['buygroups'] != '') {
                        ...
                        if (!in_array($member["groupid"], $buygroups, true)) {
                            if ($_W['ispost']) {
                ...
                if ($data['totalcnf'] == 1) {
                    if (!empty($data['optionid'])) {
                        $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual` from ' . GoodsOption::TABLE_NAME
                            . ' where id=:id and goodsid=:goodsid and uniacid=:uniacid  limit 1',
                            array(':uniacid' => $uniacid, ':goodsid' => $data['goodsid'], ':id' => $data['optionid']));
                        if (!empty($option) && $option['stock'] != -1 && empty($option['stock'])) {
                            if ($_W['ispost']) {
                                show_json(0, $data['title'] . '<br/>' . $option['title'] . ' 库存不足!');
                            } else {
                                $this->message($data['title'] . '<br/>' . $option['title'] . ' 库存不足!',
                                    mobileUrl('order'));
                            }
                        }
            ...
            $sql = 'select og.id,g.title, og.goodsid,og.optionid,ng.stotal as stock,og.total as buycount,
                        ng.gstatus as status,ng.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,g.timeend,
                        g.buylevels,g.buygroups,g.totalcnf,og.seckill,og.storeid
                    from ' . OrderGoods::TABLE_NAME . ' og
                    left join  ' . Goods::TABLE_NAME . ' g  on g.id=og.goodsid and g.uniacid=og.uniacid'
                . ' inner join ' . NewstoreGoods::TABLE_NAME . ' ng on g.id=ng.goodsid   '
                . ' where og.orderid=:orderid and og.uniacid=:uniacid and ng.storeid=:storeid';
            $order_goods = pdo_fetchall($sql,
                array(':uniacid' => $uniacid, ':orderid' => $orderid, ':storeid' => $order['storeid']));
            foreach ($order_goods as $data) {
                if (empty($data['status']) || !empty($data['deleted'])) {
                    if ($_W['ispost']) {
                        show_json(0, $data['title'] . "<br/> 已下架!");
                ...
                if ($data['totalcnf'] == 1) {
                    if (!empty($data['optionid'])) {
                        $option = pdo_fetch('select id,marketprice,stock from ' . NewstoreGoodsOption::TABLE_NAME
                            . ' where optionid=:optionid and goodsid=:goodsid and uniacid=:uniacid and storeid=:storeid  limit 1',
                            array(
                                ':uniacid' => $uniacid,
        ...
        if ($type === 'credit') {
            ...
            $fee = (float)$ps['fee'];
            $result = m('member')->setCredit($openid, 'credit2', 0 - $fee,
            ...
            if (!empty($ispeerpay)) {
                $payquery = m('finance')->isWeixinPay($_SESSION['peerpaytid'], $order['price'],
                    (is_h5app() ? true : false));
            ...
            if (!is_error($payquery) || !is_error($payquery_jie) || !empty($ispeerpay)) {
                ...
                $ret['weid'] = $log['weid'];
                $ret['uniacid'] = $log['uniacid'];
                $ret['deduct'] = (int)$_GPC['deduct'] == 1;
                if (!empty($ispeerpay)) {
                    $udata = array();
                    $udata['usay'] = trim($_GPC['peerpaymessage']);
                    pdo_update('ewei_shop_order_peerpay_payinfo', $udata,
                        array('pid' => $ispeerpay['id'], 'openid' => $_W['openid']));
                }
                $pay_result = m('order')->payResult($ret);
                @session_start();
                $_SESSION[EWEI_SHOPV2_PREFIX . '_order_pay_complete'] = 1;
    ...
    public function success()
    {
        ...
        $ispeerpay = m('order')->checkpeerpay($orderid);
        if (!empty($ispeerpay)) {
            $peerpay = (float)$_GPC['peerpay'];
            $openid = pdo_fetchcolumn('select openid from ' . Order::TABLE_NAME . ' where id=:orderid and uniacid=:uniacid limit 1',
                array(':orderid' => $orderid, ':uniacid' => $uniacid));
            $order['price'] = $ispeerpay['realprice'];
            $peerpayuid = m('member')->getInfo($_W['openid']);
            $peerprice = pdo_fetch('SELECT `price` FROM ' . OrderPeerpayPayinfo::TABLE_NAME . ' WHERE uid = :uid ORDER BY id DESC LIMIT 1',
                array(':uid' => $peerpayuid['id']));
            $share = false;
            if (com('coupon')) {
            ...
            if (!empty($order['istrade'])) {
                if ($order['status'] == 1 && $order['tradestatus'] == 1) {
                    $order['price'] = $order['dowpayment'];
                } elseif ($order['status'] == 1 && $order['tradestatus'] == 2) {
                    $order['price'] = $order['betweenprice'];
                }
            }
            $merchid = $order['merchid'];
            $goods = pdo_fetchall('select og.goodsid,og.price,g.title,g.thumb,og.total,g.credit,og.optionid,
                        og.optionname as optiontitle,g.isverify,g.storeids
                    from ' . OrderGoods::TABLE_NAME . ' og '
            ...
            if (!empty($order['addressid'])) {
                ...
                if (!is_array($address)) {
                    $address = pdo_fetch('select * from  ' . MemberAddress::TABLE_NAME . ' where id=:id limit 1',
            ...
            if (!empty($order['storeid'])) {
                if (0 < $merchid) {
                    $store = pdo_fetch('select * from  ' . MerchStore::TABLE_NAME . ' where id=:id limit 1',
                        array(':id' => $order['storeid']));
                } else {
                    $store = pdo_fetch('select * from  ' . Store::TABLE_NAME . ' where id=:id limit 1',
                        array(':id' => $order['storeid']));
                }
            }
            $stores = false;
            if ($order['isverify']) {
                ...
                if (p('newstore') && !empty($order['isnewstore'])) {
                    $stores = pdo_fetchall('select * from ' . Store::TABLE_NAME
                        . ' where id = :id  and uniacid=:uniacid and status=1  order by displayorder desc,id desc',
                        array(':uniacid' => $_W['uniacid'], ':id' => $order['storeid']));
                    ...
                    $stores = pdo_fetchall('select * from ' . MerchStore::TABLE_NAME
                        . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid
                                and merchid=:merchid and status=1 order by displayorder desc,id desc',
                        array(':uniacid' => $_W['uniacid'], ':merchid' => $merchid));
                } else {
                    $stores = pdo_fetchall('select * from ' . Store::TABLE_NAME
                        . ' where id in (' . implode(',', $storeids) . ') and uniacid=:uniacid
                                and status=1 order by displayorder desc,id desc',
                        array(':uniacid' => $_W['uniacid']));
                }
            }
            if (p('lottery')) {
        ...
        if (!empty($order['virtual']) && !empty($order['virtual_str'])) {
            ...
            $virtualtemp = pdo_fetch('SELECT linktext, linkurl FROM ' . VirtualType::TABLE_NAME
                . ' WHERE id=:id AND uniacid=:uniacid LIMIT 1',
                array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid']));
        }
        $plugincoupon = com('coupon');
        if ($plugincoupon) {
    ...
    public function peerpay()
    {
        ...
        $peerpay = (int)pdo_fetchcolumn('select orderid from ' . OrderPeerpay::TABLE_NAME . ' where orderid=:id and uniacid=:uniacid limit 1',
            array(':id' => $orderid, ':uniacid' => $uniacid));
        if (!empty($peerpay)) {
            header('location: ' . mobileUrl('order/pay/peerpayshare', array('id' => $peerpay)));
            exit();
        }
        $order = pdo_fetch('select * from ' . Order::TABLE_NAME . ' where id=:id and uniacid=:uniacid limit 1',
            array(':id' => $orderid, ':uniacid' => $uniacid));
        ...
        if ($_W['ispost']) {
            ...
            $data['peerpay_realprice'] = ($PeerPay['peerpay_price'] <= $order['price'] ? round($order['price'] - $PeerPay['peerpay_privilege'],
                2) : (double)$order['price']);
        ...
        $goods = pdo_fetchall('SELECT g.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,
                    og.price,og.title as gtitle,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,
                    og.seckill,og.seckill_taskid,og.sendtype,og.expresscom,og.expresssn,og.express,og.sendtime,
                    og.finishtime,og.remarksend
                FROM ' . OrderGoods::TABLE_NAME . ' og
                    left join ' . Goods::TABLE_NAME . ' g on og.goodsid = g.id
                    left join ' . GoodsOption::TABLE_NAME . " op on og.optionid = op.id  where $scondition and og.uniacid=:uniacid order by og.id asc",
            array(':id' => $orderid, ':uniacid' => $uniacid));
        $address = iunserializer($order['address']);
        $member = m('member')->getMember($openid, true);
    ...
    public function peerpayshare()
    {
        ...
        $peerid = (int)$_GPC['id'];
        $uniacid = $_W['uniacid'];
        $peerpay = pdo_fetch('select p.*,o.openid from ' . OrderPeerpay::TABLE_NAME . ' p
                join ' . Order::TABLE_NAME . ' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1',
        ...
        $ordergoods = pdo_fetch('select * from ' . OrderGoods::TABLE_NAME . ' where orderid=:id and uniacid=:uniacid limit 1',
    ...
    public function peerpaydetail()
    {
        ...
        $peerid = (int)$_GPC['id'];
        $uniacid = $_W['uniacid'];
        $peerpay = pdo_fetch('select p.*,o.openid,o.address,o.isparent,o.id as oid from ' . OrderPeerpay::TABLE_NAME . ' p
                join ' . Order::TABLE_NAME . ' o on o.id=p.orderid where p.orderid=:id and p.uniacid=:uniacid limit 1',
            array(':id' => $peerid, ':uniacid' => $uniacid));
        if (empty($peerpay)) {
            header('location: ' . mobileUrl('order'));
        ...
        $goods = pdo_fetchall('SELECT g.id,og.goodsid,og.total,g.title,g.thumb,g.type,g.status,og.price,
                    og.title as gtitle,og.optionname as optiontitle,og.optionid,op.specs,g.merchid,og.seckill,
                    og.seckill_taskid, og.sendtype,og.expresscom,og.expresssn,og.express,og.sendtime,og.finishtime,
                    og.remarksend FROM ' . OrderGoods::TABLE_NAME . ' og '
            . ' left join ' . Goods::TABLE_NAME . ' g on og.goodsid = g.id '
            . ' left join ' . GoodsOption::TABLE_NAME . ' op on og.optionid = op.id '
            . ' where ' . $scondition . ' and og.uniacid=:uniacid order by og.id asc',
            array(":id" => $peerpay["orderid"], ":uniacid" => $uniacid));
        $address = (!empty($peerpay['address']) ? iunserializer($peerpay['address']) : '');
        if ($peerpay['peerpay_type'] == 0) {
            $price = $peerpay['peerpay_realprice'];
        } else {
            $message = pdo_fetchall('SELECT * FROM ' . OrderPeerpayPayinfo::TABLE_NAME . ' WHERE pid = :pid ORDER BY id DESC LIMIT 3',
                array(':pid' => $peerpay['id']));
