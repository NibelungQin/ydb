...
class Order_Refund_EweiShopV2MobilePage extends MobileLoginPage
{
    protected function globalData()
    {
        ...
        $openid = $_W['openid'];
        $orderid = (int)$_GPC['id'];
        $order = pdo_fetch('select id,status,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,
                        finishtime,isverify,`virtual`,refundstate,merchid,random_code,iscycelbuy,paytype
        ...
        if ($order['status'] == 2) {
            $goods = pdo_fetchall('select og.goodsid, og.price, og.total, og.optionname, g.cannotrefund,
                        g.thumb, g.title,g.isfullback
        ...
        if ($fullback_log) {
            $fullbackgoods = pdo_fetch('select refund from ' . FullbackGoods::TABLE_NAME
                . ' where goodsid = ' . $fullback_log['goodsid'] . ' and uniacid = ' . $uniacid . ' ');
    ...
    public function main()
    {
        ...
        if (0 < $order['refundstate']) {
            if (!empty($refundid)) {
                $refund = pdo_fetch('select * from ' . OrderRefund::TABLE_NAME
    ...
    public function submit()
    {
        ...
        if ($order['refundstate'] == 0) {
            ...
            pdo_update('ewei_shop_order', array('refundid' => $refundid, 'refundstate' => $refundstate),
                array('id' => $orderid, 'uniacid' => $uniacid));
        } else {
            pdo_update('ewei_shop_order', array('refundstate' => $refundstate),
                array('id' => $orderid, 'uniacid' => $uniacid));
