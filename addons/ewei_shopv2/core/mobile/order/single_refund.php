...
class Order_Single_Refund_EweiShopV2MobilePage extends MobileLoginPage
{
    protected function globalData()
    {
        global $_W;
        global $_GPC;
        $order_goodsid = (int)$_GPC['id'];
        $openid = $_W['openid'];
        $uniacid = $_W['uniacid'];
        $order = pdo_fetch('select o.id,o.price,o.couponprice,o.iscycelbuy,o.status,o.virtual,o.isverify,
                    o.refundstate,o.finishtime,o.deductprice,o.deductcredit2,o.dispatchprice,o.deductenough,
                    o.merchdeductenough,g.cannotrefund,og.single_refundid,og.single_refundstate,
                    og.single_refundtime,og.realprice as og_realprice,o.grprice,og.consume,o.ispackage,og.sendtime
        ...
        if ($order['iscycelbuy'] == 1) {
            $order_goods = pdo_fetch('select * from ' . CycelbuyPeriods::TABLE_NAME
                . (' where orderid = ' . $order['id'] . ' and status != 0'));
        ...
        $fullback_goods_count = pdo_fetchcolumn('select count(og.id)
        ...
        $order_goods = pdo_fetchall('select og.id,og.single_refundid,og.single_refundstate,r.applyprice
                from ' . OrderGoods::TABLE_NAME . ' og '
            . ' left join ' . OrderSingleRefund::TABLE_NAME . ' r on r.id=og.single_refundid '
        ...
        if ($is_last) {
            ...
            $order_discount = $order['deductprice'] + $order['deductcredit2'] + $order['deductenough']
                + $order['merchdeductenough'] + $order['couponprice'];
    ...
    public function main()
    {
        ...
        if (!empty($single_refundid)) {
            $refund = pdo_fetch('select * from ' . OrderSingleRefund::TABLE_NAME
                . ' where id=:id and ordergoodsid=:ordergoodsid and uniacid=:uniacid limit 1',
    ...
    public function submit()
    {
        ...
        $price = (float)$_GPC['price'];
        $rtype = (int)$_GPC['rtype'];
        ...
        if ($order['single_refundstate'] == 0) {
            ...
            pdo_update('ewei_shop_order_goods',
                array('single_refundid' => $single_refundid, 'single_refundstate' => $refundstate),
                array('id' => $order_goodsid, 'uniacid' => $uniacid));
        } else {
            $refund['status'] = 0;
            pdo_update('ewei_shop_order_goods', array('single_refundstate' => $refundstate),
                array('id' => $order_goodsid, 'uniacid' => $uniacid));
            pdo_update('ewei_shop_order_single_refund', $refund,
                array('id' => $single_refundid, 'uniacid' => $uniacid));
        }

        pdo_update('ewei_shop_order', array('refundstate' => 3, 'refundtime' => 0),
            array('id' => $order['id'], 'uniacid' => $uniacid));
    ...
    public function cancel()
    {
        ...
        pdo_update('ewei_shop_order_single_refund', $change_refund,
            array('id' => $single_refundid, 'uniacid' => $uniacid));
        pdo_update('ewei_shop_order_goods', array('single_refundstate' => 0),
            array('id' => $order_goodsid, 'uniacid' => $uniacid));
        $order_goods = pdo_fetchall('select single_refundid,single_refundstate,single_refundtime
                from ' . OrderGoods::TABLE_NAME . ' where orderid=:orderid',
        ...
        if (empty($apply_refund_num) && empty($refund_num)) {
            pdo_update('ewei_shop_order', array('refundstate' => 0, 'refundtime' => 0),
                array('id' => $order['id'], 'uniacid' => $uniacid));
        }
    ...
    public function receive()
    {
        ...
        $single_refundid = (int)$_GPC['single_refundid'];
        $refund = pdo_fetch('select * from ' . OrderSingleRefund::TABLE_NAME
            . ' where id=:id and ordergoodsid=:ordergoodsid and uniacid=:uniacid limit 1',
            array(':id' => $single_refundid, ':ordergoodsid' => $order_goodsid, ':uniacid' => $uniacid));
        ...
        $is_single_refund = pdo_fetchcolumn('select count(id) from ' . OrderGoods::TABLE_NAME
            . ' where orderid=:orderid and (single_refundstate=1 or single_refundstate=2)',
            array(':orderid' => $order['id']));
