...
class Common_EweiShopV2Model
{
    ...
    public function getSysset($key = '', $uniacid = 0)
    {
        ...
        if (!empty($key)) {
            if (is_array($key)) {
                foreach ($key as $k) {
                    $retsets[$k] = $allset[$k] ?? array();
                }
            } else {
                $retsets = $allset[$key] ?? array();
            }
    ...
    public function updateSysset($values, $uniacid = 0)
    {
        ...
        $setdata = pdo_fetch('select * from ' . tablename('ewei_shop_sysset') . ' where uniacid=:uniacid limit 1',
            array(':uniacid' => $uniacid));
    ...
    public function deleteSysset($key, $uniacid = 0)
    {
        ...
        $setdata = pdo_fetch('select id, sets from ' . tablename('ewei_shop_sysset') . ' where uniacid=:uniacid limit 1',
            array(':uniacid' => $uniacid));
...
//dump(iserializer($values));exit;
        if (empty($setdata)) {
            ...
            $res = pdo_update('ewei_shop_sysset', array('plugins' => iserializer($plugins)),
                array('id' => $setdata['id']));
    ...
    public function alipay_build($params, $alipay = array(), $type = 0, $openid = '')
    {
        ...
        if (0 < $pay['alipay_id']) {
            ...
            if (empty($type)) {
                ...
                $config['return_url'] = mobileUrl('order/pay_alipay/complete',
                    array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
            } else {
                if ($type == 20) {
                    $config['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                    $config['return_url'] = mobileUrl('creditshop/detail/creditshop_complete',
                        array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
                } else {
                    if ($type == 21) {
                        $config['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                        $config['return_url'] = mobileUrl('creditshop/log/dispatch_complete',
                            array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
                    } else {
                        if ($type == 6) {
                            $config['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                            $config['return_url'] = mobileUrl('threen/register/threen_complete',
                                array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
                        } else {
                            if ($type == 22) {
                                $config['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                                $config['return_url'] = mobileUrl('membercard/detail/membercard_complete',
                                    array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
                            } else {
                                $config['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                                $config['return_url'] = mobileUrl('order/pay_alipay/recharge_complete',
                                    array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
                            }
                        }
                    }
        ...
        if (empty($type)) {
            ...
            $set['return_url'] = mobileUrl('order/pay_alipay/complete',
                array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
        } else {
            if ($type == 20) {
                $set['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                $set['return_url'] = mobileUrl('creditshop/detail/creditshop_complete',
                    array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
            } else {
                if ($type == 21) {
                    $set['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                    $set['return_url'] = mobileUrl('creditshop/log/dispatch_complete',
                        array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
                } else {
                    if ($type == 6) {
                        $set['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                        $set['return_url'] = mobileUrl('threen/register/threen_complete',
                            array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
                    } else {
                        if ($type == 22) {
                            $config['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                            ...
                            $set['notify_url'] = $_W['siteroot'] . 'addons/ewei_shopv2/payment/alipay/notify.php';
                            $set['return_url'] = mobileUrl('order/pay_alipay/recharge_complete',
                                array('openid' => $openid, 'fromwechat' => is_weixin() ? 1 : 0), true);
                        }
                    }
                }
...
', true);
    ...
    public function public_build($isapp = false)
    {
        ...
        if (!empty($set['weixin_id']) && $isapp == false) {
            $payments = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_payment') . ' WHERE uniacid=:uniacid AND id=:id',
                array(':uniacid' => $_W['uniacid'], ':id' => $set['weixin_id']));
    ...
    public function sendredpack($params)
    {
        ...
        if (strpos($params['openid'], 'sns_wa_') !== false) {
            ...
            if (!empty($set['weixin_id'])) {
                $payment = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_payment') . ' WHERE uniacid=:uniacid AND id=:id',
                    array(':uniacid' => $_W['uniacid'], ':id' => $set['weixin_id']));
                ...
                $payment = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_payment') . ' WHERE uniacid=:uniacid AND `type`=\'0\'',
        ...
        libxml_disable_entity_loader(true);
        $arr = json_decode(json_encode(simplexml_load_string($resp['content'], 'SimpleXMLElement', LIBXML_NOCDATA)),
            true);
    ...
    public function createNO($table, $field, $prefix)
    {
        ...
        while (1) {
            $count = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_' . $table) . (' where ' . $field . '=:billno limit 1'),
                array(':billno' => $billno));
    ...
    public function html_images($detail = '', $enforceQiniu = false)
    {
        ...
        preg_match_all('/<img.*?src=[\\\\\'| \\"](.*?(?:[\\.gif|\\.jpg|\\.png|\\.jpeg]?))[\\\\\'|\\"].*?[\\/]?>/',
            $detail, $imgs);
        $images = array();
    ...
    public function getSec($uniacid = 0)
    {
        ...
        $set = pdo_fetch('select sec from ' . tablename('ewei_shop_sysset') . ' where uniacid=:uniacid limit 1',
            array(':uniacid' => $uniacid));
        ...
        if (0 < $data['alipay_id']) {
            $paymentalis = pdo_fetch('SELECT alipay_sec FROM ' . tablename('ewei_shop_payment') . ' WHERE id = :id and uniacid=:uniacid and paytype = 1 ',
                array(':uniacid' => $_W['uniacid'], ':id' => $data['alipay_id']));
    ...
    public function getCopyright($ismanage = false)
    {
        ...
        if (!empty($merchid)) {
            $merch = pdo_fetch('select logo from ' . tablename('ewei_shop_merch_user') . ' where id = :id and uniacid = :uniacid ',
                array(':id' => $merchid, ':uniacid' => $_W['uniacid']));
