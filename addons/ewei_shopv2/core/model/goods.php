...
class Goods_EweiShopV2Model 
{
	...
	public function getList($args = array()) 
	{
		...
		if (!($random)) 
		{
			$sql = 'SELECT id,costprice,title,subtitle,thumb,thumb_url' . $officsql . ',marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay' . "\r\n" . '            FROM ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY ' . $order . ' ' . $orderby . ' LIMIT ' . (($page - 1) * $pagesize) . ',' . $pagesize;
			$total = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ', $params);
		}
		else 
		{
			$sql = 'SELECT id,costprice,title,thumb,thumb_url' . $officsql . ',marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,bargain,hascommission,nocommission,commission,commission1_rate,commission1_pay' . "\r\n" . '            FROM ' . tablename('ewei_shop_goods') . ' where 1 ' . $condition . ' ORDER BY rand() LIMIT ' . $pagesize;
			$total = $pagesize;
		}
		...
		foreach ($list as $key => $value ) 
		{
			if ($value['hasoption'] == 1) 
			{
				...
				foreach ($options as $k => $v ) 
				{
                    ...
                    array_push($costmax, $v['costprice']);
				}
				$value['marketprice'] = max($pricemax);
                $value['costprice'] = max($costmax);
			}
	...
	public function getCommission($goods, $level, $set) 
	{
        ...
        $commission_set = m('common')->getPluginset('commission');
		if($commission_set['calcutype']==2){
		    //如果按商品利润分佣
		...
		{
			$commission = ((1 <= $set['level'] ? round(($level['commission1'] * ($goods['marketprice']-$costprice)) / 100, 2) : 0));
		}
		else 
		{
			$commission = ((1 <= $set['level'] ? round(($set['commission1'] * ($goods['marketprice']-$costprice)) / 100, 2) : 0));
		}
		return $commission;
	}
	...
	//生成广告套餐商品推广海报
	public function createcode_advertisement($parameter) {
		global $_W;
		$path = IA_ROOT . '/addons/ewei_shopv2/data/goodscode/' . $_W['uniacid'] . '/';
		$o_id = $parameter['o_id'];
		$goodsid = $parameter['goodsid'];
