...
class Member_EweiShopV2Model
{
    public function getInfo($openid = "")
    {
        ...
        if ($uid == 0) {
            $info = pdo_fetch(
                "select * from " . tablename("ewei_shop_member") . " where openid=:openid and uniacid=:uniacid limit 1",
            ...
            if (empty($info)) {
                ...
                if (!empty($condition)) {
                    $info = pdo_fetch(
                        "select * from " . tablename("ewei_shop_member") . " where " . $condition . "  and uniacid=:uniacid limit 1",
                        array(":uniacid" => $_W["uniacid"], ":openid" => $openid)
                    );
                    if (!empty($info)) {
            ...
            $info = pdo_fetch(
                "select * from " . tablename("ewei_shop_member") . " where id=:id  and uniacid=:uniacid limit 1",
    ...
    public function getMember($openid = "")
    {
        ...
        if ($uid == 0) {
            $info = pdo_fetch(
                'select * from ' . Member::TABLE_NAME . ' where  openid=:openid and uniacid=:uniacid limit 1',
                array(':uniacid' => $_W['uniacid'], ':openid' => $openid)
            );
            if (empty($info)) {
                $condition = '';
                ...
                if (!empty($condition)) {
                    $info = pdo_fetch(
                        ...
                        . ' where ' . $condition . '  and uniacid=:uniacid limit 1',
                        array(':uniacid' => $_W['uniacid'], ':openid' => $openid)
                    );
                    if (!empty($info)) {
                        $info['bindsns'] = $bindsns;
            ...
            $info = pdo_fetch(
                'select * from ' . Member::TABLE_NAME . ' where id=:id and uniacid=:uniacid limit 1',
                array(':uniacid' => $_W['uniacid'], ':id' => $openid)
            );
        }
    ...
    public function getMobileMember($mobile)
    {
        global $_W;
        $info = pdo_fetch(
            "select * from " . tablename("ewei_shop_member") . " where mobile=:mobile and uniacid=:uniacid limit 1",
            array(":uniacid" => $_W["uniacid"], ":mobile" => $mobile)
        );
        if (!empty($info)) {
            $info = $this->updateCredits($info);
        }
    ...
    public function setCredit($openid = '', $credittype = 'credit1', $credits = 0, $log = array())
    {
        ...
        if (!empty($uid)) {
            $value = pdo_fetchcolumn(
                'SELECT ' . $credittype . ' FROM ' . tablename('mc_members') . ' WHERE `uid` = :uid',
                array(':uid' => $uid)
            );
            ...
            pdo_update(
                'ewei_shop_member',
                array($credittype => $newcredit),
                array('uniacid' => $_W['uniacid'], 'openid' => $openid)
            );
        } else {
            $value = pdo_fetchcolumn(
                'SELECT ' . $credittype . ' FROM ' . tablename('ewei_shop_member') . ' WHERE  uniacid=:uniacid and openid=:openid limit 1',
                array(':uniacid' => $_W['uniacid'], ':openid' => $openid)
            );
            $newcredit = $credits + $value;
            if ($newcredit <= 0) {
    ...
    public function getCredit($openid = '', $credittype = 'credit1')
    {
        ...
        // 匹配 openid
        $item = pdo_fetch(
            'SELECT ' . $credittype . ' FROM ' . tablename('ewei_shop_member') . ' WHERE openid=:openid and uniacid=:uniacid limit 1',
            array(':uniacid' => $_W['uniacid'], ':openid' => $openid)
        );
        ...
        if (empty($item)) {
            // 匹配 openid_wa
            $item = pdo_fetch(
                'SELECT ' . $credittype . ' FROM ' . tablename('ewei_shop_member') . ' WHERE openid_wa=:openid and uniacid=:uniacid limit 1',
                array(':uniacid' => $_W['uniacid'], ':openid' => $openid)
            );
        }
    ...
    public function getCredits($openid = '', $credittypes = array('credit1', 'credit2'))
    {
        ...
        if (!empty($uid)) {
            return pdo_fetch(
                'SELECT ' . $types . ' FROM ' . McMembers::TABLE_NAME . ' WHERE `uid` = :uid limit 1',
                array(':uid' => $uid)
            );
        }
        $item = pdo_fetch(
            'SELECT ' . $types . ' FROM ' . Member::TABLE_NAME . ' WHERE openid=:openid and uniacid=:uniacid limit 1',
            array(':uniacid' => $_W['uniacid'], ':openid' => $openid)
        );
        if (empty($item)) {
            $item = pdo_fetch(
                'SELECT ' . $types . ' FROM ' . Member::TABLE_NAME . ' WHERE openid_wa=:openid and uniacid=:uniacid limit 1',
                array(':uniacid' => $_W['uniacid'], ':openid' => $openid)
            );
        }
        if (empty($item)) {
    ...
    public function checkMember()
    {
        ...
        $openid = $_W['openid'];
        if ($_W['routes'] === 'order.pay_alipay' || $_W['routes'] === 'creditshop.log.dispatch_complete'
            || $_W['routes'] === 'threen.register.threen_complete'
            || $_W['routes'] === 'creditshop.detail.creditshop_complete'
            || $_W['routes'] === 'order.pay_alipay.recharge_complete' || $_W['routes'] === 'order.pay_alipay.complete'
            || $_W['routes'] === 'newmr.alipay' || $_W['routes'] === 'newmr.callback.gprs'
            || $_W['routes'] === 'newmr.callback.bill' || $_W['routes'] === 'account.sns' || $_W['plugin'] === 'mmanage'
            || $_W['routes'] === 'live.send.credit' || $_W['routes'] === 'live.send.coupon'
            || $_W['routes'] === 'index.share_url') {
            return null;
        }
        if ($shopset['wap']['open'] && (($shopset['wap']['inh5app'] && is_h5app())
                || (empty($shopset['wap']['inh5app']) && empty($openid)))) {
            return null;
        }
        if (empty($openid) && !EWEI_SHOPV2_DEBUG) {
            $diemsg = (is_h5app() ? 'APP正在维护, 请到公众号中访问' : '请在微信客户端打开链接');
            ExceptionUtil::exit("<!DOCTYPE html>\r\n
                  <html>\r\n
                      <head>\r\n
                          ...
                          <div class='page_msg'>
                              <div class='inner'>
                                  <span class='msg_icon_wrp'><i class='icon80_smile'></i></span>
                                      <div class='msg_content'><h4>" . $diemsg . "</h4></div>
                              </div>
                          </div>\r\n
                      </body>\r\n
                  </html>", true);
        ...
        if (empty($member) && !empty($openid)) {
            ...
            if (method_exists(m('member'), 'memberRadisCountDelete')) {   // todo
    ...
    public function getLevels($all = true)
    {
        ...
        return pdo_fetchall(
            "select * from " . tablename("ewei_shop_member_level") . " where uniacid=:uniacid" . $condition . " order by level asc",
    ...
    public function getLevel($openid)
    {
        ...
        if (!empty($member) && !empty($member["level"])) {
            $level = pdo_fetch(
                "select * from " . tablename("ewei_shop_member_level") . " where id=:id and uniacid=:uniacid limit 1",
                array(":id" => $member["level"], ":uniacid" => $_W["uniacid"])
            );
            if (!empty($level)) {
                return $level;
            }
    ...
    public function getOneGoodsLevel($openid, $goodsid)
    {
        ...
        $data = array();
        $levels = pdo_fetchall(
            "select * from " . tablename("ewei_shop_member_level") . " where uniacid=:uniacid and buygoods=1 and level and level > :level order by level asc",
            array(":uniacid" => $uniacid, ":level" => $level)
        );
        if (!empty($levels)) {
    ...
    public function getGoodsLevel($openid, $orderid)
    {
        ...
        $order_goods = pdo_fetchall(
            "select goodsid from " . tablename("ewei_shop_order_goods") . " where orderid=:orderid and uniacid=:uniacid",
    ...
    public function upgradeLevel($openid, $orderid = 0)
    {
        ...
        if (empty($leveltype)) {
            $ordermoney = pdo_fetchcolumn(
                "select sum(price) from " . tablename("ewei_shop_order") . " where uniacid=:uniacid and openid=:openid and status=3 limit 1",
                array(":uniacid" => $_W["uniacid"], ":openid" => $member["openid"])
            );
            $ordermoney = floatval($ordermoney);
            $level = pdo_fetch(
                "select * from " . tablename("ewei_shop_member_level") . " where uniacid=:uniacid  and enabled=1 and " . $ordermoney . " >= ordermoney and ordermoney>0  order by level desc limit 1",
                array(":uniacid" => $_W["uniacid"])
            );
        } else {
            if ($leveltype == 1) {
                $ordercount = pdo_fetchcolumn(
                    "select count(*) from " . tablename("ewei_shop_order") . " where openid=:openid and status=3 and uniacid=:uniacid ",
                    array(":uniacid" => $_W["uniacid"], ":openid" => $member["openid"])
                );
                $level = pdo_fetch(
                    "select * from " . tablename("ewei_shop_member_level") . " where uniacid=:uniacid and enabled=1 and " . $ordercount . " >= ordercount and ordercount>0  order by level desc limit 1",
                    array(":uniacid" => $_W["uniacid"])
                );
            }
        }
    ...
    public function upgradeLevelByLevelId($openid, $LevelID)
    {
        ...
        $level = pdo_fetch(
            "select *  from " . tablename("ewei_shop_member_level") . " where uniacid=:uniacid and enabled=1 and id=:id",
            array(":uniacid" => $_W["uniacid"], ":id" => $LevelID)
        );
        if (empty($level)) {
            return null;
        }
    ...
    public function getGroups()
    {
        global $_W;
        return pdo_fetchall(
            "select * from " . tablename("ewei_shop_member_group") . " where uniacid=:uniacid order by id asc",
            array(":uniacid" => $_W["uniacid"])
        );
    }
    ...
    public function setGroups($openid, $group_ids, $reason = "")
    {
        ...
        foreach ($diff_ids as $groupid) {
            pdo_insert("ewei_shop_member_group_log", array(
    ...
    public function checkMemberFromPlatform($openid = "", $acc = "")
    {
        ...
        if (empty($member)) {
            $mc = mc_fetch(
                $uid,
                array("realname", "nickname", "mobile", "avatar", "resideprovince", "residecity", "residedist")
            );
            $member = array(
                "uniacid" => $_W["uniacid"],
                "uid" => $uid,
    ...
    public function wxuser($appid, $secret, $snsapi = "snsapi_base", $expired = "600")
    {
        ...
        if ($wxuser === null) {
            ...
            if (!empty($json["errcode"]) && ($json["errcode"] == "40029" || $json["errcode"] == "40163")) {
                $url = "http://" . $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"] . ((strpos(
                    $_SERVER["REQUEST_URI"],
                    "?"
                ) ? "" : "?"));
                $parse = parse_url($url);
