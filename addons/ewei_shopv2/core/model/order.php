...
use Ydb\Entity\Manual\FullbackLog;
...
use Ydb\Entity\Manual\NewstoreGoods;
...
use Ydb\Entity\Manual\VirtualType;
...
class Order_EweiShopV2Model
{
    public function fullback($orderid)
    {
        global $_W;
        $uniacid = $_W['uniacid'];
        $order_goods = pdo_fetchall('select o.openid,og.optionid,og.goodsid,og.price,og.total from ' . OrderGoods::TABLE_NAME . ' as og
                    left join ' . Order::TABLE_NAME . ' as o on og.orderid = o.id
                    where og.uniacid = ' . $uniacid . ' and og.orderid = ' . $orderid . ' ');

        foreach ($order_goods as $key => $value) {
            if (0 < $value['optionid']) {
                $goods = pdo_fetch('select g.hasoption,g.id,go.goodsid,go.isfullback from ' . Goods::TABLE_NAME . ' as g
                left join ' . GoodsOption::TABLE_NAME . ' as go on go.goodsid = :id and go.id = ' . $value['optionid'] . '
                 where g.id=:id and g.uniacid=:uniacid limit 1',
                    array(':id' => $value['goodsid'], ':uniacid' => $uniacid));
            } else {
                $goods = pdo_fetch('select * from ' . Goods::TABLE_NAME . ' where id=:id and uniacid=:uniacid limit 1',
                    array(':id' => $value['goodsid'], ':uniacid' => $uniacid));
            }

            if (0 < $goods['isfullback']) {
                $fullbackgoods = pdo_fetch('SELECT id,minallfullbackallprice,maxallfullbackallprice,
                                minallfullbackallratio,maxallfullbackallratio,`day`,
                                fullbackprice,fullbackratio,status,hasoption,marketprice,`type`,startday
                            FROM ' . FullbackGoods::TABLE_NAME
                    . ' WHERE uniacid = ' . $uniacid . ' and goodsid = ' . $value['goodsid'] . ' limit 1');
                if (!empty($fullbackgoods) && $goods['hasoption'] && 0 < $value['optionid']) {
                    $option = pdo_fetch('select id,title,allfullbackprice,allfullbackratio,fullbackprice,
                            fullbackratio,`day`
                ...
                if (!empty($fullbackgoods)) {
                    ...
                    $has_record = pdo_fetch('select id from ' . FullbackLog::TABLE_NAME . (' where ' . $condition),
                        $params);
    ...
    public function fullbackstop($orderid)
    {
        ...
        $uniacid = $_W['uniacid'];
        $shopset = $_S['shop'];
        $fullback_log = pdo_fetch('select * from ' . FullbackLog::TABLE_NAME
            . ' where uniacid = ' . $uniacid . ' and orderid = ' . $orderid . ' ');
        pdo_update('ewei_shop_fullback_log', array('isfullback' => 1),
            array('id' => $fullback_log['id'], 'uniacid' => $uniacid));
    }
    ...
    public function payResult($params)
    {
        ...
        $order = pdo_fetch('select id,uniacid,ordersn, price,openid,dispatchtype,addressid,carrier,status,
                    isverify,deductcredit2,`virtual`,isvirtual,couponid,isvirtualsend,isparent,paytype,merchid,agentid,
                    createtime,buyagainprice,istrade,tradestatus,iscycelbuy
                from ' . Order::TABLE_NAME . ' where  ordersn=:ordersn and uniacid=:uniacid limit 1',
            array(':uniacid' => $_W['uniacid'], ':ordersn' => $ordersn));
        ...
        if (!empty($ispeerpay)) {
            $peerpay_info = (double)pdo_fetchcolumn('select SUM(price) price from ' . OrderPeerpayPayinfo::TABLE_NAME . ' where pid=:pid limit 1',
        ...
        if ($params['from'] == 'return') {
            ...
            if (empty($order['dispatchtype'])) {
                $address = pdo_fetch('select realname,mobile,address from ' . MemberAddress::TABLE_NAME . ' where id=:id limit 1',
            ...
            if ($order['istrade'] == 0) {
                if ($order['status'] == 0) {
                    ...
                    if ($order['isparent'] == 1) {
                        $merchSql = 'SELECT id,merchid FROM ' . Order::TABLE_NAME
                            . ' WHERE uniacid = ' . (int)$order['uniacid'] . ' AND parentid = ' . (int)$order['id'];
    ...
    {
        global $_W;
        $list = pdo_fetchall('select id,ordersn,status,finishtime,couponid,merchid  from ' . Order::TABLE_NAME
            . ' where  parentid=:parentid and uniacid=:uniacid',
            array(':parentid' => $orderid, ':uniacid' => $_W['uniacid']));
        return $list;
    }
    ...
    public function payVirtualSend($orderid = 0, $ispeerpay = false)
    {
        ...
        $order = pdo_fetch('select id,uniacid,ordersn, price,openid,dispatchtype,addressid,carrier,status,
                    ...
                    agentid,createtime,buyagainprice,istrade,tradestatus,iscycelbuy
                from ' . Order::TABLE_NAME . ' where  id=:id and uniacid=:uniacid limit 1',
            array(':uniacid' => $_W['uniacid'], ':id' => $orderid));
        $order_goods = pdo_fetch('select g.virtualsend,g.virtualsendcontent
                    from ' . OrderGoods::TABLE_NAME . ' og '
            . ' left join ' . Goods::TABLE_NAME . ' g on g.id=og.goodsid '
            . ' where og.orderid=:orderid and og.uniacid=:uniacid limit 1',
            array(':uniacid' => $order['uniacid'], ':orderid' => $orderid));
        $time = time();
    ...
    public function afterPayResult($order, $ispeerpay = false)
    {
        if (p('task')) {
            if (0 < $order['deductcredit2']) {
                $order['price'] = (float)$order['price'] + (float)$order['deductcredit2'];
            }

            if (0 < $order['deductcredit']) {
                $order['price'] = (float)$order['price'] + (float)$order['deductprice'];
            }
            ...
            $goodslist = pdo_fetchall('SELECT goodsid FROM ' . OrderGoods::TABLE_NAME
                . ' WHERE orderid = :orderid AND uniacid = :uniacid',
                array(':orderid' => $order['id'], ':uniacid' => $order['uniacid']));
            ...
            if (pdo_fetchcolumn('select count(*) from ' . Order::TABLE_NAME
                    . (' where openid = \'' . $order['openid'] . '\' and uniacid = ' . $order['uniacid'])) == 1) {
        ...
        foreach ($goods as $g) {
            ...
            if (!empty($gcredit)) {
                if (strexists($gcredit, '%')) {
                    $credits += (int)((float)str_replace('%', '', $gcredit) / 100 * $g['realprice']);
                } else {
                    $credits += (int)$g['credit'] * $g['total'];
                }
            }
        }
    ...
    {
        global $_W;
        $order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status
                from ' . Order::TABLE_NAME . ' where id=:id limit 1',
            array(':id' => $orderid));
        $goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.money,og.optionid,
                    g.total as goodstotal,og.optionid,g.sales,g.salesreal
                from ' . OrderGoods::TABLE_NAME . ' og '
            . ' left join ' . Goods::TABLE_NAME . ' g on g.id=og.goodsid '
        ...
        foreach ($goods as $g) {
            ...
            if (!empty($gbalance)) {
                if (strexists($gbalance, '%')) {
                    $balance += round((float)str_replace('%', '', $gbalance) / 100 * $g['realprice'], 2);
                } else {
        ...
        if (0 < $balance) {
            ...
            if ($type == 1) {
                if ($order['status'] == 3) {
                    m('member')->setCredit($order['openid'], 'credit2', $balance,
                        array(0, $shopset['name'] . '购物赠送余额 订单号: ' . $order['ordersn']));
                }
            } elseif (($type == 2) && 1 <= $order['status']) {
                m('member')->setCredit($order['openid'], 'credit2', 0 - $balance,
                    array(0, $shopset['name'] . '购物取消订单扣除赠送余额 订单号: ' . $order['ordersn']));
            }
        }
    }
    ...
    {
        global $_W;
        $order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,status,isparent,
                paytype,isnewstore,storeid,istrade,status from ' . Order::TABLE_NAME . ' where id=:id limit 1',
            array(':id' => $orderid));
        ...
        $goods = pdo_fetchall('select og.goodsid,og.seckill,og.total,g.totalcnf,og.realprice,g.credit,
                    og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.type
                from ' . OrderGoods::TABLE_NAME . ' og ' . ' left join ' . Goods::TABLE_NAME . ' g on g.id=og.goodsid '
            . (' where ' . $condition . ' and og.uniacid=:uniacid '),
            $param);
        $credits = 0;
        ...
        foreach ($goods as $g) {
            if (0 < $newstoreid) {
                ...
                $goods_item = pdo_fetch('select total as goodstotal from ' . Goods::TABLE_NAME
                    . ' where id=:id and uniacid=:uniacid limit 1',
                    array(':id' => $g['goodsid'], ':uniacid' => $_W['uniacid']));
            ...
            if (!empty($stocktype)) {
                ...
                if (!empty($g['optionid'])) {
                    ...
                    if (0 < $newstoreid) {
                        $store_goods_option = m('store')->getOneStoreGoodsOption($g['optionid'], $g['goodsid'],
                            $newstoreid);
                    ...
                    if (!empty($option) && $option['stock'] != -1) {
                        ...
                        if ($stock != -1) {
                            if (0 < $newstoreid) {
                                pdo_update('ewei_shop_newstore_goods_option', array('stock' => $stock), array(
                                    'uniacid' => $_W['uniacid'],
                                    'goodsid' => $g['goodsid'],
                                    'id' => $store_goods_option['id']
                                ));
                            } else {
                                pdo_update('ewei_shop_goods_option', array('stock' => $stock), array(
                                    'uniacid' => $_W['uniacid'],
                                    'goodsid' => $g['goodsid'],
                                    'id' => $g['optionid']
                                ));
                ...
                if (((!empty($g['goodstotal']) && $stocktype == -1) || $stocktype == 1) && $g['goodstotal'] != -1) {
                    ...
                    if ($totalstock != -1) {
                        if (0 < $newstoreid) {
                            pdo_update('ewei_shop_newstore_goods', array('stotal' => $totalstock),
                                array('uniacid' => $_W['uniacid'], 'id' => $store_goods['id']));
                        } else {
                            pdo_update('ewei_shop_goods', array('total' => $totalstock),
                                array('uniacid' => $_W['uniacid'], 'id' => $g['goodsid']));
                        }
                    }
                }
            }
            ...
            if ($type == 0) {
                ...
                if (0 < $credits && $table_flag) {
                    ...
                    $send_record = pdo_fetch('SELECT * FROM ' . OrderBuySend::TABLE_NAME
                        . ' WHERE orderid = :orderid AND uniacid = :uniacid AND openid = :openid',
                        array(
                            ':orderid' => $orderid,
                            ':uniacid' => $_W['uniacid'],
                            ':openid' => $order['openid']
        ...
        if ($table_flag) {
            $send_record = pdo_fetch('SELECT * FROM ' . OrderBuySend::TABLE_NAME
                . ' WHERE orderid = :orderid AND uniacid = :uniacid AND openid = :openid',
                array(':orderid' => $orderid, ':uniacid' => $_W['uniacid'], ':openid' => $order['openid']));
        ...
        if (0 < $credits) {
            ...
            if ($type == 3) {
                if ($order['status'] == 3 || $flag == true) {
                    m('member')->setCredit($order['openid'], 'credit1', $credits,
                        array(0, $shopset['name'] . '购物积分 订单号: ' . $order['ordersn']));
                    m('notice')->sendMemberPointChange($order['openid'], $credits, 0, 3);
                }
            } elseif (($type == 2) && $order['status'] == 3) {
                m('member')->setCredit($order['openid'], 'credit1', 0 - $credits,
                    array(0, $shopset['name'] . '购物取消订单扣除积分 订单号: ' . $order['ordersn']));
            ...
            $money = com_run('sale::getCredit1', $order['openid'], (double)$order['price'], $order['paytype'],
                1, 1);
    ...
    public function getTotals($merch = 0)
    {
        ...
        $totals['all'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . Order::TABLE_NAME
            . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and deleted=0'),
            $paras);
        $totals['status_1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . Order::TABLE_NAME
            . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and status=-1 and refundtime=0 and deleted=0'),
            $paras);
        $totals['status0'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . Order::TABLE_NAME
            . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and status=0 and paytype<>3 and deleted=0'),
            $paras);
        $totals['status1'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . Order::TABLE_NAME
            . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and ( status=1 or ( status=0 and paytype=3) ) and deleted=0'),
            $paras);
        $totals['status2'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . Order::TABLE_NAME
            . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and ( status=2 or (status = 1 and sendtype > 0) ) and deleted=0'),
            $paras);
        $totals['status3'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . Order::TABLE_NAME
            . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and status=3 and deleted=0'),
            $paras);
        $totals['status4'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . Order::TABLE_NAME
            . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and (refundstate>0 and refundid<>0 or (refundtime=0 and refundstate=3)) and deleted=0'),
            $paras);
        $totals['status5'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . Order::TABLE_NAME
            . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0 and refundtime<>0 and deleted=0'),
            $paras);
        return $totals;
    }
    ...
    public function getFormartDiscountPrice($isd, $gprice, $gtotal = 1)
    {
        ...
        if (!empty($isd)) {
            if (strexists($isd, '%')) {
                $dd = (float)str_replace('%', '', $isd);
                if (0 < $dd && $dd < 100) {
                    $price = round($dd / 100 * $gprice, 2);
                }
            } else {
                if (0 < (float)$isd) {
                    $price = round((float)($isd * $gtotal), 2);
                }
            }
        }
    ...
    public function getGoodsDiscountPrice($g, $level, $type = 0)
    {
        ...
        if (!empty($level['id'])) {
            $level = pdo_fetch('select * from ' . MemberLevel::TABLE_NAME . ' where id=:id and uniacid=:uniacid and enabled=1 limit 1',
                array(':id' => $level['id'], ':uniacid' => $_W['uniacid']));
        ...
        if (empty($g['is_task_goods']) && (0 < (float)$g['buyagain']) && m('goods')->canBuyAgain($g)) {
            $canbuyagain = true;
            ...
            if (is_array($discounts)) {
                ...
                if (!isset($discounts['type']) || empty($discounts['type'])) {
                    if (!empty($discounts[$key])) {
                        $dd = (float)$discounts[$key];
                        if (0 < $dd && $dd < 10) {
                            $price2 = round($dd / 10 * $gprice, 2);
                        }
                    } else {
                        $dd = (float)((empty($discounts[$key . '_pay']) ? 0 : $discounts[$key . '_pay']) * $total);
                        $md = (float)$level['discount'];
    ...
    public function getChildOrderPrice(
        &$order,
        &$goods,
        &$dispatch_array,
        $merch_array,
            ...
            if (0 < $merchid) {
                ...
                if (!empty($enoughs)) {
                    ...
                    foreach ($enoughs as $e) {
                        if ((float)$e['enough'] <= $ggprice && 0 < (float)$e['money']) {
                            ...
                            $merch_enough_total += (float)$e['money'];
                            break;
                        }
                    }
                }
    ...
    public function validate_city_express($address)
    {
        ...
        $city_express = pdo_fetch('SELECT * FROM ' . CityExpress::TABLE_NAME . ' WHERE uniacid=:uniacid and merchid=0 limit 1',
        ...
        if (!empty($city_express['enabled'])) {
            ...
            if (!empty($address)) {
                if (empty($address['lng']) || empty($address['lat'])) {
                    $data = m('util')->geocode($address['province'] . $address['city'] . $address['area'] . $address['street'] . $address['address'],
                        $city_express['geo_key']);
                    if ($data['status'] == 1 && 0 < $data['count']) {
                        $location = explode(',', $data['geocodes'][0]['location']);
                        $addres = $address;
                        $addres['lng'] = $location[0];
                        $addres['lat'] = $location[1];
                        pdo_update('ewei_shop_member_address', $addres,
                            array('id' => $addres['id'], 'uniacid' => $_W['uniacid']));
    ...
    public function compute_express_price($city_express, $lng, $lat)
    {
        ...
        if ($distance < $city_express['range']) {
            ...
            if ($distance <= $city_express['start_km'] * 1000) {
                $city_express_data['price'] = (int)$city_express['start_fee'];
            }

            if ($city_express['start_km'] * 1000 < $distance && $distance <= $city_express['start_km'] * 1000 + $city_express['pre_km'] * 1000) {
                $km = $distance - (int)($city_express['start_km'] * 1000);
                $city_express_data['price'] = (int)($city_express['start_fee'] + $city_express['pre_km_fee'] * ceil($km / 1000));
            }

            if ($city_express['fixed_km'] * 1000 <= $distance) {
                $city_express_data['price'] = (int)$city_express['fixed_fee'];
            }
        }
    ...
    public function getOrderDispatchPrice($goods, $member, $address, $saleset = false, $merch_array, $t, $loop = 0)
    {
        ...
        foreach ($goods as $g) {
            ...
            if (!empty($g['issendfree'])) {
                ...
                if ($seckillinfo && $seckillinfo['status'] == 0) {
                } elseif ((float)$g['edmoney'] <= $totalprice_array[$g['goodsid']] && 0 < (float)$g['edmoney']) {
                    if (empty($new_area)) {
                        $gareas = explode(';', $g['edareas']);
                    } else {
                        $gareas = explode(';', $g['edareas_code']);
            ...
            if ($g['dispatchtype'] == 1) {
                    ...
                    if (!empty($user_city)) {
                        ...
                        if (!empty($citys) && in_array($user_city_code, $citys) && !empty($citys)) {
                            ...
                            if (!empty($nodispatch_array['goodid'])
                                && in_array($g['goodsid'], $nodispatch_array['goodid'])) {
                                $has_goodsid = 1;
                            }
                ...
                } elseif ($city_express_data['state'] == 1) {
                    if (0 < $g['dispatchprice'] && !$sendfree) {
                        if ($city_express_data['is_sum'] == 1) {
                            $dispatch_price += $g['dispatchprice'];
                        } elseif ($dispatch_price < $g['dispatchprice']) {
                            $dispatch_price = $g['dispatchprice'];
                        }
                    }
                } else {
                ...
                if ($city_express_data['state'] == 0 && $city_express_data['is_dispatch'] == 1) {
                    ...
                    if (!empty($dispatch_data)) {
                        ...
                        $isdispatcharea = (int)$dispatch_data['isdispatcharea'];
                        ...
                        if (!empty($user_city)) {
                            if (empty($isdispatcharea)) {
                                if (empty($new_area)) {
                                    ...
                                    $citys = m('dispatch')->getAllNoDispatchAreas($dispatch_data['nodispatchareas_code'],
                                        1);
                                }
                                ...
                                if (empty($isnoarea)) {
                                    $isnoarea = m('dispatch')->checkOnlyDispatchAreas($user_city_code,
                                        $dispatch_data);
                                }
                            }
                    ...
                    if (!$sendfree) {
                        ...
                        } elseif ($dispatch_price < $city_express_data['price']) {
        ...
        if ($city_express_data['state'] == 1 && $g['dispatchtype'] == 0
            && $city_express_data['is_sum'] == 0 && $dispatch_price < $city_express_data['price']) {
            $dispatch_price = $city_express_data['price'];
        }
        ...
        if (!empty($dispatch_array)) {
            ...
            foreach ($dispatch_array as $k => $v) {
                ...
                $dispatch_info[$dispatch_data['id']]['freeprice'] = (int)$dispatch_data['freeprice'];
            }
            ...
            foreach ($merch_array as $key => $value) {
                ...
                if (0 < $merchid) {
                    ...
                    if (!empty($merchset['enoughfree'])) {
                        if ((float)$merchset['enoughorder'] <= 0) {
                            $dispatch_price = $dispatch_price - $dispatch_merch[$merchid];
                            $dispatch_merch[$merchid] = 0;
                        } elseif ((float)$merchset['enoughorder'] <= $merch_array[$merchid]['ggprice']) {
        ...
        if ($saleset && !empty($saleset['enoughfree'])) {
            ...
            if ($loop == 0) {
                if ((float)$saleset['enoughorder'] <= 0) {
                    $saleset_free = 1;
                } elseif ((float)$saleset['enoughorder'] <= $realprice - $seckill_payprice) {
            ...
            if ($saleset_free == 1) {
                ...
                if ($is_nofree == 1 && $loop == 0) {
                    if ($goods_num == 1) {
                        $new_data1 = $this->getOrderDispatchPrice($goods, $member, $address, $saleset, $merch_array,
                            $t, 1);
                        $dispatch_price = $new_data1['dispatch_price'];
                    } else {
                        $new_data2 = $this->getOrderDispatchPrice($new_goods, $member, $address, $saleset,
                            $merch_array, $t, 1);
    ...
    public function changeParentOrderPrice($parent_order)
    {
        global $_W;
        $id = $parent_order['id'];
        $item = pdo_fetch('SELECT price,ordersn2,dispatchprice,changedispatchprice FROM ' . Order::TABLE_NAME . ' WHERE id = :id and uniacid=:uniacid',
    ...
    public function getOrderCommission($orderid, $agentid = 0)
    {
        ...
        if (empty($agentid)) {
            $item = pdo_fetch('select agentid from ' . Order::TABLE_NAME . ' where id=:id and uniacid=:uniacid Limit 1',
        ...
        $order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn,
                    g.productsn,og.productsn as option_productsn, og.total,og.price,og.optionname as optiontitle,
                    og.realprice,og.changeprice,og.oldprice,og.commission1,og.commission2,og.commission3,
                    og.commissions,og.diyformdata,og.diyformfields
                from ' . OrderGoods::TABLE_NAME . ' og '
            . ' left join ' . Goods::TABLE_NAME . ' g on g.id=og.goodsid '
            . ' where og.uniacid=:uniacid and og.orderid=:orderid ',
            array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
            ...
            if (!empty($level) && !empty($agentid)) {
                ...
                if (!empty($m1)) {
                    if (is_array($commissions)) {
                        $commission1 += isset($commissions['level1']) ? (float)$commissions['level1'] : 0;
                    } else {
                ...
                if (!empty($m2)) {
                    if (is_array($commissions)) {
                        $commission2 += isset($commissions['level2']) ? (float)$commissions['level2'] : 0;
                    } else {
                ...
                if (!empty($m3)) {
                    if (is_array($commissions)) {
                        $commission3 += isset($commissions['level3']) ? (float)$commissions['level3'] : 0;
                    } else {
    ...
    public function checkOrderGoods($orderid)
    {
        ...
        $item = pdo_fetch('select * from ' . Order::TABLE_NAME . '  where  id = :id and uniacid=:uniacid limit 1',
            array(':id' => $orderid, ':uniacid' => $uniacid));
        if ((empty($item['isnewstore']) || empty($item['storeid'])) && empty($item['istrade'])) {
            $order_goods = pdo_fetchall('select og.id,g.title, og.goodsid,og.optionid,g.total as stock,
                        og.total as buycount,g.status,g.deleted,g.maxbuy,g.usermaxbuy,g.istime,g.timestart,
                        g.timeend,g.buylevels,g.buygroups,g.totalcnf,og.seckill
                    from  ' . OrderGoods::TABLE_NAME . ' og '
                . ' left join ' . Goods::TABLE_NAME . ' g on og.goodsid = g.id '
                . ' where og.orderid=:orderid and og.uniacid=:uniacid ',
                array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
            ...
            foreach ($order_goods as $data) {
                ...
                $seckillinfo = plugin_run('seckill::getSeckill', $data['goodsid'], $data['optionid'], true,
                    $_W['openid']);
                if (($seckillinfo && $seckillinfo['status'] == 0) || !empty($ispeerpay)) {
                } elseif ($data['totalcnf'] == 1) {
                    if (!empty($data['optionid'])) {
                        $option = pdo_fetch('select id,title,marketprice,goodssn,productsn,stock,`virtual`
                                from ' . GoodsOption::TABLE_NAME
                    ...
                    } elseif (($data['stock'] != -1) && empty($data['stock'])) {
                        $flag = 1;
            ...
            $sql = 'select g.id,g.title,ng.gstatus,g.deleted from ' . OrderGoods::TABLE_NAME . ' og
                        left join  ' . Goods::TABLE_NAME . ' g  on g.id=og.goodsid and g.uniacid=og.uniacid'
                . ' inner join ' . NewstoreGoods::TABLE_NAME . ' ng on ng.goodsid = g.id AND ng.storeid=' . $item['storeid']
                . ' where og.orderid=:orderid and og.uniacid=:uniacid';
    ...
    {
        global $_W;
        $sql = 'SELECT p.*,o.openid FROM ' . OrderPeerpay::TABLE_NAME . ' AS p
                JOIN ' . Order::TABLE_NAME . ' AS o ON p.orderid = o.id
                WHERE p.orderid = :orderid AND p.uniacid = :uniacid AND (p.status = 0 OR p.status=1) AND o.status >= 0 LIMIT 1';
    ...
    public function peerStatus($param)
    {
        ...
        if (!empty($param['tid'])) {
            $sql = 'SELECT id FROM ' . OrderPeerpayPayinfo::TABLE_NAME . ' WHERE tid = :tid';
    ...
    {
        global $_W;
        $num = pdo_fetchcolumn('select SUM(og.total)  from ' . OrderGoods::TABLE_NAME . ' og
		 inner join ' . Goods::TABLE_NAME . ' g on og.goodsid = g.id
    ...
    {
        global $_W;
        $num = pdo_fetchcolumn('select COUNT(1)  from ' . OrderGoods::TABLE_NAME . ' og
		 inner join ' . Goods::TABLE_NAME . ' g on og.goodsid = g.id
        ...
        $num2 = pdo_fetchcolumn('select COUNT(1)  from ' . OrderGoods::TABLE_NAME . ' og
             inner join ' . Goods::TABLE_NAME . ' g on og.goodsid = g.id
             where og.uniacid=:uniacid  and og.orderid =:orderid and g.type=5',
            array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
        $num2 = (int)$num2;
    ...
    {
        global $_W;
        $num = pdo_fetchcolumn('select COUNT(1)  from ' . VerifygoodsLog::TABLE_NAME . ' vl
		 inner join ' . Verifygoods::TABLE_NAME . ' v on vl.verifygoodsid = v.id
    ...
    {
        global $_W;
        $count = (int)substr_count($ordersn, $str);
        return $count;
    }
    ...
    public function getOrderVirtual($order = array())
    {
        ...
        $ordervirtual = array();
        $virtual_type = pdo_fetch('select `fields` from ' . VirtualType::TABLE_NAME
            . ' where id=:id and uniacid=:uniacid and merchid = :merchid limit 1 ',
            array(':id' => $order['virtual'], ':uniacid' => $_W['uniacid'], ':merchid' => $order['merchid']));
    ...
    public function dada_send($order)
    {
        ...
        $cityexpress = pdo_fetch('SELECT * FROM ' . CityExpress::TABLE_NAME
    ...
    public function CheckoodsStock($orderid = '', $type = 0)
    {
        global $_W;
        $order = pdo_fetch('select id,ordersn,price,openid,dispatchtype,addressid,carrier,`status`,isparent,
                paytype,isnewstore,storeid,istrade from ' . Order::TABLE_NAME . ' where id=:id limit 1',
            array(':id' => $orderid));
        ...
        $goods = pdo_fetchall('select og.goodsid,og.total,g.totalcnf,og.realprice,g.credit,og.optionid,
                    g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.type
                from ' . OrderGoods::TABLE_NAME . ' og '
            . ' left join ' . Goods::TABLE_NAME . ' g on g.id=og.goodsid '
            . (' where ' . $condition . ' and og.uniacid=:uniacid '),
            $param);
