...
class Plugin_EweiShopV2Model
{
    ...
    {
        $dbplugin = pdo_fetchall('select * from ' . tablename('ewei_shop_plugin') . ' where identity=:identyty limit  1',
            array(':identity' => $pluginName));
    ...
    public function getList($status = '')
    {
        ...
        if (p('grant')) {
            $acid = pdo_fetch('SELECT acid,uniacid FROM ' . tablename('account_wechats') . ' WHERE uniacid=:uniacid limit 1',
                array(':uniacid' => $_W['uniacid']));
            $item = pdo_fetch('select plugins from ' . tablename('ewei_shop_perm_plugin') . ' where acid=:acid limit 1',
                array(':acid' => $acid['acid']));
            $setting = pdo_fetch('select * from ' . tablename('ewei_shop_system_plugingrant_setting') . ' where 1 = 1 limit 1 ');

            foreach ($plugins as $key => $value) {
                if (!strstr($item['plugins'], $value['identity']) && !strstr($setting['plugin'],
                        $value['identity']) && !strstr($setting['com'], $value['identity'])) {
    ...
    public function permission($pluginName, $uniacid = 0)
    {
        ...
        if ($permset) {
            $plugins = pdo_fetchcolumn('SELECT plugins FROM ' . tablename('ewei_shop_perm_plugin') . ' WHERE acid =:id limit 1',
                array(':id' => $id));
