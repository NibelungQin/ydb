...
class Shop_EweiShopV2Model
{
    ...
    public function getCategory($refresh = false)
    {
        ...
        if (empty($allcategory) || $refresh) {
            $parents = array();
            $children = array();
            $category = pdo_fetchall('SELECT * FROM ' . Category::TABLE_NAME
                . ' WHERE uniacid =:uniacid AND enabled=1 ORDER BY parentid ASC, displayorder DESC',
    ...
    public function getFullCategory($fullname = false, $enabled = false)
    {
        ...
        $allcategory = array();
        $sql = 'SELECT * FROM ' . Category::TABLE_NAME . ' WHERE uniacid=:uniacid ';
    ...
    public function getAllCategory($refresh = false)
    {
        ...
        if (empty($allcategory) || $refresh) {
            $allcategory = pdo_fetchall('SELECT id,parentid,uniacid,name,thumb FROM ' . Category::TABLE_NAME
