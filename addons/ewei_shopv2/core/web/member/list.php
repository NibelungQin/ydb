...
class Member_List_EweiShopV2WebPage extends WebPage {
    ...
    public function detail() {
        ...
        if ($hasabonus) {
            ...
            $member['aagenttowns'] = iunserializer($member['aagenttowns']);
        }
        ...
        if ($_W['ispost']) {
            $plugin_com->upgradeLevelByAgent($member['id']);
                    ...
                    if (!empty($adata)) {
                        ...
                        if ($adata['agentid'] != $member['agentid']) {
                            if (cv('commission.agent.changeagent')) {
                                $qr_member = m('member')->getMember($adata['agentid']);
                                //如果可以修改上级信息
                                $plugin_com->promoter_qr($member, $qr_member);
            ...
            if ($hasglobonus) {
                if (cv('globonus.partner.check')) {
                    ...
                    if (!empty($gdata)) {
                        ...
                        p('globonus')->upgradeLevelByAgent($member['openid']);
                    }
                }
            }
            ...
            if ($hasabonus) {
                if (cv('abonus.agent.check')) {
                    ...
                    if (!empty($aadata)) {
                        ...
                        $aagenttowns = is_array($_GPC['aagenttowns']) ? $_GPC['aagenttowns'] : array();
                        ...
                        $aadata['aagenttowns'] = iserializer($aagenttowns);
                        ...
                        if (is_array($_GPC['aagenttowns'])) {
                            $log .= '<br/>代理乡镇:' . implode(',', $_GPC['aagenttowns']);
                        }
