...
class Order_Index_EweiShopV2WebPage extends WebPage
{
    ...
    protected function selectOrderPrice($day = 0, $is_all = false, $is_avg = false)
    {
        ...
        $where = ' and (( status > 0 and (paytime between :createtime1 and :createtime2))
         			or ((createtime between :createtime1 and :createtime2 ) and status>=0 and paytype=3))';
        ...
        $sql = 'select id,price,openid,' . $time . ' from ' . tablename('ewei_shop_order')
            . ' where uniacid = :uniacid and ismr = 0 and isparent = 0 and deleted=0 ' . $where;
