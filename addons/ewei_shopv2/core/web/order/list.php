...
use Ydb\Entity\Manual\Dispatch;
...
use Ydb\Entity\Manual\MerchSaler;
...
class Order_List_EweiShopV2WebPage extends WebPage
{
    public function test()
    {
        $list = pdo_fetchall('SELECT id,agentid from ' . Member::TABLE_NAME . ' WHERE agentid>0', []);
        foreach ($list as $k => $v) {
            $member = m('member')->getInfo($v['id']);
            $qr_member = m('member')->getInfo($v['agentid']);
    ...
    protected function orderData($status, $st)
    {
        ...
        $sendtype = $_GPC['sendtype'] ?? 0;
        $condition = ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0
         				and o.isparent=0 and o.istrade=0 and o.iscycelbuy=0';
        $uniacid = $_W['uniacid'];
        ...
        if (!empty($searchtime) && is_array($_GPC['time'])
            && in_array($searchtime, array('create', 'pay', 'send', 'finish'))) {
            $starttime = strtotime($_GPC['time']['start']);
            $endtime = strtotime($_GPC['time']['end']);
            $condition .= ' AND o.' . $searchtime . 'time >= :starttime AND o.' . $searchtime . 'time <= :endtime ';
            $paras[':starttime'] = $starttime;
            $paras[':endtime'] = $endtime;
            $priceCondition .= ' AND o.' . $searchtime . 'time >= ' . $starttime
                . ' AND o.' . $searchtime . 'time <= ' . $endtime . ' ';
            $timeCondition .= ' AND o.' . $searchtime . 'time >= ' . $starttime
                . ' AND o.' . $searchtime . 'time <= ' . $endtime . ' ';
        ...
        if ($_GPC['paytype'] != '') {
            ...
            } elseif ($_GPC['paytype'] == '4') {
                $condition .= ' AND o.paytype = 3 AND is_cashier = 1 ';
                $priceCondition .= ' AND o.paytype = 3 AND is_cashier = 1 ';
            } else {
                $condition .= ' AND o.paytype =' . (int)$_GPC['paytype'];
                $priceCondition .= ' AND o.paytype =' . (int)$_GPC['paytype'];
            }
        }
        ...
        if (!empty($_GPC['searchfield']) && !empty($_GPC['keyword'])) {
            ...
            if ($searchfield == 'ordersn') {
                ...
                $condition .= ' AND ( locate(:keyword,a.realname)>0 or locate(:keyword,a.mobile)>0
                 					or locate(:keyword,o.carrier)>0 or locate(:keyword,o.address)>0)';
                $priceCondition .= ' AND ( locate(\'' . $_GPC['keyword'] . '\',a.realname)>0
                 					or locate(\'' . $_GPC['keyword'] . '\',a.mobile)>0
                 					or locate(\'' . $_GPC['keyword'] . '\',o.carrier)>0
                 					or locate(\'' . $_GPC['keyword'] . '\',o.address)>0)';
            } elseif ($searchfield == 'location') {
                $condition .= ' AND ( locate(:keyword,o.address)>0 or locate(:keyword,o.address_send)>0 )';
                $priceCondition .= ' AND (o.address LIKE \'%' . $_GPC['keyword'] . '%\'
                 						OR o.address_send LIKE \'%' . $_GPC['keyword'] . '%\' ) ';
            } elseif ($searchfield == 'expresssn') {
                $condition .= ' AND locate(:keyword,o.expresssn)>0';
            } elseif ($searchfield == 'saler') {
                $condition .= ' AND (locate(:keyword,sm.realname)>0 or locate(:keyword,sm.mobile)>0
                 					or locate(:keyword,sm.nickname)>0 or locate(:keyword,s.salername)>0 )';
            } elseif ($searchfield == 'verifycode') {
                $condition .= ' AND (verifycode=:keyword or locate(:keyword,o.verifycodes)>0)';
            } elseif ($searchfield == 'store') {
                $condition .= ' AND (locate(:keyword,store.storename)>0 or locate(:keyword,merstore.storename)>0)';
                $sqlcondition = ' left join ' . tablename('ewei_shop_store') . ' store
                 					on store.id = o.verifystoreid and store.uniacid=o.uniacid
                 				  left join ' . tablename('ewei_shop_merch_store') . ' merstore
                 				    on merstore.id = o.verifystoreid and merstore.uniacid=o.uniacid';
            } elseif ($searchfield == 'goodstitle') {
                $sqlcondition = ' inner join ( select DISTINCT(og.orderid) from ' . tablename('ewei_shop_order_goods') . ' og
                 					left join ' . tablename('ewei_shop_goods') . (' g on g.id=og.goodsid
                 				where og.uniacid = \'' . $uniacid . '\'
                 				 	and (locate(:keyword,g.title)>0)) gs on gs.orderid=o.id');
            } elseif ($searchfield == 'goodssn') {
                $sqlcondition = ' inner join ( select DISTINCT(og.orderid) from ' . tablename('ewei_shop_order_goods') . ' og
                 					left join ' . tablename('ewei_shop_goods') . (' g on g.id=og.goodsid
                 				where og.uniacid = \'' . $uniacid . '\' and (((locate(:keyword,g.goodssn)>0))
                 				 	or (locate(:keyword,og.goodssn)>0))) gs on gs.orderid=o.id');
            } elseif ($searchfield == 'goodsoptiontitle') {
                $sqlcondition = ' inner join ( select  DISTINCT(og.orderid) from ' . tablename('ewei_shop_order_goods') . ' og
                 					left join ' . tablename('ewei_shop_goods') . (' g on g.id=og.goodsid
                 				where og.uniacid = \'' . $uniacid . '\'
                 				 	and (locate(:keyword,og.optionname)>0)) gs on gs.orderid=o.id');
            } elseif ($searchfield == 'merch') {
                if ($merch_plugin) {
                    $condition .= ' AND (locate(:keyword,merch.merchname)>0)';
                    $sqlcondition = ' left join ' . tablename('ewei_shop_merch_user') . ' merch
                     					on merch.id = o.merchid and merch.uniacid=o.uniacid';
                }
            } else {
                if ($searchfield == 'selfget') {
                    $condition .= ' AND (locate(:keyword,store.storename)>0)';
                    $sqlcondition = ' left join ' . tablename('ewei_shop_store') . ' store
                     					on store.id = o.storeid and store.uniacid=o.uniacid';
                }
            }
        }
        ...
        $statuscondition = '';
        if ($status !== '') {
            $statuscondition = OrderRepository::generateStatusConditionSQL($status);
            $priceStatus = $statuscondition;
        }

        $agentid = (int)$_GPC['agentid'];
        $p = p('commission');
        $level = 0;

        if ($p) {
            $cset = $p->getSet();
            $level = (int)$cset['level'];
        }

        $olevel = (int)$_GPC['olevel'];
        if (!empty($agentid) && 0 < $level) {
            ...
            if (empty($olevel)) {
                if (1 <= $level) {
                    $condition .= ' and  ( o.agentid=' . (int)$_GPC['agentid'];
                }
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 '
            || !empty($sqlcondition)) {
            if ($searchfield == 'member') {
                ...
                $sql = 'select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince,
       						a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,
                      		d.dispatchname,m.nickname,m.id as mid,m.realname as mrealname,m.mobile as mmobile,m.uid,
                      		r.rtype,r.status as rstatus,o.sendtype,o.city_express_state
					  	from ' . Order::TABLE_NAME . ' o '
                    . ' left join ' . OrderRefund::TABLE_NAME . ' r on r.id =o.refundid '
                    . ' left join ' . Member::TABLE_NAME . ' m on m.openid=o.openid and m.uniacid =  o.uniacid '
                    . ' left join ' . MemberAddress::TABLE_NAME . ' a on a.id=o.addressid '
                    . ' left join ' . Dispatch::TABLE_NAME . ' d on d.id = o.dispatchid '
                    . (' ' . $sqlcondition . ' where ' . $condition . ' ' . $statuscondition
                        . ' ORDER BY ' . $orderbuy . ' DESC  ');
            } elseif ($searchfield == 'saler') {
                $condition .= ' AND (locate(:keyword,sm.realname)>0 or locate(:keyword,sm.mobile)>0
                 			or locate(:keyword,sm.nickname)>0 or locate(:keyword,s.salername)>0 )';
                $sql = 'select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince,
       						a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,
                      		d.dispatchname, r.rtype,r.status as rstatus,o.sendtype,o.city_express_state
						from ' . Order::TABLE_NAME . ' o '
                    . ' left join ' . OrderRefund::TABLE_NAME . ' r on r.id =o.refundid '
                    . ' left join ' . MemberAddress::TABLE_NAME . ' a on a.id=o.addressid '
                    . ' left join ' . Dispatch::TABLE_NAME . ' d on d.id = o.dispatchid '
                    . ' left join ' . Saler::TABLE_NAME . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid '
                    . ' left join ' . Member::TABLE_NAME . ' sm on sm.openid = s.openid and sm.uniacid=s.uniacid '
                    . (' ' . $sqlcondition . ' where ' . $condition . ' ' . $statuscondition
                        . ' ORDER BY ' . $orderbuy . ' DESC  ');
            } elseif ($searchfield == 'mid') {
                $paras[':keyword'] = htmlspecialchars_decode($_GPC['keyword'], ENT_QUOTES);
            ...
            $sql = 'select * from ' . Order::TABLE_NAME
            ...
            if (!empty($list)) {
                ...
                $refundid_array = pdo_fetchall('SELECT id,rtype,status as rstatus
						FROM ' . OrderRefund::TABLE_NAME . (' WHERE id IN (' . $refundid . ')'),
                    array(), 'id');
                $openid_array = pdo_fetchall('SELECT openid,nickname,id as mid,realname as mrealname,uid,
       						mobile as mmobile
						FROM ' . Member::TABLE_NAME
                    . (' WHERE openid IN (' . $openid . ') AND uniacid=' . $_W['uniacid']),
                    array(), 'openid');
                $addressid_array = pdo_fetchall('SELECT id,realname as arealname,mobile as amobile,
       						province as aprovince ,city as acity , area as aarea,address as aaddress
						FROM ' . MemberAddress::TABLE_NAME
                    . (' WHERE id IN (' . $addressid . ')'),
                    array(), 'id');
                $dispatchid_array = pdo_fetchall('SELECT id,dispatchname
						FROM ' . Dispatch::TABLE_NAME . (' WHERE id IN (' . $dispatchid . ')'),
                    array(), 'id');
                $verifyopenid_array = pdo_fetchall('SELECT sm.id as salerid,sm.nickname as salernickname,
       						sm.openid,s.salername
						FROM ' . Saler::TABLE_NAME . ' s
						LEFT JOIN ' . Member::TABLE_NAME . (' sm
						 	ON sm.openid = s.openid and sm.uniacid=s.uniacid
						WHERE s.openid IN (' . $verifyopenid . ')'),
                    array(), 'openid');
        ...
        if (!empty($list)) {
            ...
            foreach ($list as &$value) {
                ...
                if ($is_merchname == 1) {
                    $value['merchname'] = $merch_user[$value['merchid']]['merchname']
                        ? $merch_user[$value['merchid']]['merchname'] : '';
                }
                ...
                if ($s == 1) {
                    ...
                    } elseif (empty($value['addressid'])) {
                        $value['status'] = '待取货';
                    } elseif (0 < $value['sendtype']) {
                        $value['status'] = '部分发货';
                    }
                }
                ...
                if ($s == -1) {
                    ...
                    if (!empty($value['refundid']) && !empty($_GPC['export'])) {
                        $applyprice = pdo_fetchcolumn('select applyprice
									from ' . OrderRefund::TABLE_NAME
                            . ' where id=:id and uniacid=:uniacid limit 1',
                            array(':id' => $value['refundid'], ':uniacid' => $_W['uniacid']));
                    }
                }
                ...
                if ($value['dispatchtype'] == 1 || !empty($value['isverify'])
                    || !empty($value['virtual']) || !empty($value['isvirtual'])) {
                    $value['address'] = '';
                ...
                $order_goods = pdo_fetchall('select g.id,g.title,og.title as gtitle,g.thumb,g.invoice,g.goodssn,
                    			...
                    			og.price,og.optionname as optiontitle, og.realprice,og.changeprice,og.oldprice,
       							og.commission1,og.commission2,og.commission3,og.commissions,og.diyformdata,
       							og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,
       							g.ispresell,g.costprice,op.costprice as option_costprice,og.expresssn,og.expresscom,
       							og.express,og.sendtype,g.status as giftstatus,og.single_refundid,og.single_refundstate,
       							og.id as ogid,og.nocommission
							from ' . OrderGoods::TABLE_NAME . ' og '
                    . ' left join ' . Goods::TABLE_NAME . ' g on g.id=og.goodsid '
                    . ' left join ' . GoodsOption::TABLE_NAME . ' op on og.optionid = op.id '
                    . ' where og.uniacid=:uniacid and og.orderid=:orderid order by og.single_refundstate desc ',
                ...
                foreach ($order_goods as &$og) {
                    ...
                    if ($og['seckill']) {
                        ...
                        $og['seckill_room'] = plugin_run('seckill::getRoomInfo', $og['seckill_taskid'],
                            $og['seckill_roomid']);
                    }
...
';
                    ...
                    $goods .= ' 单价: ' . $og['price'] / $og['total'] . ' 折扣后: ' . $og['realprice'] / $og['total']
                        . ' 数量: ' . $og['total'] . ' 总价: ' . $og['price'] . ' 折扣后: ' . $og['realprice'] . '
 ';
                    if (p('diyform') && !empty($og['diyformfields']) && !empty($og['diyformdata'])) {
                        $diyformdata_array = p('diyform')->getDatas(iunserializer($og['diyformfields']),
                            iunserializer($og['diyformdata']), 1);
                        $diyformdata = '';
                        $dflag = 1;
...
';
                    ...
                    if ($value['agentid'] == $agentid) {
                        ...
                        foreach ($level1_commissions as $c) {
                            ...
                            if (empty($commissions)) {
                                $commission_level += $commission['level' . $agentLevel['id']] ?? $commission['default'];
                            } else {
                                $commission_level += isset($commissions['level1']) ? (float)$commissions['level1'] : 0;
                            }
                        }
                    } elseif (array_key_exists($value['agentid'], $agent['level1_agentids'])) {
                        $value['level'] = 2;

                        if (0 < $agent['level2']) {
                            $level2_commissions = pdo_fetchall('select commission2,commissions
										from ' . OrderGoods::TABLE_NAME . ' og '
                                . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                                . ' where og.orderid=:orderid and  o.agentid in ( ' . implode(',',
                                    array_keys($agent['level1_agentids'])) . ')  and o.uniacid=:uniacid',
                                array(':orderid' => $value['id'], ':uniacid' => $uniacid));
                            ...
                            foreach ($level2_commissions as $c) {
                                ...
                                if (empty($commissions)) {
                                    ...
                                    $commission_level += isset($commissions['level2']) ? (float)$commissions['level2'] : 0;
                                }
                            }
                        }
                    } elseif (array_key_exists($value['agentid'], $agent['level2_agentids'])) {
                        $value['level'] = 3;

                        if (0 < $agent['level3']) {
                            $level3_commissions = pdo_fetchall('select commission3,commissions
                                        from ' . OrderGoods::TABLE_NAME . ' og '
                                . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                                . ' where og.orderid=:orderid and  o.agentid in ( ' . implode(',',
                                    array_keys($agent['level2_agentids'])) . ')  and o.uniacid=:uniacid',
                                array(':orderid' => $value['id'], ':uniacid' => $uniacid));
                            ...
                            foreach ($level3_commissions as $c) {
                                ...
                                if (empty($commissions)) {
                                    ...
                                    $commission_level += isset($commissions['level3']) ? (float)$commissions['level3'] : 0;
                                }
                            }
                        }
                    }
            ...
            $omember = pdo_fetchall('select openid,nickname,id as mid,realname as mrealname,mobile as mmobile,uid
					from ' . Member::TABLE_NAME
                . ' where openid in(' . $oopenid . ') and uniacid = :uniacid',
                array(':uniacid' => $_W['uniacid']), 'openid');
            $verifyopenid_array = pdo_fetchall('SELECT sm.id as salerid,sm.nickname as salernickname,
       					sm.openid,s.salername
					FROM ' . Saler::TABLE_NAME . ' s
					 	LEFT JOIN ' . Member::TABLE_NAME . (' sm ON sm.openid = s.openid and sm.uniacid=s.uniacid
					WHERE s.openid IN (' . $verifyopenid . ') and s.uniacid = :uniacid'),
                array(':uniacid' => $_W['uniacid']), 'openid');
        ...
        if ($_GPC['export'] == 1) {
            ...
            foreach ($list as &$row) {
                ...
                $row['expresssn'] = strexists($row['expresssn'],
                    ...
                    if (!empty($verifyinfo)) {
                        foreach ($verifyinfo as $k => $v) {
                            ...
                            if (!empty($verifyopenid)) {
                                ...
                                $row['salerinfo'] .= '[' . $verify_member['salerid'] . ']'
                                    . $verify_member['salername'] . '(' . $verify_member['salernickname'] . ')';
                            }
                        }
                    }
                }
                ...
                if (!empty($row['verifystoreid'])) {
                    if (0 < $row['merchid']) {
                        $row['storeinfo'] = pdo_fetchcolumn('select storename
									from ' . MerchStore::TABLE_NAME
                            . ' where id=:storeid limit 1 ',
                            array(':storeid' => $row['verifystoreid']));
                    } else {
                        $row['storeinfo'] = pdo_fetchcolumn('select storename
                ...
                if (!empty($row['storeid'])) {
                    if (0 < $row['merchid']) {
                        $row['pickname'] = pdo_fetchcolumn('select storename
								from ' . MerchStore::TABLE_NAME
                            . ' where id=:storeid limit 1 ',
                ...
                if (p('diyform') && !empty($row['diyformfields']) && !empty($row['diyformdata'])) {
                    $diyformdata_array = p('diyform')->getDatas(iunserializer($row['diyformfields']),
                        iunserializer($row['diyformdata']));
                    $diyformdata = '';
...
';
                ...
                if ($row['isverify'] && is_array($verifyinfo) && empty($row['dispatchtype'])) {
                    ...
                    if ($v['verified'] || $row['verifytype'] == 1) {
                        $v['storename'] = pdo_fetchcolumn('select storename
                                    from ' . MerchStore::TABLE_NAME
                            . ' where id=:id limit 1',
                            array(':id' => $v['verifystoreid']));
                        ...
                        $v['nickname'] = pdo_fetchcolumn('select nickname
                                    from ' . Member::TABLE_NAME
                            . ' where openid=:openid and uniacid=:uniacid limit 1',
                            array(':openid' => $v['verifyopenid'], ':uniacid' => $_W['uniacid']));
                        $v['salername'] = pdo_fetchcolumn('select salername
                                    from ' . MerchSaler::TABLE_NAME
                            . ' where openid=:openid and uniacid=:uniacid and merchid = :merchid limit 1',
                            array(
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0'
            || !empty($sqlcondition)) {
            if ($searchfield == 'member') {
                $priceCondition .= ' AND (nickname LIKE \'' . $_GPC['keyword'] . '%\'
                 			OR realname LIKE \'' . $_GPC['keyword'] . '%\'
                 			OR mobile LIKE \'' . $_GPC['keyword'] . '%\') ';
                $priceCondition1 = ' AND (nickname LIKE \'' . $_GPC['keyword'] . '%\'
                 			OR realname LIKE \'' . $_GPC['keyword'] . '%\'
                 			OR mobile LIKE \'' . $_GPC['keyword'] . '%\') ';
                $openidArr = pdo_fetchall('SELECT openid FROM ' . Member::TABLE_NAME
                    . ' WHERE uniacid = ' . $_W['uniacid'] . $priceCondition1);
                ...
                if (!empty($openidArr)) {
                    ...
                    $orderPrice = pdo_fetch('SELECT count(1) as count,sum(price) as sumprice
							FROM ' . Order::TABLE_NAME . ' o
							WHERE o.uniacid = ' . $_W['uniacid'] . ' AND o.deleted=0 AND o.isparent=0
							 	AND o.openid IN (' . $inOpenid . ')' . $timeCondition . $statuscondition);
                } else {
                ...
                $orderPrice = pdo_fetch('SELECT count(1) as count,sum(o.price) as sumprice
							FROM ' . Order::TABLE_NAME . ' o
							left join ' . MemberAddress::TABLE_NAME . ' a on o.addressid = a.id
							WHERE o.uniacid = ' . $_W['uniacid'] . '  AND o.deleted=0
							 	AND o.isparent=0 ' . $priceCondition . $statuscondition);
                $totalmoney = $orderPrice['sumprice'];
                $total = $orderPrice['count'];
            ...
            } elseif ($status === '' && empty($_GPC['keyword']) && empty($_GPC['time'])) {
                $redis = redis();
                ...
                if (!is_error($redis)) {
                    if ($redis->get($redis_key_total) != false && $redis->get($redis_key_totalmoney) != false) {
                        ...
                        $t = pdo_fetch('SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice
                                FROM ' . Order::TABLE_NAME . ' o '
                            . ' left join ' . OrderRefund::TABLE_NAME . ' r on r.id =o.refundid '
                            . ' left join ' . Saler::TABLE_NAME . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid'
                            . ' left join ' . Member::TABLE_NAME . ' m on m.openid = s.openid and m.uniacid=s.uniacid' . (' ' . $sqlcondition
            ...
            $t = pdo_fetch('SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice
        ...
        $stores = pdo_fetchall('select id,storename
					from ' . Store::TABLE_NAME . ' where uniacid=:uniacid ',
    ...
    public function updateChildOrderPay()
    {
        ...
        $sql = 'select id,parentid from ' . Order::TABLE_NAME
            . ' where parentid>0 and status>0 and paytype=0 and uniacid=:uniacid';
        $list = pdo_fetchall($sql, $params);

        if (!empty($list)) {
            foreach ($list as $k => $v) {
                $params[':orderid'] = $v['parentid'];
                $sql1 = 'select paytype from ' . Order::TABLE_NAME
                    . ' where id=:orderid and status>0 and paytype>0 and uniacid=:uniacid';
