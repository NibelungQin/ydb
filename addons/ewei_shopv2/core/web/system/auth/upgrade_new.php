...
define('EWEI_SHOPV2_NEW_AUTH_URL', 'http://service.yidaoit.com');
...
class System_Auth_Upgrade_new_EweiShopV2WebPage extends SystemPage
{
	public function main() 
	{
		...
		if (!(empty($result['result']['auth_date_end']))) 
		{
			...
			else if (!(empty($residue)) && ($residue < 0)) 
			{
	...
	public function process() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			switch ($type) 
			{
				...
				break;
				case 'files': $resp = ihttp_post(EWEI_SHOPV2_NEW_AUTH_URL . '/download', array('ip' => $auth['ip'], 'id' => $auth['id'], 'code' => $auth['code'], 'domain' => trim(preg_replace('/http(s)?:\\/\\//', '', rtrim($_W['siteroot'], '/'))), 'md5' => $content));
				...
				break;
				case 'upgrades': if (empty($content) || ($content == 'success')) 
				{
