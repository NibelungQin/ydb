...
if (!(class_exists('AbonusModel'))) {
    class AbonusModel extends PluginModel
    {
        ...
        public function getBonus($openid = '', $params = array())
        {
            ...
            if (in_array('ok4', $params)) {
                $ret['ok4'] = pdo_fetchcolumn('select ifnull(sum(paymoney4),0)   from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and status=1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            ...
            if (in_array('lock4', $params)) {
                $ret['lock3'] = pdo_fetchcolumn('select ifnull(sum(paymoney4),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and status<>1 and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            if (in_array('total', $params)) {
                $ret['total'] = pdo_fetchcolumn('select ifnull(sum(paymoney1),0)+ifnull(sum(paymoney2),0)  + ifnull(sum(paymoney3),0) + ifnull(sum(paymoney4),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            ...
            if (in_array('total4', $params)) {
                $ret['total4'] = pdo_fetchcolumn('select ifnull(sum(paymoney4),0)  from ' . tablename('ewei_shop_abonus_billp') . ' where openid=:openid and uniacid=:uniacid limit 1', array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
            }
            return $ret;
        }
        /**
        ...
        public function upgradeLevelByOrder($openid)
        {
            ...
            while ($leveltype_arr) {
                ...
                if (($leveltype == 4) || ($leveltype == 5)) {
                    ...
                    if ($leveltype == 4) {
                        $newlevel = pdo_fetch('select * from ' . tablename('ewei_shop_abonus_level') . ' where uniacid=:uniacid  and ' . $ordermoney . ' >= zg_ordermoney and zg_ordermoney>0  order by zg_ordermoney desc limit 1', array(':uniacid' => $_W['uniacid']));
                        if (empty($newlevel)) {
                            continue;
                        }
                        if (!(empty($oldlevel['id']))) {
                            ...
                            if ($newlevel['zg_ordermoney'] < $oldlevel['zg_ordermoney']) {
                                continue;
                            }
                        }
                    } else if ($leveltype == 5) {
                        $newlevel = pdo_fetch('select * from ' . tablename('ewei_shop_abonus_level') . ' where uniacid=:uniacid  and ' . $ordercount . ' >= zg_ordercount and zg_ordercount>0  order by zg_ordercount desc limit 1', array(':uniacid' => $_W['uniacid']));
                        if (empty($newlevel)) {
                            continue;
                        }
                        if (!(empty($oldlevel['id']))) {
                            ...
                            if ($newlevel['zg_ordercount'] < $oldlevel['zg_ordercount']) {
                                continue;
                            }
                        }
                    }
                    ...
                    foreach ($agents as $agent) {
                        ...
                        if ($leveltype == 0) {
                            ...
                            $newlevel = pdo_fetch('select * from ' . tablename('ewei_shop_abonus_level') . ' where uniacid=:uniacid and ' . $ordermoney . ' >= first_ordermoney and first_ordermoney>0  order by first_ordermoney desc limit 1', array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                                continue;
                            }
                            if (!(empty($oldlevel['id']))) {
                            ...
                            $newlevel = pdo_fetch('select * from ' . tablename('ewei_shop_abonus_level') . ' where uniacid=:uniacid  and ' . $ordercount . ' >= first_ordercount and first_ordercount>0  order by first_ordercount desc limit 1', array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                                continue;
                            }
                            if (!(empty($oldlevel['id']))) {
                ...
                if (($leveltype == 6) || ($leveltype == 8)) {
                    ...
                    foreach ($agents as $agent) {
                        ...
                        if ($leveltype == 6) {
                            ...
                            $newlevel = pdo_fetch('select * from ' . tablename('ewei_shop_abonus_level') . ' where uniacid=:uniacid  and ' . $downcount . ' >= c_downcount and c_downcount>0  order by c_downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
                            if (!(empty($oldlevel['id']))) {
                    ...
                    if ($leveltype == 7) {
                        ...
                        $newlevel = pdo_fetch('select * from ' . tablename('ewei_shop_abonus_level') . ' where uniacid=:uniacid  and ' . $downcount . ' >= first_downcount and first_downcount>0  order by first_downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
                        if (!(empty($oldlevel['id']))) {
                        ...
                        $newlevel = pdo_fetch('select * from ' . tablename('ewei_shop_abonus_level') . ' where uniacid=:uniacid  and ' . $downcount . ' >= first_c_downcount and first_c_downcount>0  order by first_c_downcount desc limit 1', array(':uniacid' => $_W['uniacid']));
                        if (!(empty($oldlevel['id']))) {
        ...
        public function getBonusData($year = 0, $month = 0, $week = 0, $openid = '')
        {
            ...
            $ordermoney = 0;
            $bonusmoney = 0;
            $orders = pdo_fetchall('select id,openid,price,addressid,costprice, address from ' . tablename('ewei_shop_order') . ' where uniacid=' . $_W['uniacid'] . ' and addressid<>0 and status=3  and isabonus=0 and finishtime + ' . $settletimes . '>= ' . $starttime . ' and  finishtime + ' . $settletimes . '<=' . $endtime, array(), 'id');
            $pcondition = '';
            if (!(empty($openid))) {
            ...
            foreach ($orders as $o) {
                ...
                if ($commission['calcutype'] == 2) {
                    //如果按订单利润计算佣金
                    $reward_share = ($o['price'] - $o['costprice']) * $set['bonusrate'] / 100;
                }
                foreach ($aagents as &$a) {
                    if (empty($set['selfbuy'])) {
                    ...
                    if ($a['aagenttype'] == 1) {
                        if (in_array($address['province'], $a['aagentprovinces'])) {
                            $bonusmoney1 = round(($reward_share * $a['bonus1']) / 100, 2);
                        }
                        if (in_array($address['province'] . $address['city'], $a['aagentcitys'])) {
                            $bonusmoney2 = round(($reward_share * $a['bonus2']) / 100, 2);
                        }
                        if (in_array($address['province'] . $address['city'] . $address['area'], $a['aagentareas'])) {
                            $bonusmoney3 = round(($reward_share * $a['bonus3']) / 100, 2);
                        }
                    } else if ($a['aagenttype'] == 2) {
                        if (in_array($address['province'] . $address['city'], $a['aagentcitys'])) {
                            $bonusmoney2 = round(($reward_share * $a['bonus2']) / 100, 2);
       ...
       * */
        public function packagegoods_getBonusData($openid, $orderid,$is_finish=0)
        {
            global $_W;
            $set = $this->getSet();
            //如果没有设置区域奖励比例或者区域奖励比例为0
            if (empty($set['bonusrate']) || ($set['bonusrate'] <= 0)) {
                $set['bonusrate'] = 100;
            }
            //查询是否已生成奖励信息
            $log = pdo_fetchall('SELECT id,openid,bonusmoney FROM ' . tablename('ewei_shop_packagegoods_abonus_log') . ' WHERE uniacid=:uniacid AND orderid=:orderid ',array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid));
            if($is_finish==1){
                $result_arr=[];
                foreach ($log as $k=>$v){
            ...
            //查询订单
            $orders = pdo_fetch('select o.id,o.addressid,o.address,o.openid,o.price,o.freight,g.abonus_type,g.abonus from '
                . tablename('ewei_shop_packagegoods_order') . ' o ' . ' left join ' . tablename('ewei_shop_packagegoods_goods') . ' g on g.id=o.goodid '
                . ' where o.uniacid=:uniacid and o.id=:orderid and o.isabonus=0 and o.openid=:openid  ', array(':uniacid' => $_W['uniacid'], ':orderid' => $orderid, ':openid' => $openid)
            );
            //获取全部区域代理信息
            $aagents = pdo_fetchall('select l.id as l_id,m.id,m.openid,m.aagentlevel,m.aagenttype,m.aagentprovinces,m.aagentcitys,m.aagentareas, l.bonus1,l.bonus2,l.bonus3 from ' . tablename('ewei_shop_member') . ' m ' . '  left join ' . tablename('ewei_shop_abonus_level') . ' l on l.id = m.aagentlevel ' . '  where m.uniacid=:uniacid and  m.isaagent=1 and m.aagentstatus=1 ', array(':uniacid' => $_W['uniacid']));
            $address = iunserializer($orders['address']);
            if (!(is_array($address))) {
                $address = pdo_fetch('select * from ' . tablename('ewei_shop_member_address') . ' where id=:id limit 1', array(':id' => $orders['addressid']));
            }
            if (empty($address) || !(is_array($address))) {
                return;
            }
            $reward_share = ($orders['price'] * $set['bonusrate']) / 100;//区域奖励金额
            $money_arr = array();//佣金数组
            $abonus = unserialize($orders['abonus']);
            foreach ($aagents as &$a) {
                $a['money'] = 0;
                if (empty($set['selfbuy'])) {
                    if ($a['openid'] == $orders['openid']) {
                        return;
                    }
                }
                foreach ($abonus as $key => $val) {
                    if ($val['level'] == 0) {
                        $a['bonus1'] = $set['bonus1'];
                        $a['bonus2'] = $set['bonus2'];
                        $a['bonus3'] = $set['bonus3'];
                ...
                if ($a['aagenttype'] == 1) {
                    ...
                    if (in_array($address['province'], $a['aagentprovinces'])) {
                        //如果地址属于此代理的范围
                        $a['money'] = round(($reward_share * $a['bonus1']) / 100, 2);
                    }
                    //如果代理商是市级代理
                    if (in_array($address['province'] . $address['city'], $a['aagentcitys'])) {
                        $a['money'] += round(($reward_share * $a['bonus2']) / 100, 2);
                    }
                    //如果代理商是县级代理
                    if (in_array($address['province'] . $address['city'] . $address['area'], $a['aagentareas'])) {
                        $a['money'] += round(($reward_share * $a['bonus3']) / 100, 2);
                    }
                } else if ($a['aagenttype'] == 2) {
                    //如果代理商是市级代理
                    if (in_array($address['province'] . $address['city'], $a['aagentcitys'])) {
                ...
                if ($a['money']) {
                    $all_money = $this->floor_decimals($a['money'], 2);            //四舍五入取两位小数
                    $money_arr[$a['openid']] = $all_money;
                }
            }
            $result_arr = array();
