...
class Index_EweiShopV2Page extends PluginWebPage 
{
	...
	{
		global $_W;
	    if (cv('achievement.bonus'))
		{
			header('location: ' . webUrl('achievement/bonus/status0'));
			exit();
		}
		else if (cv('achievement.bonus.send')) 
		{
			header('location: ' . webUrl('achievement/bonus/send'));
			exit();
		}
		else if (cv('achievement.notice')) 
		{
			header('location: ' . webUrl('achievement/notice'));
			exit();
		}
		else if (cv('achievement.set')) 
		{
			header('location: ' . webUrl('achievement/set'));
			exit();
		}
	}
	public function notice() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			m('common')->updatePluginset(array( 'achievement' => array('tm' => $data) ));
			plog('achievement.notice.edit', '修改通知设置');
			show_json(1);
		}
		$data = m('common')->getPluginset('achievement');
	...
	public function set() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			m('common')->updatePluginset(array('achievement' => $data));
			...
			plog('achievement.set.edit', '修改基本设置<br>' . '内购分红 -- ' . $selfbuy . ' <br>成为分销商条件 -- ' . $become);
			show_json(1, array('url' => webUrl('achievement/set', array('tab' => str_replace('#tab_', '', $_GPC['tab'])))));
		}
		$styles = array();
