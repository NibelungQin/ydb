...
class Partner_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.nickname as parentname,p.avatar as parentavatar from ' . tablename('ewei_shop_member') . ' dm ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_achievement_level') . ' l on l.id = dm.partnerlevel' . ' where dm.uniacid = ' . $_W['uniacid'] . ' and (dm.ispartner =1 or dm.ispartner=-1)  ' . $condition . ' ORDER BY dm.partnertime desc';
		...
		$total = pdo_fetchcolumn('select count(dm.id) from' . tablename('ewei_shop_member') . ' dm  ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_achievement_level') . ' l on l.id = dm.partnerlevel' . ' where dm.uniacid =' . $_W['uniacid'] . ' and (dm.ispartner =1 or dm.ispartner=-1) ' . $condition, $params);
		...
		if ($_GPC['export'] == '1') {
			ca('achievement.partner.export');
			plog('achievement.partner.export', '导出店铺数据');
	...
	'title'   => '店铺数据-' . date('Y-m-d-H-i', time()),
	'columns' => array(
		...
		array('title' => '店铺等级', 'field' => 'levelname', 'width' => 12),
		array('title' => '累计分红', 'field' => 'bonus', 'width' => 12),
		array('title' => '注册时间', 'field' => 'createtime', 'width' => 12),
		array('title' => '成为店铺时间', 'field' => 'partneragenttime', 'width' => 12),
	...
	public function delete()
	{
		...
		foreach ($members as $member) {
			...
			plog('achievement.partner.delete', '取消店铺资格 <br/>店铺信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
		}
	...
	public function query()
	{
		...
		include $this->template('achievement/query');
	}
	...
	public function check()
	{
		...
		foreach ($members as $member) {
			...
			if ($status == 1) {
				...
				plog('achievement.partner.check', '审核店铺 <br/>店铺信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
				...
				plog('achievement.partner.check', '取消审核 <br/>店铺信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
	...
	public function partnerblack()
	{
		...
		foreach ($members as $member) {
			...
			if ($partnerblack == 1) {
				...
				plog('achievement.partner.partnerblack', '设置黑名单 <br/>店铺信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
			else {
				pdo_update('ewei_shop_member', array('ispartner' => 1, 'partnerstatus' => 1, 'partnerblack' => 0), array('id' => $member['id']));
				plog('achievement.partner.partnerblack', '取消黑名单 <br/>店铺信息:  ID: ' . $member['id'] . ' /  ' . $member['openid'] . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
			}
		}
