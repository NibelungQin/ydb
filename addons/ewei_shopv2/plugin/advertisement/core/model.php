...
class AdvertisementModel extends PluginModel
{
	...
	public function groupsShare() {
		global $_W;
		$uniacid = $_W['uniacid'];
		$share = pdo_fetch('select share_title,share_icon,share_desc,share_url from ' . tablename('ewei_shop_advertisement_set') . ' where uniacid=:uniacid ', array(':uniacid' => $uniacid));
		...
		$_W['shopshare'] = array(
			...
			'link' => (!(empty($share['share_url'])) ? $share['share_url'] : mobileUrl('advertisement', array('shareid' => $myid), true))
		);
	}
