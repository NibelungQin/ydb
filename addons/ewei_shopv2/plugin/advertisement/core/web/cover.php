...
class Cover_EweiShopV2Page extends PluginWebPage
{
	public function main()
	{
		global $_W;
		global $_GPC;
		$rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => 'ewei_shopv2广告套餐入口设置'));
		if (!empty($rule)) {
		...
		if ($_W['ispost']) {
			...
			$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'ewei_shopv2广告套餐入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
			...
			$cover_data = array(
				...
				'url' => mobileUrl('advertisement')
			);
			pdo_insert('cover_reply', $cover_data);
			plog('advertisement.cover.edit', '修改广告套餐入口设置');
			show_json(1);
		}

		$url = mobileUrl('advertisement', NULL, true);
