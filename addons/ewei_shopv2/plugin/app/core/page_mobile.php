...
function app_json($result = null)
{
    ...
    if (DevelopmentUtil::isProductionEnvironment()) {
        ExceptionUtil::exit(json_encode(array_merge($ret, $result)));
    }

    if (DevelopmentUtil::notCITestingEnvironment()) {
        echo json_encode(array_merge($ret, $result));
    }
}
...
class AppMobilePage extends PluginMobilePage
{
    ...
    public function __construct()
    {
        ...
        if ($_GPC['r'] != 'app.cacheset' && strexists($_GPC['openid'],
                'sns_wa') || isset($_GPC['comefrom']) && $_GPC['comefrom'] == 'wxapp') {
            $iswxapp = true;
            $this->iswxapp = true;
        }
    ...
    private function checkUniacid()
    {
        ...
        $bind = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_wxapp_bind') . ' WHERE wxapp=:wxapp LIMIT 1',
            array(':wxapp' => $_W['uniacid']));
        if (!empty($bind)) {
