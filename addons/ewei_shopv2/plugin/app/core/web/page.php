...
class App_Page_EweiShopV2PluginWebPage extends PluginWebPage
{
    public function main()
    {
        ...
        $list = pdo_fetchall(
            'SELECT id, `name`, `type`, createtime, lasttime, status, isdefault FROM ' . tablename('ewei_shop_wxapp_page') . ' WHERE ' . $condition . ' ORDER BY isdefault DESC, id DESC' . $limit,
            $params
        );
        $total = pdo_fetchcolumn(
            'SELECT COUNT(*) FROM ' . tablename('ewei_shop_wxapp_page') . ' WHERE ' . $condition,
            $params
        );
    ...
    protected function post()
    {
            ...
            if (!empty($isdefault)) {
                ...
                if ($data['page']['type'] == 2 || $data['page']['type'] == 20) {
                    ...
                    pdo_update(
                        ...
                        array('isdefault' => 0),
                        array('uniacid' => $_W['uniacid'], 'type' => $data['page']['type'])
                    );
                }
            }
            if (!empty($id)) {
        ...
        $diyadvs = pdo_fetchall(
            'select id, `name` from ' . tablename('ewei_shop_wxapp_startadv') . ' where status=1 and uniacid=:uniacid  order by id desc',
    ...
    public function status()
    {
        ...
        $list = pdo_fetchall(
            'SELECT id, `name`, isdefault FROM ' . tablename('ewei_shop_wxapp_page') . (' WHERE id in( ' . $id . ' ) AND uniacid=:uniacid'),
            array(':uniacid' => $_W['uniacid'])
        );
        if (!empty($list)) {
            foreach ($list as $row) {
                ...
                pdo_update(
                    'ewei_shop_wxapp_page',
                    array('status' => $status),
                    array('id' => $row['id'], 'uniacid' => $_W['uniacid'])
                );
                plog(
                    'app.page.edit',
                    '修改页面状态 ID: ' . $row['id'] . ' 页面名称: ' . $row['name'] . ' 页面状态: ' . $status == 1 ? '启用' : '禁用'
                );
            }
        }
        show_json(1);
    ...
    public function setdefault()
    {
        ...
        $item = pdo_fetch(
            'SELECT id, `name`, `status`, isdefault FROM ' . tablename('ewei_shop_wxapp_page') . ' WHERE id=:id AND uniacid=:uniacid LIMIT 1',
    ...
    public function goods_query()
    {
        ...
        $ds = pdo_fetchall(
            'SELECT id,title,thumb,marketprice,productprice,share_title,share_icon,description,minprice,costprice,total,sales,islive,liveprice FROM ' . tablename('ewei_shop_goods') . (' WHERE 1 ' . $condition . ' order by createtime desc'),
            $params
        );
