...
class App_Setting_EweiShopV2PluginWebPage extends PluginWebPage
{
    public function main()
    {
        ...
        $tmsg_list = pdo_fetchall('SELECT id, `name` FROM ' . tablename('ewei_shop_wxapp_tmessage') . ' WHERE uniacid=:uniacid AND status=1',
        ...
        if (!empty($filename) && !empty($tmp_name)) {
            ...
            if ($ext != '.pem') {
                ...
                if ($fileinput == 'weixin_cert_file') {
                    $errinput = 'CERT文件格式错误';
                } elseif ($fileinput == 'weixin_key_file') {
                    $errinput = 'KEY文件格式错误';
                } else {
