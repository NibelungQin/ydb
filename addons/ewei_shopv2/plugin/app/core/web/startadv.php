...
class Startadv_EweiShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $list = pdo_fetchall(
            'SELECT id, `name`, createtime, lastedittime, status FROM ' . tablename('ewei_shop_wxapp_startadv') . ' WHERE ' . $condition . ' ORDER BY id DESC' . $limit,
            $params
        );
        $total = pdo_fetchcolumn(
            'SELECT COUNT(*) FROM ' . tablename('ewei_shop_wxapp_startadv') . ' WHERE ' . $condition,
            $params
        );
    ...
    protected function post()
    {
        ...
        if (!empty($id)) {
            $advs = pdo_fetch(
                'SELECT * FROM ' . tablename('ewei_shop_wxapp_startadv') . ' WHERE id=:id and uniacid=:uniacid limit 1 ',
    ...
    public function status()
    {
        ...
        $list = pdo_fetchall(
            'SELECT id, `name` FROM ' . tablename('ewei_shop_wxapp_startadv') . (' WHERE id in( ' . $id . ' ) AND uniacid=:uniacid'),
        ...
        if (!empty($list)) {
            foreach ($list as $row) {
                pdo_update(
                    'ewei_shop_wxapp_startadv',
                    array('status' => $status),
                    array('id' => $row['id'], 'uniacid' => $_W['uniacid'])
                );
                plog(
                    'app.startadv.edit',
                    '广告页面状态 ID: ' . $row['id'] . ' 广告名称: ' . $row['name'] . ' 广告状态: ' . $status == 1 ? '启用' : '禁用'
                );
            }
        }
