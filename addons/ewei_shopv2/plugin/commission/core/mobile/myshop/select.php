...
class Commission_Myshop_Select_EweiShopV2PluginMobilePage extends CommissionMobileLoginPage
{
	public function main()
	{
		...
		if (!empty($shop['selectgoods'])) {
			...
			if (!empty($goodsids)) {
				$goods = pdo_fetchall('select id,title,marketprice,thumb from ' . tablename('ewei_shop_goods') . ' where uniacid=:uniacid and id in ( ' . trim($shop['goodsids']) . ')',
