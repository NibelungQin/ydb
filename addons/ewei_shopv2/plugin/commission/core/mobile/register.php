...
class Commission_Register_EweiShopV2PluginMobilePage extends CommissionMobileLoginPage
{
	public function main(){
		...
		if ($_W['ispost']) {
			...
			if (0 < $icode) {
				if (p('offic')) {
					$iagent = pdo_fetch('select * from ' . tablename('ewei_shop_member')
						. ' where mobile = :mobile and uniacid = :uniacid limit 1', array(':mobile' => $icode, ':uniacid' => intval($_W['uniacid']))
					);
		...
		else if ($set['become'] == 5) {
			header('location: ' . mobileUrl('packagegoods'));exit();

//			暂时不用
//			$packagegoods = pdo_fetch('select id,title,thumb,packageprice as marketprice from' . tablename('ewei_shop_packagegoods_goods')
//				. ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $set['become_packagegoodsid'], ':uniacid' => $_W['uniacid'])
//			);
//			$goodscount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_packagegoods_order')
//				. ' where goodid=:goodid and openid=:openid and status>=' . $order_status . '  limit 1', array(':goodid' => $set['become_packagegoodsid'], ':openid' => $openid)
//			);;
//			if ($goodscount <= 0) {
//				$status = 0;
//				$buy_goods = $packagegoods;
//			}else {
//				$to_check_agent = true;
//				$status = 1;
//			}
