...
use Ydb\Entity\Manual\CommissionClickcount;
use Ydb\Entity\Manual\CommissionLevel;
use Ydb\Entity\Manual\CommissionRepurchase;
use Ydb\Entity\Manual\CommissionShop;
use Ydb\Entity\Manual\CommissionShopPrice;
...
use Ydb\Entity\Manual\SeckillTaskGoods;
use Ydb\Entity\Manual\WeishopBill;
...
if (!(class_exists('CommissionModel'))) {
    ...
    {
        public function get_shop_price($shop_id, $goods_id)
        {
            global $_W;
            global $_GPC;
            $shop_price = pdo_fetch('select goods_id,price
                    from ' . CommissionShopPrice::TABLE_NAME
                . ' where shop_id=:shop_id and uniacid=:uniacid and goods_id=:goods_id limit 1',
                array(':shop_id' => $shop_id, ':goods_id' => $goods_id, ':uniacid' => $_W['uniacid']));
            return $shop_price;
        }
        ...
        public function getSet($uniacid = 0)
        {
            ...
            $set['texts'] = array(
                ...
                'commission_detail' => (empty($set['texts']['commission_detail'])
                    ? '提现明细'
                    : (($set['texts']['commission_detail'] == '佣金明细'
                        ? '提现明细'
        ...
        //计算三级分销佣金---大礼包
        public function calculate_packagegoods($orderid = 0, $update = true, $order_agentid = null)
        {
            global $_W;
            $set = $this->getSet();
            $levels = $this->getLevels();
            $order = pdo_fetch(
                'select agentid,price from '
                . PackagegoodsOrder::TABLE_NAME . ' where id=:id limit 1',
                array(':id' => $orderid)
            );
            ...
            $hascommission = false;
            $goods = pdo_fetchall(
                'select og.id,og.price,og.goodid,og.optionid,og.commissions,g.hascommission,g.nocommission,
                        g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,g.commission3_rate,
                        g.commission3_pay,g.commission
                    from ' . PackagegoodsOrder::TABLE_NAME . '  og '
                . ' left join ' . PackagegoodsGoods::TABLE_NAME . ' g on g.id = og.goodid'
                . ' where og.id=:orderid and og.uniacid=:uniacid',
            ...
            if (0 < $set['level']) {
                foreach ($goods as &$cinfo) {
                    $price = $cinfo['price'];
                    $goods_commission = ((!(empty($cinfo['commission'])) ? json_decode($cinfo['commission'],
                        true) : ''));
                    if (empty($cinfo['nocommission'])) {//是否参与分销 0参与 1不参与
                        $hascommission = true;
                        if ($cinfo['hascommission'] == 1) {//是否有分销
                            if (empty($goods_commission['type'])) {//启用独立佣金比例设置
                                $cinfo['commission1'] = array(
                                    'default' => (1 <= $set['level']
                                        ? ((0 < $cinfo['commission1_rate']
                                            ? round(($cinfo['commission1_rate'] * $price) / 100, 2) . ''
                                            : round($cinfo['commission1_pay'] * $cinfo['total'], 2)))
                                        : 0)
                                );
                                $cinfo['commission2'] = array(
                                    'default' => (2 <= $set['level']
                                        ? ((0 < $cinfo['commission2_rate']
                                            ? round(($cinfo['commission2_rate'] * $price) / 100, 2) . ''
                                            : round($cinfo['commission2_pay'] * $cinfo['total'], 2)))
                                        : 0)
                                );
                                $cinfo['commission3'] = array(
                                    'default' => (3 <= $set['level']
                                        ? ((0 < $cinfo['commission3_rate']
                                            ? round(($cinfo['commission3_rate'] * $price) / 100, 2) . ''
                                            : round($cinfo['commission3_pay'] * $cinfo['total'], 2)))
                                        : 0)
                                );
                                foreach ($levels as $level) {
                                    $cinfo['commission1']['level' . $level['id']] = ((0 < $cinfo['commission1_rate']
                                        ? round(($cinfo['commission1_rate'] * $price) / 100, 2) . ''
                                        : round($cinfo['commission1_pay'] * $cinfo['total'], 2)));
                                    $cinfo['commission2']['level' . $level['id']] = ((0 < $cinfo['commission2_rate']
                                        ? round(($cinfo['commission2_rate'] * $price) / 100, 2) . ''
                                        : round($cinfo['commission2_pay'] * $cinfo['total'], 2)));
                                    $cinfo['commission3']['level' . $level['id']] = ((0 < $cinfo['commission3_rate']
                                        ? round(($cinfo['commission3_rate'] * $price) / 100, 2) . ''
                                        : round($cinfo['commission3_pay'] * $cinfo['total'], 2)));
                                }
                            } else {
                                ...
                                while ($i < $set['level']) {
                                    if (!(empty($goods_commission['default']['option' . $cinfo['optionid']][$i]))) {
                                        if (strexists($goods_commission['default']['option' . $cinfo['optionid']][$i],
                                            '%')) {
                                            $dd = (float)str_replace('%', '',
                                                $goods_commission['default']['option' . $cinfo['optionid']][$i]);
                                            if ((0 < $dd) && ($dd < 100)) {
                                            ...
                                            $temp_price[$i] = round(
                                                $goods_commission['default']['option' . $cinfo['optionid']][$i] * $cinfo['total'],
                                                2);
                                        }
                                    }
                                    ++$i;
                                    ...
                                    while ($i < $set['level']) {
                                        if (!(empty($goods_commission['level' . $level['id']]['option' . $cinfo['optionid']][$i]))) {
                                            if (strexists($goods_commission['level' . $level['id']]['option' . $cinfo['optionid']][$i],
                                                '%')) {
                                                $dd = (float)str_replace('%', '',
                                                    $goods_commission['level' . $level['id']]['option' . $cinfo['optionid']][$i]);
                                                if ((0 < $dd) && ($dd < 100)) {
                                                    $temp_price[$i] = round(($dd / 100) * $price, 2);
                                                } else {
                                                    $temp_price[$i] = 0;
                                                ...
                                                $temp_price[$i] = round($goods_commission['level' . $level['id']]['option' . $cinfo['optionid']][$i],
                                                    2);
                                            }
                                        }
                                        ++$i;
                            ...
                            $cinfo['commission1'] = array(
                                'default' => (1 <= $set['level'] ? round(($set['commission1'] * $price) / 100,
                                        2) . '' : 0)
                            );
                            $cinfo['commission2'] = array(
                                'default' => (2 <= $set['level'] ? round(($set['commission2'] * $price) / 100,
                                        2) . '' : 0)
                            );
                            $cinfo['commission3'] = array(
                                'default' => (3 <= $set['level'] ? round(($set['commission3'] * $price) / 100,
                                        2) . '' : 0)
                            );
                            foreach ($levels as $level) {
                                $cinfo['commission1']['level' . $level['id']] = ((1 <= $set['level']
                                    ? round(($level['commission1'] * $price) / 100, 2) . ''
                                    : 0));
                                $cinfo['commission2']['level' . $level['id']] = ((2 <= $set['level']
                                    ? round(($level['commission2'] * $price) / 100, 2) . ''
                                    : 0));
                                $cinfo['commission3']['level' . $level['id']] = ((3 <= $set['level']
                                    ? round(($level['commission3'] * $price) / 100, 2) . ''
                                    : 0));
                            }
                        }
                    } else {
                    ...
                    if ($update) {
                        ...
                        if (!(empty($agentid))) {
                            ...
                            if (($m1['isagent'] == 1) && ($m1['status'] == 1)) {
                                ...
                                $commissions['level1'] = ((empty($l1) ? round($cinfo['commission1']['default'],
                                    2) : round($cinfo['commission1']['level' . $l1['id']], 2)));
                                if (!(empty($m1['agentid']))) {
                                    $m2 = m('member')->getMember($m1['agentid']);
                                    $l2 = $this->getLevel($m2['openid']);
                                    $commissions['level2'] = ((empty($l2) ? round($cinfo['commission2']['default'],
                                        2) : round($cinfo['commission2']['level' . $l2['id']], 2)));
                                    if (!(empty($m2['agentid']))) {
                                        $m3 = m('member')->getMember($m2['agentid']);
                                        $l3 = $this->getLevel($m3['openid']);
                                        $commissions['level3'] = ((empty($l3) ? round($cinfo['commission3']['default'],
                                            2) : round($cinfo['commission3']['level' . $l3['id']], 2)));
                                    }
                                }
                            }
                        }
                        pdo_update(
                            'ewei_shop_packagegoods_order',
                            array(
            ...
            if (!($hascommission)) {
                pdo_update('ewei_shop_packagegoods_order', array('agentid' => 0), array('id' => $orderid));
            }
            return $goods;
        }

        //升级大礼包分销订单校正
        public function checkOrderConfirm_packagegoods($orderid = '0')
        {
            global $_W;
            global $_GPC;
            if (empty($orderid)) {
            ...
            $order = pdo_fetch(
                'select id,openid,orderno,price,agentid,paytime
                    from ' . PackagegoodsOrder::TABLE_NAME
            ...
            } elseif (!(empty($order['officcode'])) && p('offic')) {
                $parent = pdo_fetch(
                    'select * from ' . Member::TABLE_NAME
                    . ' where mobile = :mobile and uniacid = :uniacid limit 1 ',
                    array(':mobile' => trim($order['officcode']), ':uniacid' => (int)$_W['uniacid'])
                );
            } else {
            ...
            $time = time();
            $become_child = (int)$set['become_child'];
            if ($parent_is_agent && ($become_child == 1) && empty($member['agentid'])
                && ($member['id'] != $parent['id']) && empty($member['fixagentid'])) {    //	是否固定上级分销商
                ...
                if ($author) {
                    ...
                    pdo_update('ewei_shop_member', array('agentid' => $parent['id'], 'childtime' => $time),
                        array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                }
                if ($author) {
                ...
                $this->sendMessage($parent['openid'],
                    array('nickname' => $member['nickname'], 'childtime' => $time),
            ...
            if (!(empty($agentid))) {
                $res = pdo_update('ewei_shop_packagegoods_order', array('agentid' => $agentid),
                    array('id' => $orderid));
                $this->calculate_packagegoods($orderid, true, ($res ? $agentid : null));
                return;
            }
            $this->calculate_packagegoods($orderid);
        }
        ...
        public function calculate($orderid = 0, $update = true, $order_agentid = null)
        {
            ...
            $levels = $this->getLevels();
            $order = pdo_fetch(
                'select agentid,price,costprice,goodsprice,deductcredit2,discountprice,isdiscountprice,
                        dispatchprice,couponprice,changeprice,ispackage,packageid
                    from ' . Order::TABLE_NAME . ' where id=:id limit 1',
                array(':id' => $orderid)
            );
            $commission = m('common')->getPluginset('commission');
            ...
            $agentid = ((!($order_agentid === null) ? $order_agentid : $order['agentid']));
            $hascommission = false;
            $goods = pdo_fetchall(
                'select og.id,og.costprice,og.realprice,og.total,g.hasoption,og.goodsid,og.optionid,
                        g.hascommission,g.nocommission, g.commission1_rate,g.commission1_pay,g.commission2_rate,
                        g.commission2_pay,g.commission3_rate,g.commission3_pay,g.commission,og.commissions,
                        og.seckill,og.seckill_taskid,og.seckill_timeid
            ...
            if (0 < $set['level']) {
                foreach ($goods as &$cinfo) {
                    if ($commission['calcutype'] == 2) {
                        $price = ($cinfo['realprice'] * $rate - $cinfo['costprice']);
                        if ($price < 0) {
                            $price = 0;
                        }
                    } else {
                    ...
                    //如果是秒杀活动商品的话
                    if ($cinfo['seckill']) {
                        $seckill_goods = pdo_fetch('select commission1,commission2,commission3
                                from ' . SeckillTaskGoods::TABLE_NAME
                            . ' where  goodsid=:goodsid and optionid =:optionid and taskid=:taskid
                                    and timeid=:timeid and uniacid=:uniacid limit 1',
                            array(
                    ...
                    if (!(empty($seckill_goods))) {
                        ...
                        $cinfo['commission1'] = array(
                            'default' => (1 <= $set['level']
                                ? $seckill_goods['commission1'] * $cinfo['total'] : 0)
                        );
                        $cinfo['commission2'] = array(
                            'default' => (2 <= $set['level']
                                ? $seckill_goods['commission2'] * $cinfo['total'] : 0)
                        );
                        $cinfo['commission3'] = array(
                            'default' => (3 <= $set['level']
                                ? $seckill_goods['commission3'] * $cinfo['total'] : 0)
                        );
                        foreach ($levels as $level) {
                        ...
                        if (empty($cinfo['nocommission'])) {
                            ...
                            if ($cinfo['hascommission'] == 1) {
                                    ...
                                    while ($i < $set['level']) {
                                        if (!(empty($goods_commission['default']['option0'][$i]))) {
                                            if (strexists($goods_commission['default']['option0'][$i], '%')) {
                                                $dd = (float)str_replace('%', '',
                                                    $goods_commission['default']['option0'][$i]);
                                                if ((0 < $dd) && ($dd < 100)) {
                                                    $temp_price[$i] = round(($dd / 100) * $price, 2);
                                                } else {
                                                    $temp_price[$i] = 0;
                                                ...
                                                $temp_price[$i] =
                                                    round($goods_commission['default']['option0'][$i] * $cinfo['total'],
                                                        2);
                                            }
                                        }
                                        ++$i;
                                        ...
                                        while ($i < $set['level']) {
                                            if (!(empty($goods_commission['level' . $level['id']]['option0'][$i]))) {
                                                if (strexists($goods_commission['level' . $level['id']]['option0'][$i],
                                                    '%')) {
                                                    $dd = (float)str_replace('%', '',
                                                        $goods_commission['level' . $level['id']]['option0'][$i]);
                                                    if ((0 < $dd) && ($dd < 100)) {
                                                        $temp_price[$i] = round(($dd / 100) * $price, 2);
                                                    } else {
                                                        $temp_price[$i] = 0;
                                                    ...
                                                    $temp_price[$i] =
                                                        round($goods_commission['level' . $level['id']]['option0'][$i] * $cinfo['total'],
                                                            2);
                                                }
                                            }
                                            ++$i;
                                    ...
                                    while ($i < $set['level']) {
                                        if (!(empty($goods_commission['default']['option' . $cinfo['optionid']][$i]))) {
                                            if (strexists($goods_commission['default']['option' . $cinfo['optionid']][$i],
                                                ...
                                                $temp_price[$i] =
                                                    round($goods_commission['default']['option' . $cinfo['optionid']][$i] * $cinfo['total'],
                                                        2);
                                            }
                                        }
                                        ++$i;
                                        ...
                                        while ($i < $set['level']) {
                                            if (!(empty($goods_commission['level' . $level['id']]['option' . $cinfo['optionid']][$i]))) {
                                                if (strexists($goods_commission['level' . $level['id']]['option' . $cinfo['optionid']][$i],
                                                    ...
                                                    $temp_price[$i] =
                                                        round($goods_commission['level' . $level['id']]['option' . $cinfo['optionid']][$i] * $cinfo['total'],
                                                            2);
                                                }
                                            }
                                            ++$i;
                                ...
                                $cinfo['commission1'] = array(
                                    'default' => (1 <= $set['level']
                                        ? round(($set['commission1'] * $price) / 100, 2) . ''
                                        : 0)
                                );
                                $cinfo['commission2'] = array(
                                    'default' => (2 <= $set['level']
                                        ? round(($set['commission2'] * $price) / 100, 2) . ''
                                        : 0)
                                );
                                $cinfo['commission3'] = array(
                                    'default' => (3 <= $set['level']
                                        ? round(($set['commission3'] * $price) / 100, 2) . ''
                                        : 0)
                                );
                                foreach ($levels as $level) {
                                    $cinfo['commission1']['level' . $level['id']] = ((1 <= $set['level']
                            ...
                            if (0 < $order['ispackage']) {
                                ...
                                if (!(empty($cinfo['optionid']))) {
                                    $packoption = pdo_fetch('select commission1,commission2,commission3
                                            from ' . PackageGoodsOption::TABLE_NAME
                                        . ' where uniacid = ' . $_W['uniacid'] . ' and pid = ' . $order['packageid']
                                        . ' and optionid = ' . $cinfo['optionid'] . ' ');
                                } else {
                                    $packoption = pdo_fetch('select commission1,commission2,commission3
                                            from ' . PackageGoods::TABLE_NAME
                                        . ' where uniacid = ' . $_W['uniacid'] . ' and pid = ' . $order['packageid']
                                        . ' and goodsid = ' . $cinfo['goodsid'] . ' ');
                                }
                                $cinfo['commission1'] = array(
                                    'default' => (1 <= $set['level']
                                        ? $packoption['commission1'] : 0)
                                );
                                $cinfo['commission2'] = array(
                                    'default' => (2 <= $set['level']
                                        ? $packoption['commission2'] : 0)
                                );
                                $cinfo['commission3'] = array(
                                    'default' => (3 <= $set['level']
                                        ? $packoption['commission3'] : 0)
                                );
                                foreach ($levels as $level) {
                    ...
                    if ($update) {
                        ...
                        if (!(empty($agentid))) {
                            ...
                            if (($m1['isagent'] == 1) && ($m1['status'] == 1)) {
                                $l1 = $this->getLevel($m1['openid']);
                                $commissions['level1'] = ((empty($l1)
                                    ? round($cinfo['commission1']['default'], 2)
                                    : round($cinfo['commission1']['level' . $l1['id']], 2)));
                                if (!(empty($m1['agentid']))) {
                                    $m2 = m('member')->getMember($m1['agentid']);
                                    $l2 = $this->getLevel($m2['openid']);
                                    $commissions['level2'] = ((empty($l2)
                                        ? round($cinfo['commission2']['default'], 2)
                                        : round($cinfo['commission2']['level' . $l2['id']], 2)));
                                    if (!(empty($m2['agentid']))) {
                                        $m3 = m('member')->getMember($m2['agentid']);
                                        $l3 = $this->getLevel($m3['openid']);
                                        $commissions['level3'] = ((empty($l3)
                                            ? round($cinfo['commission3']['default'], 2)
                                            : round($cinfo['commission3']['level' . $l3['id']], 2)));
                                    }
                                }
                            }
                        }
        ...
        public function getOrderCommissions($orderid = 0, $ogid = 0)
        {
            global $_W;
            $set = $this->getSet();
            $agentid = pdo_fetchcolumn('select agentid from ' . Order::TABLE_NAME
                . ' where id=:id limit 1',
                array(':id' => $orderid));
            $goods = pdo_fetch('select commission1,commission2,commission3
                    from ' . OrderGoods::TABLE_NAME
                . ' where id=:id and orderid=:orderid and uniacid=:uniacid and nocommission=0 limit 1',
                array(':id' => $ogid, ':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
            $commissions = array('level1' => 0, 'level2' => 0, 'level3' => 0);
            if (0 < $set['level']) {
                ...
                if (!(empty($agentid))) {
                    ...
                    if (($m1['isagent'] == 1) && ($m1['status'] == 1)) {
                        $l1 = $this->getLevel($m1['openid']);
                        $commissions['level1'] = ((empty($l1)
                            ? round($commission1['default'], 2)
                            : round($commission1['level' . $l1['id']], 2)));
                        if (!(empty($m1['agentid']))) {
                            $m2 = m('member')->getMember($m1['agentid']);
                            $l2 = $this->getLevel($m2['openid']);
                            $commissions['level2'] = ((empty($l2)
                                ? round($commission2['default'], 2)
                                : round($commission2['level' . $l2['id']], 2)));
                            if (!(empty($m2['agentid']))) {
                                $m3 = m('member')->getMember($m2['agentid']);
                                $l3 = $this->getLevel($m3['openid']);
                                $commissions['level3'] = ((empty($l3)
                                    ? round($commission3['default'], 2)
                                    : round($commission3['level' . $l3['id']], 2)));
                            }
                        }
                    }
                }
        ...
        public function getInfo($openid, $options = null)
        {
            ...
            if (isset($options['starttime']) && isset($options['endtime'])) {
                $options['starttime'] = ((strexists($options['starttime'],
                    '-') ? strtotime($options['starttime']) : $options['starttime']));
            ...
            $commission_wait = 0;
            $commission_fail = 0;
            $current_commission_total = 0;//当前时间段佣金
            $level1 = 0;
            $level2 = 0;
            $level3 = 0;
            $order10 = 0;
            ...
            if (1 <= $level) {
                if (in_array('ordercount0', $options)) {
                    $level1_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,
                                count(distinct o.id) as ordercount
                            from ' . Order::TABLE_NAME . ' o '
                        . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                        . ' where o.agentid=:agentid and o.status>=0 and og.status1>=0 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1',
                ...
                if (in_array('ordercount', $options)) {
                    $level1_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,
                            ...
                            from ' . Order::TABLE_NAME . ' o '
                        . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                        . ' where o.agentid=:agentid and o.status>=1 and og.status1>=0 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1',
                        array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                ...
                if (in_array('ordercount3', $options)) {
                    $level1_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,
                                count(distinct o.id) as ordercount
                            from ' . Order::TABLE_NAME . ' o '
                        . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                        . ' where o.agentid=:agentid and o.status>=3 and og.status1>=0 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0 ' . $where_time . ' limit 1',
                        array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                ...
                if (in_array('current_money', $options)) {
                    $level1_commissions = pdo_fetchall('select og.commission1,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                    ...
                    foreach ($level1_commissions as $c) {
                        ...
                        if (empty($commissions)) {
                            $current_commission_total += ((isset($commission['level' . $agentLevel['id']])
                                ? $commission['level' . $agentLevel['id']]
                                : isset($commission['default'])
                                    ? $commission['default']
                                    : 0));
                        } else {
                            $current_commission_total += ((isset($commissions['level1'])
                                ? (float)$commissions['level1'] : 0));
                        }
                    }
                }
                if (in_array('total', $options)) {
                    $level1_commissions = pdo_fetchall('select og.commission1,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid=:agentid and o.status>=1 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    foreach ($level1_commissions as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                            $commission_total += ((isset($commission['level' . $agentLevel['id']])
                                ? $commission['level' . $agentLevel['id']]
                                : isset($commission['default'])
                                    ? $commission['default']
                                    : 0));
                        } else {
                            $commission_total += ((isset($commissions['level1'])
                                ? (float)$commissions['level1'] : 0));
                        }
                    }
                }
                if (in_array('ok', $options)) {
                    $level1_commissions = pdo_fetchall('select og.commission1,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid=:agentid and o.status>=3 and og.nocommission=0
                                and (' . $time . ' - o.finishtime > ' . $day_times . ') and og.status1=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    foreach ($level1_commissions as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                            $commission_ok += ((isset($commission['level' . $agentLevel['id']])
                                ? $commission['level' . $agentLevel['id']]
                                : $commission['default']));
                        } else {
                ...
                if (in_array('lock', $options)) {
                    $level1_commissions1 = pdo_fetchall('select og.commission1,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid=:agentid and o.status>=3 and og.nocommission=0
                                and (' . $time . ' - o.finishtime <= ' . $day_times . ')  and og.status1=0
                    ...
                    foreach ($level1_commissions1 as $c) {
                        ...
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                            $commission_lock += ((isset($commission['level' . $agentLevel['id']])
                                ? $commission['level' . $agentLevel['id']]
                                : $commission['default']));
                        } else {
                ...
                if (in_array('apply', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid=:agentid and o.status>=3 and og.status1=1 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    foreach ($level1_commissions2 as $c) {
                        ...
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                            $commission_apply += ((isset($commission['level' . $agentLevel['id']])
                                ? $commission['level' . $agentLevel['id']] : $commission['default']));
                        } else {
                ...
                if (in_array('check', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid=:agentid and o.status>=3 and og.status1=2 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    foreach ($level1_commissions2 as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                            $commission_check += ((isset($commission['level' . $agentLevel['id']])
                                ? $commission['level' . $agentLevel['id']] : $commission['default']));
                        } else {
                ...
                if (in_array('pay', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid=:agentid and o.status>=3 and og.status1=3 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    foreach ($level1_commissions2 as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                            $commission_pay += ((isset($commission['level' . $agentLevel['id']])
                                ? $commission['level' . $agentLevel['id']]
                                : ((!(empty($commission)) ? $commission['default'] : 0))));
                        } else {
                ...
                if (in_array('wait', $options)) {
                    $level1_commissions2 = pdo_fetchall('select og.commission1,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid=:agentid and (o.status=2 or o.status=1) and og.status1=0
                                and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid'], ':agentid' => $member['id']));
                    foreach ($level1_commissions2 as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                            $commission_wait += ((isset($commission['level' . $agentLevel['id']])
                                ? $commission['level' . $agentLevel['id']] : $commission['default']));
                        } else {
                ...
                if (in_array('fail', $options)) {
                    ...
                    foreach ($level1_commissions2 as $c) {
                        ...
                        $commission = iunserializer($c['commission1']);
                        if (empty($commissions)) {
                            $commission_fail += ((isset($commission['level' . $agentLevel['id']])
                                ? $commission['level' . $agentLevel['id']] : $commission['default']));
                        } else {
                ...
                $level1_agentids = pdo_fetchall('select id from ' . Member::TABLE_NAME
                    . ' where agentid=:agentid and isagent=1 and status=1 and uniacid=:uniacid ',
            ...
            if ((2 <= $level) && 0 < $level1) {
                if (in_array('ordercount0', $options)) {
                    $level2_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,
                                    count(distinct o.id) as ordercount
                            from ' . Order::TABLE_NAME . ' o '
                        . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and o.status>=0 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid
                                and o.isparent=0 ' . $where_time . ' limit 1',
                ...
                if (in_array('ordercount', $options)) {
                    $level2_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,
                        ...
                        . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and o.status>=1 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid
                                and o.isparent=0 ' . $where_time . ' limit 1',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('ordercount3', $options)) {
                    $level2_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,
                                    count(distinct o.id) as ordercount
                            from ' . Order::TABLE_NAME . ' o '
                        . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and o.status>=3 and og.status2>=0 and og.nocommission=0 and o.uniacid=:uniacid
                                and o.isparent=0 ' . $where_time . ' limit 1',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('current_money', $options)) {
                    $level2_commissions = pdo_fetchall('select og.commission2,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid
                                and o.isparent=0' . $where_time,
                        array(':uniacid' => $_W['uniacid']));
                    foreach ($level2_commissions as $c) {
                        ...
                        if (empty($commissions)) {
                            ...
                            $current_commission_total += ((isset($commissions['level2']) ? $commissions['level2'] : 0));
                        }
                    }
                }
                if (in_array('total', $options)) {
                    $level2_commissions = pdo_fetchall('select og.commission2,og.commissions
                        ...
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                    foreach ($level2_commissions as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission2']);
                ...
                if (in_array('ok', $options)) {
                    $level2_commissions = pdo_fetchall('select og.commission2,og.commissions
                        ...
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3
                                and og.status2=0 and og.nocommission=0  and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                    foreach ($level2_commissions as $c) {
                        $commissions = iunserializer($c['commissions']);
                        $commission = iunserializer($c['commission2']);
                ...
                if (in_array('lock', $options)) {
                    $level2_commissions1 = pdo_fetchall('select og.commission2,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and (' . $time . ' - o.finishtime <= ' . $day_times . ') and og.status2=0
                                and o.status>=3 and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('apply', $options)) {
                    $level2_commissions2 = pdo_fetchall('select og.commission2,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and o.status>=3 and og.status2=1 and og.nocommission=0 and o.uniacid=:uniacid
                                and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('check', $options)) {
                    $level2_commissions3 = pdo_fetchall('select og.commission2,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and o.status>=3 and og.status2=2 and og.nocommission=0 and o.uniacid=:uniacid
                                and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('pay', $options)) {
                    $level2_commissions3 = pdo_fetchall('select og.commission2,og.commissions
                        ...
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and o.status>=3 and og.status2=3 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                    foreach ($level2_commissions3 as $c) {
                        $commissions = iunserializer($c['commissions']);
                ...
                if (in_array('wait', $options)) {
                    $level2_commissions3 = pdo_fetchall('select og.commission2,og.commissions
                        ...
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and (o.status=1 or o.status=2) and og.status2=0 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                    foreach ($level2_commissions3 as $c) {
                        $commissions = iunserializer($c['commissions']);
                ...
                if (in_array('fail', $options)) {
                    $level2_commissions3 = pdo_fetchall('select og.commission2,og.commissions
                        ...
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid '
                        . ' where o.agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
                                and o.status=3 and og.status2=-1 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                    foreach ($level2_commissions3 as $c) {
                        $commissions = iunserializer($c['commissions']);
                ...
                $level2_agentids = pdo_fetchall('select id from ' . Member::TABLE_NAME
                    . ' where agentid in( ' . implode(',', array_keys($level1_agentids)) . ')
            ...
            if ((3 <= $level) && 0 < $level2) {
                if (in_array('ordercount0', $options)) {
                    $level3_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,
                                    count(distinct og.orderid) as ordercount
                            from ' . Order::TABLE_NAME . ' o '
                        . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                        . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
                                and o.status>=0 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid
                                and o.isparent=0 ' . $where_time . ' limit 1',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('ordercount', $options)) {
                    $level3_ordercount = pdo_fetch('select sum(og.realprice) as ordermoney,
                        ...
                        . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                        . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
                                and o.status>=1 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid
                                and o.isparent=0 ' . $where_time . ' limit 1',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('ordercount3', $options)) {
                    $level3_ordercount3 = pdo_fetch('select sum(og.realprice) as ordermoney,
                                    count(distinct og.orderid) as ordercount
                            from ' . Order::TABLE_NAME . ' o '
                        . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                        . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
                                and o.status>=3 and og.status3>=0 and og.nocommission=0 and o.uniacid=:uniacid
                                and o.isparent=0 ' . $where_time . ' limit 1',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('current_money', $options)) {
                    $level3_commissions = pdo_fetchall('select og.commission3,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
                                and o.status>=1 and og.nocommission=0 and o.uniacid=:uniacid
                    ...
                    foreach ($level3_commissions as $c) {
                        ...
                        if (empty($commissions)) {
                            ...
                            $current_commission_total += ((isset($commissions['level3'])
                ...
                if (in_array('ok', $options)) {
                    $level3_commissions = pdo_fetchall('select og.commission3,og.commissions
                        ...
                        . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
                                and (' . $time . ' - o.finishtime > ' . $day_times . ') and o.status>=3
                                and og.status3=0  and og.nocommission=0 and o.uniacid=:uniacid
                                and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                    foreach ($level3_commissions as $c) {
                        $commissions = iunserializer($c['commissions']);
                ...
                if (in_array('lock', $options)) {
                    $level3_commissions1 = pdo_fetchall('select og.commission3,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
                                and o.status>=3 and (' . $time . ' - o.finishtime <= ' . $day_times . ')
                                and og.status3=0  and og.nocommission=0 and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('apply', $options)) {
                    $level3_commissions2 = pdo_fetchall('select og.commission3,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
                                and o.status>=3 and og.status3=1 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('check', $options)) {
                    $level3_commissions3 = pdo_fetchall('select og.commission3,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
                                and o.status>=3 and og.status3=2 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                ...
                if (in_array('pay', $options)) {
                    $level3_commissions3 = pdo_fetchall('select og.commission3,og.commissions
                        ...
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
                                and o.status>=3 and og.status3=3 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                    foreach ($level3_commissions3 as $c) {
                        $commissions = iunserializer($c['commissions']);
                ...
                if (in_array('wait', $options)) {
                    $level3_commissions3 = pdo_fetchall('select og.commission3,og.commissions
                        ...
                        . ' left join  ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                        . ' where o.agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
                                and ( o.status=1 or o.status=2) and og.status3=0 and og.nocommission=0
                                and o.uniacid=:uniacid and o.isparent=0',
                        array(':uniacid' => $_W['uniacid']));
                    foreach ($level3_commissions3 as $c) {
                        $commissions = iunserializer($c['commissions']);
                ...
                $level3_agentids = pdo_fetchall('select id from ' . Member::TABLE_NAME
                    . ' where uniacid=:uniacid and agentid in( ' . implode(',', array_keys($level2_agentids)) . ')
            ...
            $member['current_commission_total'] = round($current_commission_total, 2);//时间段总佣金
        ...
        public function getAgents($orderid = 0)
        {
            ...
            global $_GPC;
            $agents = array();
            $order = pdo_fetch('select id,agentid,openid from ' . Order::TABLE_NAME
        ...
        public function getCommission($goods)
        {
            ...
            if ($goods['hascommission'] == 1) {
                ...
                if ($goods_commission['type'] == 0) {
                    $commission = ((1 <= $set['level']
                        ? ((0 < $goods['commission1_rate']
                            ? ($goods['commission1_rate'] * $goods['marketprice']) / 100
                            : $goods['commission1_pay']))
                        : 0));
                } else {
                    $price_all = array();
                ...
                if (!(empty($level))) {
                    $commission = ((1 <= $set['level'] ? round(($level['commission1'] * $goods['marketprice']) / 100,
                        2) : 0));
                } else {
                    $commission = ((1 <= $set['level'] ? round(($set['commission1'] * $goods['marketprice']) / 100,
                        2) : 0));
                }
            }
            return $commission;
        ...
        private function createImage($url)
        {
            if (DevelopmentUtil::isTestingEnvironment()) {
                return imagecreatefrompng(__DIR__ . '/../../../preview-custom.jpg');
            }
        ...
        public function createGoodsImage($goods)
        {
            ...
            if (!(is_file($path . $file))) {
                ...
                imagedestroy($black);
                $goods_qrcode_file = tomedia(m('qrcode')->createGoodsQrcode($userinfo['id'],
                        $goods['id'])) . '?' . time();
        ...
        public function createShopImage()
        {
            ...
            if (!(is_file($path . $file))) {
                set_time_limit(0);
                @ini_set('memory_limit', DevelopmentUtil::isTestingEnvironment() ? '2G' : '256M');
                $font = IA_ROOT . '/addons/ewei_shopv2/static/fonts/msyh.ttf';
                $target = imagecreatetruecolor(640, 1225);
                $bc = imagecolorallocate($target, 0, 3, 51);
                $cc = imagecolorallocate($target, 240, 102, 0);
        ...
        public function checkAgent($openid = '')
        {
            ...
            if ($parent_is_agent && ($parent['id'] != $member['agentid']) && $parent['openid'] != $openid) {
                ...
                $clickcount = pdo_fetchcolumn('select count(*)
                        from ' . CommissionClickcount::TABLE_NAME
                    . ' where uniacid=:uniacid and openid=:openid and from_openid=:from_openid limit 1',
                    array(
                        ':uniacid' => $_W['uniacid'],
                        ':openid' => $openid,
                ...
                if ($clickcount <= 0) {
                    ...
                    pdo_update('ewei_shop_member', array('clickcount' => $parent['clickcount'] + 1),
                        array('uniacid' => $_W['uniacid'], 'id' => $parent['id']));
                }
            }
            if ($member['isagent'] == 1) {
            ...
            if ($open_redis) {
                ...
                if (!($first)) {
                    $first = pdo_fetchcolumn('select count(*)
                            from ' . Member::TABLE_NAME . ' where uniacid=:uniacid limit 1',
                        array(':uniacid' => $_W['uniacid']));
                    m('member')->memberRadisCount($redis_key, $first);
                }
            } else {
                $first = 0;
                $member1 = pdo_fetch('select id,openid
                        from ' . Member::TABLE_NAME . ' where uniacid=:uniacid',
                    array(':uniacid' => $_W['uniacid']));
                if ($member1) {
                    $first = 1;
                    $member2 = pdo_fetch('select id,openid
                        from ' . Member::TABLE_NAME . ' where uniacid=:uniacid and id<>:fid',
                        array(':uniacid' => $_W['uniacid'], ':fid' => $member1['id']));
                    if ($member2) {
                        $first = 2;
                    }
                }
            ...
            if ($parent_is_agent && empty($member['agentid']) && $member['id'] != $parent['id']) {
                if (empty($become_child)) {
                    if (empty($member['fixagentid'])) {
                        ...
                        //修改无限级身份
                        $this->promoter_qr($member, $parent);
                        if ($author) {
                            $author_set = $author->getSet();
                            if (!(empty($author_set['become'])) && (($author_set['become'] == '2') || ($author_set['become'] == '5'))) {
                                $can_author = false;
                    ...
                    pdo_update('ewei_shop_member', array('inviter' => $parent['id']),
                        array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                }
            }
            $order_status = (((int)$set['become_order'] == 0 ? 1 : 3));
            ...
            if (empty($set['become'])) {
                ...
                $ordercount = pdo_fetchcolumn('select count(*) from ' . Order::TABLE_NAME
                    . ' where uniacid=:uniacid and openid=:openid and status>=' . $order_status . ' limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                if ((int)$set['become_ordercount'] <= $ordercount) {
                    $to_check_agent = true;
                }
            } elseif ($set['become'] == 3) {
                $moneycount = pdo_fetchcolumn(
                    'select sum(goodsprice) from ' . Order::TABLE_NAME
                    . ' where uniacid=:uniacid and openid=:openid and status>=' . $order_status . ' limit 1',
                    array(':uniacid' => $_W['uniacid'], ':openid' => $openid)
                );
                if ((float)$set['become_moneycount'] <= $moneycount) {
                    $to_check_agent = true;
                }
            } elseif ($set['become'] == 4) {
                $goods = pdo_fetch('select id,title,thumb,marketprice from ' . Goods::TABLE_NAME
                    . ' where id=:id and uniacid=:uniacid limit 1',
                    array(':id' => $set['become_goodsid'], ':uniacid' => $_W['uniacid']));
                $goodscount = pdo_fetchcolumn(
                    'select count(*) from ' . OrderGoods::TABLE_NAME . ' og '
                    . ' left join ' . Order::TABLE_NAME . ' o on o.id = og.orderid'
                    . ' where og.goodsid=:goodsid and o.openid=:openid and o.status>=' . $order_status . '  limit 1',
                    array(':goodsid' => $set['become_goodsid'], ':openid' => $openid)
                );
                if (0 < $goodscount) {
                    $to_check_agent = true;
                }
            ...
            if ($to_check_agent) {
                pdo_update('ewei_shop_member',
                    array('isagent' => 1, 'status' => $become_check, 'agenttime' => ($become_check == 1 ? $time : 0)),
                    array('uniacid' => $_W['uniacid'], 'id' => $member['id']));
                if ($become_check == 1) {
        ...
        public function checkOrderConfirm($orderid = '0')
        {
            ...
            $order = pdo_fetch(
                'select id,openid,ordersn,goodsprice,agentid,paytime,officcode
        ...
        public function checkOrderPay($orderid = '0')
        {
            ...
            $order = pdo_fetch(
                'select id,openid,ordersn,goodsprice,agentid,paytime from ' . Order::TABLE_NAME
            ...
            if ($parent_is_agent && ($become_child == 2) && empty($member['agentid'])
                && ($member['id'] != $parent['id']) && empty($member['fixagentid'])) {//是否固定上级分销商
                $member['agentid'] = $parent['id'];
                ...
                if (empty($order['agentid'])) {
                    ...
                    pdo_update('ewei_shop_order', array('agentid' => $parent['id']),
                        array('id' => $orderid));
            ...
            if (!($isagent)) {
                if (((int)$set['become'] == 4) && !(empty($set['become_goodsid']))) {
                    if (empty($set['become_order'])) {
                        $order_goods = pdo_fetchall('select goodsid
                                from ' . OrderGoods::TABLE_NAME
            ...
            if (!(empty($member['agentid']))) {
                $parent = m('member')->getMember($member['agentid']);
                if (!(empty($parent)) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
                    $order_goods = pdo_fetchall('select g.id,g.title,og.total,og.price,og.realprice,
                                og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.commission1,og.commissions
                            from ' . OrderGoods::TABLE_NAME . ' og '
                        . ' left join ' . Goods::TABLE_NAME . ' g on g.id=og.goodsid '
                    ...
                    foreach ($order_goods as $og) {
                        ...
                        $goods .= ' 单价: ' . ($og['realprice'] / $og['total'])
                            . ' 数量: ' . $og['total'] . ' 总价: ' . $og['realprice'] . '); ';
                ...
                if (((int)$set['become'] == 4) && !(empty($set['become_goodsid']))) {
                    if (empty($set['become_order'])) {
                        $order_goods = pdo_fetchall('select goodsid
                            ...
                            . ' where orderid=:orderid and uniacid=:uniacid  ',
                            array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']), 'goodsid');
                        if (array_key_exists($set['become_goodsid'], $order_goods)
                            && empty($member['partnerblack'])) {
                            pdo_update('ewei_shop_member', array(
        ...
        public function checkOrderFinish($orderid = '')
        {
            ...
            if (!($isagent) && ($set['become_order'] == '1')) {
                if (($set['become'] == '4') && !(empty($set['become_goodsid']))) {
                    $order_goods = pdo_fetchall('select goodsid from ' . OrderGoods::TABLE_NAME
                        . ' where orderid=:orderid and uniacid=:uniacid  ',
                        array(':uniacid' => $_W['uniacid'], ':orderid' => $order['id']), 'goodsid');
                    if (array_key_exists($set['become_goodsid'], $order_goods) && empty($member['agentblack'])) {
                        pdo_update('ewei_shop_member', array(
                    ...
                    if ($set['become'] == '2') {
                        ...
                        $moneycount = pdo_fetchcolumn('select sum(goodsprice) 
                                from ' . Order::TABLE_NAME
                            . ' where openid=:openid and status>=3 and uniacid=:uniacid limit 1',
                            array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                        $can = (float)$set['become_moneycount'] <= $moneycount;
            ...
            if ($isagent) {
                ...
                if ($ispartner) {
                    $plugin_globonus->upgradeLevelByAgent($openid);
                    return;
                }
                $become_check = (int)$set['become_check'];
                if ($set['become_order'] == '1') {
        ...
        public function orderFinishTask($order, $self_buy = false, $member)
        {
            ...
            $order_goods_list = pdo_fetchall('SELECT commissions FROM ' . OrderGoods::TABLE_NAME
        ...
        public function getShop($m)
        {
            ...
            $shop = pdo_fetch('select * from ' . CommissionShop::TABLE_NAME
        ...
        public function getLevels($all = true, $default = false)
        {
            ...
            global $_S;
            if ($all) {
                $levels = pdo_fetchall('select * from ' . CommissionLevel::TABLE_NAME
                    . ' where uniacid=:uniacid order by commission1 asc',
                    array(':uniacid' => $_W['uniacid']));
            } else {
                $levels = pdo_fetchall('select * from ' . CommissionLevel::TABLE_NAME
                    . ' where uniacid=:uniacid and (ordermoney>0 or commissionmoney>0) order by commission1 asc',
                    array(':uniacid' => $_W['uniacid']));
            }
            if ($default) {
                $default = array(
                    'id' => '0',
                    'levelname' => (empty($_S['commission']['levelname'])
                        ? '默认等级' : $_S['commission']['levelname']),
        ...
        public function upgradeLevelByOrder($openid)
        {
            ...
            while ($leveltype_arr) {
                ...
                if (($leveltype == 4) || ($leveltype == 5)) {
                    ...
                    //自购完成订单金额，自购完成订单数量
                    $orders = pdo_fetch('select sum(og.realprice) as ordermoney,
                                count(distinct og.orderid) as ordercount
                            from ' . Order::TABLE_NAME . ' o '
                        . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                        . ' where o.openid=:openid and o.status>=3 and o.uniacid=:uniacid limit 1',
                        array(':uniacid' => $_W['uniacid'], ':openid' => $openid));
                    $ordermoney = $orders['ordermoney'];
                    $ordercount = $orders['ordercount'];
                    if ($leveltype == 4) { //如果未自购完成订单金额计算
                        $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                            . ' where uniacid=:uniacid  and ' . $ordermoney . ' >= zg_ordermoney and zg_ordermoney>0
                                order by zg_ordermoney desc limit 1',
                            array(':uniacid' => $_W['uniacid']));
                        if (empty($newlevel)) {
                            continue;
                        }
                        ...
                        $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                            . ' where uniacid=:uniacid  and ' . $ordercount . ' >= zg_ordercount and zg_ordercount>0
                                order by zg_ordercount desc limit 1',
                            array(':uniacid' => $_W['uniacid']));
                        if (empty($newlevel)) {
                            continue;
                        }
                    ...
                    $this->sendMessage($m['openid'],
                        array('nickname' => $m['nickname'], 'oldlevel' => $oldlevel, 'newlevel' => $newlevel),
                        TM_COMMISSION_UPGRADE);
                } elseif ((0 <= $leveltype) && ($leveltype <= 3)) {
                    $agents = array();
                    ...
                    if (!(empty($m['agentid']))) {
                        ...
                        if (!(empty($m1))) {
                            ...
                            if (!(empty($m1['agentid'])) && ($m1['isagent'] == 1) && ($m1['status'] == 1)) {
                                ...
                                if (!(empty($m2)) && ($m2['isagent'] == 1) && ($m2['status'] == 1)) {
                                    $agents[] = $m2;
                                    if (empty($set['selfbuy']) && !(empty($m2['agentid']))
                                        && ($m2['isagent'] == 1) && ($m2['status'] == 1)) {
                                        $m3 = m('member')->getMember($m2['agentid']);
                                        if (!(empty($m3)) && ($m3['isagent'] == 1) && ($m3['status'] == 1)) {
                                            $agents[] = $m3;
                                        }
                    ...
                    foreach ($agents as $agent) {
                        ...
                        if ($leveltype == 0) {
                            ...
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid and ' . $ordermoney . ' >= ordermoney and ordermoney>0
                                    order by ordermoney desc limit 1',
                                array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                                continue;
                            }
                            ...
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid and ' . $ordermoney . ' >= first_ordermoney
                                        and first_ordermoney>0
                                    order by first_ordermoney desc limit 1',
                                array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                                continue;
                            }
                            if (!(empty($oldlevel['id']))) {
                                ...
                                if ($newlevel['first_ordermoney'] < $oldlevel['first_ordermoney']) {
                                    continue;
                                }
                            }
                        } elseif ($leveltype == 2) {
                            ...
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid  and ' . $ordercount . ' >= ordercount and ordercount>0
                            ...
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid  and ' . $ordercount . ' >= first_ordercount
                                        and first_ordercount>0
                                    order by first_ordercount desc limit 1',
                                array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                                continue;
                            }
                            if (!(empty($oldlevel['id']))) {
                                ...
                                if ($newlevel['first_ordercount'] < $oldlevel['first_ordercount']) {
                                    continue;
                                }
                            }
                        }
                        pdo_update('ewei_shop_member', array('agentlevel' => $newlevel['id']),
                            array('id' => $agent['id']));
                        $this->sendMessage($agent['openid'],
                            array('nickname' => $agent['nickname'], 'oldlevel' => $oldlevel, 'newlevel' => $newlevel),
                            TM_COMMISSION_UPGRADE);
                    }
                }
            }
        ...
        public function upgradeLevelByAgent($openid)
        {
            ...
            foreach ($agents as $k => $agent) {
                ...
                while ($leveltype_arr) {
                    ...
                    switch ($leveltype) {
                        ...
                        case 0:
                            ...
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid and ' . $ordermoney . ' >= ordermoney
                                    order by ordermoney desc limit 1',
                                array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                                $newlevelarr[0] = $oldlevel['id'];
                        ...
                        case 4:
                            ...
                            $orders = pdo_fetch('select sum(og.realprice) as ordermoney,
                                ...
                                . ' left join  ' . OrderGoods::TABLE_NAME . ' og on og.orderid=o.id '
                                . ' where o.openid=:openid and o.status>=3 and o.uniacid=:uniacid limit 1',
                                array(':uniacid' => $_W['uniacid'], ':openid' => $agent['openid']));
                            $ordermoney = empty($orders['ordermoney']) ? 0 : $orders['ordermoney'];
                            $ordercount = $orders['ordercount'];
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid  and ' . $ordermoney . ' >= zg_ordermoney
                                        and zg_ordermoney>0
                                    order by zg_ordermoney desc limit 1',
                                array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                        ...
                        case 5:
                            ...
                            $ordercount = $orders['ordercount'];
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid  and ' . $ordercount . ' >= zg_ordercount
                                        and zg_ordercount>0
                                    order by zg_ordercount desc limit 1',
                                array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                        ...
                        case 6:
                            ...
                            $downs1 = pdo_fetchall('select id from ' . Member::TABLE_NAME
                                . ' where agentid=:agentid and uniacid=:uniacid ',
                                array(':agentid' => $agent['id'], ':uniacid' => $_W['uniacid']), 'id');
                            $downcount += count($downs1);
                            if (!(empty($downs1))) {
                                $downs2 = pdo_fetchall('select id from ' . Member::TABLE_NAME
                                    . ' where agentid in( ' . implode(',',
                                        array_keys($downs1)) . ') and uniacid=:uniacid',
                                    array(':uniacid' => $_W['uniacid']), 'id');
                                $downcount += count($downs2);
                                if (!(empty($downs2))) {
                                    $downs3 = pdo_fetchall('select id from ' . Member::TABLE_NAME
                                        . ' where agentid in( ' . implode(',',
                                            array_keys($downs2)) . ') and uniacid=:uniacid',
                                        array(':uniacid' => $_W['uniacid']), 'id');
                                    $downcount += count($downs3);
                                }
                            }
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid  and ' . $downcount . ' >= downcount and downcount>0
                                        order by downcount desc limit 1',
                                array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                                $newlevelarr[6] = $oldlevel['id'];
                            } else {
                        ...
                        case 7:
                            //一级下级人数满（分销商+非分销商）
                            $downcount = pdo_fetchcolumn('select count(*)
                                    from ' . Member::TABLE_NAME
                                . ' where agentid=:agentid and uniacid=:uniacid ',
                                array(':agentid' => $agent['id'], ':uniacid' => $_W['uniacid']));
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid  and ' . $downcount . ' >= first_downcount
                                        and first_downcount>0
                                    order by first_downcount desc limit 1',
                                array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                        ...
                        //下级总人数满(分销商)
                        case 8:
                            $commission_level = pdo_fetchall('select *
                                    from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid and id>:id ORDER BY id desc',
                                array(':uniacid' => $_W['uniacid'], ':id' => $oldlevel['id']));
                            if (empty($commission_level)) {
                                $newlevelarr[8] = $oldlevel['id'];
                            }
                            $newlevelarr[8] = $oldlevel['id'];
                            //从高等级往低等级走
                            foreach ($commission_level as $v) {
                                ...
                                for ($a = 0; $a < $v['hierarchy']; $a++) {
                                    $childids = pdo_fetchall('select id
                                            from ' . Member::TABLE_NAME
                                        . ' where uniacid=:uniacid and isagent=1
                                                and agentid in(' . $fatherids[$a] . ')',
                                        array(':uniacid' => $_W['uniacid']));
                                    if (empty($childids)) {
                                        break;
                                    }
                                ...
                                $team_identity = pdo_fetchcolumn('select count(*)
                                        from ' . Member::TABLE_NAME
                                    . ' where uniacid=:uniacid and agentid in(' . implode(',', $memberids) . ') '
                                    . $where,
                                    array(':uniacid' => $_W['uniacid']));
                        ...
                        case 9:
                            ...
                            $newlevelarr[9] = $oldlevel['id'];
                            foreach ($commission_level as $v) {
                                if (empty($v['first_team_downcount'])) {
                                    continue;
                                }
                                ...
                                $first_identity = pdo_fetchcolumn('select count(*)
                                        from ' . Member::TABLE_NAME
                                    . ' where uniacid=:uniacid and agentid=:agentid ' . $where,
                                    array(':uniacid' => $_W['uniacid'], ':agentid' => $agent['id']));
                        ...
                        case 10:
                            //已提现佣金总金额满
                            $info = $this->getInfo($agent['id'], array('pay'));
                            $commissionmoney = $info['commission_pay'];
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid  and ' . $commissionmoney . ' >= commissionmoney
                                        and commissionmoney>0  order by commissionmoney desc limit 1',
                                array(':uniacid' => $_W['uniacid']));
                            if (empty($newlevel)) {
                ...
                //修改会员信息
                if ($new_level > 0) {
                    pdo_update('ewei_shop_member', array('agentlevel' => $new_level), array('id' => $agent['id']));
                }
            }
        }
                ...
                if (($leveltype == 6) || ($leveltype == 8)) {
                    ...
                    foreach ($agents as $agent) {
                        ...
                        if ($leveltype == 6) {
                            $downs1 = pdo_fetchall('select id from ' . Member::TABLE_NAME
                                . ' where agentid=:agentid and uniacid=:uniacid ',
                                array(':agentid' => $m['id'], ':uniacid' => $_W['uniacid']), 'id');
                            $downcount += count($downs1);
                            if (!(empty($downs1))) {
                                $downs2 = pdo_fetchall('select id from ' . Member::TABLE_NAME
                                    . ' where agentid in( ' . implode(',', array_keys($downs1)) . ')
                                            and uniacid=:uniacid',
                                    array(':uniacid' => $_W['uniacid']), 'id');
                                $downcount += count($downs2);
                                if (!(empty($downs2))) {
                            ...
                            $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                                . ' where uniacid=:uniacid  and ' . $downcount . ' >= team_downcount
                                        and team_downcount>0  order by team_downcount desc limit 1',
                                array(':uniacid' => $_W['uniacid']));
                            if (!(empty($oldlevel['id']))
                                && $newlevel['team_downcount'] < $oldlevel['team_downcount']) {
                                continue;
                            }
                        }
                        if (empty($newlevel)) {
                    ...
                    if ($leveltype == 7) {
                        $downcount = pdo_fetchcolumn('select count(*) from ' . Member::TABLE_NAME
                            . ' where agentid=:agentid and uniacid=:uniacid ',
                            array(':agentid' => $m['id'], ':uniacid' => $_W['uniacid']));
                        $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                            . ' where uniacid=:uniacid  and ' . $downcount . ' >= first_downcount and first_downcount>0
                                order by first_downcount desc limit 1',
                            array(':uniacid' => $_W['uniacid']));
                        if (!(empty($oldlevel['id']))) {
                            if ($newlevel['first_downcount'] < $oldlevel['first_downcount']) {
                        ...
                        $newlevel = pdo_fetch('select * from ' . CommissionLevel::TABLE_NAME
                            . ' where uniacid=:uniacid  and ' . $downcount . ' >= first_team_downcount
                                    and first_team_downcount>0
                                order by first_team_downcount desc limit 1',
                            array(':uniacid' => $_W['uniacid']));
                        if (!(empty($oldlevel['id']))
                            && $newlevel['first_team_downcount'] < $oldlevel['first_team_downcount']) {
                            continue;
                        }
                    }
                    if (empty($newlevel)) {
        ...
        public function sendMessage($openid = '', $data = array(), $message_type = '')
        {
            ...
            if (($message_type == TM_COMMISSION_AGENT_NEW) && empty($usernotice['commission_agent_new'])) {
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        'first' => array(
                            'value' => '亲爱的' . $data['nickname'] . '，您新增了一个下级' . $data['nickname'],
                            'color' => '#ff0000'
                        ),
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
                        'keyword2' => array('title' => '业务状态', 'value' => '新增下级通知', 'color' => '#000000'),
                        'keyword3' => array(
                            'title' => '业务内容',
                            'value' => '您新增了一个下级' . $data['nickname'],
                            'color' => '#000000'
                        ),
                        'remark' => array('value' => "\n" . '感谢您的支持', 'color' => '#000000')
                    );
                    ...
                    $message = str_replace(array('[昵称]', '[下级昵称]', '[时间]'),
                        array(
                            $data['nickname'],
                            $data['nickname'],
                            date('Y-m-d H:i:s',
                    ...
                    $msg = array(
                        ...
                        'keyword2' => array(
                            'value' => (!(empty($tm['commission_agent_newtitle']))
                                ? $tm['commission_agent_newtitle'] : '新增下线通知'),
                            'color' => '#73a68d'
                        ),
            ...
            } elseif (($message_type == TM_COMMISSION_ORDER_PAY) && empty($usernotice['commission_order_pay'])) {
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        'first' => array(
                            'value' => '亲爱的' . $member['nickname'] . '，您的下级' . $data['nickname'] . '已付款',
                            'color' => '#ff0000'
                        ),
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
                    ...
                    $message = str_replace(
                        array('[昵称]', '[下级昵称]', '[时间]', '[订单编号]', '[订单金额]', '[商品详情]'),
                        array(
                            $member['nickname'],
                            $data['nickname'],
                    ...
                    $msg = array(
                        'keyword1' => array('value' => '会员通知', 'color' => '#73a68d'),
                        'keyword2' => array(
                            'value' => (!(empty($tm['commission_order_paytitle']))
                                ? $tm['commission_order_paytitle'] : '下线付款通知')
                        ),
                        'keyword3' => array('value' => $message)
                    );
                    return $this->sendNotice($openid, $tm, 'commission_order_pay_advanced', $data, $member, $msg);
                }
            } elseif (($message_type == TM_COMMISSION_ORDER_FINISH) && empty($usernotice['commission_order_finish'])) {
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        'first' => array(
                            'value' => '亲爱的' . $member['nickname'] . '，您的下级' . $data['nickname'] . '已确认收货',
                            'color' => '#ff0000'
                        ),
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
                    ...
                    $msg = array(
                        'keyword1' => array('value' => '会员通知', 'color' => '#73a68d'),
                        'keyword2' => array(
                            'value' => (!(empty($tm['commission_order_finishtitle']))
                                ? $tm['commission_order_finishtitle'] : '下线确认收货通知'),
                            'color' => '#73a68d'
                        ),
                        'keyword3' => array('value' => $message, 'color' => '#73a68d')
                    );
            ...
            } elseif (($message_type == TM_COMMISSION_APPLY) && empty($usernotice['commission_apply'])) {
                if ($tm['is_advanced']) {
                    if ($tm['commission_apply_close_advanced']) {
                        return false;
                    }
                    $tag = 'commission_apply';
                    $text = '您的' . $set['texts']['commission']
                        . '提现申请已提交！' . "\n" . date('Y-m-d H:i') . "\n";
                    $message = array(
                        'first' => array(
                            'value' => '亲爱的' . $member['nickname']
                                . '，您的' . $set['texts']['commission'] . '提现申请已提交',
                            'color' => '#ff0000'
                        ),
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', 'color' => '#000000'),
                    ...
                    $message = str_replace(
                        array('[昵称]', '[时间]', '[金额]', '[提现方式]'),
                        array($member['nickname'], date('Y-m-d H:i:s', time()), $data['commission'], $data['type']),
                        $message);
                    $msg = array(
                        'keyword1' => array('value' => '会员通知', 'color' => '#73a68d'),
                        'keyword2' => array(
            ...
            } elseif (($message_type == TM_COMMISSION_CHECK) && empty($usernotice['commission_check'])) {
                if ($tm['is_advanced']) {
                    ...
                    $message = str_replace(array('[昵称]', '[时间]', '[金额]', '[提现方式]'),
                        array($member['nickname'], date('Y-m-d H:i:s', time()), $data['commission'], $data['type']),
                        $message);
                    $msg = array(
                        'keyword1' => array('value' => '会员通知', 'color' => '#73a68d'),
            ...
            } elseif (($message_type == TM_COMMISSION_PAY) && empty($usernotice['commission_pay'])) {
                if ($tm['is_advanced']) {
            ...
            } elseif (($message_type == TM_COMMISSION_UPGRADE) && empty($usernotice['commission_upgrade'])) {
                if ($tm['is_advanced']) {
                    ...
                    $message = array(
                        ...
                        'keyword3' => array(
                            'title' => '业务内容',
                            'value' => '您的分销商等级从' . $data['oldlevel']['levelname'] . '升级为' . $data['oldlevel']['levelname'] . '特此通知!',
                            'color' => '#000000'
                        ),
                        'remark' => array('value' => "\n" . '感谢您的支持', 'color' => '#000000')
                    );
                    ...
                    $message = str_replace(array(
                        ...
                        $data['oldlevel']['levelname'],
                        $data['oldlevel']['commission1'] . '%',
                        $data['oldlevel']['commission2'] . '%',
                        $data['oldlevel']['commission3'] . '%',
                        $data['newlevel']['levelname'],
                        $data['newlevel']['commission1'] . '%',
                        $data['newlevel']['commission2'] . '%',
                        $data['newlevel']['commission3'] . '%'
                    ), $message);
                    $msg = array(
                        'keyword1' => array('value' => '会员通知', 'color' => '#73a68d'),
                        'keyword2' => array(
            ...
            } elseif (($message_type == TM_COMMISSION_BECOME) && empty($usernotice['commission_become'])) {
                if ($tm['is_advanced']) {
                    ...
                    $message = str_replace(array('[昵称]', '[时间]'),
                        array($data['nickname'], date('Y-m-d H:i:s', $data['agenttime'])), $message);
                    $msg = array(
                        'keyword1' => array('value' => '会员通知', 'color' => '#73a68d'),
                        'keyword2' => array(
            ...
            } elseif (($message_type == TM_COMMISSION_APPLYMONEY) && empty($usernotice['commission_apply'])) {
                if ($tm['is_advanced']) {
                    ...
                    $message = str_replace(array('[昵称]', '[时间]', '[金额]', '[提现方式]'),
                        array($member['nickname'], $time, $data['commission'], $data['type']), $message);
                    $msg = array(
                        'keyword1' => array('value' => '会员通知', 'color' => '#73a68d'),
                        'keyword2' => array(
                            'value' => (!(empty($tm['commission_applymoneytitle'])) ? $tm['commission_applymoneytitle'] : '分销商体现通知'),
                            'color' => '#73a68d'
                        ),
            ...
            if ($tm['is_advanced'] && (($tag == 'commission_agent_new')
                    || ($tag == 'commission_order_pay') || ($tag == 'commission_order_finish'))) {
                $res = $this->getAgentsByMember($toopenid, (int)$tm[$tag . '_advanced_notice']);
                $datas[] = array(
                    'name' => $set['texts']['down'],
                ...
                foreach ($res as $key => $value) {
                    if ($key == 0) {
                        ...
                        } elseif ($tag == 'commission_order_finish') {
        ...
        protected function sendNotice($touser, $tm, $tag, $datas, $member, $msg)
        {
            if (!(empty($tm['is_advanced'])) && !(empty($tm[$tag]))) {
                $advanced_template = pdo_fetch('select *
                        from ' . MemberMessageTemplate::TABLE_NAME
                    . ' where id=:id and uniacid=:uniacid limit 1',
                    array(':id' => $tm[$tag], ':uniacid' => $_W['uniacid']));
                if (!(empty($advanced_template))) {
                    $url = ((!(empty($advanced_template['url'])) ? $this->replaceTemplate($advanced_template['url'],
                        $tag, $datas, $member) : ''));
            ...
            } elseif (empty($msg['keyword2']['value'])) {
                return true;
            } else {
        ...
        public function getLastApply($mid, $type = -1)
        {
            ...
            $sql = 'select type,alipay,bankname,bankcard,realname from ' . CommissionApply::TABLE_NAME
        ...
        public function getRepurchase($openid, array $time)
        {
            ...
            foreach ($time as $value) {
                ...
                if (!(empty($repurchase_price))) {
                    $order_price = (double)pdo_fetchcolumn('SELECT SUM(price) as price
                            FROM ' . Order::TABLE_NAME
                        . ' WHERE `uniacid`=:uniacid AND `openid`=:openid AND `status`>2
                    ...
                    $residue_price = (double)pdo_fetchcolumn('SELECT SUM(repurchase)
        ...
        public function getAllDown($openid)
        {
            ...
            $uid = (int)$openid;
            if ($uid == 0) {
                $info = pdo_fetch('select id,openid,uniacid,agentid,isagent,status
                        from ' . Member::TABLE_NAME
            ...
            $agents = pdo_fetchall('select id,openid,uniacid,agentid,isagent,nickname,agenttime,createtime,
                        avatar,status,realname,mobile,weixin
                    from ' . Member::TABLE_NAME
                . ' where uniacid=:uniacid and agentid=:agentid',
                array(':uniacid' => $_W['uniacid'], ':agentid' => $uid));
            $ids = array();
            $openids = array();
            $users = array();
         ...
         * @retur array  奖励数组
         */
        public function getWeishopData($year = 0, $month = 0, $week = 0, $openid = '')
        {
            global $_W;
            $set = $this->getSet();
            $commission = m('common')->getPluginset('commission');
            ...
            $settletimes = (int)$set['settledays'] * 86400;
            if ((1 <= $week) && ($week <= 4)) {
                $weekdays = array();
                $i = $starttime;
                while ($i <= $endtime) {
            ...
            $bill = pdo_fetch('select * from ' . WeishopBill::TABLE_NAME
                . ' where uniacid=:uniacid and `year`=:year and `month`=:month and `week`=:week limit 1',
                array(':uniacid' => $_W['uniacid'], ':year' => $year, ':month' => $month, ':week' => $week));
            ...
            $bonusordermoney = 0;
            $bonusmoney = 0;
            $pcondition = ' AND is_shop_order=1';
            if (!(empty($openid))) {
                $member = m('member')->getMember($openid);
                $pcondition = 'AND finishtime>' . $member['partnertime'];
            }
            $orders = pdo_fetchall('select agentid,shop_diff_price,id,openid,price
                    from ' . Order::TABLE_NAME
                . ' where uniacid=' . $_W['uniacid'] . ' and status=3 and is_weishop_bonus=0
                        and finishtime + ' . $settletimes . '>= ' . $starttime . '
                        and  finishtime + ' . $settletimes . '<=' . $endtime . ' ' . $pcondition,
                array(), 'id');
            $pcondition = '';
            if (!(empty($openid))) {
            ...
            foreach ($orders as $k => $v) {
                $member = m('member')->getMember($v['agentid']);
                if (!in_array($member, $partners)) {
                    $partners[] = m('member')->getMember($v['agentid']);
                }
            }
            foreach ($orders as $o) {
                $ordermoney += $o['price'];
                $bonusordermoney += $o['shop_diff_price'];

                foreach ($partners as &$p) {
                    $price = $o['shop_diff_price'];
        ...
        //shop_member用户插入或建立关系
        public function promoter_qr($member, $qrmember)
        {
            $user_id = $member['id'];
            $old_flag = $member['gflag'];
            $old_generation = $member['generation'];
            $p_generation = $qrmember['generation'];
            $p_gflag = $qrmember['gflag'];
            $my_generation = 1;
            $my_gflag = ",-1,";
            if (!empty($qrmember['gflag'])) {
                $my_generation = $p_generation + 1;
                $my_gflag = $p_gflag . $qrmember['id'] . ",";
            }
            $add_generation = $my_generation - $old_generation;
            //修改被邀请人关系链
            pdo_update('ewei_shop_member', array('gflag' => $my_gflag, 'generation' => $my_generation),
                array('id' => $member['id']));
            //修改邀请人以及邀请人上级的关系链
            $pcondition = " WHERE gflag LIKE " . '"' . "%," . $user_id . ",%" . '"';
            $query = "update  " . pdo()->tablename('ewei_shop_member')
                . "set gflag=replace(gflag,'" . $old_flag . "','" . $my_gflag . "'),
                        generation=generation+" . $add_generation;
            $query .= $pcondition;
            pdo()->query($query);
        }
    }
