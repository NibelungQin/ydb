...
class Commission_Agent_EweiShopV2PluginWebPage extends PluginWebPage
{
    public function main()
    {
        ...
        if ($_GPC['followed'] != '') {
            if ($_GPC['followed'] == 2) {
                ...
                $condition .= ' and f.follow=' . (int)$_GPC['followed'] . ' and f.unfollowtime=0 ';
            }
        }
        ...
        if ($_GPC['agentlevel'] != '') {
            $condition .= ' and dm.agentlevel=' . (int)$_GPC['agentlevel'];
        }
        if ($_GPC['isagentblack'] != '') {
            $condition .= ' and dm.agentblack=' . (int)$_GPC['isagentblack'];
        }
        if ($_GPC['status'] != '') {
            $condition .= ' and dm.status=' . (int)$_GPC['status'];
        }
        if ($_GPC['agentblack'] != '') {
            $condition .= ' and dm.agentblack=' . (int)$_GPC['agentblack'];
        }
        $sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname,p.id as pid,p.nickname as parentname,
                    p.avatar as parentavatar,f.follow as followed, f.unfollowtime
                from ' . Member::TABLE_NAME . ' dm '
            . ' left join ' . Member::TABLE_NAME . ' p on p.id = dm.agentid '
            . ' left join ' . CommissionLevel::TABLE_NAME . ' l on l.id = dm.agentlevel '
            . ' left join ' . McMappingFans::TABLE_NAME . ' f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid']
            . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isagent =1  ' . $condition
            . ' ORDER BY dm.agenttime desc';
        if (empty($_GPC['export'])) {
            $sql .= ' limit ' . (($pindex - 1) * $psize) . ',' . $psize;
        }
        $list = pdo_fetchall($sql, $params);
        $total = pdo_fetchcolumn('select count(dm.id) from ' . Member::TABLE_NAME . ' dm  '
            . ' left join ' . Member::TABLE_NAME . ' p on p.id = dm.agentid '
    ...
    public function delete()
    {
        ...
        $members = pdo_fetchall('SELECT * FROM ' . Member::TABLE_NAME
            . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
        foreach ($members as $member) {
            pdo_update('ewei_shop_member', array('isagent' => 0, 'status' => 0), array('id' => $member['id']));
            plog('commission.agent.delete',
                '取消分销商资格 <br/>分销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid']
    ...
    public function user()
    {
        ...
        $level11 = pdo_fetchcolumn('select count(*) from ' . Member::TABLE_NAME
            . ' where isagent=0 and agentid=:agentid and uniacid=:uniacid limit 1',
            array(':agentid' => $agentid, ':uniacid' => $_W['uniacid']));
        $condition = '';
        $params = array();
        $hasagent = false;
        if (empty($level) && !(empty($this->set['level']))) {
            ...
            $condition .= ' )';
            $hasagent = true;
        } elseif (($level == 1) && (0 < $this->set['level'])) {
            if (0 < $level1) {
                $condition = ' and dm.agentid=' . $member['id'];
                $hasagent = true;
            }
        } elseif (($level == 2) && (1 < $this->set['level'])) {
            if (0 < $level2) {
                $condition = ' and dm.agentid in( ' . implode(',', array_keys($member['level1_agentids'])) . ')';
                $hasagent = true;
            }
        } elseif (($level == 3) && (2 < $this->set['level'])) {
            if (0 < $level3) {
        ...
        if ($_GPC['isagent'] != '') {
            $condition .= ' and dm.isagent=' . (int)$_GPC['isagent'];
        }
        if ($_GPC['agentlevel'] != '') {
            $condition .= ' and dm.agentlevel=' . (int)$_GPC['agentlevel'];
        }
        ...
        if ($hasagent) {
            $total = pdo_fetchcolumn('select count(dm.id) from ' . Member::TABLE_NAME . ' dm '
                . ' left join ' . Member::TABLE_NAME . ' p on p.id = dm.agentid '
                . ' left join ' . McMappingFans::TABLE_NAME . ' f on f.openid=dm.openid '
                . ' where dm.uniacid =' . $_W['uniacid'] . '  ' . $condition,
                $params);
            $sql = 'select dm.*,p.nickname as parentname,p.avatar as parentavatar,l.levelname as levelname
                    from ' . Member::TABLE_NAME . ' dm '
                . ' left join ' . Member::TABLE_NAME . ' p on p.id = dm.agentid '
                . ' left join ' . CommissionLevel::TABLE_NAME . ' l on l.id = dm.agentlevel '
                . ' left join ' . McMappingFans::TABLE_NAME . ' f on f.openid=dm.openid  and f.uniacid=' . $_W['uniacid']
            ...
            foreach ($list as &$row) {
                ...
                if ($row['agentid'] == $member['id']) {
                    $row['level'] = 1;
                } elseif (array_key_exists($row['agentid'], $member['level1_agentids'])) {
                    $row['level'] = 2;
                } elseif (array_key_exists($row['agentid'], $member['level2_agentids'])) {
                    $row['level'] = 3;
                }
            }
        }
    ...
    public function query()
    {
        ...
        $ds = pdo_fetchall('SELECT id,avatar,nickname,openid,realname,mobile
    ...
    public function check()
    {
        ...
        $members = pdo_fetchall('SELECT id,openid,agentid,nickname,realname,mobile,status
                FROM ' . Member::TABLE_NAME
            . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
        $time = time();
        foreach ($members as $member) {
            ...
            if ($status == 1) {
                ...
                plog('commission.agent.check',
                    '审核分销商 <br/>分销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid']
                    . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
                ...
                pdo_update('ewei_shop_member', array('status' => 0, 'agenttime' => 0),
                    array('id' => $member['id'], 'uniacid' => $_W['uniacid']));
                plog('commission.agent.check',
                    '取消审核 <br/>分销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid']
                    . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
            }
        }
        show_json(1, array('url' => referer()));
    ...
    public function agentblack()
    {
        ...
        $members = pdo_fetchall('SELECT id,openid,nickname,realname,mobile,agentblack
                FROM ' . Member::TABLE_NAME
            . ' WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
        foreach ($members as $member) {
            ...
            if ($agentblack == 1) {
                ...
                plog('commission.agent.agentblack',
                    '设置黑名单 <br/>分销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid']
                    . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
            } else {
                pdo_update('ewei_shop_member', array('agentblack' => 0), array('id' => $member['id']));
                plog('commission.agent.agentblack',
                    '取消黑名单 <br/>分销商信息:  ID: ' . $member['id'] . ' /  ' . $member['openid']
                    . '/' . $member['nickname'] . '/' . $member['realname'] . '/' . $member['mobile']);
            }
        }
        show_json(1, array('url' => referer()));
