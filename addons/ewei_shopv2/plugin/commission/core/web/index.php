...
class Commission_Index_EweiShopV2PluginWebPage extends PluginWebPage
{
    ...
    public function notice()
    {
        ...
        if (isset($data['openid1'])) {
            if (!(empty($data['openid1']))) {
                ...
                @$salers1 = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('ewei_shop_member') . ' where openid in (' . implode(',',
                        $openids1) . ') and uniacid=' . $_W['uniacid']);
            }
        }
        $salers2 = array();
        if (isset($data['openid2'])) {
            if (!(empty($data['openid2']))) {
                ...
                @$salers2 = pdo_fetchall('select id,nickname,avatar,openid from ' . tablename('ewei_shop_member') . ' where openid in (' . implode(',',
                        $openids2) . ') and uniacid=' . $_W['uniacid']);
            }
        }
        if ($_W['ispost']) {
        ...
        $template_lists = pdo_fetchall('SELECT id,title,typecode FROM ' . tablename('ewei_shop_member_message_template') . ' WHERE uniacid=:uniacid ',
    ...
    public function set()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (is_null($data['leveltype'])) {
                $data['leveltype'] = [];
            }
            $data['leveltype'] = json_encode($data['leveltype']);
            ...
            // 关注公众号奖励设置项
            $data['subcredit'] = (int)$_GPC['subcredit'];
            $data['submoney'] = $_GPC['submoney'];
            $data['reccredit'] = (int)$_GPC['reccredit'];
            $data['recmoney'] = $_GPC['recmoney'];
            $data['reccredit_totle'] = (int)$_GPC['reccredit_totle'];
            $data['recmoney_totle'] = (float)$_GPC['recmoney_totle'];
            $data['paytype'] = (int)$_GPC['paytype'];
            ...
            plog('commission.set.edit',
                '修改基本设置<br>' . '分销内购 -- ' . $selfbuy . '<br>成为下线条件 -- ' . $become_child . '<br>成为分销商条件 -- ' . $become);
            show_json(1,
                array('url' => webUrl('commission/set', array('tab' => str_replace('#tab_', '', $_GPC['tab'])))));
        }
        $styles = array();
        ...
        $data = m('common')->getPluginset('commission');
        $data['leveltype'] = json_decode($data['leveltype'], true);
        $goods = false;
        if (!(empty($data['become_goodsid']))) {
            $goods = pdo_fetch('select id,title,thumb from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1 ',
                array(':id' => $data['become_goodsid'], ':uniacid' => $_W['uniacid']));
        }
        $item = $data;
