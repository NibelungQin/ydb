...
class Commission_Level_EweiShopV2PluginWebPage extends PluginWebPage {
...
//        $shop_identity = pdo_fetchall('select id as identity_id,levelname from '. tablename('ewei_shop_globonus_level') .' where uniacid=:uniacid',array(':uniacid'=>$_W['uniacid']));
        $identity_data = array(array('identity_id'=>0,'levelname'=>'分销默认等级','identity_type'=>1));
...
//        }
        if ($_W['ispost']) {
            $data = array(
                ...
                'team_downcount' => intval($_GPC['team_downcount']),
                'first_team_downcount' => intval($_GPC['first_team_downcount']),
            );
            
            if(!empty($data['team_downcount']) && !empty($_GPC['team_identity_id'])){
            ...
            if(!empty($data['first_team_downcount']) && !empty($_GPC['first_team_identity_id'])){
