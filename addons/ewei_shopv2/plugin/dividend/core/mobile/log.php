...
class Dividend_Log_EweiShopV2PluginMobilePage extends DividendMobileLoginPage
{
    ...
    public function get_list()
    {
        ...
        $params = [':mid' => $member['id'], ':uniacid' => $_W['uniacid']];
        $status = trim($_GPC['status']);
        if ($status != '') {
            if ($status == -1) {
                $condition .= ' and (status=' . (int)$status . ' or status=-2)';
            } else {
                $condition .= ' and status=' . (int)$status;
            }
        }
        $dividendcount = pdo_fetchcolumn('
            select sum(dividend)
            from ' . DividendApply::TABLE_NAME . '
            where mid=:mid and uniacid=:uniacid and status>-1 limit 1',
    ...
    public function orders()
    {
        ...
        $apply = pdo_fetch('
            select * from ' . DividendApply::TABLE_NAME . '
            where id=:id and `mid`=:mid and uniacid=:uniacid limit 1',
            [':id' => $id, ':mid' => $member['id'], ':uniacid' => $_W['uniacid']]);
        if (empty($apply)) {
            show_json(0, ['message' => '未找到提现申请!']);
        }
        $orderids = explode(',', $apply['orderids']);
        if (!is_array($orderids) || count($orderids) <= 0) {
            show_json(0, ['message' => '未找到订单信息!']);
        }
        $list = pdo_fetchall('
            select id,headsid, ordersn,price,goodsprice, dispatchprice,createtime, paytype,dividend,
                   dividend_status,dividend_content
            from ' . Order::TABLE_NAME . '
            where  id in ( ' . $apply['orderids'] . ' ) LIMIT ' . ($pindex - 1) * $psize . ',' . $psize);
        $total = pdo_fetchcolumn('
            select count(*) from ' . Order::TABLE_NAME . '
        ...
        foreach ($list as &$row) {
            ...
            if (!empty($dividend)) {
                ...
                if (2 <= $row['dividend_status']) {
                    $orderpay = (isset($dividend['dividend_price']) ? (float)$dividend['dividend_price'] : 0);
                }
            }
            $set_array = array();
        ...
        unset($row);
        show_json(1, ['list' => $list, 'pagesize' => $psize, 'total' => $total, 'totaldividend' => $totaldividend]);
    }
}