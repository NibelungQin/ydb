...
class Dividend_Order_EweiShopV2PluginMobilePage extends DividendMobileLoginPage
{
    ...
    public function get_list()
    {
        ...
        $list = pdo_fetchall('
            select id,ordersn,openid,createtime,price,status,dividend
            from ' . Order::TABLE_NAME . '
            where headsid = ' . $member['id'] . $condition . ' and uniacid = :uniacid
            order by id desc
        ...
        if (!empty($list)) {
            foreach ($list as &$row) {
                ...
                $goods = pdo_fetchall('
                    select og.id,og.goodsid,g.thumb,og.price,og.optionname,g.title
                    from ' . OrderGoods::TABLE_NAME . ' og
                        left join ' . Goods::TABLE_NAME . ' g on g.id = og.goodsid
                    where og.orderid = :orderid and og.uniacid = :uniacid order by og.createtime desc',
                    array(':orderid' => $row['id'], ':uniacid' => $_W['uniacid']));
