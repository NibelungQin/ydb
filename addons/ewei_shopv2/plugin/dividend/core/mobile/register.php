...
class Dividend_Register_EweiShopV2PluginMobilePage extends DividendMobileLoginPage
{
    public function main()
    {
        ...
        if ($member['isagent'] != 1 || $member['status'] != 1) {
            YdbHttpUtil::header('location: ' . mobileUrl('commission/register'));
            ExceptionUtil::exit('请先成为分销商', true);
        }
        if ($member['isheads'] == 1 && $member['headsstatus'] == 1) {
            YdbHttpUtil::header('location: ' . mobileUrl('dividend'));
            return;
        }
        if ($member['headsblack']) {
            include($this->template());
            ExceptionUtil::exit('团长身份已列入黑名单', true);
        }
        $apply_set = array();
            ...
            if ($template_flag == 1) {
                ...
                if ($check == 1) {
                    $this->model->sendMessage($member['openid'],
                        array('nickname' => $member['nickname'], 'headstime' => $m_data['headstime']),
                        TM_DIVIDEND_BECOME);
                }
                ...
                if ($check == 1) {
                    $this->model->sendMessage($member['openid'],
                        array('nickname' => $member['nickname'], 'headstime' => $data['headstime']),
                        TM_DIVIDEND_BECOME);
                }
                if (!empty($member['uid'])) {
                    m('member')->mc_update($member['uid'],
                        array('realname' => $data['realname'], 'mobile' => $data['mobile']));
                }
            }
        ...
        if ($set['condition'] == 1) {
            ...
            $membercount = pdo_fetchcolumn('
                ...
                where  agentid = ' . $member['id'] . ' and id <> ' . $member['id'] . '  and uniacid=:uniacid  limit 1',
                array(':uniacid' => $_W['uniacid']));
            if ($membercount < (int)$set['downline']) {
                $status = 0;
        ...
        } elseif ($set['condition'] == 2) {
            $status = 1;
            $commissiondownlinecount = pdo_fetchcolumn('
                select count(*) from ' . Member::TABLE_NAME . '
                where  agentid = ' . $member['id'] . ' and isagent = 1
                    and id <> ' . $member["id"] . '  and uniacid=:uniacid  limit 1',
                array(':uniacid' => $_W['uniacid']));
            if ($commissiondownlinecount < (float)$set['commissiondownline']) {
                $status = 0;
