...
class Dividend_Bank_EweiShopV2PluginMobilePage extends DividendWebPage
{
    public function main()
    {
        ...
        $total = pdo_fetchcolumn('
            SELECT count(1) FROM ' . DividendBank::TABLE_NAME . '
    ...
    public function delete()
    {
        ...
        $items = pdo_fetchall('
            SELECT id FROM ' . DividendBank::TABLE_NAME . '
    ...
    public function status()
    {
        ...
        $items = pdo_fetchall('
            ...
            WHERE id in( ' . $id . ' ) AND uniacid=' . $_W['uniacid']);
        foreach ($items as $item) {
            pdo_update('ewei_shop_dividend_bank', array('status' => (int)$_GPC['status']),
                array('id' => $item['id']));
        }
        show_json(1, array('url' => referer()));
    }
