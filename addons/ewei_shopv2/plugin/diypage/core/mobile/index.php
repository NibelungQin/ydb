...
if (!defined('IN_IA')) {
	ExceptionUtil::exit('Access Denied');
}

class Diypage_Index_EweiShopV2PluginMobilePage extends PluginMobilePage
{
	...
	public function getInfo()
	{
		...
		if ($urlData[0] == 'goodsids' || $urlData[0] == 'category' || $urlData[0] == 'groups') {
			...
			if ($urlData[0] == 'category') {
				$pcate = $urlValue[0];
				$goodsql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,productprice,minprice,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,hascommission,commission1_pay,commission,total,description,bargain,nocommission,`type`,ispresell,`virtual`,hasoption,video FROM ' . tablename('ewei_shop_goods') . ' WHERE FIND_IN_SET(' . $pcate . ',cates) AND status > 0 AND deleted = 0 AND uniacid =' . $_W['uniacid'] . ' limit 0,' . $goodsnum;
				...
				if ($urlData[0] == 'goodsids') {
					...
					if (!empty($goodsids)) {
						...
						$sql = 'SELECT id,displayorder,title,subtitle,thumb,marketprice,productprice,minprice,maxprice,hascommission,nocommission,commission,commission1_rate,marketprice,commission,commission1_pay,maxprice,isdiscount,isdiscount_time,isdiscount_discounts,sales,salesreal,total,description,bargain,`type`,ispresell,`virtual`,hasoption,video FROM ' . tablename('ewei_shop_goods') . ' WHERE id in(' . $goodsid . ') AND uniacid =' . $_W['uniacid'] . ' limit 0,' . $goodsnum;
						$count = pdo_fetch('SELECT count(id) as count FROM ' . tablename('ewei_shop_goods') . ' WHERE id in(' . $goodsid . ') AND uniacid =' . $_W['uniacid']);
	...
	public function getHomeDiypageMoreGoods(){
		global $_W;
		global $_GPC;
		$template_id = intval($_GPC['template_id']);
		$itemid = $_GPC['itemid'];
		$page_index = intval($_GPC['page_index']);
		$page_size = intval($_GPC['page_size']);
		$title="goods";
		$item = [];
		$check_page_goods = $itemid;
		$this->model->getDiypagePageItemsByPage($item,$itemid,$title,$page_index,$page_size,$check_page_goods,$template_id);
	}
}
