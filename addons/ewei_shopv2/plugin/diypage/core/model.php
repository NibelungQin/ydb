...
class DiypageModel extends PluginModel
{
    ...
    public function getDiypagePageItemsByPage($item,$itemid,$title,$page_index=1,$page_size=10,$check_page_goods,$template_id = 0){
        global  $_W;
        if(!empty($template_id)){
            $wapset = m('common')->getSysset('wap');
            $where = ' where id=:id and uniacid=:uniacid';
            $params = array(':id' => $template_id, ':uniacid' => $_W['uniacid']);
            ...
            $check_page_goods = '';
            $last = end($page['data']['items']);
            if($last['id']=='goods'){
                $check_page_goods = key($page['data']['items']);
            }
           $item = $last;
        }
        $total_goodss_num = 0;
        if(isset($page['data']['items'][$itemid]['data'])){
            $total_goodss_num = count($page['data']['items'][$itemid]['data']);
        }
            $creditshop = !empty($item['params']['goodstype']) ? true : false;
            if ($item['params']['goodsdata'] == '0') {
                if (!empty($item['data']) && is_array($item['data'])) {
                    $goodsids = array();
                    if($check_page_goods==$itemid){
                        $item['data'] = PageUtil::page_array($page_size,$page_index,$item['data'],1);
     ...
     * @return bool|void
     */
    public function getFrontPage($id, $mobile = false)
    {
        global $_W;
            ...
            if (!empty($page['data']['items']) && is_array($page['data']['items'])) {
                foreach ($page['data']['items'] as $itemid => &$item) {
                    if ($item['id'] == 'goods') {
                        $page_index=1;
                        $page_size=1000;
                        $item = $this->getDiypagePageItemsByPage($item,$itemid,$item['id'],$page_index,$page_size,$check_page_goods);
                        if (empty($item['data'])) {
        ...
        if ($do == 'add') {
            if (!empty($type)) {
                ...
                if ($getpagetype != $pagetype) {
                    ...
                    YdbHttpUtil::header('location: ' . $url);
                    ExceptionUtil::exit();
                }
    ...
    protected function verifymobile($id = 0, $type = 0)
    {
        ...
        if ($_GPC['r'] == 'diypage') {
            if ($type == 2 && $page['home'] == $id) {
                YdbHttpUtil::header('location: ' . mobileUrl(NULL, array('mid' => $_GPC['mid'])));
            } else {
                if ($type == 3 && $page['member'] == $id) {
                    YdbHttpUtil::header('location: ' . mobileUrl('member', array('mid' => $_GPC['mid'])));
                } else {
                    if ($type == 4 && $page['commission'] == $id) {
                        YdbHttpUtil::header('location: ' . mobileUrl('commission', array('mid' => $_GPC['mid'])));
                    }
                }
            }
        }
