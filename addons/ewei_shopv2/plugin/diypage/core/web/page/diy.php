...
class Diypage_Page_Diy_EweiShopV2PluginWebPage extends PluginWebPage
{
	...
	protected function post($do)
	{
		global $_W;
		global $_GPC;
		if(!empty($_GPC['jsonData'])){
			$json_array_data = json_decode(html_entity_decode($_GPC['jsonData']),true);
			array_walk($json_array_data,function($value,$key) use (&$_GPC){
				$_GPC[$key] = $value;
			});
		}
