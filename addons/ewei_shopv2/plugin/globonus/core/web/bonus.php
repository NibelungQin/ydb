...
class Bonus_EweiShopV2Page extends PluginWebPage
{
	...
	public function detail()
	{
		...
		if ($_GPC['export'] == 1) {
			...
			plog('globonus.bonus.detail.export', '导出结算单店铺数据 ID: ' . $data['id'] . ' 单号: ' . $data['billno']);
	...
	public function build()
	{
		...
		if ($_W['ispost']) {
			...
			$bill = array('uniacid' => $_W['uniacid'],
				'billno' => m('common')->createNO('globonus_bill', 'billno', 'GB'),//编号
				...
				'partnercount' => $data['partnercount'],//股东数
	...
	public function payp()
	{
		...
		if (!empty($member)) {
			$result = m('finance')->pay($partner['openid'], $moneytype, $pay, $partner['payno'], '店铺奖励', false);
			...
			if (is_error($result)) {
				...
				$this->model->upgradeLevelByAgent($partner['openid']);
