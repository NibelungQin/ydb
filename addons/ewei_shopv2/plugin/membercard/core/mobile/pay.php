...
class Pay_EweiShopV2Page extends PluginMobileLoginPage
{
	public function main()
	{
		...
		if (is_h5app()) {
			$payinfo = array('wechat' => !empty($sec['app_wechat']['merchname']) && !empty($set['pay']['app_wechat']) && !empty($sec['app_wechat']['appid']) && !empty($sec['app_wechat']['appsecret']) && !empty($sec['app_wechat']['merchid']) && !empty($sec['app_wechat']['apikey']) && 0 < $order['total'] ? true : false, 'alipay' => false, 'mcname' => $sec['app_wechat']['merchname'], 'card_id' => $order['member_card_id'], 'orderno' => $order['orderno'], 'money' => $order['total'], 'attach' => $_W['uniacid'] . ':5', 'type' => 5, 'orderid' => $orderid, 'credit' => $credit);
		}
