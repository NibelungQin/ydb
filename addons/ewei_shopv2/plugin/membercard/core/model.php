...
class MembercardModel extends PluginModel
{
	...
	public function querycoupon($couponid)
	{
		...
		foreach ($couponid as $ck => $cv) {
			...
			$cpsql = 'SELECT id,catid,couponname,bgcolor,enough,coupontype,timestart,timeend,discount,deduct,backtype,backmoney,backcredit,backredpack,createtime,total,limitgoodcatetype,limitgoodids,limitgoodcatetype,limitgoodcateids FROM ' . tablename('ewei_shop_coupon') . $where;
