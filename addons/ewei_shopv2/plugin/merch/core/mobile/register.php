<?php
class Merch_Register_EweiShopV2PluginMobilePage extends PluginMobileLoginPage
{
	...
	public function main() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$upass = $_GPC['upass'];
            $parent_mobile = trim($_GPC['parent_mobile']);//推荐人手机号
			if (empty($uname)) 
			{
				show_json(0, '请填写帐号!');
			}
			...
			if(!empty($parent_mobile)){
			   ...
			   $parents=m('member')->getMobileMember($parent_mobile);
			   if(!$parents){
                   show_json(0, '推荐人手机号不存在!');
               }
            }
			...
			if($parents){
			    $data['parent_mobile']=$parents['mobile'];
                $data['parent_openid']=$parents['openid'];
            }
			if ($template_flag == 1)
			{
				$mdata = $_GPC['mdata'];
