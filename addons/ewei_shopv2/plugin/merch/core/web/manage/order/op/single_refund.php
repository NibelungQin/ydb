...
class Merch_Order_Op_Single_Refund_EweiShopV2MerchWebPage extends MerchWebPage
{
    protected function opData()
    {
        ...
        global $_GPC;
        $id = intval($_GPC['id']);
        $item = pdo_fetch('select o.*,og.id as ordergoodsid,og.single_refundid,og.single_refundstate,og.single_refundtime from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id=og.orderid' . ' where og.id=:ordergoodsid limit 1',
        ...
        if (!empty($item['single_refundid'])) {
            $refund = pdo_fetch('select * from ' . tablename('ewei_shop_order_single_refund') . ' where id=:id limit 1',
                array(':id' => $item['single_refundid']));
            $refund['imgs'] = iunserializer($refund['imgs']);
        }

        return array('id' => $id, 'item' => $item, 'refund' => $refund, 'r_type' => OrderRefund::REFUND_TYPE);
    }
    ...
    public function submit()
    {
        ...
        if ($_W['ispost']) {
            ...
            if (empty($refund['refundno'])) {
                ...
                pdo_update('ewei_shop_order_single_refund', array('refundno' => $refund['refundno']),
                    array('id' => $refund['id']));
            }
            ...
            if ($refundstatus == 0) {
                ...
                if ($refundstatus == 3) {
                    ...
                    if ($raid == 0) {
                        $raddress = pdo_fetch('select * from ' . tablename('ewei_shop_refund_address') . ' where isdefault=1 and uniacid=:uniacid and merchid=0 limit 1',
                            array(':uniacid' => $uniacid));
                    } else {
                        $raddress = pdo_fetch('select * from ' . tablename('ewei_shop_refund_address') . ' where id=:id and uniacid=:uniacid and merchid=0 limit 1',
                            array(':id' => $raid, ':uniacid' => $uniacid));
                    }

                    if (empty($raddress)) {
                        $raddress = pdo_fetch('select * from ' . tablename('ewei_shop_refund_address') . ' where uniacid=:uniacid and merchid=0 order by id desc limit 1',
                            array(':uniacid' => $uniacid));
                    }
                    ...
                    pdo_update('ewei_shop_order_single_refund', $change_refund,
                        array('id' => $item['single_refundid']));
                    ...
                    if ($refundstatus == 5) {
                        ...
                        if ($refundstatus == 10) {
                            ...
                            pdo_update('ewei_shop_order_single_refund', $refund_data,
                                array('id' => $item['single_refundid'], 'uniacid' => $uniacid));
                            $order_data = array();
                            $order_data['single_refundstate'] = 9;
                            pdo_update('ewei_shop_order_goods', $order_data,
                                array('id' => $item['ordergoodsid'], 'uniacid' => $uniacid));
                            m('notice')->sendOrderMessage($item['id'], true);
                        } else {
                            if ($refundstatus == 1) {
                                if (0 < $item['parentid']) {
                                    $parent_item = pdo_fetch('SELECT id,ordersn,ordersn2,price,transid,paytype,apppay FROM ' . tablename('ewei_shop_order') . ' WHERE id = :id and uniacid=:uniacid Limit 1',
                                        array(':id' => $item['parentid'], ':uniacid' => $_W['uniacid']));
                                ...
                                if (!empty($item['transid']) && $item['paytype'] == 22 && empty($item['apppay']) && strexists($item['borrowopenid'],
                                        '2088')) {
                                    $item['paytype'] = 23;
                                }
                                ...
                                if ($item['paytype'] == 1) {
                                    m('member')->setCredit($item['openid'], 'credit2', $pay_refund_price, array(
                                        0,
                                        $shopset['name'] . ('退款: ' . $pay_refund_price . '元 订单号: ') . $item['ordersn']
                                    ));
                                    $result = true;
                                    $refundtype = 0;
                                } else {
                                    if ($item['paytype'] == 21) {
                                        if ($item['apppay'] == 2) {
                                            $result = m('finance')->wxapp_refund($item['openid'], $ordersn,
                                                $refund['refundno'], $order_price * 100, $pay_refund_price * 100,
                                                !empty($item['apppay']) ? true : false);
                                        } else {
                                            if (0 < $pay_refund_price) {
                                                if (empty($item['isborrow'])) {
                                                    $result = m('finance')->refund($item['openid'], $ordersn,
                                                        $refund['refundno'], $order_price * 100,
                                                        $pay_refund_price * 100,
                                                        !empty($item['apppay']) ? true : false);
                                                } else {
                                                    $result = m('finance')->refundBorrow($item['borrowopenid'],
                                                        $ordersn, $refund['refundno'], $order_price * 100,
                                                        $pay_refund_price * 100, !empty($item['ordersn2']) ? 1 : 0);
                                                }
                                            }
                                        }
                                        ...
                                        if ($item['paytype'] == 22) {
                                            ...
                                            if (!empty($item['apppay'])) {
                                                ...
                                                if (!empty($sec['alipay_pay'])) {
                                                    ...
                                                    $config = array(
                                                        'app_id' => $sec['alipay_pay']['appid'],
                                                        'privatekey' => $sec['alipay_pay']['private_key'],
                                                        'publickey' => '',
                                                        'alipublickey' => '',
                                                        'sign_type' => $sign_type
                                                    );
                                            ...
                                            if ($item['paytype'] == 23 && !empty($item['isborrow'])) {
                                                $result = m('finance')->refundBorrow($item['borrowopenid'], $ordersn,
                                                    $refund['refundno'], $order_price * 100, $pay_refund_price * 100,
                                                ...
                                                if (0 < $pay_refund_price) {
                                                    $result = m('finance')->pay($item['openid'], 1,
                                                        $pay_refund_price * 100, $refund['refundno'],
                                                        $shopset['name'] . ('退款: ' . $pay_refund_price . '元 订单号: ') . $item['ordersn']);
                                                }
                                ...
                                if ($refundstatus == -1) {
                                    pdo_update('ewei_shop_order_single_refund',
                                        array('reply' => $refundcontent, 'status' => -1, 'endtime' => $time),
                                        array('id' => $item['single_refundid']));
                                    plog('order.op.refund.submit',
                                        '订单退款拒绝 ID: ' . $item['id'] . ' 订单号: ' . $item['ordersn'] . ' 原因: ' . $refundcontent);
                                    pdo_update('ewei_shop_order_goods', array('single_refundstate' => 8),
                                        array('id' => $item['ordergoodsid'], 'uniacid' => $uniacid));
                                    $is_single_refund = pdo_fetchcolumn('select count(id) from ' . tablename('ewei_shop_order_goods') . 'where orderid=:orderid and (single_refundstate=1 or single_refundstate=2)',
                                        array(':orderid' => $item['id']));

                                    if (empty($is_single_refund)) {
                                        pdo_update('ewei_shop_order', array('refundtime' => $time),
                                            array('id' => $item['id'], 'uniacid' => $uniacid));
                                    }
        ...
        $refund_address = pdo_fetchall('select * from ' . tablename('ewei_shop_refund_address') . ' where uniacid=:uniacid and merchid=0',
    ...
    public function refund_after($item, $time, $shopset)
    {
        global $_W;
        global $_GPC;
        $goods = pdo_fetch('SELECT og.goodsid,og.total,g.totalcnf,og.realprice,g.money,og.optionid,g.total as goodstotal,og.optionid,g.sales,g.salesreal,g.credit,og.seckill,og.consume FROM ' . tablename('ewei_shop_order_goods') . 'og' . ' left join ' . tablename('ewei_shop_goods') . ' g on og.goodsid=g.id ' . ' WHERE og.id=:ordergoodsid and og.uniacid=:uniacid limit 1',
            array(':ordergoodsid' => $item['ordergoodsid'], ':uniacid' => $_W['uniacid']));
        $consume = iunserializer($goods['consume']);
        $credit1 = m('ordergoods')->getGoodsCredit1($goods);
        if ($item['status'] == 3 && 0 < $credit1) {
            m('member')->setCredit($item['openid'], 'credit1', 0 - $credit1,
                array(0, $shopset['name'] . '购物取消订单扣除积分 订单号: ' . $item['ordersn']));
        ...
        if (0 < $credit2) {
            if (1 <= $item['status']) {
                m('member')->setCredit($item['openid'], 'credit2', 0 - $credit2,
                    array(0, $shopset['name'] . '购物取消订单扣除赠送余额 订单号: ' . $item['ordersn']));
            }
        }

        if (!empty($consume)) {
            if (!empty($consume['consume_deduct'])) {
                m('member')->setCredit($item['openid'], 'credit1', $consume['consume_deduct'],
                    array(0, $shopset['name'] . '返还抵扣积分 订单号: ' . $item['ordersn']));
            }

            if (!empty($consume['consume_deduct2'])) {
                m('member')->setCredit($item['openid'], 'credit2', $consume['consume_deduct2'],
                    array(0, $shopset['name'] . '返还抵扣余额 订单号: ' . $item['ordersn']));
            }
        }

        m('ordergoods')->setStock($goods);
        pdo_update('ewei_shop_order_goods',
            array('single_refundstate' => 9, 'single_refundtime' => $time, 'nocommission' => 1),
            array('id' => $item['ordergoodsid'], 'uniacid' => $_W['uniacid']));
        $order_goods = pdo_fetchall('select single_refundid,single_refundstate,single_refundtime from ' . tablename('ewei_shop_order_goods') . ' where orderid=:orderid',
            array(':orderid' => $item['id']));
        $refund_num = 0;
        $apply_refund_num = 0;
        ...
        $salesreal = pdo_fetchcolumn('select ifnull(sum(total),0) from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid ' . ' where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1',
            array(':goodsid' => $item['ordergoodsid'], ':uniacid' => $_W['uniacid']));
    ...
    public function main()
    {
        ...
        if (0 <= $refund['status']) {
            ...
            if ($refund['status'] == -1) {
                ...
                $step_array[3]['done'] = 1;
                $step_array[3]['title'] = '拒绝' . OrderRefund::REFUND_TYPE[$refund['rtype']];
                ...
                if ($refund['status'] == -2) {
                    ...
                    $step_array[3]['done'] = 1;
                    $step_array[3]['title'] = '客户取消' . OrderRefund::REFUND_TYPE[$refund['rtype']];
                    $step_array[3]['time'] = $refund['refundtime'];
                }
            }
        }

        $goods = pdo_fetch('SELECT g.*, og.goodssn as option_goodssn, og.productsn as option_productsn,og.total,g.type,og.optionname,og.optionid,og.price as orderprice,og.realprice,og.changeprice,og.oldprice,og.commission1,og.commission2,og.commission3,og.commissions ' . $diyformfields . ' FROM ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on og.goodsid=g.id ' . ' WHERE og.id=:ordergoodsid and og.uniacid=:uniacid',
            array(':ordergoodsid' => $refund['ordergoodsid'], ':uniacid' => $_W['uniacid']));
