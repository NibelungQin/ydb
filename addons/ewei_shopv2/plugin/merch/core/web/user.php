...
class User_EweiShopV2Page extends PluginWebPage 
{
	public function main() 
	{
        ...
        $parent_openid = trim($_GPC['parent_openid']);
		if (!(empty($keyword))) 
		{
		...
		if(!empty($parent_openid)){
            $condition .= ' and u.parent_openid=' ."'". $parent_openid."'";
        }
		if ($_GPC['groupid'] != '')
		{
	...
	protected function post() 
	{
        ...
        if (!(empty($item['parent_openid'])))
        {
            $parent_user = m('member')->getMember($item['parent_openid']);
        }
		...
		if ($_W['ispost']) 
		{
			...
			$data = array('uniacid' => $_W['uniacid'], 'merchname' => trim($_GPC['merchname']), 'salecate' => trim($_GPC['salecate']), 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']), 'address' => trim($_GPC['address']), 'tel' => trim($_GPC['tel']), 'lng' => $_GPC['map']['lng'], 'lat' => $_GPC['map']['lat'], 'accounttime' => strtotime($_GPC['accounttime']), 'accounttotal' => intval($_GPC['accounttotal']), 'maxgoods' => intval($_GPC['maxgoods']), 'groupid' => intval($_GPC['groupid']), 'cateid' => intval($_GPC['cateid']), 'isrecommand' => intval($_GPC['isrecommand']), 'remark' => trim($_GPC['remark']), 'status' => $status, 'desc' => trim($_GPC['desc1']), 'logo' => save_media($_GPC['logo']), 'parent_openid' => trim($_GPC['parent_openid']),'payopenid' => trim($_GPC['payopenid']), 'payrate' => trim($_GPC['payrate'], '%'), 'pluginset' => iserializer($_GPC['pluginset']), 'creditrate' => intval($_GPC['creditrate']), 'iscredit' => intval($_GPC['iscredit']), 'iscreditmoney' => intval($_GPC['iscreditmoney']));
			if ($diyform_flag) 
			{
