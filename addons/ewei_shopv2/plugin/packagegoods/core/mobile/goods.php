...
class Goods_EweiShopV2Page extends PluginMobileLoginPage 
{
	...
	public function main() {
		...
		$uniacid = $_W['uniacid'];
		$id = intval($_GPC['id']);
		$groupsset = pdo_fetch('select description,packagegoods_description from ' . tablename('ewei_shop_packagegoods_set') . "\r\n\t\t\t\t\t" . 'where uniacid = :uniacid ', array(':uniacid' => $uniacid));
		$groupsset['packagegoods_description'] = m('ui')->lazy($groupsset['packagegoods_description']);
		...
		if (empty($id) || empty($goods)) {
			$this->message('你访问的礼包不存在或已删除!', mobileUrl('packagegoods'), 'error');
		}
		if (!(empty($goods['thumb_url']))) {
			$goods['thumb_url'] = array_merge(iunserializer($goods['thumb_url']));
		}
		$goods = set_medias($goods, 'thumb');
		$goods['fightnum'] = pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_packagegoods_order') . ' where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ', array(':goodid' => $goods['id'], ':uniacid' => $uniacid));
		$goods['fightnum'] = $goods['teamnum'] + $goods['fightnum'];
		$goods['content'] = m('ui')->lazy($goods['content']);
		if (empty($goods)) {
			$this->message('礼包已下架或被删除!', mobileUrl('packagegoods'), 'error');
		}

		$url = array('pay' => mobileUrl('packagegoods/goods/pay'), 'lottery' => mobileUrl('packagegoods/goods/lottery'));
		$set = $_W['shopset'];
		$_W['shopshare'] = array(
			...
			'link' => mobileUrl('packagegoods/goods', array('id' => $id,'mid'=>$_W['mid']), true)
		);
	...
	public function goodsCheck(){
		...
		try {
			...
			$openid = $_W['openid'];
			if (empty($id)) {
				show_json(0, array('message' => '礼包不存在！'));
			}
			$goods = pdo_fetch('select * from ' . tablename('ewei_shop_packagegoods_goods') . "\r\n\t\t\t\t\t" . 'where id = :id and status = :status and uniacid = :uniacid and deleted = 0 order by displayorder desc', array(':id' => $id, ':uniacid' => $uniacid, ':status' => 1));
			if (empty($goods)) {
				show_json(0, array('message' => '礼包不存在！'));
			...
			if ($goods['stock'] <= 0) {
				show_json(0, array('message' => '您选择的礼包库存不足，请浏览其他礼包或联系平台！'));
			}

			if (empty($goods['status'])) {
				show_json(0, array('message' => '您选择的礼包已经下架，请浏览其他礼包或联系平台！'));
			}
...
//				show_json(0, array('message' => '您已到达此礼包购买上限，请浏览其他礼包或联系平台！'));
//			}
			...
			if (empty($goods['stock'])) {
				show_json(0, array('message' => '礼包库存为0，暂时无法购买，请浏览其他礼包！'));
			}
			$specArr = array();
	...
	public function openGroups(){
		...
		if (empty($id)) {
			$this->message('你访问的商品不存在或已删除!', mobileUrl('packagegoods'), 'error');
		}
		...
		if (empty($goods)) {
			$this->message('商品已下架或被删除!', mobileUrl('packagegoods'), 'error');
		}
		include $this->template();
	}
