...
class Refund_EweiShopV2Page extends PluginMobileLoginPage 
{
	protected function globalData() 
	{
		...
		if (empty($order)) {
			if (!($_W['isajax'])) {
				header('location: ' . mobileUrl('packagegoods/orders'));exit();
			}else {
				show_json(0, '订单未找到');
			}
		}
		...
		$goodRefund = false;
		$groupsSet = pdo_fetch('select goodsid,refundday from ' . tablename('ewei_shop_packagegoods_set') . 'where uniacid = :uniacid ', array(':uniacid' => $uniacid));
		...
		if (!(empty($_err))) {
			if ($_W['isajax']) {
				...
				$this->message($_err, mobileUrl('packagegoods/index'), 'error');
			}
		}
	...
	public function main() {
		...
		global $_GPC;
		try {
			extract($this->globalData());//把值赋值给键
			if ($order['status'] == '-1') {
			...
			if (0 < $order['refundstate']) {
				if (!(empty($refundid))) {
					$refund = pdo_fetch('select * from ' . tablename('ewei_shop_packagegoods_order_refund') . ' where id=:id and uniacid=:uniacid and orderid=:orderid limit 1', array(':id' => $refundid, ':uniacid' => $uniacid, ':orderid' => $orderid));
	...
	public function submit(){
		...
		if ($order['refundstate'] == 0) {
			...
			pdo_insert('ewei_shop_packagegoods_order_refund', $refund);
			$refundid = pdo_insertid();
			pdo_update('ewei_shop_packagegoods_order', array('refundid' => $refundid, 'refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
		}else {
			pdo_update('ewei_shop_packagegoods_order', array('refundstate' => $refundstate), array('id' => $orderid, 'uniacid' => $uniacid));
			pdo_update('ewei_shop_packagegoods_order_refund', $refund, array('id' => $refundid, 'uniacid' => $uniacid));
		}
		p('packagegoods')->sendTeamMessage($orderid, true);
		show_json(1);
	}
	//取消订单退款申请
	public function cancel(){
		...
		pdo_update('ewei_shop_packagegoods_order_refund', $change_refund, array('id' => $refundid, 'uniacid' => $uniacid));
		pdo_update('ewei_shop_packagegoods_order', array('refundstate' => 0), array('id' => $orderid, 'uniacid' => $uniacid));
		show_json(1);
	}
	...
	public function receive(){
		...
		pdo_update('ewei_shop_packagegoods_order_refund', $refund_data, array('id' => $refundid, 'uniacid' => $uniacid));
		$order_data = array();
		$order_data['refundstate'] = -1;
		$order_data['status'] = 3;
		$order_data['refundtime'] = $time;
		$order_data['finishtime'] = $time;
		pdo_update('ewei_shop_packagegoods_order', $order_data, array('id' => $orderid, 'uniacid' => $uniacid));
		show_json(1);
	}
	public function refundexpress(){
		...
		$this->model->packagegoodsShare();
		include $this->template('packagegoods/refund/express');
	}
}
?>