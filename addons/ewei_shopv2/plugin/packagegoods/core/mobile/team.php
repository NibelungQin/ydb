...
class Team_EweiShopV2Page extends PluginMobileLoginPage 
{
	...
	public function detail() 
	{
		...
		if (empty($teamid)) 
		{
			$this->message('该团不存在!', mobileUrl('packagegoods/index'), 'error');
		}
		...
		foreach ($orders as $key => $value ) 
		{
			...
			{
				$orders[$key]['avatar'] = '../addons/ewei_shopv2/plugin/packagegoods/template/mobile/default/images/user/' . mt_rand(1, 20) . '.jpg';
			}
		}
		...
		$set = $_W['shopset'];
		$_W['shopshare'] = array('title' => '还差' . $n . '人，我参加了“' . $goods['title'] . '”拼团，快来吧。盼你如南方人盼暖气~', 'imgUrl' => (!(empty($goods['share_icon'])) ? tomedia($goods['share_icon']) : tomedia($goods['thumb'])), 'desc' => (!(empty($goods['share_title'])) ? $goods['share_title'] : $goods['title']), 'link' => mobileUrl('packagegoods/team/detail', array('teamid' => $teamid), true));
		include $this->template();
	}
