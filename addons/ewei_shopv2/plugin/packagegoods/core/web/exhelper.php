...
class Exhelper_EweiShopV2Page extends PluginWebPage 
{
	...
	public function shortedit() 
	{
		...
		foreach ($goods as $good ) 
		{
			...
			if (empty($shorttitle)) 
			{
				plog('packagegoods.exhelper.short.edit', '清空商品简称 ID: ' . $good['id'] . ' 商品名称: ' . $good['title']);
			}
			else 
			{
				plog('packagegoods.exhelper.short.edit', '修改商品简称 ID: ' . $good['id'] . ' 商品名称: ' . $good['title'] . ' 商品简称: ' . $shorttitle);
			}
		}
	...
	protected function expresspost() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				plog('packagegoods.exhelper.expressedit', '修改快递单信息 ID: ' . $id);
			}
			else 
			{
				...
				$id = pdo_insertid();
				plog('packagegoods.exhelper.expressadd', '添加快递单模板 ID: ' . $id);
			}
			...
			show_json(1, array('url' => webUrl('packagegoods/exhelper/expressedit', array('id' => $id))));
			exit();
		}
		include $this->template('packagegoods/exhelper/expresspost');
	}
	...
	public function invoicepost() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			{
				pdo_update('ewei_shop_exhelper_express', $data, array('id' => $id));
				plog('packagegoods.exhelper.invoniceedit', '修改发货单信息 ID: ' . $id);
			}
			else 
			{
				$data['uniacid'] = $_W['uniacid'];
				pdo_insert('ewei_shop_exhelper_express', $data);
				$id = pdo_insertid();
				plog('packagegoods.exhelper.invoniceadd', '添加发货单模板 ID: ' . $id);
			}
			if (!(empty($data['isdefault']))) 
			{
			...
			show_json(1, array('url' => webUrl('packagegoods/exhelper/invoiceedit', array('id' => $id))));
			exit();
		}
		include $this->template('packagegoods/exhelper/invoicepost');
	}
	...
	protected function senderpost() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (!(empty($id))) 
			{
				...
				plog('packagegoods.exhelper.senderedit', '修改发件人模板 ID: ' . $id);
			}
			else 
			{
				pdo_insert('ewei_shop_exhelper_senduser', $data);
				$id = pdo_insertid();
				plog('packagegoods.exhelper.senderadd', '添加发件人模板 ID: ' . $id);
			}
			if (!(empty($data['isdefault']))) 
			{
			...
			show_json(1, array('url' => webUrl('packagegoods/exhelper/sender', array('id' => $id, 'tab' => 'sender'))));
		}
		include $this->template('packagegoods/exhelper/senderpost');
	}
	public function sendersetdefault() 
	{
		...
		if (!(empty($item))) 
		{
			pdo_update('ewei_shop_exhelper_senduser', array('isdefault' => 0), array('uniacid' => $_W['uniacid']));
			pdo_update('ewei_shop_exhelper_senduser', array('isdefault' => 1), array('id' => $id));
			plog('packagegoods.exhelper.senderedit', '设置 默认发件人 ID: ' . $id . ' 发件人: ' . $item['sendername'] . ' ');
		}
		show_json(1);
	}
	public function senderdelete() 
	{
		...
		foreach ($items as $item ) 
		{
			...
			plog('packagegoods.exhelper.senderdelete', '删除 发件人模板 ID: ' . $item['id'] . '  <br/>发件人: ' . $item['title'] . '/发件人电话: ' . $item['sendertel'] . '/发件城市: ' . $item['sendercity'] . '/发件地址: ' . $item['senderaddress'] . ' ');
		}
		show_json(1, array('url' => referer()));
	}
	public function single() 
	{
		...
		global $_GPC;
		$tab = $_GPC['tab'];
		$paytype = Order::ORDER_TYPE_TYPE_OLD;
	...
	public function getdata() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			include $this->template('packagegoods/exhelper/print_tpl_single');
		}
	}
	public function getorder() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			extract($temps);
			include $this->template('packagegoods/exhelper/print_tpl_single_detail');
		}
	}
	...
	public function getorderinfo() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			include $this->template('packagegoods/exhelper/print_tpl_dosend');
		}
	}
	public function dosend() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if ($orderinfo['status'] == 1) 
			{
				...
				p('packagegoods')->sendTeamMessage($orderinfo['id']);
				plog('packagegoods.exhelper.dosend', '一键发货 订单号: ' . $orderinfo['orderno'] . ' <br/>快递公司: ' . $_GPC['expresscom'] . ' 快递单号: ' . $_GPC['expresssn']);
	...
	public function getdatabatch() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			$temps = $this->model->getTemp();
			extract($temps);
			include $this->template('packagegoods/exhelper/print_tpl_batch_detail');
		}
	}
	...
	public function dosendbatch() 
	{
		...
		if ($_W['ispost']) 
		{
			...
			if (($orderinfo['status'] == 1) && (($orderinfo['is_team'] == 0) || ($orderinfo['success'] == 1))) 
			{
				...
				plog('packagegoods.exhelper.batchdosend', '一键发货 订单号: ' . $orderinfo['orderno'] . ' <br/>快递公司: ' . $_GPC['expresscom'] . ' 快递单号: ' . $_GPC['expresssn']);
				exit(json_encode(array('result' => 'success')));
			}
		}
	}
