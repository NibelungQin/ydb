...
class Refund_EweiShopV2Page extends PluginWebPage 
{
	...
	protected function opData(){
		...
		if (!(empty($refundid))) {
			$refund = pdo_fetch('select * from ' . tablename('ewei_shop_packagegoods_order_refund') . ' where id=:id limit 1', array(':id' => $refundid));
...
//			}
			...
			$list = pdo_fetchall(
				'SELECT o.addressid,o.goodid,o.pay_type,o.price,o.creditmoney,o.is_team,o.refundid,ore.*,g.title,g.thumb,g.packageprice,g.thumb,g.thumb_url' . "\r\n"
				...
				. '                FROM ' . tablename('ewei_shop_packagegoods_order_refund') . ' as ore' . "\r\n"
...
//					),
	...
	public function detail() {
		...
		try {
			...
			$refund = pdo_fetch(
				'SELECT ore.*,o.price,o.credit,o.freight,o.paytime,o.pay_type,o.orderno,o.goodid,g.title,g.thumb,g.thumb_url,g.packageprice' . "\r\n"
				. '                FROM ' . tablename('ewei_shop_packagegoods_order_refund') . ' as ore' . "\r\n\t\t\t\t"
				. 'right join ' . tablename('ewei_shop_packagegoods_order') . ' as o on o.id = ore.orderid' . "\r\n\t\t\t\t"
				. 'left join ' . tablename('ewei_shop_packagegoods_goods') . ' as g on g.id = o.goodid' . "\r\n\t\t\t\t"
	...
	public function submit(){
		...
		if ($_W['ispost']) {
			...
			$setting = pdo_fetch('SELECT refundday FROM ' . tablename('ewei_shop_packagegoods_set') . ' WHERE uniacid = :uniacid ', array(':uniacid' => $_W['uniacid']));
			$refundtime = $item['finishtime'] + (3600 * 24 * $setting['refundday']);
			if (($item['status'] == 3) && ($refundtime < time())) {
				pdo_update('ewei_shop_packagegoods_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $_W['uniacid']));
			...
			if (empty($refund['refundno'])) {
				$refund['refundno'] = m('common')->createNO('packagegoods_order_refund', 'refundno', 'PR');
				pdo_update('ewei_shop_packagegoods_order_refund', array('refundno' => $refund['refundno']), array('id' => $refund['id']));
			}
			...
			if ($refundstatus == 0) {//处理申请
				show_json(1);
			}else if ($refundstatus == 3) {//确认收货
				$raid = $_GPC['raid'];
				...
				pdo_update('ewei_shop_packagegoods_order_refund', $change_refund, array('id' => $item['refundid']));
				p('packagegoods')->sendTeamMessage($item['id'], true);
			}else if ($refundstatus == 5) {//修改物流
				...
				pdo_update('ewei_shop_packagegoods_order_refund', $refund_data, array('id' => $item['refundid'], 'uniacid' => $uniacid));
				$order_data = array();
				$order_data['refundstate'] = 0;
				$order_data['status'] = 3;
				$order_data['refundtime'] = $time;
				pdo_update('ewei_shop_packagegoods_order', $order_data, array('id' => $item['id'], 'uniacid' => $uniacid));
				p('packagegoods')->sendTeamMessage($item['id'], true);
			}else if ($refundstatus == 1) {//同意退款
				...
				$order = pdo_fetch('SELECT id,orderno,credit,creditmoney,price,freight,status,pay_type,is_team,apppay FROM ' . tablename('ewei_shop_packagegoods_order') . "\r\n" . '                    WHERE id = :orderid and uniacid=:uniacid', array(':orderid' => $item['id'], ':uniacid' => $uniacid));
				...
				pdo_update('ewei_shop_packagegoods_order', array('refundstate' => 0, 'status' => -1, 'refundtime' => $time), array('id' => $item['id'], 'uniacid' => $uniacid));
				$sales = pdo_fetch('select id,sales,stock from ' . tablename('ewei_shop_packagegoods_goods') . ' where id = :id and uniacid = :uniacid ', array(':id' => $item['goodid'], ':uniacid' => $uniacid));
				pdo_update('ewei_shop_packagegoods_goods', array('sales' => $sales['sales'] - 1, 'stock' => $sales['stock'] + 1), array('id' => $sales['id'], 'uniacid' => $uniacid));
				plog('packagegoods.refund', '订单退款 ID: ' . $item['id'] . ' 订单号: ' . $item['orderno']);
			}else if ($refundstatus == -1) {//驳回申请
				pdo_update('ewei_shop_packagegoods_order_refund', array('reply' => $refundcontent, 'refundstatus' => -1, 'endtime' => $time), array('id' => $item['refundid']));
				p('packagegoods')->sendTeamMessage($item['id'], true);
				plog('packagegoods.refund', '订单退款拒绝 ID: ' . $item['id'] . ' 订单号: ' . $item['orderno'] . ' 原因: ' . $refundcontent);
				pdo_update('ewei_shop_packagegoods_order', array('refundstate' => 0), array('id' => $item['id'], 'uniacid' => $uniacid));
