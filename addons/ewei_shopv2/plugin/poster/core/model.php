...
class PosterModel extends PluginModel 
{
	...
	public function scanTime($openid, $from_openid, $poster) {
		...
		global $_W;
		global $_GPC;
		$scancount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_poster_scan')
			. ' where openid=:openid  and posterid=:posterid and uniacid=:uniacid limit 1', array(':openid' => $openid, ':posterid' => $poster['id'], ':uniacid' => $_W['uniacid'])
		);
		if ($scancount <= 0) {
	...
	public function createCommissionPoster($openid, $goodsid = 0, $type = 0){
		...
		$poster = pdo_fetch('select * from ' . tablename('ewei_shop_poster')
			. ' where uniacid=:uniacid and `type`=:type and isdefault=1 limit 1', array(':uniacid' => $_W['uniacid'], ':type' => $type)
		);
		if (empty($poster)) {
			return '';
		}
	...
	public function getQR($poster, $member, $goodsid = 0) {
		...
		if ($poster['type'] == 2) {
			...
			$qr = pdo_fetch('select * from ' . tablename('ewei_shop_poster_qr')
				. ' where openid=:openid and acid=:acid and type=4 limit 1', array(':openid' => $member['openid'], ':acid' => $acid)
			);
