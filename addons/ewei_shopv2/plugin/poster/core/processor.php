...
class PosterProcessor extends PluginProcessor
{
    ...
    public function respond($obj = NULL)
    {
        ...
        file_put_contents(IA_ROOT."/data/logs/aaaaa.txt", "json2=======" . var_export($obj->member, true) . "\r\n", FILE_APPEND);
    ...
    private function responseSubscribe($obj)
    {
        ...
        file_put_contents(IA_ROOT."/data/logs/aaaaa.txt", "json2=======" . var_export($member, true) . "\r\n", FILE_APPEND);
        if (empty($ticket))
        {
            return $this->responseDefault($obj);
        }
                            ...
                            if (empty($member['agentid']))
                            {
                                ...
                                if (empty($member['fixagentid']))
                                {
                                    ...
                                    if ($author)
                                    {
                                        ...
                                        pdo_update('ewei_shop_member', array('agentid' => $qrmember['id'],'fixagentid'=>1, 'childtime' => $time, 'authorid' => $authorid), array('id' => $member['id']));
                                    }
                                    else
                                    {
                                        pdo_update('ewei_shop_member', array('agentid' => $qrmember['id'], 'fixagentid'=>1,'childtime' => $time), array('id' => $member['id']));
                                    }
                                    //修改关系链
                                      $this->promoter_qr($member, $qrmember);
                                    if ($author)
                                    {
    ...
    public function promoter_qr($member, $qrmember) {
        ...
        //修改被邀请人关系链
        pdo_update('ewei_shop_member', array('gflag' => $my_gflag, 'generation' => $my_generation), array('id' => $member['id']));
        //修改邀请人以及邀请人上级的关系链
        //_file_put_contents("ewei_shop_member.txt", __LINE__.'，'.date("Ymd").'，'."Aparent_id====".$qrmember['id']."\r\n", FILE_APPEND);
        $pcondition = " WHERE gflag LIKE ".'"'."%,".$user_id.",%".'"';
        $query = "update  " . pdo()->tablename('ewei_shop_member') . "set gflag=replace(gflag,'" . $old_flag . "','" . $my_gflag . "') ,generation=generation+" . $add_generation;
        $query.=$pcondition;
        pdo()->query($query);
    }
}
