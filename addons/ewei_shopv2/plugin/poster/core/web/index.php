...
class Index_EweiShopV2Page extends PluginWebPage
{
    public function main()
    {
        ...
        $list = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_poster') . ' WHERE 1 ' . $condition . ' ORDER BY isdefault desc,createtime desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize,
            $params);
        foreach ($list as &$row) {
            $row['times'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_poster_scan') . ' where posterid=:posterid and uniacid=:uniacid',
                array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
            $row['follows'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_poster_log') . ' where posterid=:posterid and uniacid=:uniacid',
                array(':posterid' => $row['id'], ':uniacid' => $_W['uniacid']));
        }
        unset($row);
        $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('ewei_shop_poster') . ' where 1 ' . $condition . ' ',
            $params);
    ...
    protected function post()
    {
        ...
        $item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_poster') . ' WHERE id =:id and uniacid=:uniacid limit 1',
        ...
        if ($_W['ispost']) {
            ...
            $data = array(
                ...
                'ismembergroup' => intval($_GPC['ismembergroup'])
            );
            if ($data['type'] == 4) {
            ...
            if ($data['isdefault'] == 1) {
                pdo_update('ewei_shop_poster', array('isdefault' => 0),
                    array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $data['type']));
            }
            if (!(empty($id))) {
                pdo_update('ewei_shop_poster', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
                plog('poster.edit',
                    '修改超级海报 ID: ' . $id . '<br>' . (($data['isopen'] ? '允许非分销商生成自己的海报 -- 是<br>' : '允许非分销商生成自己的海报 -- 否<br>')) . (($data['bedown'] ? '扫码关注成为下线 -- 是<br>' : '扫码关注成为下线 -- 否<br>')) . (($data['beagent'] ? '扫码关注成为分销商 -- 是' : '扫码关注成为分销商 -- 否')));
            } else {
                pdo_insert('ewei_shop_poster', $data);
                $id = pdo_insertid();
                plog('poster.add',
                    '添加超级海报 ID: ' . $id . '<br>' . (($data['isopen'] ? '允许非分销商生成自己的海报<br>' : '不允许非分销商生成自己的海报<br>')) . (($data['bedown'] ? '扫码关注成为下线 -- 是<br>' : '扫码关注成为下线 -- 否<br>')) . (($data['beagent'] ? '扫码关注成为分销商 -- 是' : '扫码关注成为分销商 -- 否')));
            }
            $rule = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1',
                array(
                    ':uniacid' => $_W['uniacid'],
                    ':module' => 'ewei_shopv2',
                    ':name' => 'ewei_shopv2:poster:' . $data['type']
                ));
            if (empty($rule)) {
                $rule_data = array(
                    'uniacid' => $_W['uniacid'],
            ...
            $ruleauto = pdo_fetch('select * from ' . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name  limit 1',
                array(':uniacid' => $_W['uniacid'], ':module' => 'ewei_shopv2', ':name' => 'ewei_shopv2:poster:auto'));
            if (empty($ruleauto)) {
                $rule_data = array(
                    'uniacid' => $_W['uniacid'],
                    'name' => 'ewei_shopv2:poster:auto',
                    'module' => 'ewei_shopv2',
                    'displayorder' => 0,
                    'status' => 1
                );
            ...
            show_json(1, array(
                'url' => webUrl('poster/edit', array('id' => $id, 'tab' => str_replace('#tab_', '', $_GPC['tab'])))
            ));
        }
    ...
    public function delete()
    {
        ...
        foreach ($posters as $poster) {
            ...
            $rule = pdo_fetchall('SELECT id,rid FROM ' . tablename('rule_keyword') . ' WHERE uniacid=:uniacid AND content IN (\'' . $poster['keyword'] . '\',\'' . $poster['keyword2'] . '\')',
                array(':uniacid' => $_W['uniacid']), 'rid');
    ...
    public function setdefault()
    {
        ...
        pdo_update('ewei_shop_poster', array('isdefault' => 0),
            array('uniacid' => $_W['uniacid'], 'isdefault' => 1, 'type' => $poster['type']));
        pdo_update('ewei_shop_poster', array('isdefault' => 1),
            array('uniacid' => $_W['uniacid'], 'id' => $poster['id']));
