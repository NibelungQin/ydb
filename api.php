    ...
    class WeEngine
    {
        ...
        public function encrypt()
        {
            ...
            if (empty($this->account)) {
                ExceptionUtil::exit('Miss Account.');
            }
            $timestamp = TIMESTAMP;
            $nonce = random(5);
            ...
            ExceptionUtil::exit(json_encode($array));
        }
        ...
        public function start()
        {
            ...
            if (!$this->account->checkSign()) {
                ExceptionUtil::exit('Check Sign Fail.');
            }
            if (strtolower($_SERVER['REQUEST_METHOD']) == 'get') {
                ...
                ExceptionUtil::exit(htmlspecialchars($_GET['echostr']));
                ...
                if (empty($message)) {
                    ...
                    ExceptionUtil::exit('Request Failed');
                }
        ...
        private function booking($message)
        {
            ...
            if (empty($default_groupid)) {
                $default_groupid = pdo_fetchcolumn('SELECT groupid FROM ' . tablename('mc_groups') . ' WHERE uniacid = :uniacid AND isdefault = 1',
                ...
                if ($message['event'] == 'unsubscribe') {
                    ...
                    if (empty($member)) {
                        if (!isset($setting['passport']) || empty($setting['passport']['focusreg'])) {
                            $data = array(
                                'uniacid' => $_W['uniacid'],
                                'email' => md5($message['from']) . '@yidaoit.cn',
                                'salt' => random(8),
            ...
            if (!empty($subscribe[$this->message['type']])) {
                ...
                foreach ($subscribe[$this->message['type']] as $modulename) {
                    ...
                    if (is_error($response) && $response['errno'] == '7') {
                        $response = ihttp_request($_W['siteroot'] . 'web/' . wurl('utility/subscribe/receive'), $params,
                            array(), 10);
                    }
                }
            }
...
if (!empty($_GPC['appid'])) {
    ...
    if ($appid == 'wx570bc396a51b8ff8') {
        ...
        $id = pdo_fetchcolumn("SELECT acid FROM " . tablename('account_wechats') . " WHERE `key` = :appid",
            array(':appid' => $appid));
    }
}
if (empty($id)) {
...
if (empty($_W['account'])) {
    ExceptionUtil::exit('initial error hash or id');
}
if (empty($_W['account']['token'])) {
    ExceptionUtil::exit('initial missing token');
}
