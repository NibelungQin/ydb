...
if (!empty($_W['account']['setting']['bind_domain']) && !empty($_W['account']['setting']['bind_domain']['domain']) && strpos($_W['siteroot'],
        $_W['account']['setting']['bind_domain']['domain']) === false) {
...
if (empty($_W['session_id'])) {
    ...
    YdbHttpUtil::setcookie(session_name(), $_W['session_id']);
}
YdbHttpUtil::session_id($_W['session_id']);
...
if (!empty($_W['account']['oauth']) && $_W['account']['oauth']['level'] == '4' && empty($_W['isajax'])) {
    if (($_W['container'] == 'wechat' && !$_GPC['logout'] && empty($_W['openid'])
