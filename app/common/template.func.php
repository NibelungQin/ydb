...
function template_page($id, $flag = TEMPLATE_DISPLAY)
{
    ...
    $GLOBALS['_share'] = array(
        'desc' => $page['description'],
        'title' => $page['title'],
        'imgUrl' => tomedia($page['params']['0']['params']['thumb'])
    );;
...
function app_template($filename, $flag = TEMPLATE_DISPLAY)
{
    global $_W, $_GPC;
    if (DevelopmentUtil::isCITestingEnvironment()) {
        return '';
    }
    ...
    $compile = str_replace($paths['filename'], $_W['uniacid'] . '_' . intval($_GPC['t']) . '_' . $paths['filename'],
        $compile);

    if (DEVELOPMENT || !is_file($compile) || filemtime($source) > filemtime($compile)) {
        app_template_compile($source, $compile);
    }
    switch ($flag) {
        case TEMPLATE_DISPLAY:
        default:
...
function app_template_compile($from, $to)
{
    $path = dirname($to);
    if (!is_dir($path)) {
        load()->func('file');
    ...
    $content = app_template_parse(file_get_contents($from));
    file_put_contents($to, $content);
}

function app_template_parse($str)
{
    ...
    foreach ($check_repeat_template as $template) {
        if (preg_match_all('/{template\s+' . $template . '}/', $str, $match) > 1) {
            ...
            $str = preg_replace('/{template\s+' . $template . '}/i',
                '<?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template(' . $replace . ', TEMPLATE_INCLUDEPATH)) : (include app_template(' . $replace . ', TEMPLATE_INCLUDEPATH));?>',
                $str, 1);
    ...
    $str = preg_replace('/{template\s+(.+?)}/',
        '<?php (!empty($this) && $this instanceof WeModuleSite) ? (include $this->template($1, TEMPLATE_INCLUDEPATH)) : (include app_template($1, TEMPLATE_INCLUDEPATH));?>',
        $str);
    ...
    $str = preg_replace('/{loop\s+(\S+)\s+(\S+)\s+(\S+)}/', '<?php if(is_array($1)) { foreach($1 as $2 => $3) { ?>',
        $str);
    ...
    $str = preg_replace_callback('/{hook\s+(.+?)}/s', "app_template_modulehook_parser", $str);
    $str = preg_replace('/{\/data}/', '<?php } } ?>', $str);
    $str = preg_replace('/{\/hook}/', '<?php ; ?>', $str);
    $str = preg_replace_callback('/<\?php([^\?]+)\?>/s', "app_template_addquote", $str);
    ...
    if (!empty($GLOBALS['_W']['setting']['remote']['type'])) {
        $str = str_replace('</body>',
            "<script>var imgs = document.getElementsByTagName('img');for(var i=0, len=imgs.length; i < len; i++){imgs[i].onerror = function() {if (!this.getAttribute('check-src') && (this.src.indexOf('http://') > -1 || this.src.indexOf('https://') > -1)) {this.src = this.src.indexOf('{$GLOBALS['_W']['attachurl_local']}') == -1 ? this.src.replace('{$GLOBALS['_W']['attachurl_remote']}', '{$GLOBALS['_W']['attachurl_local']}') : this.src.replace('{$GLOBALS['_W']['attachurl_local']}', '{$GLOBALS['_W']['attachurl_remote']}');this.setAttribute('check-src', true);}}};{$business_stat_script}</script></body>",
            $str);
    } else {
...
function app_template_addquote($matchs)
{
    $code = "<?php {$matchs[1]}?>";
...
function site_navs($params = array())
{
    ...
    if (!empty($navs)) {
        foreach ($navs as &$row) {
            if (!$cid || !$ishomepage) {
                if (!strexists($row['url'], 'tel:') && !strexists($row['url'], '://') && !strexists($row['url'],
                        'www') && !strexists($row['url'], 'i=')) {
                    $row['url'] .= strexists($row['url'], '?') ? '&i=' . $_W['uniacid'] : '?i=' . $_W['uniacid'];
                }
            } else {
                if (empty($row['linkurl']) || (!strexists($row['linkurl'], 'http://') && !strexists($row['linkurl'],
                            'https://'))) {
...
function site_article($params = array())
{
    ...
    if (!empty($cid)) {
        ...
        $category_list = pdo_getall('site_category', array('uniacid' => $_W['uniacid'], 'multiid' => $multiid), array(),
            'id');
...
function site_slide_search($params = array())
{
    ...
    if (empty($params['multiid'])) {
        $multiid = pdo_fetchcolumn('SELECT default_site FROM ' . tablename('uni_settings') . ' WHERE uniacid = :id',
            array(':id' => $_W['uniacid']));
    } else {
        $multiid = intval($params['multiid']);
    }
...
function site_quickmenu()
{
    ...
    if ($_GPC['c'] == 'mc' || $_GPC['c'] == 'activity') {
        $quickmenu = pdo_fetch("SELECT html, params FROM " . tablename('site_page') . " WHERE uniacid = :uniacid AND type = '4' AND status = '1'",
        ...
        $quickmenu = pdo_fetch("SELECT html, params FROM " . tablename('site_page') . " WHERE multiid = :multiid AND type = '2' AND status = '1'",
            array(':multiid' => $multiid));
    }
    if (empty($quickmenu)) {
        return false;
    }
...
function app_template_modulehook_parser($params = array())
{
    load()->model('module');
    if (empty($params[1])) {
        return '';
