...
function app_tpl_form_field_date($name, $values = '', $withtime = false)
{
    $html = '';
...
{
    $value = (empty($values['province']) || empty($values['city']) || empty($values['district'])) ? '' : implode(' ',
        $values);
    $html = '';
...
function app_tpl_form_field_image($name, $value)
{
    $thumb = empty($value) ? 'images/global/nopic.jpg' : $value;
    $thumb = tomedia($thumb);
    $html = <<<EOF
