...
if ($do == 'display') {
    ...
    if ($_W['isajax']) {
        $psize = 10;
        $list = pdo_fetchall('SELECT id,title,thumb,type,credittype,endtime,description,credit FROM ' . tablename('activity_exchange') . " $where ORDER BY id ASC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,
            $params);
        if (!empty($list)) {
            foreach ($list as &$li) {
                $li['credittype_cn'] = activity_type_title($li['type']);
                $li['credit_cn'] = $creditnames[$li['credittype']];
                $li['url'] = $li['type'] != 3 ? url('activity/exchange/post',
                    array('exid' => $li['id'])) : url('activity/exchange/shipping', array('exid' => $li['id']));
            }
        ...
        $list = pdo_fetchall('SELECT id,title,thumb,type,credittype,endtime,description,credit FROM ' . tablename('activity_exchange') . " $where ORDER BY id ASC LIMIT 10",
            $params);
        foreach ($list as &$li) {
            $li['credittype_cn'] = activity_type_title($li['type']);
            $li['credit_cn'] = $creditnames[$li['credittype']];
    ...
    mc_credit_update($uid, $exchange['credittype'], -1 * $exchange['credit'], array(
        $uid,
        '礼品兑换:' . $exchange['title'] . ' 消耗 ' . $creditnames[$exchange['credittype']] . ':' . $exchange['credit']
    ));
    message("兑换成功,您消费了 {$exchange['credit']} {$creditnames[$exchange['credittype']]}",
        url("mc/bond/{$tradetype[$extype]}", array('type' => $extype)));
} elseif ($do == 'shipping') {
    load()->func('tpl');
    $exid = intval($_GPC['exid']);
    if (!empty($exid)) {
    ...
    if (checksubmit('submit')) {
        ...
        pdo_update('activity_exchange', array('num' => $exchange['num'] + 1),
            array('id' => $exchange['id'], 'uniacid' => $_W['uniacid']));
        mc_credit_update($uid, $exchange['credittype'], -1 * $exchange['credit'], array(
            $uid,
            '礼品兑换:' . $exchange['title'] . ' 消耗 ' . $creditnames[$exchange['credittype']] . ':' . $exchange['credit']
        ));
        message("兑换成功,您消费了 {$exchange['credit']} {$creditnames[$exchange['credittype']]}",
            url('activity/exchange/mine', array('type' => 3)));
    }
        ...
        case 3:
            ...
            if (!empty($list)) {
                foreach ($list as &$row) {
                    ...
                    if ($row['status'] == 1) {
                        $row['status_cn'] = "<a onclick=\"return confirm('确认收货吗?');\" class=\"btn btn-primary\"  href=\"" . url('activity/exchange/confirm',
                                array('id' => $row['id'])) . "\">收货</a>";
                    } else {
