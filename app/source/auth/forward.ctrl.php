            ...
            if (!empty($fan)) {
                ...
                if ($auth['hash'] == $hash) {
                    if ($_W['setting']['authmode'] == 2) {
                        ...
                        pdo_update('mc_mapping_fans', $rec,
                            array('uniacid' => $_W['uniacid'], 'acid' => $auth['acid'], 'openid' => $auth['openid']));
                    }
...
if (empty($forward)) {
    ...
    $forward = (strexists($forward, 'http://') || strexists($forward,
            'https://')) ? $forward : $_W['siteroot'] . 'app/' . $forward;
}
