...
if (!empty($_SESSION['pay_params'])) {
    if (!empty($oauth['openid'])) {
        header("Location: " . url('mc/cash/wechat',
                array('payopenid' => $oauth['openid'], 'params' => $_SESSION['pay_params'])));
        exit;
    } else {
        message('非法访问.');
    }
    ...
    if (!empty($fan)) {
        ...
        if (!is_error($userinfo) && !empty($userinfo) && !empty($userinfo['subscribe'])) {
            ...
            if (!isset($unisetting['passport']) || empty($unisetting['passport']['focusreg'])) {
                $email = md5($oauth['openid']) . '@yidaoit.cn';
    ...
    if (!is_error($userinfo)) {
        ...
        if (!empty($fan)) {
            ...
            pdo_update('mc_mapping_fans', $record,
                array('openid' => $fan['openid'], 'acid' => $_W['acid'], 'uniacid' => $_W['uniacid']));
