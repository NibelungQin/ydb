...
if ($do == 'credits') {
    ...
    if ($type == 'recorddetail') {
        ...
        $data = pdo_fetch("SELECT r.*, u.username FROM " . tablename('mc_credits_record') . ' AS r LEFT JOIN ' . tablename('users') . ' AS u ON r.operator = u.uid ' . ' WHERE r.id = :id AND r.uniacid = :uniacid AND r.credittype = :credittype ORDER BY id DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize,
            array(':uniacid' => $_W['uniacid'], ':id' => $id, ':credittype' => $credittype));
...
if ($do == 'mobile') {
    ...
    if ($_W['ispost'] && $_W['isajax']) {
        ...
        if (!empty($is_exist)) {
            ...
            if (!empty($reregister)) {
                mc_update($_W['member']['uid'],
                    array('mobile' => $mobile, 'email' => '', 'salt' => $salt, 'password' => $password));
            } else {
...
if ($do == 'password') {
    ...
    if ($_W['isajax'] && $_W['ispost']) {
        if (empty($reregister) && !empty($profile['password'])) {
            ...
            $correct = pdo_get('mc_members', array('uid' => $_W['member']['uid'], 'password' => $oldpassword),
                array('uid'));
            if (empty($correct)) {
...
if ($do == 'email') {
    ...
    if ($_W['isajax'] && $_W['ispost']) {
        ...
        $emailexists = pdo_get('mc_members',
            array('email' => $data['email'], 'uniacid' => $_W['uniacid'], 'uid <>' => $_W['member']['uid']),
            array('uid'));
...
if ($do == 'binding_account') {
    ...
    if ($_W['isajax'] && $_W['ispost']) {
        ...
        if ($type == '1') {
            if (!empty($data['email'])) {
                $userexists = pdo_get('mc_members',
                    array('email' => $data['email'], 'uniacid' => $_W['uniacid'], 'uid <>' => $_W['member']['uid']),
                    array('uid'));
            } elseif (!empty($data['mobile'])) {
                $userexists = pdo_get('mc_members',
                    array('mobile' => $data['mobile'], 'uniacid' => $_W['uniacid'], 'uid <>' => $_W['member']['uid']),
                    array('uid'));
                $data['email'] = '';
            }
            ...
            if (!empty($reregister)) {
                $member = pdo_get('mc_members', array('uniacid' => $_W['uniacid'], 'email' => $data['email']),
                    array('uid', 'salt', 'password'));
                if (empty($member)) {
                ...
                if (!empty($member_old) && !empty($member_new)) {
                    ...
                    pdo_update('mc_members', $profile_update,
                        array('uid' => $member['uid'], 'uniacid' => $_W['uniacid']));
                    cache_build_memberinfo($member['uid']);
                    pdo_delete('mc_members', array('uid' => $_W['member']['uid'], 'uniacid' => $_W['uniacid']));
                    pdo_update('coupon_record', array('uid' => $member['uid']),
