...
if ($do == 'sign_display') {
    ...
    $today_signed = pdo_get('mc_card_sign_record',
...
if ($do == 'sign_record') {
    ...
    $data = pdo_fetchall('SELECT * FROM ' . tablename('mc_card_sign_record') . $where . ' ORDER BY id DESC LIMIT ' . ($pindex - 1) * $psize . ", {$psize}",
        $params);
...
if ($do == 'notice') {
    ...
    $data = pdo_fetchall('SELECT a.*,b.* FROM ' . tablename('mc_card_notices_unread') . ' AS a LEFT JOIN ' . tablename('mc_card_notices') . ' AS b ON a.notice_id = b.id WHERE a.uniacid = b.uniacid AND a.uniacid = :uniacid AND a.uid = :uid AND a.type = :type ORDER BY a.notice_id DESC LIMIT ' . ($pindex - 1) * $psize . ", {$psize}",
        array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid'], ':type' => $type));
...
if ($do == 'recharge_record') {
    $period = $_GPC['period'];
    $period_date = ($period == '1') ? date('Y.m', strtotime('now')) : date('Y.m',
        strtotime('now' . ($_GPC['period'] * 1) . ' month'));
...
if ($do == 'consume') {
    ...
    $stores = pdo_getall('activity_stores',
        array('uniacid' => $_W['uniacid'], 'source' => COUPON_TYPE, 'status' => '1'),
