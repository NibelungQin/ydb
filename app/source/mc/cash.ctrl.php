...
if (!empty($type)) {
    ...
    $moduleid = pdo_fetchcolumn("SELECT mid FROM " . tablename('modules') . " WHERE name = :name",
        array(':name' => $params['module']));
    ...
    if ($type != 'delivery') {
        if ($_GPC['mix_pay']) {
            ...
            if ($credtis[$setting['creditbehaviors']['currency']] > 0 && in_array('mix',
                    $dos) && $credtis[$setting['creditbehaviors']['currency']] < $log['card_fee']) {
                $mix_credit_log = $log;
                ...
                pdo_update('core_paylog', array(
                    'fee' => $log['card_fee'] - $credtis[$setting['creditbehaviors']['currency']],
                    'card_fee' => $log['card_fee'] - $credtis[$setting['creditbehaviors']['currency']]
                ), array('plid' => $log['plid']));
    ...
    if ($type == 'wechat') {
        if (!empty($log['plid'])) {
            ...
            pdo_update('core_paylog', array('openid' => $_W['openid'], 'tag' => iserializer($tag)),
                array('plid' => $log['plid']));
        }
        ...
        if (empty($_GPC['notify'])) {
            if (!empty($log) && $log['status'] == '0') {
                ...
                if (!empty($we7_coupon_info) && $log['is_usecard'] == 1 && !empty($log['encrypt_code'])) {
                    ...
                    $coupon_record = pdo_get('coupon_record', array(
                ...
                if (!empty($we7_coupon_info)) {
                    ...
                    $result = mc_credit_update($log['openid'], 'credit2', -$fee,
                        array(0, $tip, $log['module'], 0, $store_id, 3));
                } else {
                    $result = mc_credit_update($_W['member']['uid'], $setting['creditbehaviors']['currency'], -$fee,
                        array($_W['member']['uid'], '消费' . $setting['creditbehaviors']['currency'] . ':' . $fee));
                }
                if (is_error($result)) {
                ...
                if (!is_error($site)) {
                    ...
                    if (method_exists($site, $method)) {
                        ...
                        echo '<iframe style="display:none;" src="' . murl('mc/cash/credit', array(
                                'notify' => 'yes',
                                'params' => $_GPC['params'],
                                'code' => $_GPC['code'],
