...
if ($do == 'credits') {
    ...
    if ($_GPC['credittype']) {
        ...
        if ($_GPC['type'] == 'order') {
            ...
            $orderpager = pagination($total, $pindex, $psize, '',
                array('before' => 0, 'after' => 0, 'ajaxcallback' => ''));
            template('mc/bond');
            exit();
        }
...
if ($do == 'addressadd') {
    if ($_W['ispost']) {
        ...
        if (!empty($_GPC['addid'])) {
            if (pdo_update('mc_member_address', $address,
                array('id' => intval($_GPC['addid']), 'uid' => $address['uid']))) {
        ...
        if (pdo_insert('mc_member_address', $address)) {
            $adres = pdo_get('mc_member_address',
                array('uniacid' => $_W['uniacid'], 'uid' => $address['uid'], 'isdefault' => 1));
            if (!empty($adres)) {
...
if ($do == 'card') {
    $mcard = pdo_fetch('SELECT * FROM ' . tablename('mc_card_members') . ' WHERE uniacid = :uniacid AND uid = :uid',
        array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid']));
    if (!empty($mcard)) {
    ...
    if (!empty($setting)) {
        ...
        if (is_array($setting['grant'])) {
            ...
            if ($coupon_id > 0) {
                $coupon = pdo_fetch('SELECT couponid,title,type FROM ' . tablename('activity_coupon') . ' WHERE uniacid = :uniacid AND couponid = :couponid',
                    array(':uniacid' => $_W['uniacid'], ':couponid' => $coupon_id));
            }
        }
    } else {
    ...
    if (!empty($setting['fields'])) {
        ...
        $reregister = 0;
        if (strlen($member_info['email']) == 39 && strexists($member_info['email'], '@yidaoit.cn')) {
            $member_info['email'] = '';
            $reregister = 1;
        }
    }
    if (checksubmit('submit')) {
        ...
        $count = pdo_fetchcolumn($sql,
            array(':uid' => $_W['member']['uid'], ':cid' => $_GPC['cardid'], ':uniacid' => $_W['uniacid']));
        if ($count >= 1) {
...
if ($do == 'mycard') {
    ...
    if (empty($mcard['openid']) && !empty($_W['openid'])) {
        pdo_update('mc_card_members', array('openid' => $_W['openid']),
            array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid']));
    }
    if (!empty($mcard['status'])) {
        $setting = pdo_fetch('SELECT * FROM ' . tablename('mc_card') . ' WHERE uniacid = :uniacid',
...
if ($do == 'consume') {
    ...
    $stores = pdo_fetchall('SELECT id,business_name FROM ' . tablename('activity_stores') . ' WHERE uniacid = :uniacid',
...
if ($do == 'mobile') {
    ...
    if (checksubmit('submit')) {
        if ($mobile_exist == 1) {
            ...
            $info = pdo_fetch('SELECT uid, password, salt FROM ' . tablename('mc_members') . ' WHERE uniacid = :uniacid AND mobile = :mobile AND uid = :uid',
                array(':uniacid' => $_W['uniacid'], ':mobile' => $oldmobile, ':uid' => $_W['member']['uid']));
            if (!empty($info)) {
                if ($info['password'] == md5($password . $info['salt'] . $_W['config']['setting']['authkey'])) {
                    pdo_update('mc_members', array('mobile' => $mobile),
                        array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid']));
            ...
            $is_exist = pdo_fetch('SELECT uid FROM ' . tablename('mc_members') . ' WHERE uniacid = :uniacid AND mobile = :mobile AND uid != :uid',
                array(':uniacid' => $_W['uniacid'], ':mobile' => $mobile, ':uid' => $_W['member']['uid']));
            if (!empty($is_exist)) {
                message('该手机号已被绑定,换个手机号试试', '', 'error');
            }
            $salt = random(8);
            $password = md5($password . $salt . $_W['config']['setting']['authkey']);
            pdo_update('mc_members', array('mobile' => $mobile, 'salt' => $salt, 'password' => $password),
                array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid']));
            message('修改手机号成功', url('mc/home'), 'success');
        }
    }
...
if ($do == 'email') {
    ...
    $reregister = false;
    if ($_W['member']['email'] == md5($_W['openid']) . '@yidaoit.cn') {
        $reregister = true;
    }
    if (checksubmit('submit')) {
        ...
        if ($type == 1) {
            if ($reregister) {
                if (!empty($_GPC['email'])) {
                    ...
                    if (($username_type == 'email' || $username_type == 'random') && preg_match(REGULAR_EMAIL,
                            $username)) {
                        $data['email'] = $username;
                        $emailexists = pdo_fetch("SELECT uid FROM " . tablename('mc_members') . " WHERE email = :email AND uniacid = :uniacid AND uid != :uid ",
                            array(
                                ':email' => $data['email'],
                                ':uniacid' => $_W['uniacid'],
                                ':uid' => $_W['member']['uid']
                    ...
                    } elseif (($username_type == 'mobile' || $username_type == 'random') && preg_match(REGULAR_MOBILE,
                            $username)) {
                        $data['mobile'] = $username;
                        $mobileexists = pdo_fetch("SELECT uid FROM " . tablename('mc_members') . " WHERE mobile = :mobile AND uniacid = :uniacid AND uid != :uid ",
                            array(
                                ':mobile' => $data['mobile'],
                                ':uniacid' => $_W['uniacid'],
                                ':uid' => $_W['member']['uid']
                        ...
                        //当用户已填写重要资料,把用户@yidaoit.cn格式的邮箱重置为空,防止用户登陆反复提示重置资料
                        $data['email'] = '';
                    } else {
            ...
            $member = pdo_fetch("SELECT `uid`,`salt`,`password` FROM " . tablename('mc_members') . " WHERE `uniacid`=:uniacid " . $sql_tmp,
                $pars_tmp);
            if (empty($member)) {
            ...
            if (!empty($member_old) && !empty($member_new)) {
                ...
                pdo_update('activity_coupon_record', array('uid' => $member['uid']),
                    array('uid' => $_W['member']['uid'], 'uniacid' => $_W['uniacid']));
                pdo_update('activity_exchange_trades', array('uid' => $member['uid']),
                    array('uid' => $_W['member']['uid'], 'uniacid' => $_W['uniacid']));
                pdo_update('activity_exchange_trades_shipping', array('uid' => $member['uid']),
                    array('uid' => $_W['member']['uid'], 'uniacid' => $_W['uniacid']));
                pdo_update('mc_credits_record', array('uid' => $member['uid']),
                    array('uid' => $_W['member']['uid'], 'uniacid' => $_W['uniacid']));
                pdo_update('mc_card_members', array('uid' => $member['uid']),
                    array('uid' => $_W['member']['uid'], 'uniacid' => $_W['uniacid']));
            }
