...
if ($foo == 'bind') {
        ...
        if (empty($mapping)) {
            ...
            if ($op == 'yes') {
                if (checksubmit('submit')) {
                    ...
                    if ($data[0] < 0) {
                        if ($data[0] == -1) {
                            ...
                            message('安全提问错误！', '', 'error');
                        }
                    }

                    $exist = pdo_fetch('SELECT * FROM ' . tablename('mc_mapping_ucenter') . ' WHERE `uniacid`=:uniacid AND `centeruid`=:centeruid',
                        array(':uniacid' => $_W['uniacid'], 'centeruid' => $data[0]));
                    if (empty($exist)) {
                        pdo_insert('mc_mapping_ucenter',
                            array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid'], 'centeruid' => $data[0]));
                ...
                if (checksubmit('submit')) {
                    $username = trim($_GPC['username']) ? trim($_GPC['username']) : message('请填写用户名！', '', 'error');
                    $password = trim($_GPC['password']) ? trim($_GPC['password']) : message('请填写密码！', '', 'error');
                    $repassword = trim($_GPC['repassword']) ? trim($_GPC['repassword']) : message('请填写确认密码！', '',
                        'error');
                    ...
                    if ($uid < 0) {
                        if ($uid == -1) {
                            message('用户名不合法！', '', 'error');
                        } elseif ($uid == -2) {
                            message('包含不允许注册的词语！', '', 'error');
                        } elseif ($uid == -3) {
                            message('用户名已经存在！', '', 'error');
                        } elseif ($uid == -4) {
                            message('邮箱格式错误！', '', 'error');
                        } elseif ($uid == -5) {
                            message('邮箱不允许注册！', '', 'error');
                        } elseif ($uid == -6) {
                            message('邮箱已经被注册！', '', 'error');
                        }
                    } else {
                        if ($_W['member']['email'] == '') {
                        ...
                        pdo_insert('mc_mapping_ucenter',
                            array('uniacid' => $_W['uniacid'], 'uid' => $_W['member']['uid'], 'centeruid' => $uid));
                        message('绑定UC账号成功！', url('mc/mc/home'), 'success');
                    }
                }
            }
    ...
    $result = pdo_delete('mc_mapping_ucenter', array('uid' => $_W['member']['uid'], 'uniacid' => $_W['uniacid']),
        ' AND ');
    if ($result === false) {
