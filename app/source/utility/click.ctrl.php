...
$total = pdo_fetchcolumn('SELECT SUM(credit_value) FROM ' . tablename('mc_handsel') . ' WHERE uniacid = :uniacid AND module = :module AND sign = :sign',
    array(':uniacid' => $_W['uniacid'], ':module' => $_GPC['module'], ':sign' => $_GPC['sign']));
...
if (empty($_GPC['tuid'])) {
    ...
    $user = pdo_fetchcolumn('SELECT uid FROM ' . tablename('mc_members') . ' WHERE uniacid = :uniacid AND uid = :uid',
        array(':uniacid' => $_W['uniacid'], ':uid' => $tuid));
    if (empty($user)) {
...
if (empty($_GPC['fuid'])) {
    ...
    $user = pdo_fetchcolumn('SELECT uid FROM ' . tablename('mc_members') . ' WHERE uniacid = :uniacid AND uid = :uid',
        array(':uniacid' => $_W['uniacid'], ':uid' => $fuid));
    if (empty($user)) {
