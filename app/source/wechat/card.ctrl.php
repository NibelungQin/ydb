...
$logo = pdo_fetchcolumn('SELECT logourl FROM  ' . tablename('coupon_setting') . ' WHERE uniacid = :aid AND acid = :cid',
    array(':aid' => $_W['uniacid'], ':cid' => $_W['acid']));
$colors = array(
    'Color010' => '#55bd47',
    'Color020' => '#10ad61',
    'Color030' => '#35a4de',
...
if ($do == 'display') {
    ...
    $data = pdo_fetchall('SELECT id,card_id,title,color,brand_name,date_info FROM ' . tablename('coupon') . $condition . ' ORDER BY id DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize,
        $parma);
...
if ($do == 'use') {
    ...
    if (is_error($code)) {
        ...
        $record = pdo_get('coupon_record',
            array('acid' => $_W['acid'], 'card_id' => $card_id, 'code' => $code['code']));
    }
    ...
    if (checksubmit()) {
        ...
        pdo_update('coupon_record', array(
            ...
            'usetime' => TIMESTAMP
        ), array('acid' => $_W['acid'], 'card_id' => $card_id, 'openid' => $openid, 'code' => $code));
