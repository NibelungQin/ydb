...
class CoreModule extends WeModule
{
    ...
    public function fieldsFormDisplay($rid = 0, $option = array())
    {
        ...
        switch ($_GPC['a']) {
                ...
                if (!empty($isexists['media_id']) && !empty($isexists['msgtype'])) {
                    switch ($isexists['msgtype']) {
                        case 'news':
                            ...
                            break;
                        case 'image':
                            $img = pdo_get('wechat_attachment', array('media_id' => $isexists['media_id']),
                                array('attachment'));
                            $replies['image'][0]['img_url'] = tomedia($img['attachment'], true);
                            $replies['image'][0]['mediaid'] = $isexists['media_id'];
                            break;
                        case 'voice':
                            $voice = pdo_get('wechat_attachment', array('media_id' => $isexists['media_id']),
                                array('filename'));
                            $replies['voice'][0]['title'] = $voice['filename'];
                            $replies['voice'][0]['mediaid'] = $isexists['media_id'];
                            break;
                        case 'video':
                            $video = pdo_get('wechat_attachment', array('media_id' => $isexists['media_id']),
                                array('tag'));
            ...
            default:
                if (!empty($rid) && $rid > 0) {
                    $isexists = pdo_fetch("SELECT id, name, module FROM " . tablename('rule') . " WHERE id = :id",
                        array(':id' => $rid));
                }
                if ($_GPC['m'] == 'special') {
                    ...
                    if ($reply_type == 'module') {
                        ...
                        $keyword = pdo_fetchall("SELECT content FROM " . tablename('rule_keyword') . " WHERE uniacid = :uniacid AND rid = :rid",
                ...
                if (!empty($isexists)) {
                    ...
                    if ($_GPC['a'] == 'reply' && (!empty($_GPC['m']) && $_GPC['m'] == 'keyword')) {
                        foreach ($this->tablename as $key => $tablename) {
                            if ($key != 'keyword') {
                                $replies[$key] = pdo_fetchall("SELECT * FROM " . tablename($tablename) . " WHERE rid = :rid ORDER BY `id`",
                                    array(':rid' => $rid));
                                switch ($key) {
                                    case 'image':
                                        foreach ($replies[$key] as &$img_value) {
                                            $img = pdo_get('wechat_attachment',
                                                array('media_id' => $img_value['mediaid']), array('attachment'));
