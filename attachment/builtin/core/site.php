...
class CoreModuleSite extends WeModuleSite
{
    ...
    public function doMobilePay()
    {
        ...
        if ($params['method'] == 'wechat') {
            ...
            message(error(0, $_W['siteroot'] . "/payment/{$type}/pay.php?i={$_W['uniacid']}&auth={$auth}&ps={$sl}"), '',
                'ajax', true);
            exit();
        }
    }
    ...
    public function doMobileDetail()
    {
        ...
        if (!empty($row['media_id']) && intval($row['media_id']) != 0) {
            $row = pdo_get('wechat_news',
                array('attach_id' => $row['media_id'], 'displayorder' => $row['displayorder']));
