    ...
    public function doWebChatlog()
    {
        ...
        if (!empty($_GPC['token'])) {
            ...
            if (!empty($_GPC['nickname']) && empty($_GPC['openid'])) {
                $user = pdo_fetch('SELECT b.openid,a.avatar,b.nickname FROM ' . tablename('mc_members') . ' AS a LEFT JOIN ' . tablename('mc_mapping_fans') . ' AS b ON a.uid = b.uid WHERE b.acid = :acid AND a.nickname = :nickname',
                    array(':nickname' => trim($_GPC['nickname']), ':acid' => $_W['acid']));
                if (empty($user['openid'])) {
                    itoast('没有找到昵称为 "' . $_GPC['nickname'] . '" 的用户', $this->createWebUrl('chatlog', array(
                        'acid' => $acid,
                ...
                $user = pdo_fetch('SELECT b.openid,a.avatar,b.nickname FROM ' . tablename('mc_members') . ' AS a LEFT JOIN ' . tablename('mc_mapping_fans') . ' AS b ON a.uid = b.uid WHERE b.acid = :acid AND b.openid = :openid',
                    array(':openid' => trim($_GPC['openid']), ':acid' => $_W['acid']));
                if (!empty($user['avatar'])) {
                    $avatar = tomedia($user['avatar']);
                    $nickname = $user['nickname'];
                }
