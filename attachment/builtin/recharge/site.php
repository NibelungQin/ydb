...
class RechargeModuleSite extends WeModuleSite
{
    ...
    public function doMobilePay()
    {
        ...
        if ($type == 'credit') {
            ...
            if ($type == 'card_times') {
                ...
                $member_card = pdo_get('mc_card_members',
            ...
            $chargerecord = pdo_fetch("SELECT * FROM " . tablename('mc_credits_recharge') . " WHERE uniacid = :uniacid AND uid = :uid AND fee = :fee AND type = :type  AND status = 0 AND tag = :tag",
                array(
                    ':uniacid' => $_W['uniacid'],
                    ':uid' => $_W['member']['uid'],
                    ':fee' => $fee,
    ...
    public function payResult($params)
    {
        ...
        load()->model('mc');
        load()->model('card');
        $order = pdo_fetch("SELECT * FROM " . tablename('mc_credits_recharge') . " WHERE tid = :tid",
        ...
        if ($params['result'] == 'success' && $params['from'] == 'notify') {
            ...
            if ($order['type'] == 'card_nums') {
                $member_card = pdo_get('mc_card_members',
                    array('uniacid' => $order['uniacid'], 'uid' => $order['uid']));
                $total_num = $member_card['nums'] + $order['tag'];
                pdo_update('mc_card_members', array('nums' => $total_num),
                    array('uniacid' => $order['uniacid'], 'uid' => $order['uid']));
                $log = array(
                ...
                $type = pdo_fetchcolumn('SELECT nums_text FROM ' . tablename('mc_card') . ' WHERE uniacid = :uniacid',
                    array(':uniacid' => $order['uniacid']));
                $total_num = $member_card['nums'] + $order['tag'];
            ...
            if ($order['type'] == 'card_times') {
                ...
                pdo_update('mc_card_members', array('endtime' => $endtime),
                    array('uniacid' => $order['uniacid'], 'uid' => $order['uid']));
                $log = array(
                    'uniacid' => $order['uniacid'],
                    'uid' => $order['uid'],
                    ...
                    'fee' => $params['fee'],
                    'tag' => $order['tag'],
                    'note' => date('Y-m-d H:i') . "通过{$paydata[$params['type']]}充值{$params['fee']}元，返{$order['tag']}天，充值后到期时间:" . date('Y-m-d',
                            $endtime),
                    'addtime' => TIMESTAMP
                );
                pdo_insert('mc_card_record', $log);
                $type = pdo_fetchcolumn('SELECT times_text FROM ' . tablename('mc_card') . ' WHERE uniacid = :uniacid',
                    array(':uniacid' => $order['uniacid']));
    ...
    protected function pay($params = array(), $mine = array())
    {
        ...
        $log = pdo_get('core_paylog',
            array('uniacid' => $_W['uniacid'], 'module' => $params['module'], 'tid' => $params['tid']));
        if (empty($log)) {
            $log = array(
                'uniacid' => $_W['uniacid'],
                'acid' => $_W['acid'],
