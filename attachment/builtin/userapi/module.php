...
class UserapiModule extends WeModule
{
    ...
    public function fieldsFormDisplay($rid = 0)
    {
        global $_W;
        if (!empty($rid)) {
            $row = pdo_fetch("SELECT * FROM " . tablename($this->tablename) . " WHERE rid = :rid ORDER BY `id` DESC",
                array(':rid' => $rid));
            $row['type'] = 1;
    ...
    public function fieldsFormSubmit($rid = 0)
    {
        ...
        $is_exists = pdo_fetchcolumn('SELECT id FROM ' . tablename($this->tablename) . ' WHERE rid = :rid',
            array(':rid' => $rid));
        if (!empty($is_exists)) {
    ...
    public function doSwitch()
    {
        ...
        $ds = array();
        foreach ($rs as $row) {
            $reply = pdo_fetch('SELECT * FROM ' . tablename($this->tablename) . ' WHERE `rid`=:rid',
                array(':rid' => $row['id']));
            $r = array();
            $r['title'] = $row['name'];
            $r['rid'] = $row['id'];
