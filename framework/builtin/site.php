...
class StoreModuleSite extends WeModuleSite
{
        ...
        if ($params['result'] == 'success' && $params['from'] == 'notify') {
            ...
            if (!empty($order)) {
                ...
                if (in_array($goods['type'], array(STORE_TYPE_ACCOUNT_RENEW, STORE_TYPE_WXAPP_RENEW))) {
                    ...
                    $account_endtime = strtotime('+' . $order['duration'] * $account_num . $goods['unit'],
                        max(TIMESTAMP, $account_info['endtime']));
                    pdo_update('account', array('endtime' => $account_endtime),
                        array('uniacid' => $order[$account_type]));
                ...
                if (!empty($modules)) {
                    foreach ($modules as $module) {
                        if (in_array($module['name'],
                                $have_module_goods) || $type == STORE_TYPE_MODULE && $module['app_support'] != 2 || $type == STORE_TYPE_WXAPP_MODULE && $module['wxapp_support'] != 2) {
                            continue;
                        }
    ...
    public function doWebGoodsPost()
    {
        ...
        if ($operate == 'post') {
            ...
            if (checksubmit('submit')) {
                ...
                if (!empty($result)) {
                    if (!empty($id)) {
                        itoast('编辑成功！', $this->createWebUrl('goodsseller',
                            array('direct' => 1, 'type' => $type, 'online' => $data['status'])), 'success');
                    } else {
                        itoast('添加成功！', $this->createWebUrl('goodsSeller', array('direct' => 1, 'type' => $type)),
                            'success');
                    }
                } else {
    ...
    public function doWebGoodsBuyer()
    {
        ...
        if ($operate == 'display') {
            ...
            $pagesize = 24;
            $type = 0;
            if (!empty($_GPC['type']) && in_array($_GPC['type'], array(
                    STORE_TYPE_MODULE,
                    STORE_TYPE_ACCOUNT,
                    STORE_TYPE_WXAPP,
                    STORE_TYPE_WXAPP_MODULE,
                    STORE_TYPE_PACKAGE,
                    STORE_TYPE_API,
                    STORE_TYPE_ACCOUNT_RENEW,
            ...
            if ((empty($type) || in_array($type,
                        array(STORE_TYPE_MODULE, STORE_TYPE_WXAPP_MODULE))) && is_array($store_goods)) {
        ...
        if ($operate == 'goods_info') {
            ...
            if (!empty($user_account) && is_array($user_account)) {
                foreach ($user_account as $key => $account) {
                    ...
                    if (in_array($goods['type'],
                            array(STORE_TYPE_MODULE, STORE_TYPE_ACCOUNT_RENEW)) && !in_array($default_account['type'],
                            array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH)) || in_array($goods['type'],
                            array(STORE_TYPE_WXAPP_MODULE, STORE_TYPE_WXAPP_RENEW)) && $default_account['type'] != 4) {
                        unset($user_account[$key]);
                    }
                    if (in_array($goods['type'], array(
                            STORE_TYPE_ACCOUNT_RENEW,
                            STORE_TYPE_WXAPP_RENEW
                        )) && $default_account['endtime'] <= 0) {
                        unset($user_account[$key]);
                    }
                    if ($goods['type'] == STORE_TYPE_PACKAGE && !empty($module_groups[$goods['module_group']]['wxapp']) && $default_account['type'] == 4) {
                        $wxapp_account_list[] = array(
                            'uniacid' => $default_account['uniacid'],
        ...
        if ($operate == 'submit_order') {
            ...
            if (in_array($goods_info['type'], array(
                STORE_TYPE_PACKAGE,
                STORE_TYPE_MODULE,
                STORE_TYPE_WXAPP_MODULE,
                STORE_TYPE_API,
            ...
            if (in_array($goods_info['type'], array(
                ...
                $history_order_endtime = pdo_getcolumn('site_store_order',
                    array('goodsid' => $goodsid, 'buyerid' => $_W['uid']), 'max(endtime)');
                $order['endtime'] = strtotime('+' . $duration . $goods_info['unit'],
                    max($history_order_endtime, time()));
            }
        ...
        if ($operate == 'pay_order') {
            ...
            $core_paylog = pdo_get('core_paylog', array(
                'module' => 'store',
                'status' => 0,
                'module' => 'store',
            ...
            'store_goods' => array(
                ...
                'menu' => array(
                    ...
                    'store_goods_wxapp_module' => array(
                        'title' => '小程序模块',
                        'url' => $this->createWebUrl('goodsbuyer',
                            array('direct' => 1, 'type' => STORE_TYPE_WXAPP_MODULE)),
                    ...
                    'store_goods_account_renew' => array(
                        'title' => '公众号续费',
                        'url' => $this->createWebUrl('goodsbuyer',
                            array('direct' => 1, 'type' => STORE_TYPE_ACCOUNT_RENEW)),
                        'icon' => 'wi wi-appjurisdiction',
                    ...
                    'store_goods_wxapp_renew' => array(
                        'title' => '小程序续费',
                        'url' => $this->createWebUrl('goodsbuyer',
                            array('direct' => 1, 'type' => STORE_TYPE_WXAPP_RENEW)),
                        'icon' => 'wi wi-appjurisdiction',
