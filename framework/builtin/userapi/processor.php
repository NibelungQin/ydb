...
class UserapiModuleProcessor extends WeModuleProcessor
{
    ...
    private function procRemote($item)
    {
        ...
        $body = "<xml>" . PHP_EOL .
            ...
            "</xml>";
        $response = ihttp_request($item['apiurl'], $body,
            array('CURLOPT_HTTPHEADER' => array('Content-Type: text/xml; charset=utf-8')));
        $result = array();
    ...
    public function respond()
    {
        ...
        if (!empty($result) && is_array($result)) {
            ...
            if ($item['cachetime'] > 0) {
                if (empty($cache)) {
                    pdo_insert('userapi_cache',
                        array('key' => $key, 'content' => iserializer($result), 'lastupdate' => TIMESTAMP));
                } else {
                    pdo_update('userapi_cache', array('content' => iserializer($result), 'lastupdate' => TIMESTAMP),
                        array('key' => $key));
                }
            }
        }
