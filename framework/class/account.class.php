...
abstract class WeAccount
{
    ...
    public static function create($acidOrAccount = array())
    {
        ...
        if (is_array($acidOrAccount) && !empty($acidOrAccount)) {
            ...
            $acidOrAccount = empty($acidOrAccount) ? $_W['account']['acid'] : (int)$acidOrAccount;
...
abstract class WeBase
{
    ...
    public function saveSettings($settings)
    {
        ...
        if (pdo_fetchcolumn("SELECT module FROM " . tablename('uni_account_modules') . " WHERE module = :module AND uniacid = :uniacid",
            array(':module' => $this->modulename, ':uniacid' => $_W['uniacid']))) {
            return pdo_update('uni_account_modules', $row, $pars) !== false;
        } else {
            return pdo_insert('uni_account_modules', array(
    ...
    protected function template($filename)
    {
        ...
        if (DEVELOPMENT || !is_file($compile) || filemtime($source) > filemtime($compile)) {
            web_template_compile($source, $compile, true);
        }
        return $compile;
    }
...
abstract class WeModuleProcessor extends WeBase
{
    ...
    protected function respText($content)
    {
        ...
        if (!empty($matchArray[1])) {
            foreach ($matchArray[1] as $emojiUSB) {
                $response['Content'] = str_ireplace("[U+{$emojiUSB}]", utf8_bytes(hexdec($emojiUSB)),
                    $response['Content']);
            }
        }
        return $response;
    ...
    protected function buildSiteUrl($url)
    {
        ...
        if (uni_is_multi_acid() && strexists($url, './index.php?i=') && !strexists($url,
                '&j=') && !empty($_W['acid'])) {
...
abstract class WeModuleWxapp extends WeBase
{
    ...
    protected function pay($order)
    {
        ...
        $paylog = pdo_get('core_paylog',
            array('uniacid' => $_W['uniacid'], 'module' => $this->module['name'], 'tid' => $order['tid']));
        if (empty($paylog)) {
            $paylog = array(
                'uniacid' => $_W['uniacid'],
                'acid' => $_W['acid'],
