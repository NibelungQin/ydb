    ...
    public function PayConsumeCode($data)
    {
        ...
        if (is_error($code)) {
            ...
            if (is_error($sumecode)) {
                ...
                pdo_update('coupon_record', array('status' => 3, 'usetime' => time()),
                    array('acid' => $this->account['acid'], 'code' => $code['code'], 'card_id' => $data['card_id']));
            }
        }
        return true;
    }
...
class MemberCard extends Card
{
    ...
    public $supply_balance = true;
    public $prerogative = '';
    public $auto_activate = false;
    public $custom_field1 = array('name_type' => 'FIELD_NAME_TYPE_COUPON', 'url' => '');
    public $activate_url = '';
    public $wx_activate = false;
    public $bonus_url = '';
    public $balance_url = '';
    public $bonus_rules = '';
    ...
    public $custom_cell1 = array(
        'name' => '账单',
        'tips' => '',
        'url' => 'http://06.yidaoit.cn/app/index.php?i=76&c=mc&a=bond&do=credits&credittype=credit2&type=record&period=1&wxref=mp.weixin.qq.com#wechat_redirect'
    );
    public $discount = '';
