    ...
    public function debug($output = true, $append = array())
    {
        ...
        if ($output) {
            ...
            if (!empty($append['error'][1])) {
                ...
                $msg = "SQL: <br/>{$append['sql']}<hr/>Params: <br/>{$params}<hr/>SQL Error: <br/>{$append['error'][2]}<hr/>Traces: <br/>{$ts}";
                trigger_error($msg);
                $logger->debug($msg);
            }
        }
...
class SqlPaser
{
    ...
    private static function parseFieldAlias($field, $alias = '')
    {
        ...
        if (strexists($field, '(')) {
            $select_fields = str_replace(array('(', ')'),
                array('(' . (!empty($alias) ? "`{$alias}`." : '') . '`', '`)'), $field);
        } else {
    ...
    public static function parseSelect($field = array(), $alias = '')
    {
        ...
        foreach ($field as $field_row) {
            if (strexists($field_row, '*')) {
                ...
                $field_row = str_replace(array('(', ')'),
                    array('(' . (!empty($alias) ? "`{$alias}`." : '') . '`', '`)'), $field_row);
                if (!strexists(strtolower($field_row), 'as')) {
                    $field_row .= " AS '{$index}'";
                }
            } else {
