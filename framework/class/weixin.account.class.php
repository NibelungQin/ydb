...
class WeiXinAccount extends WeAccount
{
    ...
    public static function __set_state($array)
    {
        ...
        $weiXinAccount->tablename = $array['tablename'];
        $weiXinAccount->apis = $array['apis'];
        $weiXinAccount->types = $array['types'];
        $weiXinAccount->account = $array['account'];
        $weiXinAccount->uniacid = $array['uniacid'];
        $weiXinAccount->menuFrame = $array['menuFrame'];
        $weiXinAccount->type = $array['type'];
        $weiXinAccount->typeName = $array['typeName'];
        $weiXinAccount->typeTempalte = $array['typeTempalte'];
        $weiXinAccount->uniaccount = $array['uniaccount'];
    ...
    {
        if (empty($this->uniaccount) || (!empty($this->uniaccount['account']) && !in_array($this->uniaccount['type'],
                    array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH)) && !defined('IN_MODULE'))) {
            return false;
        }
        return true;
    }
    ...
    public function menuCreate($menu)
    {
        ...
        if (!empty($result['errcode'])) {
            return error(-1,
                "访问微信接口错误, 错误代码: {$result['errcode']}, 错误信息: {$result['errmsg']},错误详情：{$this->errorCode($result['errcode'])}");
        }
    ...
    public function menuCurrentQuery()
    {
        if (DevelopmentUtil::isTestingEnvironment()) {
            return ['errno' => '0', 'message' => '测试环境 mock 数据'];
        }
        $token = $this->getAccessToken();
        if (is_error($token)) {
            return $token;
    ...
    public function fansQueryInfo($uniid, $isOpen = true)
    {
        ...
        $response['content'] = str_replace('"city":"' . $reg_arr[1] . '","province":"' . $reg_arr[2] . '","country":"' . $reg_arr[3] . '"',
            '"city":"' . $city . '","province":"' . $province . '","country":"' . $country . '"', $response['content']);
        $result = @json_decode($response['content'], true);
    ...
    public function barCodeCreateDisposable($barcode)
    {
        ...
        $token = $this->getAccessToken();
        $response = ihttp_request("https://api.weixin.qq.com/cgi-bin/qrcode/create?access_token=" . $token,
            json_encode($barcode));
        if (is_error($response)) {
            return $response;
        }
        ...
        if (!empty($content['errcode'])) {
            return error(-1,
                "访问微信接口错误, 错误代码: {$content['errcode']}, 错误信息: {$content['errmsg']},错误详情：{$this->errorCode($content['errcode'])}");
        }
        return $content;
    }
    ...
    public function getAccessToken()
    {
        ...
        if (empty($token) || !is_array($token) || empty($token['access_token']) || empty($token['expires_in'])) {
            ...
            return error('-1',
                '获取微信公众号授权失败, 请稍后重试！ 公众平台返回原始数据为: 错误代码-' . $errorinfo['errcode'] . '，错误信息-' . $errorinfo['errmsg']);
        }
        $record = array();
    ...
    public function fansTagEdit($tagid, $tagname)
    {
        ...
        $data = stripslashes(ijson_encode(array('tag' => array('id' => $tagid, 'name' => $tagname)),
    ...
    public function sendTplNotice($touser, $template_id, $postdata, $url = '', $topcolor = '#FF683F')
    {
        ...
        if (empty($result)) {
            ...
            return error(-1,
                "访问微信接口错误, 错误代码: {$result['errcode']}, 错误信息: {$result['errmsg']},信息详情：{$this->errorCode($result['errcode'])}");
        }
        return true;
    }
    ...
    public function uploadVideoFixed($title, $description, $path)
    {
        ...
        $data = array(
            'media' => '@' . $path,
            'description' => stripslashes(ijson_encode(array('title' => $title, 'introduction' => $description),
                JSON_UNESCAPED_UNICODE)),
        );
        $response = $this->requestApi($url, $data);
        return $response;
    ...
    public function getMaterial($media_id, $savefile = true)
    {
        ...
        if ($savefile && !empty($response['headers']['Content-disposition']) && strexists($response['headers']['Content-disposition'],
                'filename=')) {
            global $_W;
    ...
    protected function requestApi($url, $post = '')
    {
        ...
        if (empty($result)) {
            ...
            return error($result['errcode'],
                "访问公众平台接口失败, 错误: {$result['errmsg']},错误详情：{$this->errorCode($result['errcode'])}");
        }
        return $result;
    }
}