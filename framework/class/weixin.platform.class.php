...
class WeiXinPlatform extends WeiXinAccount
{
    ...
    function getComponentAccesstoken()
    {
        ...
        if (empty($accesstoken) || empty($accesstoken['value']) || $accesstoken['expire'] < TIMESTAMP) {
            ...
            if (empty($ticket)) {
                return error(1,
                    '缺少接入平台关键数据，等待微信开放平台推送数据，请十分钟后再试或是检查“授权事件接收URL”是否写错（index.php?c=account&amp;a=auth&amp;do=ticket地址中的&amp;符号容易被替换成&amp;amp;）');
            }
            $data = array(
    ...
    public function getAuthLoginUrl()
    {
        ...
        if (is_error($preauthcode)) {
            ...
            $authurl = sprintf(ACCOUNT_PLATFORM_API_LOGIN, $this->appid, $preauthcode,
    ...
    public function getOauthInfo($code = '')
    {
        ...
        $apiurl = sprintf(ACCOUNT_PLATFORM_API_OAUTH_INFO . $component_accesstoken, $this->account['key'], $this->appid,
            $code);
