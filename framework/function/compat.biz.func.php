...
if (!function_exists('fans_search')) {
    function fans_search($user, $fields = array())
    {
        ...
        $uid = intval($user);
        if (empty($uid)) {
            $uid = pdo_fetchcolumn("SELECT uid FROM " . tablename('mc_mapping_fans') . " WHERE openid = :openid AND acid = :acid",
                array(':openid' => $user, ':acid' => $_W['acid']));
            if (empty($uid)) {
                return array();
            }
        }
