...
function cron_run($id)
{
    ...
    $response = ihttp_request($_W['sitescheme'] . '127.0.0.1/' . $urlset['dirname'] . '/' . url('cron/entry',
            array('id' => $cron['cloudid'])), array(), $extra);
...
function cron_setnexttime($cron)
{
    ...
    if ($cron['type'] == 1) {
        pdo_update('core_cron', array('status' => 0, 'lastruntime' => TIMESTAMP, 'nextruntime' => TIMESTAMP),
            array('id' => $cron['id']));
        return true;
    }
    ...
    list($yearnow, $monthnow, $daynow, $weekdaynow, $hournow, $minutenow) = explode('-',
        date('Y-m-d-w-H-i', TIMESTAMP));
...
function cron_delete($ids)
{
    ...
    $ids = implode(', ', $ids);
    $corns = pdo_fetchall('SELECT id, cloudid FROM ' . tablename('core_cron') . " WHERE uniacid = :uniacid AND id IN ({$ids})",
        array(':uniacid' => $_W['uniacid']), 'cloudid');
    ...
    if (!empty($cloudid)) {
        ...
        pdo_query('DELETE FROM ' . tablename('core_cron') . " WHERE uniacid = :uniacid AND id IN ({$ids})",
