...
function isetcookie($key, $value, $expire = 0, $httponly = false)
{
    ...
    return YdbHttpUtil::setcookie(
        $_W['config']['cookie']['pre'] . $key,
        $value,
        $expire,
        $_W['config']['cookie']['path'],
        $_W['config']['cookie']['domain'],
        $secure,
        $httponly
    );
}
...
function checksubmit($var = 'submit', $allowget = false)
{
    ...
    if (defined('IN_SYS')) {
        if ($allowget
            || (($_W['ispost'] && !empty($_W['token']) && $_W['token'] == $_GPC['token'])
...
function pagination(
    ...
    $pageSize = 15,
    $url = '',
    $context = array('before' => 5, 'after' => 4, 'ajaxcallback' => '', 'callbackfuncname' => '')
) {
    global $_W;
    $pdata = array(
        'tcount' => 0,
...
{
    if (empty($data) || !is_array($data) || !array_key_exists('errno', $data) || (array_key_exists(
                'errno',
                $data
            ) && $data['errno'] == 0)) {
        return false;
...
function referer($default = '')
{
    ...
    if (!empty($reurl['host'])
        && !in_array($reurl['host'], array($_SERVER['SERVER_NAME'], 'www.' . $_SERVER['HTTP_HOST']))
        && !in_array($_SERVER['SERVER_NAME'], array($reurl['host'], 'www.' . $reurl['host']))) {
...
function cutstr($string, $length, $havedot = false, $charset = '')
{
    ...
    if (function_exists('mb_strcut')) {
        ...
        $string = str_replace(
            array('&amp;', '&quot;', '&lt;', '&gt;'),
            array($pre . '&' . $end, $pre . '"' . $end, $pre . '<' . $end, $pre . '>' . $end),
            $string
        );
        $strcut = '';
        ...
        if (basename($_SERVER['PHP_SELF']) === $_W['script_name']) {
            ...
            if (isset($_SERVER['ORIG_SCRIPT_NAME']) && basename($_SERVER['ORIG_SCRIPT_NAME']) === $_W['script_name']) {
                ...
                if (($pos = strpos($_SERVER['PHP_SELF'], '/' . $scriptName)) !== false) {
                    ...
                    if (isset($_SERVER['DOCUMENT_ROOT']) && strpos(
