...
{
    $categorys = pdo_fetchall('SELECT * FROM ' . tablename('article_category') . ' WHERE type = :type ORDER BY displayorder DESC',
        array(':type' => $type), 'id');
    return $categorys;
}
...
{
    $limit = intval($limit);
    $news = pdo_fetchall('SELECT * FROM ' . tablename('article_news') . ' WHERE is_display = 1 AND is_show_home = 1 ORDER BY displayorder DESC,id DESC LIMIT ' . $limit,
        array(), 'id');
    return $news;
}
...
function article_notice_home($limit = 5)
{
    ...
    $notice = pdo_fetchall("SELECT * FROM " . tablename('article_notice') . " WHERE is_display = 1 AND is_show_home = 1 ORDER BY displayorder DESC,id DESC LIMIT " . $limit,
        array(), 'id');
...
function article_news_all($filter = array(), $pindex = 1, $psize = 10)
{
    ...
    $news = pdo_fetchall("SELECT * FROM " . tablename('article_news') . $condition . " ORDER BY " . $order . " DESC " . $limit,
        $params, 'id');
...
function article_notice_all($filter = array(), $pindex = 1, $psize = 10)
{
    ...
    $notice = pdo_fetchall("SELECT * FROM " . tablename('article_notice') . $condition . " ORDER BY " . $order . " DESC " . $limit,
        $params, 'id');
    foreach ($notice as $key => $notice_val) {
        $notice[$key]['style'] = iunserializer($notice_val['style']);
        $notice[$key]['group'] = empty($notice_val['group']) ? array(
            'vice_founder' => array(),
            'normal' => array()
        ) : iunserializer($notice_val['group']);
        if (empty($_W['user']) && !empty($notice_val['group']) || !empty($_W['user']['groupid']) && !empty($notice_val['group']) && !in_array($_W['user']['groupid'],
                $notice[$key]['group']['vice_founder']) && !in_array($_W['user']['groupid'],
                $notice[$key]['group']['normal'])) {
            unset($notice[$key]);
        }
    }
...
function article_category_delete($id)
{
    ...
    $navs = pdo_fetchall("SELECT icon, id FROM " . tablename('site_nav') . " WHERE id IN (SELECT nid FROM " . tablename('site_category') . " WHERE id = {$id} OR parentid = '$id')",
        array(), 'id');
    if (!empty($navs)) {
        foreach ($navs as $row) {
