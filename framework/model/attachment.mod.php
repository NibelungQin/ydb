...
function attachment_newalioss_auth($key, $secret, $bucket)
{
    ...
    try {
        ...
        $ossClient->uploadFile($bucket, $filename, ATTACHMENT_ROOT . $filename);
...
function attachment_qiniu_auth($key, $secret, $bucket)
{
    ...
    list($ret, $err) = $uploadmgr->putFile($token, 'fav.ico',
        ATTACHMENT_ROOT . 'fav.ico');
    if ($err !== null) {
        $err = (array)$err;
...
function attachment_cos_auth($bucket, $appid, $key, $secret, $bucket_local = '')
{
    ...
    if (!empty($bucket_local)) {
        ...
        $uploadRet = qcloudcos\Cosapi::upload($bucket, ATTACHMENT_ROOT . 'fav.ico',
            '/fav.ico', '', 3 * 1024 * 1024, 0);
    } else {
        load()->library('cosv3');
        ...
        $uploadRet = Qcloud_cos\Cosapi::upload($bucket, ATTACHMENT_ROOT . 'fav.ico',
            '/fav.ico', '', 3 * 1024 * 1024, 0);
    }
    if ($uploadRet['code'] != 0) {
