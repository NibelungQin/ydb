    ...
    if (!empty($system_menu) && is_array($system_menu)) {
        ...
        foreach ($system_menu as $menu_name => $menu) {
            ...
            foreach ($menu['section'] as $section_name => $section) {
                ...
                $system_menu[$menu_name]['section'][$section_name]['menu'] = iarray_sort($system_menu[$menu_name]['section'][$section_name]['menu'],
                    'displayorder', 'desc');
            }
        }
        $add_top_nav = pdo_getall('core_menu', array('group_name' => 'frame', 'is_system <>' => 1),
            array('title', 'url', 'permission_name', 'displayorder'));
...
function cache_build_uninstalled_module()
{
    ...
    if (!empty($cloud_module) && !is_error($cloud_module)) {
        foreach ($cloud_module as $module) {
            ...
            $wxapp_support = !empty($module['site_branch']['wxapp_support']) && is_array($module['site_branch']['bought']) && in_array('wxapp',
                $module['site_branch']['bought']) ? $module['site_branch']['wxapp_support'] : 1;
            $app_support = !empty($module['site_branch']['app_support']) && is_array($module['site_branch']['bought']) && in_array('app',
                $module['site_branch']['bought']) ? $module['site_branch']['app_support'] : 1;
    ...
    $cache = array(
        ...
        'wxapp_count' => empty($uninstallModules['uninstalled']['wxapp']) ? 0 : count($uninstallModules['uninstalled']['wxapp'])
    );
...
function cache_build_proxy_wechatpay_account()
{
    ...
    if (!empty($uniaccounts)) {
        foreach ($uniaccounts as $uniaccount) {
            ...
            if (is_array($account) && !empty($account['key']) && !empty($account['secret']) && in_array($account['level'],
                    array(4)) &&
