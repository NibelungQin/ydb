...
define('CLOUD_GATEWAY_URL', 'http://www.yidaoit.cn/gateway.php');
define('CLOUD_GATEWAY_URL_NORMAL', 'http://www.yidaoit.cn/gateway.php');
...
function cloud_prepare()
{
    ...
    if (empty($_W['setting']['site']['key']) || empty($_W['setting']['site']['token'])) {
        return error('-1', "您的站点只有在一道宝云服务平台成功注册后，才能使用云服务的相应功能。");
    }
    return true;
}
...
function cloud_build()
{
    ...
    $dat = cloud_request('http://www.yidaoit.cn/gateway.php', $pars);
...
function cloud_download($path, $type = '')
{
    ...
    $dat = cloud_request('http://www.yidaoit.cn/gateway.php', $pars, $headers, 300);
    if (is_error($dat)) {
...
function cloud_sms_send($mobile, $content, $postdata = array())
{
    ...
    $response = cloud_request('http://www.yidaoit.cn/gateway.php', $pars);
...
function cloud_cron_create($cron)
{
    ...
    $result = cloud_request('http://www.yidaoit.cn/gateway.php', $pars);
...
function _cloud_cron_parse($result)
{
    ...
    if ($result['content'] == 'blacklist') {
        return error(-1, '抱歉，您的站点已被列入云服务黑名单，云服务一切业务已被禁止，请联系一道宝客服！');
    }
...
function cloud_auth_url($forward, $data = array())
{
    ...
    $auth_url = 'http://www.yidaoit.cn/gateway.php?__auth=' . $query;
    return $auth_url;
}
...
function cloud_flow_master_post($flow_master)
{
    ...
    $dat = cloud_request('http://www.yidaoit.cn/gateway.php', $pars, array(), 300);
    if (is_error($dat)) {
        return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
    }
