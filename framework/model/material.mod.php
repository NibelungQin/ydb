...
function material_sync($material, $exist_material, $type)
{
    ...
    foreach ($material as $news) {
        $attachid = '';
        $material_exist = pdo_get('wechat_attachment',
            array('uniacid' => $_W['uniacid'], 'media_id' => $news['media_id']));
        if (empty($material_exist)) {
            ...
            if ($type == 'image') {
                ...
                pdo_update('wechat_attachment', $material_data,
                    array('uniacid' => $_W['uniacid'], 'media_id' => $news['media_id']));
            }
            if ($type == 'voice') {
                $material_data = array(
        ...
        if ($type == 'news') {
            ...
            pdo_update('wechat_attachment', array('createtime' => $news['update_time']),
                array('media_id' => $news['media_id']));
        }
    }
                ...
                foreach ($news as &$news_row) {
                    ...
                    if (!empty($match[1])) {
                        foreach ($match[1] as $val) {
                            if ((strexists($val, 'http://') || strexists($val, 'https://')) && (strexists($val,
                                        'mmbiz.qlogo.cn') || strexists($val, 'mmbiz.qpic.cn'))) {
...
function material_strip_wechat_image_proxy($content)
{
    ...
    if (!empty($match_wechat[1])) {
        foreach ($match_wechat[1] as $val) {
            $wechat_thumb_url = urldecode(str_replace($_W['siteroot'] . 'web/index.php?c=utility&a=wxcode&do=image&attach=',
                '', $val));
...
function material_local_news_upload($attach_id)
{
    ...
    foreach ($material['news'] as $news) {
        ...
        if (!empty($news['content_source_url'])) {
            $news['content_source_url'] = safe_gpc_url($news['content_source_url'], false,
                $_W['siteroot'] . 'app/' . $news['content_source_url']);
        }
    ...
    if (empty($material['media_id'])) {
        ...
        if (!empty($material_info['news_item'])) {
            foreach ($material_info['news_item'] as $key => $info) {
                pdo_update('wechat_news', array('url' => $info['url']),
                    array('uniacid' => $_W['uniacid'], 'attach_id' => $material['id'], 'displayorder' => $key));
            }
        }
        ...
        pdo_update('wechat_attachment', array('model' => 'perm'),
            array('uniacid' => $_W['uniacid'], 'id' => $attach_id));
    }
    return $material;
}
...
function material_local_upload_by_url($url, $type = 'images')
{
    ...
    if (!empty($_W['setting']['remote']['type'])) {
        ...
        if (strexists(parse_url($url, PHP_URL_PATH), '/attachment/')) {
            $url = substr(parse_url($url, PHP_URL_PATH),
                strpos(parse_url($url, PHP_URL_PATH), '/attachment/') + strlen('/attachment/'));
        }
...
function material_news_delete($material_id)
{
    ...
    if (empty($_W['isfounder']) && !empty($permission) && !in_array('platform_material',
            $permission) && !in_array('all', $permission)) {
...
function material_news_list($server = '', $search = '', $page = array('page_index' => 1, 'page_size' => 24))
{
    ...
    $list_sql = sprintf($select_sql,
        "a.id as id, a.filename, a.attachment, a.media_id, a.type, a.model, a.tag, a.createtime, b.displayorder, b.title, b.digest, b.thumb_url, b.thumb_media_id, b.attach_id, b.url",
        " ORDER BY a.createtime DESC, b.displayorder ASC LIMIT " . ($page['page_index'] - 1) * $page['page_size'] . ", " . $page['page_size']);
    ...
    $pager = pagination($total, $page['page_index'], $page['page_size'], '',
        $context = array('before' => 5, 'after' => 4, 'isajax' => $_W['isajax']));
...
function material_list($type = '', $server = '', $page = array('page_index' => 1, 'page_size' => 24))
{
    ...
    if ($server == 'local') {
        $material_list = pdo_getslice($table, $conditions, array($page['page_index'], $page['page_size']), $total,
