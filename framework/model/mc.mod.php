...
function mc_update($uid, $fields)
{
    ...
    if (!empty($openid) && empty($uid)) {
        pdo_update('mc_mapping_fans', array('uid' => $result),
            array('uniacid' => mc_current_real_uniacid(), 'openid' => $openid));
    }
...
function mc_oauth_userinfo($acid = 0)
{
    ...
    load()->func('tpl');
    include app_template('mc/iswxapp', TEMPLATE_INCLUDEPATH);
    exit;
}
    ...
    if (!empty($_SESSION['openid']) && intval($_W['account']['level']) >= 3) {
        ...
        if (!is_error($userinfo) && !empty($userinfo) && is_array($userinfo) && !empty($userinfo['nickname'])) {
            ...
            if (!empty($fan)) {
                ...
                pdo_update('mc_mapping_fans', $record,
                    array('openid' => $_SESSION['openid'], 'acid' => $_W['acid'], 'uniacid' => $_W['uniacid']));
            } else {
                $record = array();
            ...
            if (!empty($fan['uid']) || !empty($_SESSION['uid'])) {
                ...
                $member = mc_fetch($uid,
                    array('nickname', 'gender', 'residecity', 'resideprovince', 'nationality', 'avatar'));
                $record = array();
...
function mc_require($uid, $fields, $pre = '')
{
    ...
    if (in_array('birth', $fields) || in_array('birthyear', $fields) || in_array('birthmonth',
            $fields) || in_array('birthday', $fields)) {
    ...
    if (in_array('reside', $fields) || in_array('resideprovince', $fields) || in_array('residecity',
            $fields) || in_array('residedist', $fields)) {
    ...
    if (!empty($message)) {
        ...
        if (checksubmit('submit')) {
            ...
            if (in_array('email', $fields)) {
                $emailexists = pdo_fetchcolumn("SELECT email FROM " . tablename('mc_members') . " WHERE uniacid = :uniacid AND email = :email " . $condition,
                    array(':uniacid' => $_W['uniacid'], ':email' => trim($record['email'])));
                if (!empty($emailexists)) {
                    itoast('抱歉，您填写的手机号已经被使用，请更新。', 'refresh', 'error');
                }
            }
            if (in_array('mobile', $fields)) {
                $mobilexists = pdo_fetchcolumn("SELECT mobile FROM " . tablename('mc_members') . " WHERE uniacid = :uniacid AND mobile = :mobile " . $condition,
                    array(':uniacid' => $_W['uniacid'], ':mobile' => trim($record['mobile'])));
...
function mc_account_change_operator($clerk_type, $store_id, $clerk_id)
{
    ...
    if (empty($stores) || empty($clerks)) {
        ...
        $stores = pdo_getall('activity_stores', array('uniacid' => $_W['uniacid']),
            array('id', 'business_name', 'branch_name'), 'id');
    }
    $data = array(
        'clerk_cn' => '',
        'store_cn' => '',
...
function _mc_login($member)
{
    ...
    if (!empty($member) && !empty($member['uid'])) {
        $member = pdo_get('mc_members', array('uid' => $member['uid'], 'uniacid' => $_W['uniacid']),
            array('uid', 'realname', 'mobile', 'email', 'groupid', 'credit1', 'credit2', 'credit6'));
...
function mc_handsel($touid, $fromuid, $handsel, $uniacid = '')
{
    ...
    $creditbehaviors = pdo_fetchcolumn('SELECT creditbehaviors FROM ' . tablename('uni_settings') . ' WHERE uniacid = :uniacid',
...
function mc_openid2uid($openid)
{
    ...
    if (is_string($openid)) {
        $fans_info = pdo_get('mc_mapping_fans', array('uniacid' => mc_current_real_uniacid(), 'openid' => $openid),
            array('uid'));
    ...
    if (is_array($openid)) {
        ...
        if (!empty($fans)) {
            $fans = pdo_getall('mc_mapping_fans', array('uniacid' => mc_current_real_uniacid(), 'openid' => $fans),
                array('uid', 'openid'), 'uid');
...
function mc_uid2openid($uid)
{
    ...
    if (is_array($uid)) {
        ...
        if (!empty($uids)) {
            $fans_info = pdo_getall('mc_mapping_fans', array('uniacid' => mc_current_real_uniacid(), 'uid' => $uids),
                array('uid', 'openid'), 'openid');
...
function mc_group_update($uid = 0)
{
    ...
    if (!$_W['uniaccount']['grouplevel']) {
        $_W['uniaccount']['grouplevel'] = pdo_getcolumn('uni_settings', array('uniacid' => $_W['uniacid']),
            'grouplevel');
    ...
    if ($uid <= 0) {
        ...
        $user = pdo_fetch('SELECT uid, realname, credit1, credit6, groupid FROM ' . tablename('mc_members') . ' WHERE uniacid = :uniacid AND uid = :uid',
            array(':uniacid' => $_W['uniacid'], ':uid' => $uid));
        $user['openid'] = pdo_fetchcolumn('SELECT openid FROM ' . tablename('mc_mapping_fans') . ' WHERE acid = :acid AND uid = :uid',
            array(':acid' => $_W['acid'], ':uid' => $uid));
    }
    if (empty($user)) {
        return false;
    }
    ...
    if ($groupid > 0 && $groupid != $user['groupid']) {
        ...
        cache_build_memberinfo($uid);
        mc_notice_group($user['openid'], $_W['uniaccount']['groups'][$user['groupid']]['title'],
            $_W['uniaccount']['groups'][$groupid]['title']);
    }
...
function mc_notice_recharge($openid, $uid = 0, $num = 0, $url = '', $remark = '')
{
    ...
    $time = date('Y-m-d H:i');
    if (empty($url)) {
        $url = murl('mc/bond/credits', array('credittype' => 'credit2', 'type' => 'record', 'period' => '1'), true,
            true);
    }
...
function mc_notice_credit1($openid, $uid, $credit1_num, $tip, $url = '', $remark = '谢谢惠顾，点击查看详情')
{
    ...
    $time = date('Y-m-d H:i');
    if (empty($url)) {
        $url = murl('mc/bond/credits', array('credittype' => 'credit1', 'type' => 'record', 'period' => '1'), true,
            true);
    }
...
function mc_notice_consume($openid, $title, $content, $url = '')
{
    ...
    if ($_W['account']['level'] == 4) {
        mc_notice_credit2($openid, $content['uid'], $content['credit2_num'], $content['credit1_num'], $content['store'],
            '', $content['remark']);
    }
    if ($_W['account']['level'] == 3) {
...
function mc_init_fans_info($openid, $force_init_member = false)
{
    ...
    foreach ($fans_list as $fans) {
        if (empty($fans['subscribe'])) {
            pdo_update('mc_mapping_fans', array('follow' => 0, 'unfollowtime' => TIMESTAMP),
        ...
        if ($force_init_member) {
            ...
            if (empty($fans_mapping['uid'])) {
                $email = md5($openid) . '@yidaoit.cn';
                $email_exists_member = pdo_getcolumn('mc_members', array('email' => $email), 'uid');
                if (!empty($email_exists_member)) {
                    $uid = $email_exists_member;
                } else {
                    $member_update_info['groupid'] = pdo_getcolumn('mc_groups',
                        array('uniacid' => $_W['uniacid'], 'isdefault' => 1), 'groupid');
...
function mc_parse_profile($profile)
{
    ...
    if (empty($profile['email']) || (!empty($profile['email']) && substr($profile['email'],
                -6) == 'yidaoit.cn' && strlen($profile['email']) == 39)) {
        $profile['email'] = '';
    }
    return $profile;
}
        ...
        if (!empty($list)) {
            ...
            for ($i = 0; $i < $size; $i++) {
                ...
                foreach ($buffer as $row) {
                    if (strexists($row['email'], 'yidaoit.cn')) {
                        $row['email'] = '';
                    }
