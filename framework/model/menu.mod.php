...
function menu_construct_createmenu_data($data_array, $is_conditional = false)
{
    ...
    foreach ($data_array['button'] as $button) {
        $temp = array();
        $temp['name'] = preg_replace_callback('/\:\:([0-9a-zA-Z_-]+)\:\:/',
            create_function('$matches', 'return utf8_bytes(hexdec($matches[1]));'), $button['name']);
        ...
        if (empty($button['sub_button'])) {
            ...
            foreach ($button['sub_button'] as $sub_button) {
                $sub_temp = array();
                $sub_temp['name'] = preg_replace_callback('/\:\:([0-9a-zA-Z_-]+)\:\:/',
                    create_function('$matches', 'return utf8_bytes(hexdec($matches[1]));'), $sub_button['name']);
...
function menu_update_currentself()
{
    ...
    if (!empty($default_menu_id)) {
        ...
        pdo_update('uni_account_menus', array('status' => STATUS_OFF),
            array('uniacid' => $_W['uniacid'], 'type' => MENU_CURRENTSELF, 'id !=' => $default_menu_id));
    } else {
        $insert_data = array(
            'uniacid' => $_W['uniacid'],
        ...
        pdo_update('uni_account_menus', array('status' => STATUS_OFF),
            array('uniacid' => $_W['uniacid'], 'type' => MENU_CURRENTSELF, 'id !=' => $insert_id));
    }
    return true;
}
...
function menu_update_conditional()
{
    ...
    pdo_update('uni_account_menus', array('status' => STATUS_OFF),
        array('uniacid' => $_W['uniacid'], 'type' => MENU_CONDITIONAL));
    if (!empty($conditional_menu_info['conditionalmenu'])) {
        foreach ($conditional_menu_info['conditionalmenu'] as $menu) {
            ...
            if (!empty($menu['matchrule'])) {
                $menu_info = table('menu')->accountMenuInfo(array(
                    'uniacid' => $_W['uniacid'],
                    'menuid' => $menu['menuid'],
                    'type' => MENU_CONDITIONAL
                ));
...
function menu_push($id)
{
    ...
    if ($menu_info['status'] == STATUS_OFF) {
        ...
        if ($menu_info['type'] == MENU_CURRENTSELF) {
            ...
            pdo_update('uni_account_menus', array('status' => '0'),
                array('id !=' => $menu_info['id'], 'uniacid' => $_W['uniacid'], 'type' => MENU_CURRENTSELF));
            ...
            $status = pdo_update('uni_account_menus', array('status' => STATUS_ON, 'menuid' => $result),
                array('uniacid' => $_W['uniacid'], 'id' => $menu_info['id']));
        }
        return true;
    }
