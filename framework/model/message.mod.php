...
function message_account_expire()
{
    ...
    foreach ($expire_account_list as $account) {
        ...
        if ($account_detail['endtime'] > 0 && $account_detail['endtime'] < TIMESTAMP) {
            ...
            message_notice_record($account_detail['name'] . $account_name, $account_detail['uid'], $account['uniacid'],
                $type, $message);
        }
    }
    return true;
    ...
    if ($response['code'] == 200) {
        ...
        if (!empty($worker_notice_lists)) {
            foreach ($worker_notice_lists as $list) {
                message_notice_record($list['note'], $uid, $list['uuid'], MESSAGE_WORKORDER_TYPE,
                    array('create_time' => strtotime($list['updated_at'])));
            }
        }
    }
    return true;
    ...
    foreach ($users_expire as $v) {
        ...
        if (!empty($v['mobile']) && preg_match(REGULAR_MOBILE, $v['mobile'])) {
            ...
            if (is_error($result)) {
                ...
                pdo_insert('core_sendsms_log', array(
                    'mobile' => $v['mobile'],
                    'content' => $content,
                    'result' => $result['errno'] . $result['message'],
...
function message_user_expire_notice()
{
    ...
    if (!empty($_W['user']['endtime']) && $_W['user']['endtime'] < strtotime('+7 days')) {
        ...
        message_notice_record($content, $_W['uid'], $_W['uid'], MESSAGE_USER_EXPIRE_TYPE,
            array('end_time' => $_W['user']['endtime']));
    }
    return true;
}
...
function message_list_detail($lists)
{
    ...
    foreach ($lists as &$message) {
        ...
        if ($message['type'] == MESSAGE_ORDER_TYPE) {
            $message['url'] = url('site/entry/orders',
                array('m' => 'store', 'direct' => 1, 'message_id' => $message['id']));
        }
        if ($message['type'] == MESSAGE_ACCOUNT_EXPIRE_TYPE) {
            $message['url'] = url('account/manage',
                array('account_type' => ACCOUNT_TYPE_OFFCIAL_NORMAL, 'message_id' => $message['id']));
        }
        if ($message['type'] == MESSAGE_WECHAT_EXPIRE_TYPE) {
            $message['url'] = url('account/manage',
                array('account_type' => ACCOUNT_TYPE_APP_NORMAL, 'message_id' => $message['id']));
        }

        if ($message['type'] == MESSAGE_WEBAPP_EXPIRE_TYPE) {
            $message['url'] = url('account/manage',
                array('account_type' => ACCOUNT_TYPE_WEBAPP_NORMAL, 'message_id' => $message['id']));
        }
        ...
        if ($message['type'] == MESSAGE_WORKORDER_TYPE) {
            $message['url'] = url('system/workorder/display',
                array('uuid' => $message['sign'], 'message_id' => $message['id']));
        }
    }
