...
use Ydb\Entity\Manual\Engine\ModulesBinddings;
use Ydb\Entity\Manual\Engine\ModulesRecycle;
use Ydb\Entity\Manual\Engine\UniAccountUsers;
use Ydb\Entity\Manual\Engine\UniGroup;
...
function module_entries($name, $types = array(), $rid = 0, $args = null)
{
    ...
    $bindings = pdo_getall('modules_bindings', array('module' => $name, 'entry' => $types),
        array(), '', 'displayorder DESC, eid ASC');
    $entries = array();
    foreach ($bindings as $bind) {
        if (!empty($bind['call'])) {
            $response = ihttp_request(web_url('utility/bindcall', array(
...
function module_app_entries($name, $types = array(), $args = null)
{
    ...
    foreach ($bindings as $bind) {
        if (!empty($bind['call'])) {
            ...
            $response = ihttp_request($_W['sitescheme'] . '127.0.0.1/' . $urlset['dirname'] . '/'
                . web_url('utility/bindcall',
                    array(
                        'modulename' => $bind['module'],
                        'callname' => $bind['call'],
                        'args' => $args,
                        'uniacid' => $_W['uniacid']
                    )), array('W' => base64_encode(iserializer($_W))), $extra);
            if (is_error($response)) {
                continue;
            }
            $response = json_decode($response['content'], true);
...
{
    $sql = 'SELECT * FROM ' . ModulesBinddings::TABLE_NAME . ' WHERE `eid`=:eid';
    $pars = array();
    $pars[':eid'] = $eid;
...
function module_save_group_package($package)
{
    ...
    if (!empty($package['id'])) {
        $name_exist = pdo_get('uni_group',
            array('uniacid' => 0, 'id <>' => $package['id'], 'name' => $package['name']));
    } else {
...
function module_fetch($name)
{
    ...
    if (empty($module)) {
        $sql = 'SELECT * FROM ' . Modules::TABLE_NAME . ' as a
                    LEFT JOIN ' . ModulesRecycle::TABLE_NAME . ' as b ON a.name = b.modulename
                WHERE a.name = :name AND b.modulename is NULL';
        ...
        $module_info['main_module'] = pdo_getcolumn('modules_plugin', array('name' => $module_info['name']),
            'main_module');
        if (!empty($module_info['main_module'])) {
            ...
            $module_info['plugin_list'] = pdo_getall('modules_plugin', array('main_module' => $module_info['name']),
                array(), 'name');
            if (!empty($module_info['plugin_list'])) {
                $module_info['plugin_list'] = array_keys($module_info['plugin_list']);
            }
        }
        if ($module_info['app_support'] != MODULE_SUPPORT_ACCOUNT
            && $module_info['wxapp_support'] != MODULE_SUPPORT_WXAPP
            && $module_info['webapp_support'] != MODULE_SUPPORT_WEBAPP
            && $module_info['welcome_support'] != MODULE_SUPPORT_SYSTEMWELCOME) {
            $module_info['app_support'] = MODULE_SUPPORT_ACCOUNT;
        }
        $module_info['is_relation'] = ($module_info['app_support'] == 2 && $module_info['wxapp_support'] == 2);
        ...
        if (array_key_exists($name, $module_upgrade['module_upgrade'])) {
...
function module_get_all_unistalled($status, $cache = true, $module_type = '')
{
    ...
    } elseif (is_array($uninstallModules)) {
    ...
    if (!is_array($uninstallModules)
        || empty($uninstallModules['modules'][$status][$account_type])
        || (int)$uninstallModules['cloud_m_count'] !== (int)$cloud_m_count) {
    ...
    if (!empty($module['permissions'])) {
        ...
        foreach ($module['permissions'] as $permission) {
            $data[] = array(
                ...
                'permission' => $name . '_permission_' . $permission['permission']
            );
        }
    }
    return $data;
...
function module_execute_uninstall_script($module_name, $str)
{
    global $_W;
    load()->object('cloudapi');
    load()->model('cloud');
    ...
    $recycle_module = $cloudapi->post('cache', 'get',
        array('key' => cache_system_key('recycle_module:')));
    $recycle_module = !empty($recycle_module['data']) ? $recycle_module['data'] : array();
    unset($recycle_module[$module_name]);
    $cloudapi->post('cache', 'set',
        array('key' => cache_system_key('recycle_module:'),
            $str => $recycle_module));
    cache_delete(cache_system_key('module:all_uninstall'));
    return true;
...
function module_status($module)
{
    ...
    if (!empty($cloud_m_info) && !empty($cloud_m_info['version']['version'])) {
        if (version_compare($module_info['version'], $cloud_m_info['version']['version'])) {
            $module_status['upgrade'] = array(
                'name' => $module_info['title'],
                'version' => $cloud_m_info['version']['version'],
                'upgrade' => 1
            );
        }
    } else {
    ...
    $module_upgrade_setting = is_array($module_upgrade_setting['module_upgrade'])
        ? $module_upgrade_setting['module_upgrade'] : array();
    if (!array_key_exists($module, $module_upgrade_setting) && !empty($module_status['upgrade']['upgrade'])) {
...
{
    $modules = array();
    $installed_module = pdo_getall('modules',
        array('name' => $module_list), array('version', 'name'), 'name');
    ...
    if (!empty($module_list) && is_array($module_list) && !empty($installed_module)) {
        foreach ($module_list as $key => $module) {
            ...
            if (!empty($manifest) && is_array($manifest)) {
                ...
                $module['from'] = 'local';
                if (version_compare($installed_module[$module['name']]['version'],
                        $manifest['application']['version']) == '-1') {
                    $module['upgrade'] = true;
...
function module_upgrade_new($type = 'account')
{
    ...
    if (!empty($upgrade_modules)) {
        foreach ($upgrade_modules as $key => &$module) {
            ...
            $module['link'] = web_url('module/manage-system/module_detail',
                array('name' => $module['name'], 'show' => 'upgrade'));
        }
        unset($module);
    }
...
function module_exist_in_account($module_name, $uniacid)
{
    ...
    $owner_uid = pdo_getcolumn('uni_account_users',
        array('uniacid' => $uniacid, 'role' => 'owner'), 'uid');
    if (!empty($owner_uid) && !in_array($owner_uid, $founders)) {
        $packageids = pdo_getall('uni_account_group',
            array('uniacid' => $uniacid), array('groupid'), 'groupid');
        ...
        if (!in_array('-1', $packageids)) {
            $uni_modules = array();
            $uni_groups = pdo_fetchall('SELECT `modules` FROM ' . UniGroup::TABLE_NAME
                . ' WHERE ' . "id IN ('" . implode("','", $packageids) . "') OR " . " uniacid = '{$uniacid}'");
            if (!empty($uni_groups)) {
...
function module_link_uniacid_fetch($uid, $module_name)
{
    ...
    foreach ($accounts_list as $key => $account_value) {
        if ($account_value['type'] == ACCOUNT_TYPE_APP_NORMAL) {
            ...
            foreach ($account_value['versions'] as $version_key => $version_value) {
                ...
                if (empty($version_value['modules'][0]['account'])
                    || !is_array($version_value['modules'][0]['account'])) {
                    $accounts_link_result[$key] = $account_value;
                    continue;
                }
    ...
    if (!empty($accounts_link_result)) {
        foreach ($accounts_link_result as $link_key => $link_value) {
            if (in_array($link_value['type'], array(
                    ACCOUNT_TYPE_OFFCIAL_NORMAL,
                    ACCOUNT_TYPE_OFFCIAL_AUTH
                ))
                && !empty($link_value['link_wxapp'])
                && is_array($link_value['link_wxapp'])) {
            ...
            } elseif ($link_value['type'] == ACCOUNT_TYPE_APP_NORMAL
                && !empty($link_value['versions']) && is_array($link_value['versions'])) {
...
function module_clerk_info($module_name)
{
    ...
    $sql = 'SELECT u.uid, p.permission FROM ' . UniAccountUsers::TABLE_NAME . ' u,' . UsersPermission::TABLE_NAME . ' p
        WHERE u.uid = p.uid AND u.uniacid = p.uniacid AND u.role = :role AND p.type = :type AND u.uniacid = :uniacid';
