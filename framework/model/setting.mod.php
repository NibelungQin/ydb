...
function setting_save($data = '', $key = '')
{
    ...
    if (is_array($data) && empty($key)) {
        ...
        if ($record) {
            $return = pdo_query("REPLACE INTO " . tablename('core_settings') . " (`key`, `value`) VALUES " . implode(',',
                    $record));
        }
    } else {
        $record = array();
...
<?php
/**
 * Copyright (c) 2019, Yidao. All rights reserved.
 * YIDAO PROPRIETARY/CONFIDENTIAL. Use is subject to license terms. Visit http://www.yidaoit.cc/ for more details.
 *
 */
defined('IN_IA') or exit('Access Denied');
