...
function site_cover($coverparams = array())
{
    ...
    $cover = pdo_fetch("SELECT * FROM " . tablename('cover_reply') . " WHERE `module` = :module AND uniacid = :uniacid {$where}",
        $params);
    if (empty($cover['rid'])) {
        $rule = array(
...
function site_cover_delete($page_id)
{
    ...
    $cover = pdo_fetch('SELECT * FROM ' . tablename('cover_reply') . ' WHERE uniacid = :uniacid AND module = :module AND multiid = :id',
        array(':uniacid' => $_W['uniacid'], ':module' => 'page', ':id' => $page_id));
    if (!empty($cover)) {
