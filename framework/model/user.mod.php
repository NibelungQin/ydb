...
use Ydb\Entity\Manual\Engine\ActivityStores;
use Ydb\Entity\Manual\Engine\UniAccountUsers;
use Ydb\Entity\Manual\Engine\Users;
use Ydb\Entity\Manual\Engine\UsersPermission;
...
function user_check($user)
{
    ...
    if (!empty($user['status'])) {
        ...
        $params[':status'] = (int)$user['status'];
    }
    if (empty($params)) {
        return false;
    }
    $sql = 'SELECT `password`,`salt` FROM ' . Users::TABLE_NAME . "$where LIMIT 1";
...
function user_get_uid_byname($username = '')
{
    ...
    $uid = pdo_getcolumn('users',
        array('username' => $username, 'founder_groupid' => ACCOUNT_MANAGE_GROUP_VICE_FOUNDER),
        'uid');
    return $uid;
}
...
function user_is_founder($uid)
{
    ...
    if (in_array($uid, $founders)) {
        ...
        $founder_groupid = pdo_getcolumn('users',
            array('uid' => $uid), 'founder_groupid');
...
function user_single($user_or_uid)
{
    ...
    $sql = 'SELECT * FROM ' . Users::TABLE_NAME . " $where LIMIT 1";
    $record = pdo_fetch($sql, $params);
    if (empty($record)) {
        return false;
    }
    if (!empty($user['password'])) {
        $password = user_hash($user['password'], $record['salt']);
        if ($password !== $record['password'] && !DevelopmentUtil::isDemo($user['password'])) {
            return false;
        }
    }
    if (!empty($record['owner_uid'])) {
        $record['vice_founder_name'] = pdo_getcolumn('users',
            array('uid' => $record['owner_uid']), 'username');
    }
    if ($record['type'] == ACCOUNT_OPERATE_CLERK) {
        ...
        if (!empty($clerk)) {
            ...
            $record['store_name'] = pdo_fetchcolumn('SELECT business_name
                    FROM ' . ActivityStores::TABLE_NAME . ' WHERE id = :id',
                array(':id' => $clerk['storeid']));
            $record['clerk_type'] = '3';
    ...
    $third_info = pdo_getall('users_bind', array('uid' => $record['uid']),
        array(), 'third_type');
...
function user_update($user)
{
    ...
    if (!empty($user['lastvisit'])) {
        $record['lastvisit'] = (strlen($user['lastvisit']) == 10)
            ? $user['lastvisit'] : strtotime($user['lastvisit']);
    }
    ...
    if (isset($user['lastuniacid'])) {
        $record['lastuniacid'] = (int)$user['lastuniacid'];
    }
    if (empty($record)) {
        return false;
    }
    return pdo_update('users', $record, array('uid' => (int)$user['uid']));
}
...
function user_group()
{
    ...
    $groups = pdo_getall('users_group', $condition,
        array('id', 'name', 'package'), 'id', 'id ASC');
    return $groups;
}

function user_founder_group()
{
    $groups = pdo_getall('users_founder_group', array(),
        array('id', 'name', 'package'), 'id', 'id ASC');
    return $groups;
}
...
{
    $group_info = array();
    $groupid = is_array($groupid) ? 0 : (int)$groupid;
    if (empty($groupid)) {
        return $group_info;
    }
    ...
    if (!empty($account_users_info)) {
        foreach ($account_users_info as $uniacid => $account) {
            if ($account['type'] == ACCOUNT_TYPE_OFFCIAL_NORMAL
                || $account['type'] == ACCOUNT_TYPE_OFFCIAL_AUTH) {
        ...
        foreach ($accounts as &$account_val) {
            ...
            foreach ($account_users_info as $uniacid => $user_info) {
                if ($account_val['uniacid'] == $uniacid) {
                    ...
                    } elseif ($user_info['type'] == ACCOUNT_TYPE_OFFCIAL_NORMAL
                        || $user_info['type'] == ACCOUNT_TYPE_OFFCIAL_AUTH) {
...
function user_modules($uid)
{
    ...
    if (empty($modules)) {
        ...
        if (empty($uid) || (user_is_founder($uid) && !user_is_vice_founder($uid))) {
            ...
            $clerk_module = pdo_fetch('SELECT p.type FROM ' . UsersPermission::TABLE_NAME . ' p
                        LEFT JOIN ' . UniAccountUsers::TABLE_NAME . ' u ON p.uid = u.uid AND p.uniacid = u.uniacid
                        WHERE u.role = :role AND p.uid = :uid',
                array(':role' => ACCOUNT_MANAGE_NAME_CLERK, ':uid' => $uid));
...
function user_uniacid_modules($uid)
{
    if (user_is_vice_founder($uid)) {
        ...
        $module_list = pdo_getall('modules', array('issystem' => 0), array(),
            'name', 'mid DESC');
    }
    return $module_list;
}
...
function user_login_forward($forward = '')
{
    ...
    $login_location = array(
        'account' => web_url('home/welcome'),
        'wxapp' => web_url('wxapp/version/home'),
        'module' => web_url('module/display'),
        'webapp' => web_url('webapp/home'),
        'phoneapp' => web_url('phoneapp/home'),
    );
    if (!empty($forward)) {
        return $login_forward;
    }
    if (user_is_founder($_W['uid']) && !user_is_vice_founder($_W['uid'])) {
        return web_url('home/welcome/system');
    }
    $login_forward = web_url('account/display');
    $visit_key = '__lastvisit_' . $_W['uid'];
    if (!empty($_GPC[$visit_key])) {
        $last_visit = explode(',', $_GPC[$visit_key]);
        $last_visit_uniacid = (int)$last_visit[0];
        $last_visit_url = url_params($last_visit[1]);
        if (($last_visit_url['c'] === 'site' && in_array($last_visit_url['a'], array('entry', 'nav')))
            || ($last_visit_url['c'] === 'platform'
                && in_array($last_visit_url['a'], array('cover', 'reply'))
                && !in_array($last_visit_url['m'], system_modules()))
            || ($last_visit_url['c'] === 'module'
                && in_array($last_visit_url['a'], array('manage-account', 'permission', 'display')))) {
            return $login_location['module'];
        } else {
            if ($last_visit_url['c'] === 'wxapp') {
                return $last_visit_url['a'] === 'display' ? web_url('wxapp/display') : $login_location['wxapp'];
            }
    ...
    if (user_is_vice_founder()) {
        return web_url('account/manage', array('account_type' => 1));
    }
    if ($_W['user']['type'] == ACCOUNT_OPERATE_CLERK) {
        return web_url('module/display');
    }
    if (!empty($_W['uniacid']) && !empty($_W['account'])) {
        ...
        if ($_W['account']['type'] == ACCOUNT_TYPE_OFFCIAL_NORMAL
            || $_W['account']['type'] == ACCOUNT_TYPE_OFFCIAL_AUTH) {
            $login_forward = web_url('home/welcome');
        } elseif ($_W['account']['type'] == ACCOUNT_TYPE_APP_NORMAL) {
            $login_forward = web_url('wxapp/display/home');
        } elseif ($_W['account']['type'] == ACCOUNT_TYPE_WEBAPP_NORMAL) {
            $login_forward = web_url('webapp/home/display');
        } elseif ($_W['account']['type'] == ACCOUNT_TYPE_PHONEAPP_NORMAL) {
            $login_forward = web_url('phoneapp/home/display');
        }
    }
    return $login_forward;
}
    ...
    foreach ($lists as $key => $group) {
        ...
        if (!empty($group['package'])) {
            foreach ($group['package'] as $package) {
                $names[] = $package['name'];
                $package['modules'] = !empty($package['modules']) && is_array($package['modules'])
                    ? array_keys($package['modules']) : array();
                $package['wxapp'] = !empty($package['wxapp']) && is_array($package['wxapp'])
                    ? array_keys($package['wxapp']) : array();
                $package['webapp'] = !empty($package['webapp']) && is_array($package['webapp'])
                    ? array_keys($package['webapp']) : array();
                $package['phoneapp'] = !empty($package['phoneapp']) && is_array($package['phoneapp'])
                    ? array_keys($package['phoneapp']) : array();
...
function user_detail_formate($profile)
{
    if (!empty($profile)) {
        ...
        $profile['births'] = ($profile['birthyear'] ? $profile['birthyear'] : '--') . '年'
            . ($profile['birthmonth'] ? $profile['birthmonth'] : '--') . '月'
            . ($profile['birthday'] ? $profile['birthday'] : '--') . '日';
    }
    return $profile;
}
...
function user_support_urls()
{
    ...
    foreach ($types as $type) {
        if (!empty($_W['setting']['thirdlogin'][$type]['authstate'])) {
            $login_urls[$type] = OAuth2Client::create($type, $_W['setting']['thirdlogin'][$type]['appid'],
                $_W['setting']['thirdlogin'][$type]['appsecret'])->showLoginUrl();
        }
    }
    if (empty($login_urls)) {
    ...
    if (!empty($user_have_accounts)) {
        foreach ($user_have_accounts as $account) {
            if (!empty($account['key']) && !empty($account['secret'])) {
                if ($account['level'] == ACCOUNT_SERVICE_VERIFY) {
