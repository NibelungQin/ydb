...
function wxapp_account_create($account)
{
    ...
    if (empty($_W['isfounder'])) {
        ...
        if (empty($user_info['usergroup_wxapp_limit'])) {
            ...
            pdo_insert('site_store_create_account',
                array('uid' => $_W['uid'], 'uniacid' => $uniacid, 'type' => ACCOUNT_TYPE_APP_NORMAL));
        }
    }
...
function wxapp_update_daily_visittrend()
{
    ...
    $trend = pdo_get('wxapp_general_analysis',
        array('uniacid' => $_W['uniacid'], 'type' => WXAPP_STATISTICS_DAILYVISITTREND, 'ref_date' => $yesterday));
    if (!empty($trend)) {
        return true;
    }
...
{
    return pdo_update('wxapp_versions', array('appjson' => serialize($json), 'use_default' => 0),
        array('id' => $version_id));
}
