...
class ModuleTable extends We7Table
{
    ...
    {
        return $this->query->from('modules')->where('issystem', 1)->whereor('name',
            $package_group_module)->orderby('mid', 'desc')->getall('name');
    }
    ...
    public function moduleSetRankTop($module_name)
    {
        ...
        if (!empty($exist)) {
            ...
            pdo_insert('modules_rank',
                array('uid' => $_W['uid'], 'module_name' => $module_name, 'rank' => ($max_rank + 1)));
        }
        return true;
    }
    ...
    public function uniAccountModuleInfo($module_name)
    {
        ...
        $result = $this->query->from('uni_account_modules')->where('module', $module_name)->where('uniacid',
            $_W['uniacid'])->get();
        if (!empty($result)) {
