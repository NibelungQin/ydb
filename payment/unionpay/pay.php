...
if (!empty($_POST) && verify($_POST) && $_POST['respMsg'] == 'success') {
    ...
    if (!empty($log) && $log['status'] == '0') {
        ...
        if ($log['is_usecard'] == 1 && $log['card_type'] == 2) {
            ...
            pdo_query('UPDATE ' . tablename('activity_coupon_record') . " SET status = 2, usetime = {$now}, usemodule = '{$log['module']}' WHERE uniacid = :aid AND couponid = :cid AND uid = :uid AND status = 1 LIMIT 1",
                array(':aid' => $_W['uniacid'], ':uid' => $log['openid'], ':cid' => $log['card_id']));
        }
    }
