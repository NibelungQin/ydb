    ...
    if (!empty($wechat)) {
        ...
        if ($sign == $get['sign']) {
            ...
            if (!empty($log) && $log['status'] == '0' && (($get['total_fee'] / 100) == $log['card_fee'])) {
                ...
                $mix_pay_credit_log = pdo_get('core_paylog', array(
                    'module' => $log['module'],
                    'tid' => $log['tid'],
                    'uniacid' => $log['uniacid'],
                    'type' => 'credit'
                ...
                if (!empty($mix_pay_credit_log)) {
                    ...
                    mc_credit_update($log['uid'], $setting['creditbehaviors']['currency'], -$mix_pay_credit_log['fee'],
                        array($log['uid'], '消费' . $setting['creditbehaviors']['currency'] . ':' . $fee));
                }
