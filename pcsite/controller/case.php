...
class CaseController extends Controller
{
    public function index()
    {
        ...
        $articles = pdo_fetchall('SELECT a.* ,c.id as cid,c.name FROM ' . tablename('ewei_shop_system_case') . ' AS a
                    ...
                    WHERE 1 ' . $condition . '  ORDER BY a.displayorder DESC LIMIT ') . ($pindex - 1) * $psize . ',' . $psize,
            $params);
        $total = pdo_fetchcolumn('SELECT count(1) FROM ' . tablename('ewei_shop_system_case') . ' WHERE status = 1 ',
            $params);
        ...
        $casebanner = pdo_fetch('select casebanner,background from ' . tablename('ewei_shop_system_setting') . ' where uniacid = :uniacid ',
