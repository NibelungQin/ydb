...
class LoginController extends Controller
{
    ...
    public function check()
    {
        ...
        pdo_query('DELETE FROM' . tablename('users_failed_login') . ' WHERE lastupdate < :timestamp',
            array(':timestamp' => TIMESTAMP - 300));
        ...
        if (1.2 <= IMS_VERSION) {
            ...
            if (!empty($record)) {
                ...
                if ($record['type'] == ACCOUNT_OPERATE_CLERK) {
                    ...
                    $data['msg'] = 'true';
                    $data['url'] = url('site/entry/clerkdesk',
                        array('uniacid' => $record['uniacid'], 'op' => 'index', 'm' => 'we7_coupon'));
                    echo json_encode($data);
                    exit();
                }
                ...
                if (empty($failed)) {
                    pdo_insert('users_failed_login',
                        array('ip' => CLIENT_IP, 'username' => $username, 'count' => '1', 'lastupdate' => TIMESTAMP));
                } else {
                    pdo_update('users_failed_login', array('count' => $failed['count'] + 1, 'lastupdate' => TIMESTAMP),
                        array('id' => $failed['id']));
                }
