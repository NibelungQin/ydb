...
function web_message($msg, $redirect = '', $type = '', $tips = false, $extend = array())
{
    global $_W, $_GPC;
    if ($redirect === 'refresh') {
...
let url = ' . (!empty($redirect) ? 'parent.location.href' : "''") . ";
let modalobj = util.message('" . $msg . "', '', '" . $type . "');
if (url) {
    modalobj.on('hide.bs.modal', function() {
        \$('.modal').each(function(){
            if(\$(this).attr('id') != 'modal-message') {
                \$(this).modal('hide');
            }
        });
        top.location.reload()
            ...
            $vars['type'] = $type;
            ExceptionUtil::exit(json_encode($vars), true);
        }
    }
    if (empty($msg) && !empty($redirect)) {
        $header = 'Location: ' . $redirect;
        YdbHttpUtil::header($header);
    ...
    if ($tips) {
        ...
        if (!empty($message_cookie['redirect'])) {
            YdbHttpUtil::header('Location: ' . $message_cookie['redirect']);
        } else {
            include web_template('common/message', TEMPLATE_INCLUDEPATH);
        }
    } else {
        include web_template('common/message', TEMPLATE_INCLUDEPATH);
    }
...
{
    web_message(error($code, $message), $redirect, 'ajax', false);
}

function web_itoast($message, $redirect = '', $type = '', $extend = array())
{
    web_message($message, $redirect, $type, true, $extend);
}
...
function checklogin()
{
    ...
    if (empty($_W['uid'])) {
        if (!empty($_W['setting']['copyright']['showhomepage'])) {
            web_itoast('', web_url('user/login'), 'warning');
        } else {
            web_itoast('', web_url('user/login'), 'warning');
        }
    }
...
function buildframes($framename = '')
{
    ...
    if (!$_W['isfounder'] && $status && $_W['role'] !== ACCOUNT_MANAGE_NAME_OWNER) {
        ...
        if (!is_error($module_permission) && !empty($module_permission)) {
            foreach ($module_permission as $module) {
                if (!in_array($module['type'], $sysmodules)
                    && empty($modules[$module['type']]['main_module'])
                    && ($modules[$module['type']]['app_support'] == 2)) {
                    $module = $modules[$module['type']];
                    if (!empty($module)) {
                        $frames['account']['section']['platform_module']['menu']['platform_' . $module['name']] = array(
                            'title' => $module['title'],
                            'icon' => $module['logo'],
                            'url' => web_url('home/welcome/ext', array('m' => $module['name'])),
                            'is_display' => 1,
                        );
                    }
                }
        ...
        $account_module = pdo_getall('uni_account_modules',
            array('uniacid' => $_W['uniacid'], 'shortcut' => STATUS_ON),
            array('module'), '', 'displayorder DESC, id DESC');
        if (!empty($account_module)) {
            foreach ($account_module as $module) {
                if (!in_array($module['module'], $sysmodules)) {
                    ...
                    if (!empty($module) && !empty($modules[$module['name']])
                        && empty($module['main_module']) && $module['app_support'] == 2) {
                        $frames['account']['section']['platform_module']['menu']['platform_' . $module['name']] = array(
                            'title' => $module['title'],
                            'icon' => $module['logo'],
                            'url' => web_url('home/welcome/ext', array('m' => $module['name'])),
    ...
    if (!empty($user_permission)) {
        foreach ($frames as $nav_id => $section) {
            ...
            foreach ($section['section'] as $section_id => $secion) {
                if ($nav_id == 'account') {
                    if ($status && !empty($module_permission)
                        && in_array("account*", $user_permission)
                        && $section_id !== 'platform_module'
                        && permission_account_user_role($_W['uid'], $_W['uniacid']) != ACCOUNT_MANAGE_NAME_OWNER) {
                        $frames['account']['section'][$section_id]['is_display'] = false;
                        continue;
                    } elseif (in_array("account*", $user_permission)) {
                        continue;
                    }
                }
    ...
    $eid = (int)$_GPC['eid'];
    $version_id = (int)$_GPC['version_id'];
    if ((!empty($modulename) || !empty($eid)) && !in_array($modulename, system_modules())) {
        ...
        if ($status) {
            $permission = pdo_get('users_permission',
                array('uniacid' => $_W['uniacid'], 'uid' => $_W['uid'], 'type' => $modulename), array('permission'));
            if (!empty($permission)) {
        ...
        if ($module['isrulefields'] || !empty($entries['cover']) || !empty($entries['mine'])) {
            if (!empty($module['isrulefields'])
                && !empty($_W['account'])
                && in_array($_W['account']['type'], array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH))) {
                $url = web_url('platform/reply', array('m' => $modulename, 'version_id' => $version_id));
            }
            if (empty($url) && !empty($entries['cover'])) {
                $url = web_url('platform/cover',
                    array('eid' => $entries['cover'][0]['eid'], 'version_id' => $version_id));
            }
        ...
        if ($module['settings']) {
            $frames['account']['section']['platform_module_common']['menu']['platform_module_settings'] = array(
                ...
                'url' => web_url('module/manage-account/setting',
                    array('m' => $modulename, 'version_id' => $version_id)),
                'is_display' => 1,
            );
        }
        if ($module['permissions'] && ($_W['isfounder'] || $_W['role'] == ACCOUNT_MANAGE_NAME_OWNER)) {
            $frames['account']['section']['platform_module_common']['menu']['platform_module_permissions'] = array(
                ...
                'url' => web_url('module/permission', array('m' => $modulename, 'version_id' => $version_id)),
                'is_display' => 1,
            );
        }
        if ($_W['isfounder'] || $_W['role'] == ACCOUNT_MANAGE_NAME_OWNER) {
            $frames['account']['section']['platform_module_common']['menu']['platform_module_default_entry'] = array(
                ...
                'url' => web_url('module/default-entry', array('m' => $modulename, 'version_id' => $version_id)),
                'is_display' => 1,
            );
        }
        if ($entries['home'] && !empty($_W['account'])
            && in_array($_W['account']['type'], array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH))) {
            $frames['account']['section']['platform_module_common']['menu']['platform_module_home'] = array(
                'title' => "<i class='fa fa-home'></i> 微站首页导航",
                'url' => web_url('site/nav/home', array('m' => $modulename, 'version_id' => $version_id)),
                'is_display' => 1,
            );
        }
        if ($entries['profile'] && !empty($_W['account'])
            && in_array($_W['account']['type'], array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH))) {
            $frames['account']['section']['platform_module_common']['menu']['platform_module_profile'] = array(
                'title' => "<i class='fa fa-user'></i> 个人中心导航",
                'url' => web_url('site/nav/profile', array('m' => $modulename, 'version_id' => $version_id)),
                'is_display' => 1,
            );
        }
        if ($entries['shortcut'] && !empty($_W['account'])
            && in_array($_W['account']['type'], array(ACCOUNT_TYPE_OFFCIAL_NORMAL, ACCOUNT_TYPE_OFFCIAL_AUTH))) {
            $frames['account']['section']['platform_module_common']['menu']['platform_module_shortcut'] = array(
                'title' => "<i class='fa fa-plane'></i> 快捷菜单",
                'url' => web_url('site/nav/shortcut', array('m' => $modulename, 'version_id' => $version_id)),
                'is_display' => 1,
            );
        }
        if (!empty($entries['cover'])) {
            foreach ($entries['cover'] as $key => $menu) {
                $frames['account']['section']['platform_module_common']['menu']['platform_module_cover'][] = array(
                    'title' => "{$menu['title']}",
                    'url' => web_url('platform/cover', array('eid' => $menu['eid'], 'version_id' => $version_id)),
                    'is_display' => 0,
                );
            }
        }
            ...
            if (!empty($plugin_list)) {
                ...
                foreach ($plugin_list as $plugin) {
                    $frames['account']['section']['platform_module_menu']['plugin_menu']['menu'][$modules[$plugin]['name']] = array(
                        ...
                        'url' => web_url('home/welcome/ext', array('m' => $plugin, 'version_id' => $version_id)),
                    );
                }
            }
        }
    ...
    if (defined('FRAME') && FRAME === 'wxapp') {
        ...
        if (!empty($wxapp_version['modules'])) {
            foreach ($wxapp_version['modules'] as $module) {
                ...
                $frames['wxapp']['section']['wxapp_module']['menu']['module_menu' . $module['mid']] = array(
                    ...
                    'url' => web_url('wxapp/display/switch',
                        array('module' => $module['name'], 'version_id' => $version_id)),
                    'is_display' => 1,
                );
            }
        } else {
        ...
        if (!empty($frames['wxapp']['section'])) {
            ...
            foreach ($frames['wxapp']['section'] as $wxapp_section_id => $wxapp_section) {
                if ($status && !empty($wxapp_permission) && in_array("wxapp*", $wxapp_permission)
                    && $wxapp_section_id != 'wxapp_module' && $role != ACCOUNT_MANAGE_NAME_OWNER) {
                ...
                if (!empty($wxapp_section['menu']) && $wxapp_section_id != 'wxapp_module') {
                    foreach ($wxapp_section['menu'] as $wxapp_menu_id => $wxapp_menu) {
                        ...
                        if (!in_array('wxapp*', $wxapp_permission)
                            && !in_array($wxapp_menu['permission_name'], $wxapp_permission)) {
    ...
    if (defined('FRAME') && FRAME === 'phoneapp') {
        ...
        if (!empty($phoneapp_version['modules'])) {
            foreach ($phoneapp_version['modules'] as $module) {
                ...
                $frames['phoneapp']['section']['phoneapp_module']['menu']['module_menu' . $module['mid']] = array(
                    'title' => "<img src='{$module['logo']}'> {$module['title']}",
                    'url' => url('phoneapp/display/switch',
                        array('module' => $module['name'], 'version_id' => $version_id)),
                    'is_display' => 1,
                );
            }
            ...
            $frames['phoneapp']['section']['platform_module']['menu']['platform_module_more'] = array(
                'title' => '更多应用',
                'url' => web_url('phoneapp/description'),
                'is_display' => 1,
            );
        }
        if (!empty($frames['phoneapp']['section'])) {
            ...
            foreach ($frames['phoneapp']['section'] as $phoneapp_section_id => $phoneapp_section) {
                if ($status && !empty($phoneapp_permission) && in_array("phoneapp*", $phoneapp_permission)
                    && $phoneapp_section_id !== 'phoneapp_module' && $role != ACCOUNT_MANAGE_NAME_OWNER) {
                ...
                if (!empty($phoneapp_section['menu']) && $phoneapp_section_id !== 'phoneapp_module') {
                    foreach ($phoneapp_section['menu'] as $phoneapp_menu_id => $phoneapp_menu) {
                        ...
                        if (!in_array('phoneapp*', $phoneapp_permission)
                            && !in_array($phoneapp_menu['permission_name'], $phoneapp_permission)) {
    ...
    foreach ($frames as $menuid => $menu) {
        if ((!empty($menu['founder']) && empty($_W['isfounder']))
            || (is_array($_W['setting']['store']['blacklist'])
                && in_array($_W['username'], $_W['setting']['store']['blacklist']) && $menuid === 'store')
            ...
            || !$menu['is_display']
            || ($_W['setting']['store']['status'] == 1 && $menuid === 'store'
                && (!$_W['isfounder'] || user_is_vice_founder()))) {
            continue;
        }
        $top_nav[] = array(
            'title' => $menu['title'],
...
function site_profile_perfect_tips()
{
    global $_W;
    if ($_W['isfounder']
        && (empty($_W['setting']['site']) || empty($_W['setting']['site']['profile_perfect']))
        && !defined('SITE_PROFILE_PERFECT_TIPS')) {
        $url = web_url('cloud/profile');
        return <<<EOF
$(function() {
var html =
...
'<button type="button" class="close" data-dismiss="alert">
    <span aria-hidden="true" class="wi wi-error-sign"></span><span class="sr-only">Close</span></button>'+
//					'<a href="{$url}" target="_blank">请尽快完善您在一道宝云服务平台的站点注册信息。</a>'+
'</div>'+
'</div>'+
'</div>';
$('body').prepend(html);
