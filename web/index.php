...
if (!function_exists('_calc_current_frames')) {
...
if (empty($_W['isfounder']) && !empty($_W['user'])
    && (($_W['user']['status'] == USER_STATUS_CHECK) || ($_W['user']['status'] == USER_STATUS_BAN))) {
    web_message('您的账号正在审核或是已经被系统禁止，请联系网站管理员解决！');
}
...
if (($_W['setting']['copyright']['status'] == 1) && empty($_W['isfounder'])
    && $controller != 'cloud' && $controller != 'utility' && $controller != 'account') {
    $_W['siteclose'] = true;
    if ($controller == 'account' && $action == 'welcome') {
        web_template('account/welcome');
        exit();
    }
    ...
    web_message('站点已关闭，关闭原因：' . $_W['setting']['copyright']['reason'], url('account/welcome'), 'info');
}
...
if ($_W['role'] != ACCOUNT_MANAGE_NAME_FOUNDER) {
    if ($_W['role'] == ACCOUNT_MANAGE_NAME_UNBIND_USER) {
        web_itoast('', web_url('user/third-bind'));
    }
    if (empty($_W['uniacid'])) {
        ...
        if (defined('FRAME') && FRAME == 'wxapp') {
            web_itoast('', web_url('wxapp/display'), 'info');
        }
    }
    $acl = permission_build();
    if (empty($acl[$controller][$_W['role']])
        || (!in_array($controller . '*', $acl[$controller][$_W['role']])
            && !in_array($action, $acl[$controller][$_W['role']]))) {
        web_message('不能访问, 需要相应的权限才能访问！');
    }
}
require _forward($controller, $action);
