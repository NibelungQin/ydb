...
if ($do == 'display') {
    ...
    if (checksubmit('submit')) {
        if (!empty($_GPC['delete'])) {
            pdo_query("DELETE FROM " . tablename('uni_group') . " WHERE id IN ('" . implode("','",
                    $_GPC['delete']) . "')");
    ...
    if (!empty($list)) {
        foreach ($list as &$row) {
            if (!empty($row['modules'])) {
                ...
                if (is_array($modules)) {
                    $row['modules'] = pdo_fetchall("SELECT name, title FROM " . tablename('modules') . " WHERE `name` IN ('" . implode("','",
                            $modules) . "')");
                }
            }
            if (!empty($row['templates'])) {
                ...
                if (is_array($templates)) {
                    $row['templates'] = pdo_fetchall("SELECT name, title FROM " . tablename('site_templates') . " WHERE id IN ('" . implode("','",
                            $templates) . "')");
                }
            }
        }
