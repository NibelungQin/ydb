...
if ($do == 'edit') {
    ...
    $account = pdo_fetch("SELECT * FROM " . tablename('uni_account') . " WHERE uniacid = :uniacid",
        array(':uniacid' => $uniacid));
    if (empty($account)) {
        message('抱歉，您操作的公众号不存在或是已经被删除！');
    }
    $permission = pdo_fetchall("SELECT id, uid, role FROM " . tablename('uni_account_users') . " WHERE uniacid = '$uniacid' and role != :role  ORDER BY uid ASC, role DESC",
        array(':role' => 'clerk'), 'uid');
    if (!empty($permission)) {
        $member = pdo_fetchall("SELECT username, uid FROM " . tablename('users') . " WHERE uid IN (" . implode(',',
                array_keys($permission)) . ")", array(), 'uid');
    }
    $uids = array();
...
if ($do == 'auth') {
    ...
    foreach ($uids as $v) {
        ...
        $exists = pdo_fetch("SELECT * FROM " . tablename('uni_account_users') . " WHERE uid = :uid AND uniacid = :uniacid",
            array(':uniacid' => $uniacid, ':uid' => $tmpuid));
        if (empty($exists)) {
            $data['role'] = 'operator';
...
if ($do == 'select') {
    ...
    $permission = pdo_fetchall("SELECT uid FROM " . tablename('uni_account_users') . " WHERE uniacid = '$uniacid'",
        array(), 'uid');
...
if ($do == 'user') {
    ...
    if (!empty($user)) {
        ...
        $exists = pdo_fetch("SELECT * FROM " . tablename('uni_account_users') . " WHERE uid = :uid AND uniacid = :uniacid",
            array(':uniacid' => $uniacid, ':uid' => $user['uid']));
        if (empty($exists)) {
            $data['role'] = 'operator';
            pdo_insert('uni_account_users', $data);
        } else {
