...
$role_permission = in_array($state, array(
...
if ($do == 'edit') {
    $permissions = pdo_fetchall("SELECT id, uid, role FROM " . tablename('uni_account_users') . " WHERE uniacid = '$uniacid' and role != :role  ORDER BY uid ASC, role DESC",
        array(':role' => 'clerk'), 'uid');
    $owner = pdo_get('uni_account_users', array('uniacid' => $uniacid, 'role' => 'owner'), array('uid', 'id'));
    if (!empty($permissions)) {
        $member = pdo_fetchall("SELECT username, uid FROM " . tablename('users') . " WHERE uid IN (" . implode(',',
                array_keys($permissions)) . ")", array(), 'uid');
        if (!empty($member)) {
    ...
    if (!empty($user)) {
        ...
        if (is_error($permission = permission_create_account($user['uid'],
                ACCOUNT_TYPE)) && $addtype == ACCOUNT_MANAGE_TYPE_OWNER) {
        ...
        if (empty($exists)) {
            ...
            if ($addtype == ACCOUNT_MANAGE_TYPE_VICE_FOUNDER) {
                ...
                pdo_delete('uni_account_users',
                    array('uniacid' => $uniacid, 'role' => ACCOUNT_MANAGE_NAME_VICE_FOUNDER));
            ...
            if ($addtype == ACCOUNT_MANAGE_TYPE_OWNER) {
                ...
                if ($addtype == ACCOUNT_MANAGE_TYPE_MANAGER) {
                    if ($state == ACCOUNT_MANAGE_NAME_MANAGER) {
                        iajax(4, '管理员不可操作管理员', '');
                    }
                    $data['role'] = ACCOUNT_MANAGE_NAME_MANAGER;
                } else {
                    if ($addtype == ACCOUNT_MANAGE_TYPE_OPERATOR) {
    ...
    if (!empty($module)) {
        foreach ($module as $key => $value) {
            if (in_array($account['type'], array(
                    ACCOUNT_TYPE_OFFCIAL_NORMAL,
                    ACCOUNT_TYPE_OFFCIAL_AUTH
                )) && $value['app_support'] != MODULE_SUPPORT_ACCOUNT) {
                unset($module[$key]);
            }
    ...
    if (checksubmit('submit')) {
        ...
        if ($account['type'] == ACCOUNT_TYPE_OFFCIAL_NORMAL || $account['type'] == ACCOUNT_TYPE_OFFCIAL_AUTH) {
            if (!empty($_GPC['system'])) {
                ...
                pdo_delete('users_permission',
                    array('uniacid' => $uniacid, 'uid' => $uid, 'type' => PERMISSION_ACCOUNT));
            }
        } elseif ($account['type'] == ACCOUNT_TYPE_APP_NORMAL) {
            if (!empty($_GPC['wxapp'])) {
        ...
        pdo_query("DELETE FROM " . tablename('users_permission') . " WHERE uniacid = :uniacid AND uid = :uid AND type != '" . PERMISSION_ACCOUNT . "' AND type != '" . PERMISSION_WXAPP . "'",
    ...
    $purview = pdo_fetch('SELECT * FROM ' . tablename('users_permission') . ' WHERE uniacid = :aid AND uid = :uid AND type = :type',
        array(':aid' => $uniacid, ':uid' => $uid, ':type' => $module_name));
