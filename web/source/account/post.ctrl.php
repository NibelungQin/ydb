...
if (empty($uniacid) || empty($acid)) {
    web_itoast('请选择要编辑的公众号', web_url('account/manage'), 'error');
}
$defaultaccount = uni_account_default($uniacid);
if (!$defaultaccount) {
    web_itoast('无效的acid', web_url('account/manage'), 'error');
}
...
$role_permission = in_array($state,
    array(ACCOUNT_MANAGE_NAME_FOUNDER, ACCOUNT_MANAGE_NAME_OWNER, ACCOUNT_MANAGE_NAME_VICE_FOUNDER));
...
if ($role_permission) {
    ...
    if (ACCOUNT_TYPE == ACCOUNT_TYPE_APP_NORMAL) {
        header('Location: ' . web_url('wxapp/manage/display', array('uniacid' => $uniacid, 'acid' => $acid)));
        exit;
    } else {
        $do = in_array($do, $dos) ? $do : 'modules_tpl';
    }
} else {
    web_itoast('您是该公众号的操作员，无权限操作！', web_url('account/manage'), 'error');
}
...
if ($do == 'base') {
    if (!$role_permission) {
        web_itoast('无权限操作！', web_url('account/post/modules_tpl', array('uniacid' => $uniacid, 'acid' => $acid)), 'error');
    }
    if ($_W['ispost'] && $_W['isajax']) {
        ...
        switch ($type) {
            ...
            case 'headimgsrc':
                ...
                break;
            case 'name':
                $uni_account = pdo_update('uni_account', array('name' => trim($_GPC['request_data'])),
                    array('uniacid' => $uniacid));
                $account_wechats = pdo_update(uni_account_tablename(ACCOUNT_TYPE),
                    array('name' => trim($_GPC['request_data'])), array('acid' => $acid, 'uniacid' => $uniacid));
                $result = ($uni_account && $account_wechats) ? true : false;
                break;
            case 'account' :
                $data = array('account' => trim($_GPC['request_data']));
                break;
            case 'original':
                $data = array('original' => trim($_GPC['request_data']));
                break;
            case 'level':
                $data = array('level' => intval($_GPC['request_data']));
                break;
            case 'key':
                $data = array('key' => trim($_GPC['request_data']));
                break;
            case 'secret':
                $data = array('secret' => trim($_GPC['request_data']));
                break;
            case 'token':
                $oauth = (array)uni_setting_load(array('oauth'), $uniacid);
                if ($oauth['oauth'] == $acid && $account['level'] != 4) {
                    $acid = pdo_fetchcolumn("SELECT acid FROM " . tablename('account_wechats') . " WHERE uniacid = :uniacid AND level = 4 AND secret != '' AND `key` != ''",
                        array(':uniacid' => $uniacid));
            ...
            case 'jointype':
                ...
                if ($original_type['type'] == ACCOUNT_NORMAL_LOGIN) {
                    ...
                    $update_type = pdo_update('account', array('type' => ACCOUNT_NORMAL_LOGIN),
                        array('uniacid' => $uniacid));
            ...
            case 'highest_visit':
                ...
                $result = pdo_update('uni_settings', array('statistics' => iserializer($highest_visit)),
                    array('uniacid' => $uniacid));
                break;
            case 'endtime':
                if ($_GPC['endtype'] == 1) {
        ...
        if (!in_array($type, array('qrcodeimgsrc', 'headimgsrc', 'name', 'endtime', 'jointype', 'highest_visit'))) {
            $result = pdo_update(uni_account_tablename(ACCOUNT_TYPE), $data,
                array('acid' => $acid, 'uniacid' => $uniacid));
        }
        if ($result) {
    ...
    if ($_W['setting']['platform']['authstate']) {
        ...
        if (is_error($preauthcode)) {
            ...
            $authurl = array(
                'errno' => 0,
                'url' => sprintf(ACCOUNT_PLATFORM_API_LOGIN, $account_platform->appid, $preauthcode,
                    urlencode($GLOBALS['_W']['siteroot'] . 'index.php?c=account&a=auth&do=forward'))
            );
        }
    }
    ...
    web_template('account/manage-base' . ACCOUNT_TYPE_TEMPLATE);
}
...
if ($do == 'sms') {
    ...
    web_template('account/manage-sms' . ACCOUNT_TYPE_TEMPLATE);
}
...
if ($do == 'modules_tpl') {
    ...
    if ($_W['isajax'] && $_W['ispost'] && ($role_permission)) {
        ...
        if ($_GPC['type'] == 'extend') {
            ...
            if (!empty($module) || !empty($tpl)) {
                ...
                $id = pdo_fetchcolumn("SELECT id FROM " . tablename('uni_group') . " WHERE uniacid = :uniacid",
                    array(':uniacid' => $uniacid));
                if (empty($id)) {
    ...
    if (in_array($owner['uid'], $founders)) {
        ...
        if ($owner['founder_groupid'] == ACCOUNT_MANAGE_GROUP_VICE_FOUNDER) {
            $owner['group'] = pdo_get('users_founder_group', array('id' => $owner['groupid']),
                array('id', 'name', 'package'));
        } else {
    ...
    if ($_W['role'] == ACCOUNT_MANAGE_NAME_FOUNDER && !in_array($owner['uid'],
            $founders) || $_W['role'] == ACCOUNT_MANAGE_NAME_VICE_FOUNDER && $owner['uid'] != $_W['uid']) {
        $canmodify = true;
    }
    ...
    if (!empty($extend['templates'])) {
        $extend['templates'] = pdo_getall('site_templates', array('id' => $extend['templates']),
            array('id', 'name', 'title'));
    }
    ...
    if (!empty($account_buy_modules) && is_array($account_buy_modules)) {
        foreach ($account_buy_modules as &$module) {
            ...
            $module['expire_time'] = pdo_getcolumn('site_store_order',
                array('uniacid' => $uniacid, 'type' => STORE_ORDER_FINISH, 'goodsid' => $module['goods_id']),
                'max(endtime)');
        }
    }
    ...
    if (is_array($account_buy_group) && !empty($account_buy_group)) {
        foreach ($account_buy_group as $group) {
            ...
            $account_buy_package[$group]['goods_id'] = pdo_getcolumn('site_store_goods',
                array('module_group' => $group), 'id');
            $account_buy_package[$group]['expire_time'] = pdo_getcolumn('site_store_order', array(
                'uniacid' => $uniacid,
                'type' => STORE_ORDER_FINISH,
                'goodsid' => $account_buy_package[$group]['goods_id']
            ), 'max(endtime)');
            if (TIMESTAMP > $account_buy_package[$group]['expire_time']) {
                ...
                $account_buy_package[$group]['near_expire'] = strtotime('-1 week',
                    $account_buy_package[$group]['expire_time']) < time() ? true : false;
            }
    ...
    web_template('account/manage-modules-tpl' . ACCOUNT_TYPE_TEMPLATE);
}