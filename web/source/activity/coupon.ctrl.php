...
if (!uni_user_permission_check('activity_coupon_display', false) && !uni_user_permission_check('activity_token_display',
        false)) {
...
if ($do == 'display') {
    ...
    $location_store = pdo_getall('activity_stores',
        array('uniacid' => $_W['uniacid'], 'status' => 1, 'location_id !=' => ''),
        array('id', 'location_id', 'business_name', 'branch_name', 'address'));
    $module = uni_modules();
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('coupon') . " AS c " . $join_sql . " WHERE  " . $condition_sql . " ORDER BY c.id DESC LIMIT " . ($pageindex - 1) * $psize . ',' . $psize,
        $condition);
    $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('coupon') . " AS c " . $join_sql . " WHERE  " . $condition_sql,
        $condition);
    foreach ($list as &$row) {
        ...
        if ($row['status'] == 1) {
            ...
            pdo_update('coupon', array('status' => $status),
                array('uniacid' => $_W['uniacid'], 'card_id' => $row['card_id']));
        }
    }
...
if ($do == 'post') {
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($_GPC['promotion_url_name']) && !empty($_GPC['promotion_url'])) {
            $coupon->setPromotionMenu($_GPC['promotion_url_name'], $_GPC['promotion_url_sub_title'],
                $_GPC['promotion_url']);
        }
    ...
    $location_store = pdo_getall('activity_stores',
        array('uniacid' => $_W['uniacid'], 'status' => 1, 'source' => COUPON_TYPE),
        array('id', 'location_id', 'business_name', 'branch_name', 'address'));
    ...
    $qrcode_list = pdo_getslice('qrcode_stat', array('qrcid' => $qrcode_sceneid), 10, $total,
        array('openid', 'createtime'));
    ...
    $status = pdo_update('uni_settings', array('coupon_type' => intval($_GPC['status'])),
