...
$_W['page']['title'] = '店员工作台';
if ($do == 'index') {
    $clerk_p = pdo_fetchall("SELECT * FROM " . tablename('activity_clerk_menu') . " WHERE (uniacid = :uniacid OR system = '1') AND pid = 0",
    ...
    foreach ($clerk_c as $c) {
        if (empty($c['permission'])) {
            pdo_update('activity_clerk_menu', array('permission' => 'clerk_' . $c['id']),
                array('uniacid' => $_W['uniacid'], 'id' => $c['id']));
        }
    ...
    if (is_error($user_permission)) {
        ...
        if (!empty($permission_list)) {
            ...
            pdo_update('users_permission', array('permission' => $post),
                array('uniacid' => $_W['uniacid'], 'uid' => $_W['uid'], 'type' => 'system'));
        }
