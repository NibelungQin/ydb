...
if ($do == 'exchange_enable') {
    $status = pdo_update('uni_settings', array('exchange_enable' => intval($_GPC['status'])),
...
if ($type == 'coupon') {
    ...
    if ($do == 'post') {
        ...
        $coupons = pdo_fetchall("SELECT * FROM " . tablename('coupon') . " WHERE uniacid = :uniacid AND source = " . COUPON_TYPE,
            array(':uniacid' => $_W['uniacid']), 'id');
        $coupon_exists = pdo_getall('activity_exchange', array('type' => COUPON_TYPE, 'uniacid' => $_W['uniacid']),
            array(), 'extra');
    ...
    if ($do == 'display') {
        ...
        $psize = 20;
        if (!empty($title)) {
            $cids = pdo_fetchall("SELECT * FROM " . tablename('coupon') . " WHERE uniacid = :uniacid AND title LIKE :title AND source = :source",
                array('uniacid' => $_W['uniacid'], ':title' => '%' . $title . '%', ':source' => COUPON_TYPE), 'id');
            $cids = implode('\',\'', array_keys($cids));
            $condition = ' AND extra IN(\'' . $cids . '\')';
        }
        $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('activity_exchange') . " WHERE uniacid = :uniacid AND type = :type " . $condition,
            array(':uniacid' => $_W['uniacid'], ':type' => COUPON_TYPE));
        $list = pdo_fetchall("SELECT * FROM " . tablename('activity_exchange') . " WHERE uniacid = :uniacid AND type = :type " . $condition . " ORDER BY id desc LIMIT " . ($pindex - 1) * $psize . ',' . $psize,
            array(':uniacid' => $_W['uniacid'], ':type' => COUPON_TYPE));
        if (!empty($list)) {
            foreach ($list as &$ex) {
...
if ($type == 'goods') {
    ...
    if ($do == 'post') {
        $id = intval($_GPC['id']);
        if (!empty($id)) {
            $item = pdo_fetch('SELECT * FROM ' . tablename('activity_exchange') . ' WHERE id=:id AND uniacid=:uniacid',
    ...
    if ($do == 'display') {
        ...
        $list = pdo_fetchall('SELECT * FROM ' . tablename('activity_exchange') . " $where ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,
            $params);
    ...
    if ($do == 'del') {
        $id = intval($_GPC['id']);
        if (!empty($id)) {
            $item = pdo_fetch('SELECT id FROM ' . tablename('activity_exchange') . ' WHERE id=:id AND uniacid=:uniacid',
                array(':id' => $id, ':uniacid' => $_W['uniacid']));
        }
        if (empty($item)) {
    ...
    if ($do == 'deliver') {
        $exchanges = pdo_fetchall('SELECT id, title FROM ' . tablename('activity_exchange') . ' WHERE uniacid = :uniacid ORDER BY id DESC',
            array(':uniacid' => $_W['uniacid']));
        ...
        $pindex = max(1, intval($_GPC['page']));
        $psize = 20;
        $list = pdo_fetchall("SELECT a.*, b.title,b.extra,b.thumb FROM " . tablename('activity_exchange_trades_shipping') . ' AS a LEFT JOIN ' . tablename('activity_exchange') . ' AS b ON a.exid = b.id ' . " $where ORDER BY tid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,
            $params);
        $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('activity_exchange_trades_shipping') . ' AS a LEFT JOIN ' . tablename('activity_exchange') . ' AS b ON a.exid = b.id ' . $where,
            $params);
    ...
    if ($do == 'receiver') {
        $id = intval($_GPC['id']);
        $shipping = pdo_fetch('SELECT * FROM ' . tablename('activity_exchange_trades_shipping') . ' WHERE uniacid = :uniacid AND tid = :tid',
            array(':uniacid' => $_W['uniacid'], ':tid' => $id));
        if (checksubmit('submit')) {
            $data = array(
                'name' => $_GPC['realname'],
                'mobile' => $_GPC['mobile'],
