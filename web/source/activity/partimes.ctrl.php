...
if ($do == 'display') {
    ...
    $list = pdo_fetchall('SELECT * FROM ' . tablename('activity_exchange') . " $where ORDER BY createtime DESC, id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,
        $params);
...
if ($do == 'del') {
    $id = intval($_GPC['id']);
    if (!empty($id)) {
        $item = pdo_fetch('SELECT id FROM ' . tablename('activity_exchange') . ' WHERE id=:id AND type = 5 AND uniacid=:uniacid',
            array(':id' => $id, ':uniacid' => $_W['uniacid']));
    }
    if (empty($item)) {
