...
if ($do == 'display') {
    ...
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('activity_stores') . " WHERE uniacid = :uniacid AND source = :source",
        array(':uniacid' => $_W['uniacid'], ':source' => COUPON_TYPE));
    $list = pdo_getslice('activity_stores', array('uniacid' => $_W['uniacid'], 'source' => COUPON_TYPE),
        array($pindex, $psize));
    ...
    if (!empty($location)) {
        foreach ($location as $row) {
            ...
            if ($li['available_state'] == 3) {
                $isexist = pdo_getcolumn('activity_stores',
                    array('uniacid' => $_W['uniacid'], 'location_id' => $li['poi_id']), 'id');
                if (empty($isexist)) {
                ...
                $select_type == 'sid';
                if (!empty($li['sid'])) {
                    $isexist = pdo_getcolumn('activity_stores', array('uniacid' => $_W['uniacid'], 'id' => $li['sid']),
                        'id');
                }
                if (empty($isexist)) {
                    $select_type = 'location';
                    $isexist = pdo_get('activity_stores',
