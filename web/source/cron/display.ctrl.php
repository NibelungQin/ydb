...
if ($do == 'list') {
    ...
    $psize = 15;
    $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('core_cron') . ' WHERE uniacid = :uniacid',
        array(':uniacid' => $_W['uniacid']));
    $crons = pdo_fetchall('SELECT * FROM ' . tablename('core_cron') . ' WHERE uniacid = :uniacid ORDER BY id DESC LIMIT ' . ($pindex - 1) * $psize . ", {$psize}",
        array(':uniacid' => $_W['uniacid']));
    ...
    if (!empty($crons)) {
        ...
        if (!empty($modules)) {
            ...
            $modules = pdo_fetchall('SELECT title,name FROM ' . tablename('modules') . " WHERE name IN ({$modules})",
                array(), 'name');
            $modules['task'] = array('name' => 'task', 'title' => '系统任务');
        }
    }
