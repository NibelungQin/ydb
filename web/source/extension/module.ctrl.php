...
if ($do == 'batch-install') {
    ...
    if ($_W['ispost']) {
        ...
        if (pdo_fetchcolumn("SELECT mid FROM " . tablename('modules') . " WHERE name = :name",
        ...
        if (pdo_fetchcolumn("SELECT mid FROM " . tablename('modules') . " WHERE name = :name",
            array(':name' => $manifest['application']['identifie']))) {
            exit('error');
        }
...
if ($do == 'info') {
    $m = trim($_GPC['m']);
    if ($_W['isajax']) {
        $data = pdo_fetch('SELECT name, title, ability, description FROM ' . tablename('modules') . ' WHERE name = :m',
            array(':m' => $m));
...
if ($do == 'installed') {
    ...
    $modules = pdo_fetchall("SELECT * FROM " . tablename('modules') . ' ORDER BY `issystem` DESC, `mid` ASC', array(),
        'mid');
    if (!empty($modules)) {
        foreach ($modules as $mid => $module) {
            $manifest = ext_module_manifest($module['name']);
            $modules[$mid]['official'] = empty($module['issystem']) && (strexists($module['author'],
                        'WeEngine Team') || strexists($module['author'], '一道宝团队'));
    ...
    if (is_dir($path)) {
        ...
        if ($handle = opendir($path)) {
            while (false !== ($modulepath = readdir($handle))) {
                ...
                if (!empty($status) && in_array($manifest['application']['identifie'],
                        $recycle_modules) || empty($status) && !in_array($manifest['application']['identifie'],
                        $recycle_modules)) {
                    if (is_array($manifest) && !empty($manifest['application']['identifie']) && !in_array($manifest['application']['identifie'],
                            $moduleids)) {
...
if ($do == 'permission') {
    ...
    if (!empty($module)) {
        ...
        $bindings = pdo_fetchall('SELECT * FROM ' . tablename('modules_bindings') . ' WHERE `module`=:module',
            array(':module' => $id));
        if (!empty($bindings)) {
    ...
    if (empty($module)) {
        message('你访问的模块不存在. 或许你愿意去一道宝云服务平台看看. ', 'http://v2.addons.yidaoit.cn/web/index.php?keyword=' . $_GPC['title']);
    }
    ...
    if (pdo_insert('modules', $module)) {
        ...
        if ($_GPC['flag'] && !empty($post_groups) && $module['name']) {
            foreach ($post_groups as $post_group) {
                $item = pdo_fetch("SELECT id,name,modules FROM " . tablename('uni_group') . " WHERE id = :id",
                    array(':id' => intval($post_group)));
                if (empty($item)) {
                    continue;
                }
        ...
        if (empty($module_subscribe_success)) {
            message('模块安装成功, 请按照【公众号服务套餐】【用户组】来分配权限！模块订阅消息有错误，系统已禁用该模块的订阅消息，详细信息请查看 <div><a class="btn btn-primary" style="width:80px;" href="' . url('extension/subscribe/subscribe') . '">订阅管理</a> &nbsp;&nbsp;<a class="btn btn-default" href="' . url('extension/module') . '">返回模块列表</a></div>',
                '', 'tips');
        } else {
...
if ($do == 'uninstall') {
    ...
    $module = pdo_fetch("SELECT `name`, `isrulefields`, `issystem`, `version` FROM " . tablename('modules') . " WHERE name = :name",
        array(':name' => $id));
    ...
    if ($module['isrulefields'] && !isset($_GPC['confirm'])) {
        message('卸载模块时同时删除规则数据吗, 删除规则数据将同时删除相关规则的统计分析数据？<div><a class="btn btn-primary" style="width:80px;" href="' . url('extension/module/uninstall',
                array(
                    'id' => $_GPC['id'],
                    'confirm' => 1
                )) . '">是</a> &nbsp;&nbsp;<a class="btn btn-default" style="width:80px;" href="' . url('extension/module/uninstall',
                array('id' => $_GPC['id'], 'confirm' => 0)) . '">否</a></div>', '', 'tips');
    } else {
...
if ($do == 'upgrade') {
    $id = $_GPC['m'];
    $module = pdo_fetch("SELECT mid, name, version FROM " . tablename('modules') . " WHERE name = :name",
        array(':name' => $id));
    if (empty($module)) {
        message('模块已经被卸载或是不存在！', '', 'error');
    }
        ...
        if (!is_error($info)) {
            if (empty($_GPC['flag'])) {
                if (intval($_GPC['branch']) > $info['version']['branch_id']) {
                    header('location: ' . url('cloud/redirect/buybranch',
                            array('m' => $id, 'branch' => intval($_GPC['branch']), 'is_upgrade' => 1)));
                    exit;
                }
        ...
        if (is_array($bindings[$p]) && !empty($bindings[$p])) {
            ...
            if (!empty($delete_do)) {
                pdo_query('DELETE FROM ' . tablename('modules_bindings') . " WHERE module = :module AND entry = :entry AND `call` = '' AND do NOT IN ('" . implode("','",
                        $delete_do) . "')", array(':module' => $manifest['application']['identifie'], ':entry' => $p));
                unset($delete_do);
            }
            if (!empty($delete_title)) {
                pdo_query('DELETE FROM ' . tablename('modules_bindings') . " WHERE module = :module AND entry = :entry AND `call` = '' AND title NOT IN ('" . implode("','",
                        $delete_title) . "')",
                    array(':module' => $manifest['application']['identifie'], ':entry' => $p));
                unset($delete_title);
            }
    ...
    if ($_GPC['flag'] == 1) {
        message('模块更新成功！ <br> 由于数据库更新, 可能会产生多余的字段. 你可以按照需要删除.<div><a class="btn btn-primary" href="' . url('system/database/trim') . '">现在去删除</a>&nbsp;&nbsp;&nbsp;<a class="btn btn-default" href="' . url('extension/module/') . '">返回模块列表</a></div>',
            '', 'success');
    } else {
...
if ($do == 'designer') {
    ...
    if (checksubmit() && $available[$_GPC['method']]) {
        ...
        if (empty($m['application']['identifie']) || !preg_match('/^[a-z][a-z\d_]+$/i',
                $m['application']['identifie'])) {
        ...
        $m['application']['type'] = array_key_exists($_GPC['application']['type'],
            $modtypes) ? $_GPC['application']['type'] : 'other';
        ...
        if ($_GPC['method'] == 'create') {
            ...
            if ($m['application']['setting']) {
                f_write("{$mRoot}/template/setting.html",
        ...
        if ($_GPC['method'] == 'download') {
            ...
            if ($m['application']['setting']) {
                $zip->addFromString("template/setting.html",
                    "{template 'common/header'}\r\n这里定义页面内容\r\n{template 'common/footer'}");
            }
            if ($m['icon']) {
...
<manifest xmlns="http://www.yidaoit.cn" versionCode="{$versions}">
...
class {$name}ModuleProcessor extends WeModuleProcessor {
	public function respond() {
		...
		//这里定义此模块进行消息处理时的具体过程, 请查看一道宝文档来编写你的代码
	}
}
TPL;
    }
...
class {$name}ModuleReceiver extends WeModuleReceiver {
	public function receive() {
		...
		//这里定义此模块进行消息订阅时的, 消息到达以后的具体处理过程, 请查看一道宝文档来编写你的代码
	}
}
TPL;
    }
