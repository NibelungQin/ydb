...
if ($do == 'switch') {
    ...
    $ds = array();
    foreach ($rs as $row) {
        $reply = pdo_fetch('SELECT * FROM ' . tablename('userapi_reply') . ' WHERE `rid`=:rid',
            array(':rid' => $row['id']));
        $r = array();
        $r['title'] = $row['name'];
        $r['rid'] = $row['id'];
...
if ($do == 'post') {
    ...
    if (!empty($rid)) {
        ...
        $reply['description'] = pdo_fetchcolumn('SELECT description FROM ' . tablename('userapi_reply') . ' WHERE rid = :rid',
            array(':rid' => $rid));
        if (empty($reply)) {
