...
if ($do == 'install') {
    ...
    $id = $_GPC['templateid'];
    if (pdo_fetchcolumn("SELECT id FROM " . tablename('site_templates') . " WHERE name = :name",
        array(':name' => $id))) {
    ...
    if ($id && $post_groups) {
        ...
        foreach ($post_groups as $post_group) {
            $item = pdo_fetch("SELECT id,name,templates FROM " . tablename('uni_group') . " WHERE id = :id",
...
if ($do == 'uninstall') {
    $name = pdo_fetchcolumn('SELECT name FROM ' . tablename('site_templates') . ' WHERE id = :id',
...
if ($do == 'upgrade') {
    ...
    $theme = pdo_fetch("SELECT id, name, title FROM " . tablename('site_templates') . " WHERE name = :name",
        array(':name' => $id));
    if (empty($theme)) {
        if ($batch == 1) {
            cache_write('upgrade:template', iserializer($wait_upgrade));
            message($theme['title'] . ' 模板已经被卸载或是不存在。系统将进入下一个模板的更新。<br>请勿关闭浏览器',
                url('extension/theme/upgrade', array('batch' => 1)), 'success');
        }
        ...
        if (empty($_GPC['flag'])) {
            if (intval($_GPC['branch']) > $upgrade_info['version']['branch_id']) {
                header('location: ' . url('cloud/redirect/buybranch',
                        array('m' => $id, 'branch' => intval($_GPC['branch']), 'type' => 'theme', 'is_upgrade' => 1)));
                exit;
            }
    ...
    if (empty($manifest)) {
        if ($batch == 1) {
            cache_write('upgrade:template', iserializer($wait_upgrade));
            message($theme['title'] . ' 模块安装配置文件不存在或是格式不正确。系统将进入下一个模板的更新。<br>请勿关闭浏览器',
                url('extension/theme/upgrade', array('batch' => 1)), 'success');
        }
    ...
    if (ver_compare($theme['version'], $packet['version']) != -1) {
        if ($batch == 1) {
            cache_write('upgrade:template', iserializer($wait_upgrade));
            message($theme['title'] . ' 模板版本不低于要更新的版本。系统将进入下一个模板的更新。<br>请勿关闭浏览器',
                url('extension/theme/upgrade', array('batch' => 1)), 'success');
        }
    ...
    if ($batch == 1) {
        cache_write('upgrade:template', iserializer($wait_upgrade));
        message($theme['title'] . ' 模板更新成功。系统将进入下一个模板的更新。<br>请勿关闭浏览器',
            url('extension/theme/upgrade', array('batch' => 1)), 'success');
    }
...
if ($do == 'designer') {
    ...
    if (checksubmit('submit') && $available[$_GPC['method']]) {
        ...
        $t['template']['type'] = array_key_exists($_GPC['template']['type'],
            $temtypes) ? $_GPC['template']['type'] : 'other';
        $t['template']['description'] = trim($_GPC['template']['description']);
        if (empty($t['template']['description']) || preg_match('/\*\/|\/\*|eval|\$\_/i',
        ...
        if (!empty($_GPC['settings']['variables'])) {
            foreach ($_GPC['settings']['variables'] as $key => $value) {
                $temp = array();
                if (!empty($_GPC['settings']['variables'][$key]) && preg_match('/^[a-z\d]+$/i',
                        $_GPC['settings']['variables'][$key])) {
