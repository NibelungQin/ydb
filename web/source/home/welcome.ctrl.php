...
if ($do === 'get_not_installed_modules') {
    ...
    $not_installed_modules['app'] = is_array($not_installed_modules['app']) ? array_slice($not_installed_modules['app'],
        0, 4) : array();
    $not_installed_modules['wxapp'] = is_array($not_installed_modules['wxapp']) ? array_slice($not_installed_modules['wxapp'],
        0, 4) : array();
    ...
    if (is_array($data['module']) && !empty($data['module'])) {
        foreach ($data['module'] as &$module) {
            if ($module['app_support'] == 2) {
                $module['link'] = web_url('module/manage-system/not_installed',
                    array('account_type' => ACCOUNT_TYPE_OFFCIAL_NORMAL));
            } else {
                $module['link'] = web_url('module/manage-system/not_installed',
                    array('account_type' => ACCOUNT_TYPE_APP_NORMAL));
            }
        }
    }
    iajax(0, $data);
...
if ($do === 'ext' && $_GPC['m'] !== 'store' && !$_GPC['system_welcome']) {
    ...
    if (is_error($account_api)) {
        web_message($account_api['message'], web_url('account/display'));
    }
    $check_manange = $account_api->checkIntoManage();
    if (is_error($check_manange)) {
        $account_display_url = $account_api->accountDisplayUrl();
        web_itoast('', $account_display_url);
    }
}
...
if ($do === 'platform') {
    ...
    if (empty($last_uniacid)) {
        web_itoast('', web_url('account/display'), 'info');
    }
    if (!empty($last_uniacid) && $last_uniacid != $_W['uniacid']) {
        uni_account_switch($last_uniacid, web_url('home/welcome'));
    }
    define('FRAME', 'account');
    if (empty($_W['account']['endtime']) && !empty($_W['account']['endtime']) && $_W['account']['endtime'] < time()) {
        web_itoast('公众号已到服务期限，请联系管理员并续费', web_url('account/manage'), 'info');
    }
    $notices = welcome_notices_get();

    web_template('home/welcome');
} elseif ($do === 'system') {
    define('FRAME', 'system');
    $_W['page']['title'] = '欢迎页 - 系统管理';
    if (!$_W['isfounder'] || user_is_vice_founder()) {
        header('Location: ' . web_url('account/manage', array('account_type' => 1)), true);
        exit;
    }
    ...
    web_template('home/welcome-system');
    ...
    foreach ($frames['section'] as $secion) {
        foreach ($secion['menu'] as $menu) {
            if (!empty($menu['url'])) {
                if (!empty($uni_account_module['settings']['default_entry']) && !strpos($menu['url'],
                        '&eid=' . $uni_account_module['settings']['default_entry'])) {
                    continue;
                }
    ...
    $yesterday_stat['new'] = (int)$yesterday_stat['new'];
    $yesterday_stat['cancel'] = (int)$yesterday_stat['cancel'];
    $yesterday_stat['jing_num'] = $yesterday_stat['new'] - $yesterday_stat['cancel'];
    $yesterday_stat['cumulate'] = (int)$yesterday_stat['cumulate'];
    $today_stat = pdo_get('stat_fans', array('date' => date('Ymd'), 'uniacid' => $_W['uniacid']));
    $today_stat['new'] = (int)$today_stat['new'];
    $today_stat['cancel'] = (int)$today_stat['cancel'];
    $today_stat['jing_num'] = $today_stat['new'] - $today_stat['cancel'];
    $today_stat['cumulate'] = (int)$today_stat['jing_num'] + $yesterday_stat['cumulate'];
