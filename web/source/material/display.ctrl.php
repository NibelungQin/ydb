...
if ($do == 'down') {
    ...
    if ($total == 0 || ($pindex > ceil($total / $psize))) {
        $has_str = implode(',', $has);
        pdo_query('DELETE FROM ' . tablename('wechat_attachment') . "  WHERE uniacid = :uniacid AND type = :type AND id NOT IN ({$has_str})",
            array('uniacid' => $_W['uniacid'], ':type' => $type));
        if ($type == 'news') {
            pdo_query('DELETE FROM ' . tablename('wechat_news') . "  WHERE uniacid = :uniacid AND attach_id NOT IN ({$has_str})",
        ...
        if ($type != 'news') {
            ...
            if ($type == 'image') {
                if (!empty($media) && !empty($media['attachment'])) {
                    if (strexists($media['attachment'],
                            'https://mmbiz.qlogo.cn') || (file_exists(ATTACHMENT_ROOT . $media['attachment']))) {
                        $has[] = $media['id'];
                        continue;
                    }
                }
            ...
            if (empty($media)) {
                ...
                pdo_update('wechat_attachment', $insert,
                    array('uniacid' => $_W['uniacid'], 'media_id' => $data['media_id']));
                $media_id = $media['id'];
            }
            $has[] = $media['id'];
        } else {
            ...
            if (empty($media)) {
                ...
                pdo_update('wechat_attachment', array('createtime' => $data['update_time']),
                    array('uniacid' => $_W['uniacid'], 'media_id' => $data['media_id']));
                $insert_id = $media['id'];
    ...
    if (!empty($lists)) {
        foreach ($lists as &$row) {
            if ($type == 'video') {
                ...
                $row['items'] = pdo_fetchall("SELECT * FROM " . tablename('wechat_news') . " WHERE uniacid = :uniacid AND attach_id = :attach_id ORDER BY id ASC",
                    array(':uniacid' => $_W['uniacid'], ':attach_id' => $row['id']));
            }
        }
    }
    $pager = pagination($total, $pindex, $psize);

    $groups = pdo_fetch('SELECT * FROM ' . tablename('mc_fans_groups') . ' WHERE uniacid = :uniacid AND acid = :acid',
