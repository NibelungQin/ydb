...
if ($do == 'list') {
    ...
    $record = pdo_fetchall('SELECT * FROM ' . tablename('mc_mass_record') . ' WHERE uniacid = :uniacid AND sendtime >= :time ORDER BY sendtime ASC LIMIT 7',
        array(':uniacid' => $_W['uniacid'], ':time' => $time), 'sendtime');
    for ($i = 0; $i < 7; $i++) {
...
if ($do == 'post') {
    ...
    set_time_limit(0);
    $records = pdo_fetchall('SELECT id, cron_id FROM ' . tablename('mc_mass_record') . ' WHERE uniacid = :uniacid AND sendtime >= :time AND status = 1 ORDER BY sendtime ASC LIMIT 8',
        array(':uniacid' => $_W['uniacid'], ':time' => strtotime(date('Y-m-d'))), 'id');
    if (!empty($records)) {
        ...
        pdo_query('DELETE FROM ' . tablename('mc_mass_record') . " WHERE uniacid = :uniacid AND id IN ({$ids})",
