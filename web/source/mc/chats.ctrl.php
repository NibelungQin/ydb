...
if ($do == 'chats') {
    ...
    $chat_record = pdo_getslice('mc_chats_record', array('uniacid' => $_W['uniacid'], 'openid' => $openid),
        array('1', 20), $total, array(), '', 'createtime desc');
...
if ($do == 'send') {
    ...
    if (is_error($result)) {
        ...
        if ($send['msgtype'] == 'mpnews') {
            $material = pdo_getcolumn('wechat_attachment',
                array('uniacid' => $_W['uniacid'], 'media_id' => $content['mediaid']), 'id');
        ...
        iajax(0,
            array('createtime' => date('Y-m-d H:i:s', time()), 'content' => $content, 'msgtype' => $send['msgtype']),
            '');
    }
}
