...
if ($do == 'display') {
    ...
    if (checksubmit('submit')) {
        ...
        $list = pdo_fetch("SELECT creditbehaviors FROM " . tablename('uni_settings') . " WHERE uniacid = :uniacid",
        ...
        $row = pdo_fetch("SELECT uniacid FROM " . tablename('uni_settings') . " WHERE uniacid = :uniacid",
            array(':uniacid' => $_W['uniacid']));
        if (empty($row)) {
    ...
    $list = pdo_fetch("SELECT creditnames FROM " . tablename('uni_settings') . " WHERE uniacid = :uniacid",
        array(':uniacid' => $_W['uniacid']));
...
if ($do == 'strategy') {
    ...
    $row = pdo_fetch("SELECT creditnames,creditbehaviors FROM " . tablename('uni_settings') . " WHERE uniacid = :uniacid",
        array(':uniacid' => $_W['uniacid']));
