...
if ($do == 'display') {
    ...
    $list = pdo_fetchall("SELECT uid, uniacid, email, nickname, realname, mobile, {$select_credit} FROM " . tablename('mc_members') . $where . ' ORDER BY uid DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize,
        $params);
    ...
    if (count($list) == 1 && $list[0]['uid'] && !empty($keyword)) {
        ...
        foreach ($list as &$li) {
            if (empty($li['email']) || (!empty($li['email']) && substr($li['email'],
                        -6) == 'yidaoit.cn' && strlen($li['email']) == 39)) {
                $li['email'] = '未完善';
            }
        }
        $status = 0;
    ...
    if ($uid) {
        foreach ($creditnames as $index => $creditname) {
            if (($_GPC[$index . '_type'] == 1 || $_GPC[$index . '_type'] == 2) && $_GPC[$index . '_value']) {
                ...
                $return = mc_credit_update($uid, $index, $value,
                    array($_W['uid'], trim($_GPC['remark']), 'system', $clerk['id'], $clerk['store_id']));
                if (is_error($return)) {
                    message($return['message']);
                }
                $openid = pdo_fetchcolumn('SELECT openid FROM ' . tablename('mc_mapping_fans') . ' WHERE acid = :acid AND uid = :uid',
...
if ($do == 'modal') {
    if ($_W['isajax']) {
        $uid = intval($_GPC['uid']);
        $data = pdo_fetch("SELECT uid, nickname, realname, email, mobile, uniacid, {$select_credit} FROM " . tablename('mc_members') . ' WHERE uid = :uid AND uniacid = :uniacid',
            array(':uniacid' => $_W['uniacid'], ':uid' => $uid));
        if (empty($data['email']) || (!empty($data['email']) && substr($data['email'],
                    -6) == 'yidaoit.cn' && strlen($data['email']) == 39)) {
            $data['email'] = '未完善';
        }
...
if ($do == 'credit_record') {
    ...
    $pindex = max(1, intval($_GPC['page']));
    $psize = 50;
    $total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('mc_credits_record') . ' WHERE uid = :uid AND uniacid = :uniacid AND credittype = :credittype ',
        array(':uniacid' => $_W['uniacid'], ':uid' => $uid, ':credittype' => $type));
    $data = pdo_fetchall("SELECT r.*, u.username FROM " . tablename('mc_credits_record') . ' AS r LEFT JOIN ' . tablename('users') . ' AS u ON r.operator = u.uid ' . ' WHERE r.uid = :uid AND r.uniacid = :uniacid AND r.credittype = :credittype ORDER BY id DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize,
        array(':uniacid' => $_W['uniacid'], ':uid' => $uid, ':credittype' => $type));
    $pager = pagination($total, $pindex, $psize);
...
if ($do == 'stat') {
    ...
    if (!empty($credits)) {
        ...
        foreach ($credits as $li) {
            $data[$li]['add'] = round(pdo_fetchcolumn('SELECT SUM(num) FROM ' . tablename('mc_credits_record') . ' WHERE uniacid = :id AND uid = :uid AND createtime > :start AND createtime < :end AND credittype = :type AND num > 0',
                array(
                    ':id' => $_W['uniacid'],
                    ':uid' => $uid,
                    ':start' => $starttime,
            ...
            $data[$li]['del'] = abs(round(pdo_fetchcolumn('SELECT SUM(num) FROM ' . tablename('mc_credits_record') . ' WHERE uniacid = :id AND uid = :uid AND createtime > :start AND createtime < :end AND credittype = :type AND num < 0',
                array(
                    ':id' => $_W['uniacid'],
                    ':uid' => $uid,
                    ':start' => $starttime,
