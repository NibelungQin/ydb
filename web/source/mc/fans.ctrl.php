...
if ($do == 'display') {
    ...
    $fans_list_sql = sprintf($select_sql, "f.* ",
        " GROUP BY f.`fanid`" . $orderby . " LIMIT " . ($pageindex - 1) * $pagesize . "," . $pagesize);
    ...
    $fans['total'] = pdo_getcolumn("mc_mapping_fans",
        array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'follow' => 1), 'count(*)');
}
...
if ($do == 'batch_edit_fans_tag') {
    ...
    foreach ($tags as $tag) {
        ...
        foreach ($openid_list as $openid) {
            ...
            pdo_update('mc_mapping_fans', array('groupid' => $groupid),
...
if ($do == 'download_fans') {
    ...
    $same_account_exist = pdo_getall('account_wechats',
        array('key' => $_W['account']['key'], 'uniacid <>' => $_W['uniacid']), array(), 'uniacid');
    if (!empty($same_account_exist)) {
        pdo_update('mc_mapping_fans', array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid']),
            array('uniacid' => array_keys($same_account_exist)));
    }
    ...
    if (!is_error($wechat_fans_list)) {
        ...
        for ($i = 0; $i < $total_page; $i++) {
            ...
            pdo_update('mc_mapping_fans', array('follow' => 1, 'uniacid' => $_W['uniacid'], 'acid' => $_W['acid']),
                array('openid' => $wechat_fans));
        }
...
if ($do == 'sync') {
    ...
    if ($type == 'all') {
        ...
        $pageindex++;
        $sync_fans = pdo_getslice('mc_mapping_fans',
            array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid'], 'follow' => '1'), array($pageindex, 100), $total,
            array(), 'openid', 'fanid DESC');
