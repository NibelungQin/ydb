...
if ($do == 'default') {
    if ($_W['account']['level'] > 2) {
        $groups_data = pdo_fetch('SELECT * FROM ' . tablename('mc_fans_groups') . ' WHERE uniacid = :uniacid AND acid = :acid',
            array(':uniacid' => $_W['uniacid'], ':acid' => $_W['acid']));
...
if ($do == 'news') {
    ...
    if (!empty($data)) {
        foreach ($data as &$da) {
            $da['replies'] = pdo_fetchall('SELECT id,title,thumb FROM ' . tablename('news_reply') . ' WHERE rid = :rid ORDER BY `displayorder` DESC',
                array(':rid' => $da['id']));
            if (!empty($da['replies'])) {
...
if ($do == 'fans') {
    ...
    $data = pdo_fetchall('SELECT fanid,openid,nickname,followtime,tag FROM ' . tablename('mc_mapping_fans') . $condition . $limit,
        $param, 'fanid');
    if (!empty($data)) {
        foreach ($data as &$da) {
            $da['selected'] = 0;
...
if ($do == 'post') {
    ...
    if ($post['msg_type'] == 'mpnews') {
        ...
        $rule = pdo_fetch('SELECT * FROM ' . tablename('rule') . ' WHERE uniacid = :uniacid AND id = :id',
            array(':uniacid' => $_W['uniacid'], ':id' => $rid));
        if (empty($rule)) {
            message(error(-1, '规则不存在'), '', 'ajax');
        }
        $replies = pdo_fetchall('SELECT * FROM ' . tablename('news_reply') . ' WHERE rid = :rid ORDER BY `displayorder` DESC',
            array(':rid' => $rid));
        $thumb_message = '';
...
if ($do == 'send') {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('mc_mass_record') . $condition . " ORDER BY `id` DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,
        $pars);
    ...
    if (!empty($list)) {
        foreach ($list as &$li) {
            if ($li['msgtype'] == 'news') {
                ...
                if ($rid > 0) {
                    $li['rid'] = $rid;
                    $li['rule_name'] = pdo_fetchcolumn('SELECT name FROM ' . tablename('rule') . ' WHERE id = :id',
                        array(':id' => $rid));
                } else {
