...
if ($do == 'user') {
    ...
    if (empty($data)) {
        ...
        exit(json_encode(array(
            ...
            'html' => $html,
            'card' => $card,
            'group' => $_W['account']['groups'],
            'grouplevel' => $_W['account']['grouplevel']
        )));
    }
}
...
if ($do == 'consume') {
    ...
    $log = $note . $log;
    if ($credit2 > 0) {
        $status = mc_credit_update($uid, 'credit2', -$credit2,
            array(0, $log, 'system', $_W['user']['clerk_id'], $_W['user']['store_id'], $_W['user']['clerk_type']));
        if (is_error($status)) {
            exit($status['message']);
        }
    }
    if ($credit1 > 0) {
        $status = mc_credit_update($uid, 'credit1', -$credit1,
            array(0, $log, 'system', $_W['user']['clerk_id'], $_W['user']['store_id'], $_W['user']['clerk_type']));
        if (is_error($status)) {
            exit($status['message']);
        }
    ...
    if (!empty($grant_credit1_enable)) {
        ...
        mc_credit_update($uid, 'credit1', $num,
            array(0, $tips, 'system', $_W['user']['clerk_id'], $_W['user']['store_id'], $_W['user']['clerk_type']));
    }
...
if ($do == 'credit') {
    ...
    $status = mc_credit_update($uid, $type, $num, array(
        $_W['user']['uid'],
        trim($_GPC['remark']),
        'system',
        $_W['user']['clerk_id'],
...
if ($do == 'card') {
    ...
    if (pdo_insert('mc_card_members', $record)) {
        pdo_update('mc_card', array('snpos' => $card['snpos']),
            array('uniacid' => $_W['uniacid'], 'id' => $card['id']));
        $notice = '';
...
if ($do == 'group') {
    ...
    if ($credit6 != $user['credit6']) {
        mc_credit_update($uid, 'credit6', (-$user['credit6'] + $credit6), array(
            0,
            "通过修改贡献值,来变更会员用户组",
            'group',
            $_W['user']['clerk_id'],
    ...
    $_W['member'] = $user;
    $_W['openid'] = pdo_fetchcolumn('SELECT openid FROM ' . tablename('mc_mapping_fans') . ' WHERE acid = :acid AND uid = :uid',
        array(':acid' => $_W['acid'], ':uid' => $user['uid']));
    mc_group_update();
    exit('success');
}
template('mc/trade');