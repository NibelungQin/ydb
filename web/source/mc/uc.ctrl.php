...
$uc = pdo_fetch("SELECT `uc`,`passport` FROM " . tablename('uni_settings') . " WHERE uniacid = :weid",
    array(':weid' => $_W['weid']));
...
if (checksubmit('submit')) {
    ...
    if ($uc['status'] == '1') {
        ...
        $uc['charset'] = empty($_GPC['charset']) ? message('请填写UCenter的字符集！', referer(),
            'error') : trim($_GPC['charset']);
        if ($connect == 'mysql') {
            $uc['dbhost'] = empty($_GPC['dbhost']) ? message('请填写UCenter数据库主机地址！', referer(),
                'error') : trim($_GPC['dbhost']);
            $uc['dbuser'] = empty($_GPC['dbuser']) ? message('请填写UCenter数据库用户名！', referer(),
                'error') : trim($_GPC['dbuser']);
            $uc['dbpw'] = empty($_GPC['dbpw']) ? message('请填写UCenter数据库密码！', referer(), 'error') : trim($_GPC['dbpw']);
            $uc['dbname'] = empty($_GPC['dbname']) ? message('请填写UCenter数据库名称！', referer(),
                'error') : trim($_GPC['dbname']);
            $uc['dbcharset'] = empty($_GPC['dbcharset']) ? message('请填写UCenter数据库字符集！', referer(),
                'error') : trim($_GPC['dbcharset']);
            $uc['dbtablepre'] = empty($_GPC['dbtablepre']) ? message('请填写UCenter数据表前缀！', referer(),
                'error') : trim($_GPC['dbtablepre']);
            ...
            $uc['dbconnect'] = intval($_GPC['dbconnect']);
            $uc['api'] = empty($_GPC['api']) ? message('请填写UCenter 服务端的URL地址！', referer(),
                'error') : trim($_GPC['api']);
            $uc['ip'] = empty($_GPC['ip']) ? message('请填写UCenter的IP！', referer(), 'error') : trim($_GPC['ip']);
        }
    }
    $rec['uc'] = iserializer($uc);
    $row = pdo_fetch("SELECT uniacid FROM " . tablename('uni_settings') . " WHERE uniacid = :wid LIMIT 1",
        array(':wid' => intval($_W['weid'])));
    if (!empty($row)) {
