...
if ($do == 'switch') {
    ...
    if (empty($uniacid) && !empty($version_id)) {
        ...
        itoast('', url('wxapp/display/switch', array('module' => $module_name, 'version_id' => $version_id)),
            'success');
    }
    if (!empty($uniacid)) {
        if (empty($version_id)) {
            itoast('', url('account/display/switch', array('uniacid' => $uniacid, 'module_name' => $module_name)),
                'success');
        }
        if ($version_info['uniacid'] != $uniacid) {
            itoast('', url('account/display/switch',
                array('uniacid' => $uniacid, 'module_name' => $module_name, 'version_id' => $version_id)), 'success');
        } else {
            wxapp_save_switch($version_info['uniacid']);
            wxapp_update_last_use_version($version_info['uniacid'], $version_id);
            itoast('', url('wxapp/display/switch', array('module' => $module_name, 'version_id' => $version_id)),
