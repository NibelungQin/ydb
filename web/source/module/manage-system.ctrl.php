    ...
    foreach ($module_list as &$module) {
        ...
        if (empty($manifest)) {
            if (in_array($module['name'], array_keys($cloud_m_query_module))) {
                ...
                if (version_compare($module['version'],
                        $cloud_branch_version) == -1 || ($cloud_m_info['displayorder'] < $best_branch['displayorder'] && !empty($cloud_m_info['version'])) || (!empty($module_info['site_branch_id']) && $cloud_m_info['site_branch']['id'] > $module_info['site_branch_id'])) {
                    $module['upgrade'] = true;
                } else {
                    $module['upgrade'] = false;
                }
...
if ($do == 'upgrade') {
    ...
    if (empty($manifest['platform']['main_module']) && !file_exists($module_path . 'processor.php') && !file_exists($module_path . 'module.php') && !file_exists($module_path . 'receiver.php') && !file_exists($module_path . 'site.php')) {
        web_itoast('模块缺失文件，请检查模块文件中site.php, processor.php, module.php, receiver.php 文件是否存在！', '', 'error');
    }
    ...
    if (!empty($manifest['platform']['plugin_list'])) {
        ...
        foreach ($manifest['platform']['plugin_list'] as $plugin) {
            pdo_insert('modules_plugin',
                array('main_module' => $manifest['application']['identifie'], 'name' => $plugin));
        }
    }
        ...
        if (is_array($bindings[$point_name]) && !empty($bindings[$point_name])) {
            foreach ($bindings[$point_name] as $entry) {
                ...
                if ($entry['title'] && $entry['do']) {
                    ...
                    $module_binding = pdo_get('modules_bindings', array(
                        'module' => $manifest['application']['identifie'],
                        'entry' => $point_name,
                        'title' => $entry['title'],
                        'do' => $entry['do']
                    ));
            ...
            if (!empty($not_delete_do)) {
                pdo_query("DELETE FROM " . tablename('modules_bindings') . " WHERE module = :module AND entry = :entry AND `call` = '' AND do NOT IN ('" . implode("','",
                        $not_delete_do) . "')",
                    array(':module' => $manifest['application']['identifie'], ':entry' => $point_name));
                unset($not_delete_do);
            }
            if (!empty($not_delete_title)) {
                pdo_query("DELETE FROM " . tablename('modules_bindings') . " WHERE module = :module AND entry = :entry AND `call` = '' AND title NOT IN ('" . implode("','",
                        $not_delete_title) . "')",
                    array(':module' => $manifest['application']['identifie'], ':entry' => $point_name));
                unset($not_delete_title);
            }
            if (!empty($not_delete_call)) {
                pdo_query("DELETE FROM " . tablename('modules_bindings') . " WHERE module = :module AND  entry = :entry AND do = '' AND title = '' AND `call` NOT IN ('" . implode("','",
                        $not_delete_call) . "')",
                    array(':module' => $manifest['application']['identifie'], ':entry' => $point_name));
    ...
    web_itoast('模块更新成功！', url('module/manage-system', array('account_type' => ACCOUNT_TYPE)), 'success');
}
...
if ($do == 'install') {
    ...
    if (!empty($is_recycle_module)) {
        ...
        cache_build_uninstalled_module();
        cache_build_module_info($module_name);
        web_itoast('已恢复', url('module/manage-system/installed', array('account_type' => ACCOUNT_TYPE)), '', 'success');
    }

    if (empty($_W['isfounder'])) {
        web_itoast('您没有安装模块的权限', '', 'error');
    }
    $module_info = module_fetch($module_name);
    if (!empty($module_info)) {
        web_itoast('模块已经安装或是唯一标识已存在！', '', 'error');
    }
    $manifest = ext_module_manifest($module_name);
    if (!empty($manifest)) {
        $result = cloud_m_prepare($module_name);
        if (is_error($result)) {
            web_itoast($result['message'], url('module/manage-system/not_installed', array('account_type' => ACCOUNT_TYPE)),
                'error');
        }
    } else {
        $result = cloud_prepare();
        if (is_error($result)) {
            web_itoast($result['message'], url('cloud/profile'), 'error');
        }
        ...
        if (!is_error($module_info)) {
            ...
            web_itoast($module_info['message'], '', 'error');
        }
    }
    if (empty($manifest)) {
        web_itoast('模块安装配置文件不存在或是格式不正确，请刷新重试！',
            web_url('module/manage-system/not_installed', array('account_type' => ACCOUNT_TYPE)), 'error');
    }
    if (!empty($manifest['platform']['main_module'])) {
        $plugin_exist = pdo_get('modules_plugin', array(
            'main_module' => $manifest['platform']['main_module'],
            'name' => $manifest['application']['identifie']
        ));
        if (empty($plugin_exist)) {
            web_itoast('请先更新主模块后再安装插件', web_url('module/manage-system/installed'), 'error', array(
                array(
                    'title' => '查看主程序',
                    'url' => web_url('module/manage-system/module_detail',
    ...
    if (!$_W['ispost'] || empty($_GPC['flag'])) {
        web_template('system/module-group');
        exit;
    }
    if (!empty($manifest['platform']['plugin_list'])) {
        foreach ($manifest['platform']['plugin_list'] as $plugin) {
    ...
    if (pdo_insert('modules', $module)) {
        ...
        header('Location: ' . web_url('module/manage-system/install_success', array('account_type' => ACCOUNT_TYPE)));
        exit;
    } else {
        web_itoast('模块安装失败, 请联系模块开发者！');
    }
}
...
if ($do == 'uninstall') {
    ...
    if (!empty($module['plugin_list']) && empty($_GPC['confirm'])) {
        ...
        if (!empty($plugin_list) && is_array($plugin_list)) {
            ...
            unset($plugin);
            $message .= '<a class="btn btn-primary" href="' . web_url('module/manage-system/uninstall',
                    array('confirm' => 1, 'name' => $name)) . '">停用模块及插件</a></ul>';
        }
        if (!empty($message)) {
            web_itoast($message, '', 'tips');
        }
    }
    ...
    web_itoast('模块已放入回收站！', web_url('module/manage-system', array('account_type' => ACCOUNT_TYPE)), 'success');
}
...
if ($do == 'recycle_uninstall') {
    ...
    if (!isset($_GPC['confirm'])) {
        ...
        if ($module['isrulefields']) {
            $message .= '是否删除相关规则和统计分析数据<div><a class="btn btn-primary" style="width:80px;" href="' . web_url('module/manage-system/recycle_uninstall',
                    array(
                        'module_name' => $name,
                        'confirm' => 1
                    )) . '">是</a> &nbsp;&nbsp;<a class="btn btn-default" style="width:80px;" href="' . web_url('module/manage-system/recycle_uninstall',
                    array('account_type' => ACCOUNT_TYPE, 'module_name' => $name, 'confirm' => 0)) . '">否</a></div>';
        }
        if (!empty($message)) {
            web_itoast($message, '', 'tips');
        }
    ...
    $uninstall_result = module_execute_uninstall_script($name, $_GPC['confirm'], 'value');
    if (is_error($uninstall_result)) {
        web_itoast($uninstall_result['message'], web_url('module/manage-system'), 'error');
    }
    web_itoast('模块已卸载！', web_url('module/manage-system', array('account_type' => ACCOUNT_TYPE)), 'success');
}
...
if ($do == 'filter') {
    ...
    $pager = pagination($total, $pageindex, $pagesize, '',
        array('before' => 5, 'after' => 4, 'ajaxcallback' => true, 'callbackfuncname' => 'filter'));
    iajax(0, array('modules' => $modules, 'pager' => $pager));
}
web_template('module/manage-system' . ACCOUNT_TYPE_TEMPLATE);