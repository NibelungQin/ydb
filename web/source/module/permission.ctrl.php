    ...
    if (!empty($uid)) {
        foreach ($module_and_plugins as $key => $plugin) {
            ...
            foreach ($all_permission[$plugin]['permission'] as $key => $value) {
                ...
                if (in_array($value['permission'], $have_permission[$plugin]) || in_array('all',
                        $have_permission[$plugin])) {
        ...
        if (!empty($permission) && is_array($permission)) {
            foreach ($module_and_plugins as $name) {
                ...
                if (empty($have_permission[$name])) {
                    ...
                    pdo_update('users_permission', array('permission' => $module_permission),
                        array('uniacid' => $_W['uniacid'], 'uid' => $uid, 'type' => $name));
                }
            }
        } else {
            $permission = 'all';
    ...
    if (empty($operator_id)) {
        ...
        if (!empty($user)) {
            $delete_account_users = pdo_delete('uni_account_users',
                array('uid' => $operator_id, 'role' => 'clerk', 'uniacid' => $_W['uniacid']));
