...
if ($do == 'list') {
    ...
    $record = pdo_getall('mc_mass_record', array('uniacid' => $_W['uniacid'], 'sendtime >=' => $time), array(),
        'sendtime', 'sendtime ASC', array(1, 7));
    ...
    for ($i = 0; $i < 8; $i++) {
        ...
        $massdata = pdo_fetch('SELECT id, `groupname`, `msgtype`, `group`, `attach_id`, `media_id`, `sendtime` FROM ' . tablename('mc_mass_record') . ' WHERE uniacid = :uniacid AND sendtime BETWEEN :starttime AND :endtime AND status = 1',
        ...
        if (!empty($massdata)) {
            ...
            switch ($massdata['msgtype']) {
                case 'news':
                    ...
                    $massdata['media']['items'] = pdo_getall('wechat_news',
                        array('attach_id' => $massdata['attach_id']));
...
if ($do == 'post') {
    ...
    if (checksubmit('submit')) {
        ...
        if ($type == 1) {
            ...
            $records = pdo_fetchall("SELECT id, cron_id FROM " . tablename('mc_mass_record') . ' WHERE uniacid = :uniacid AND sendtime BETWEEN :starttime AND :endtime AND status = 1 ORDER BY sendtime ASC LIMIT 8',
                array(':uniacid' => $_W['uniacid'], ':starttime' => $starttime, ':endtime' => $endtime), 'id');
            if (!empty($records)) {
    ...
    web_template('platform/mass-post');
}
...
if ($do == 'send') {
    ...
    $lists = pdo_getall('mc_mass_record', array('uniacid' => $_W['uniacid'], 'acid' => $_W['acid']), array(), '',
    ...
    web_template('platform/mass-send');
}