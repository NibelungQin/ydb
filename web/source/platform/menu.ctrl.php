...
if ($do == 'display') {
    ...
    if ($type == MENU_CONDITIONAL) {
        ...
        if (is_error($update_conditional_menu)) {
            web_itoast($update_conditional_menu['message'], '', 'error');
        }
    }
    ...
    $data = pdo_fetchall("SELECT * FROM " . tablename('uni_account_menus') . $condition . " ORDER BY type ASC, status DESC,id DESC LIMIT " . ($pindex - 1) * $psize . "," . $psize,
        $params);
...
if ($do == 'copy') {
    ...
    $menu = menu_get($id);
    if (empty($menu)) {
        web_itoast('菜单不存在或已经删除', web_url('platform/menu/display'), 'error');
    }
    if ($menu['type'] != MENU_CONDITIONAL) {
        web_itoast('该菜单不能复制', web_url('platform/menu/display'), 'error');
    }
    ...
    $id = pdo_insertid();
    web_itoast('', web_url('platform/menu/post', array('id' => $id, 'copy' => 1, 'type' => MENU_CONDITIONAL)));
}
    ...
    if (empty($type)) {
        if (!$_W['isajax']) {
            ...
            if (is_error($update_self_menu)) {
                web_itoast($update_self_menu['message'], '', 'info');
            }
        }
        ...
        if (!empty($menu['data'])) {
            ...
            if (!empty($menu['data']['button'])) {
                foreach ($menu['data']['button'] as &$button) {
                    if (!empty($button['url'])) {
                        $button['url'] = preg_replace('/(.*)redirect_uri=(.*)&response_type(.*)wechat_redirect/', '$2',
                            $button['url']);
                    ...
                    if (empty($button['sub_button'])) {
                        ...
                        foreach ($button['sub_button'] as &$subbutton) {
                            if (!empty($subbutton['url'])) {
                                $subbutton['url'] = preg_replace('/(.*)redirect_uri=(.*)&response_type(.*)wechat_redirect/',
                                    '$2', $subbutton['url']);
    ...
    if ($_W['isajax'] && $_W['ispost']) {
        ...
        if (is_error($result)) {
            ...
            if ($post['type'] == MENU_CURRENTSELF) {
                if (!empty($id)) {
                    pdo_update('uni_account_menus', $insert,
                        array('uniacid' => $_W['uniacid'], 'type' => MENU_CURRENTSELF, 'id' => $id));
                } else {
                ...
                if ($post['status'] == STATUS_OFF && $post['id'] > 0) {
                    pdo_update('uni_account_menus', $insert,
                        array('uniacid' => $_W['uniacid'], 'type' => MENU_CONDITIONAL, 'id' => $post['id']));
                } else {
                    pdo_insert('uni_account_menus', $insert);
                }
                iajax(0, '创建菜单成功', web_url('platform/menu/display', array('type' => MENU_CONDITIONAL)));
            }
        }
    }
    web_template('platform/menu');
...
if ($do == 'delete') {
    ...
    if (is_error($result)) {
        web_itoast($result['message'], referer(), 'error');
    }
    web_itoast('删除菜单成功', referer(), 'success');
}
    ...
    if ($current_menu['type'] == 'click') {
        if (!empty($current_menu['media_id']) && empty($current_menu['key'])) {
            ...
            if ($wechat_attachment['type'] == 'news') {
                $material = pdo_get('wechat_news',
                    array('uniacid' => $_W['uniacid'], 'attach_id' => $wechat_attachment['id']));
            ...
            if ($keyword_info[0] == 'keyword') {
                ...
                $material['child_items'][0] = pdo_get('rule_keyword', array('rid' => $rule_info['id']),
                    array('content'));
