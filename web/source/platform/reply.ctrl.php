...
if ($do == 'display') {
    if ($m == 'keyword' || !in_array($m, $sysmods)) {
        ...
        if (!empty($_GPC['keyword'])) {
            if ($_GPC['search_type'] == 'keyword') {
                $rule_keyword_rid_list = pdo_getall('rule_keyword', array('content LIKE' => "%{$_GPC['keyword']}%"),
                    array('rid'), 'rid', array('id DESC'));
        ...
        if (!empty($_GPC['keyword']) && $_GPC['search_type'] == 'keyword' && empty($rule_keyword_rid_list)) {
            ...
            if (!empty($replies)) {
                foreach ($replies as &$item) {
                    ...
                    if (!in_array($item['module'],
                        array("basic", "news", "images", "voice", "video", "music", "wxcard", "reply"))) {
    ...
    if ($m == 'default' || $m == 'welcome') {
        ...
        if (!empty($setting[$m])) {
            $rule_keyword_id = pdo_getcolumn('rule_keyword',
                array('uniacid' => $_W['uniacid'], 'content' => $setting[$m]), 'rid');
...
if ($do == 'post') {
    if ($m == 'keyword' || $m == 'userapi' || !in_array($m, $sysmods)) {
        ...
        if ($_W['isajax'] && $_W['ispost']) {
            ...
            $result = pdo_getall('rule_keyword', array('uniacid' => $_W['uniacid'], 'content IN' => $keyword_arr),
                array('rid'));
            if (!empty($result)) {
                $keywords = array();
        ...
        web_template('platform/reply-post');
    }
    if ($m == 'special') {
        $type = trim($_GPC['type']);
        $setting = uni_setting_load('default_message', $_W['uniacid']);
        ...
        if ($setting[$type]['type'] == 'module') {
            ...
            $rule_id = pdo_getcolumn('rule_keyword',
                array('uniacid' => $_W['uniacid'], 'content' => $setting[$type]['keyword']), 'rid');
    ...
    if ($m == 'default' || $m == 'welcome') {
        if (checksubmit('submit')) {
            ...
            $item = pdo_fetch("SELECT uniacid FROM " . tablename('uni_settings') . " WHERE uniacid=:uniacid",
    ...
    if ($m == 'apply') {
        ...
        foreach ($installedmodulelist as $key => &$value) {
            ...
            $value['official'] = empty($value['issystem']) && (strexists($value['author'],
                        'WeEngine Team') || strexists($value['author'], ''));
        }
        unset($value);
        ...
        $current_user_permissions = pdo_getall('users_permission',
            array('uid' => $_W['user']['uid'], 'uniacid' => $_W['uniacid']), array(), 'type');
...
if ($do == 'change_status') {
    ...
    if ($m == 'service') {
        ...
        $userapi_config = pdo_getcolumn('uni_account_modules',
            array('uniacid' => $_W['uniacid'], 'module' => 'userapi'), 'settings');
...
if ($do == 'change_keyword_status') {
    ...
    if (!empty($result)) {
        ...
        if ($result['status'] == 1) {
            ...
            $rule_keyword = pdo_update('rule_keyword', array('status' => 0),
                array('uniacid' => $_W['uniacid'], 'rid' => $id));
        } else {
            $rule = pdo_update('rule', array('status' => 1), array('id' => $id));
            $rule_keyword = pdo_update('rule_keyword', array('status' => 1),
                array('uniacid' => $_W['uniacid'], 'rid' => $id));
        }
