...
if ($_W['isajax']) {
    if ($do == 'search_key') {
        ...
        $data = pdo_fetchall('SELECT content FROM ' . tablename('rule_keyword') . " WHERE (uniacid = 0 OR uniacid = :uniacid) AND status != 0 " . $condition . ' ORDER BY uniacid DESC,displayorder DESC LIMIT 100',
...
if ($do == 'message') {
    ...
    if (checksubmit()) {
        ...
        foreach ($setting as $modulename => &$message_handler) {
            ...
            if (!empty($message_handler['module']) && $message_handler['type'] == 'module' && !in_array($message_handler['module'],
                    $ms)) {
    ...
    foreach ($mtypes as $k => $v) {
        ...
        $row['handles'] = array();
        foreach ($_W['account']['modules'] as $m) {
            if (is_array($_W['account']['modules'][$m['name']]['handles']) && in_array($k,
                    $_W['account']['modules'][$m['name']]['handles'])) {
                $row['handles'][] = array(
                    'name' => $m['name'],
                    'title' => $_W['account']['modules'][$m['name']]['title']
                );
            }
        }
        $ds[] = $row;
