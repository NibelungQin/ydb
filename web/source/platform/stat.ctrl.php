...
if ($do == 'history') {
    ...
    if (!empty($list)) {
        foreach ($list as $index => &$history) {
            if (!empty($history['from_user'])) {
                $tag = pdo_fetchcolumn('SELECT tag FROM ' . tablename('mc_mapping_fans') . ' WHERE uniacid = :id AND openid = :oid',
                    array(':id' => $_W['uniacid'], ':oid' => $history['from_user']));
                if (is_base64($tag)) {
            ...
            if ($history['type'] == 'text') {
                ...
                $history['message'] = '<a href="' . url('platform/stat/browser',
                        array('attach' => $history['message'])) . '" class="btn btn-success btn-sm" target="_blank">查看图片</a>';
    ...
    if (!empty($rids)) {
        $rules = pdo_fetchall("SELECT name, id, module FROM " . tablename('rule') . " WHERE id IN (" . implode(',',
                $rids) . ")", array(), 'id');
        foreach ($rules as $key => &$li) {
            if ($li['module'] == 'cover') {
                $cover_reply = pdo_fetch('SELECT module,do FROM ' . tablename('cover_reply') . ' WHERE rid = :rid',
                    array(':rid' => $key));
                if (!in_array($cover_reply['module'], array('mc', 'site', 'card'))) {
                    $eid = pdo_fetchcolumn('SELECT eid FROM ' . tablename('modules_bindings') . ' WHERE module = :m AND do = :do AND entry = :entry',
                        array(':m' => $cover_reply['module'], ':do' => $cover_reply['do'], ':entry' => 'cover'));
    ...
    if (!empty($_GPC['export'])) {
        ...
        if (!empty($list)) {
            foreach ($list as $row) {
                $nickname = pdo_fetchcolumn("SELECT nickname FROM " . tablename('mc_mapping_fans') . " WHERE openid = :openid",
                    array(':openid' => $row['from_user']));
                $html .= $nickname . "\t ,";
                if ($row['type'] == 'text') {
                ...
                if (empty($row['rid']) && $row['module']) {
                    $module = pdo_fetchcolumn("SELECT title from " . tablename('modules') . " WHERE  name = :name",
                        array(':name' => $row['module']));
                    $html .= $module . "\t , ";
...
if ($do == 'keyword') {
    ...
    if ($foo == 'hit') {
        ...
        if (!empty($kids)) {
            $keywords = pdo_fetchall("SELECT content, id FROM " . tablename('rule_keyword') . " WHERE id IN (" . implode(',',
                    $kids) . ")", array(), 'id');
        }
...
if ($do == 'trend') {
    ...
    $endtime = empty($_GPC['time']['end']) ? TIMESTAMP : strtotime($_GPC['time']['end']) + 86399;
    $list = pdo_fetchall("SELECT createtime, hit  FROM " . tablename('stat_rule') . " WHERE uniacid = '{$_W['uniacid']}' AND rid = :rid AND createtime >= :createtime AND createtime <= :endtime ORDER BY createtime ASC",
        array(':rid' => $id, ':createtime' => $starttime, ':endtime' => $endtime));
    $day = $hit = array();
    if (!empty($list)) {
        foreach ($list as $row) {
    ...
    $list = pdo_fetchall("SELECT createtime, hit, rid, kid FROM " . tablename('stat_keyword') . " WHERE uniacid = '{$_W['uniacid']}' AND rid = :rid AND createtime >= :createtime AND createtime <= :endtime ORDER BY createtime ASC",
        array(':rid' => $id, ':createtime' => $starttime, ':endtime' => $endtime));
    if (!empty($list)) {
        foreach ($list as $row) {
        ...
        $keywordnames = pdo_fetchall("SELECT content, id FROM " . tablename('rule_keyword') . " WHERE id IN (" . implode(',',
                array_keys($keywords)) . ")", array(), 'id');
    }
