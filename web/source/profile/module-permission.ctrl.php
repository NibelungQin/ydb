...
if ($do == 'display') {
    ...
    $user_permissions = pdo_getall('users_permission',
        array('uniacid' => $_W['uniacid'], 'type' => $module_name, 'uid <>' => ''), '', 'uid');
...
if ($do == 'post') {
    ...
    if (checksubmit()) {
        ...
        if (empty($have_permission)) {
            pdo_insert('users_permission',
                array('uniacid' => $_W['uniacid'], 'uid' => $uid, 'type' => $module_name, 'permission' => $permission));
        } else {
            pdo_update('users_permission', array('permission' => $permission),
                array('uniacid' => $_W['uniacid'], 'uid' => $uid, 'type' => $module_name));
        }
        ...
        if (empty($role)) {
            ...
            pdo_update('uni_account_users', array('role' => 'operator'),
...
if ($do == 'delete') {
    ...
    if (empty($operator_id)) {
        ...
        if (!empty($user)) {
            $delete_account_users = pdo_delete('uni_account_users',
                array('uid' => $operator_id, 'role' => 'operator', 'uniacid' => $_W['uniacid']));
            $delete_user_permission = pdo_delete('users_permission',
                array('uid' => $operator_id, 'type' => $module_name, 'uniacid' => $_W['uniacid']));
