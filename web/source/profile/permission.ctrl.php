...
if (!empty($entries)) {
    foreach ($entries as $index1 => &$entry1) {
        foreach ($entry1 as $index2 => &$entry2) {
            ...
            $data = pdo_fetch('SELECT * FROM ' . tablename('modules_bindings') . ' WHERE eid = :eid',
                array(':eid' => $eid));
            $entry2['eid'] = $eid;
            if ($entry2['from'] == 'call') {
                $entry2['eid'] = 0;
...
$userdata = pdo_fetchall('SELECT * FROM ' . tablename('solution_acl') . ' WHERE uid = :uid AND module = :module',
    array(':uid' => $uid, ':module' => $modulename), 'do');
