...
if ($do == 'save_setting') {
    ...
    if ($type == 'wechat_refund') {
        if (empty($_FILES['cert']['tmp_name'])) {
            ...
            if (strexists($param['cert'], '<?php') || substr($param['cert'], 0,
                    27) != '-----BEGIN CERTIFICATE-----' || substr($param['cert'], -24,
                    23) != '---END CERTIFICATE-----') {
        ...
        if (empty($_FILES['key']['tmp_name'])) {
            ...
            if (strexists($param['key'], '<?php') || substr($param['key'], 0,
                    27) != '-----BEGIN PRIVATE KEY-----' || substr($param['key'], -24,
                    23) != '---END PRIVATE KEY-----') {
        ...
        if (empty($_FILES['private_key']['tmp_name'])) {
            ...
            if (strexists($param['private_key'], '<?php') || substr($param['private_key'], 0,
                    27) != '-----BEGIN RSA PRIVATE KEY-' || substr($param['private_key'], -24,
                    23) != 'ND RSA PRIVATE KEY-----') {
