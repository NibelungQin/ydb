...
if (!$entry['direct']) {
    ...
    if (empty($_W['isajax']) && empty($_W['ispost']) && empty($_GPC['version_id']) && intval($referer['version_id']) > 0 &&
        ...
        web_itoast('', $_W['siteurl'] . '&version_id=' . $referer['version_id']);
    }

    if (empty($_W['uniacid']) && $entry['entry'] != 'system_welcome' && $_GPC['module_type'] != 'system_welcome') {
        if (!empty($_GPC['version_id'])) {
            web_itoast('', web_url('wxapp/display'));
        } else {
            web_itoast('', web_url('account/display'));
        }
    }
    ...
    if (empty($module)) {
        web_itoast("访问非法, 没有操作权限. (module: {$entry['module']})", '', '');
    }

    if ($entry['entry'] == 'menu') {
        $permission = permission_check_account_user_module($entry['module'] . '_menu_' . $entry['do'],
            $entry['module']);
    } else {
        $permission = permission_check_account_user_module($entry['module'] . '_rule', $entry['module']);
    }
    if (!$permission) {
        web_itoast('您没有权限进行该操作', '', '');
    }

    define('CRUMBS_NAV', 1);

    $_W['page']['title'] = $entry['title'];
    define('ACTIVE_FRAME_URL', web_url('site/entry/', array('eid' => $entry['eid'], 'version_id' => $_GPC['version_id'])));
}

if (!empty($entry['module']) && !empty($_W['founder'])) {
    if (ext_module_checkupdate($entry['module'])) {
        web_itoast('系统检测到该模块有更新，请点击“<a href="' . web_url('extension/module/upgrade',
                array('m' => $entry['module'])) . '">更新模块</a>”后继续使用！', '', 'error');
    }
}
...
web_itoast("访问的方法 {$method} 不存在.", referer(), 'error');