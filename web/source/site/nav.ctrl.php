...
if ($do == 'homemenu_display') {
    ...
    $navs = pdo_getall('site_nav', array('uniacid' => $_W['uniacid'], 'position' => '1', 'multiid' => $multiid),
        array(), '', array('displayorder DESC', 'id ASC'));
    $navigations = array();
    if (!empty($navs)) {
        foreach ($navs as $nav) {
...
if ($do == 'homemenu_post') {
    ...
    $url = ((strexists($post['url'], 'http://') || strexists($post['url'], 'https://')) && !strexists($post['url'],
            '#wechat_redirect')) ? $post['url'] . '#wechat_redirect' : $post['url'];
