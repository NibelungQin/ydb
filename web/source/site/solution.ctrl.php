...
if ($do == 'operator') {
    ...
    $wechatmembers = pdo_fetchall('SELECT memberid FROM ' . tablename('uni_account_users') . " WHERE weid=:weid",
        array(':weid' => $_W['weid']), 'memberid');
...
if ($do == 'menu') {
    ...
    if (!empty($uid)) {
        $haspermission = pdo_fetch("SELECT id FROM " . tablename('uni_account_users') . " WHERE memberid = :memberid",
            array(':memberid' => $uid));
    }
    ...
    if (checksubmit('submit')) {
        ...
        pdo_delete('modules_solution_bindings',
            array('acid' => $_W['weid'], 'memberid' => $uid, 'module' => $modulename));
    ...
    $allmenus = array();
    $bindings = pdo_fetchall('SELECT * FROM ' . tablename('modules_bindings') . " WHERE module = :module AND entry IN ('menu', 'cover') ORDER BY entry ASC",
    ...
    foreach ($bindings as $binding) {
        if (empty($binding['call'])) {
            $allmenus[] = array(
                ...
                'state' => $binding['state'],
                'title' => $binding['title'],
                'url' => $binding['entry'] == 'cover' ? url('rule/cover',
                    array('eid' => $binding['eid'])) : url('site/module/' . $binding['do'],
                    array('name' => $binding['module'], 'weid' => $_W['weid']))
            );
        } else {
            $call = $binding['call'];
...
if ($do == 'management') {
    ...
    if (ext_module_checkupdate($modulename)) {
        message('系统检测到该模块有更新，请点击“<a href="' . url('extension/module/upgrade',
                array('id' => $modulename)) . '">更新模块</a>”后继续使用！', '', 'error');
    }
    if (!empty($_W['isfounder'])) {
        $menus = array();
        $bindings = pdo_fetchall('SELECT * FROM ' . tablename('modules_bindings') . " WHERE module = :module ORDER BY eid ASC",
            array(':module' => $modulename));
        ...
        $mymenus = pdo_fetchall($sql,
            array(':memberid' => $_W['uid'], ':acid' => $_W['weid'], ':module' => $modulename));
        ...
        if (!empty($eids)) {
            $bindings = pdo_fetchall('SELECT * FROM ' . tablename('modules_bindings') . " WHERE eid IN (" . implode(',',
                    $eids) . ") ORDER BY eid ASC");
            if (!empty($bindings) && is_array($bindings)) {
                foreach ($bindings as $opt) {
                    $menus[] = array(
                        $opt['title'],
