...
if ($do == 'check') {
    ...
    $response = cloud_request('http://v2.addons.yidaoit.cn/gateway.php', $params);
                    ...
                    if (!empty($maps)) {
                        ...
                        foreach ($maps as $map) {
                            if (!is_array($map) || empty($map['checksum'])) {
                                ...
                                $checksum_found = true;
                                $clouds['/addons/' . $match[1] . $map['path']] = array(
                                    'path' => '/addons/' . $match[1] . $map['path'],
                    ...
                    if (!empty($maps)) {
                        ...
                        foreach ($maps as $map) {
                            if (!is_array($map) || empty($map['checksum'])) {
                                ...
                                $checksum_found = true;
                                $clouds['/app/themes/' . $match[1] . $map['path']] = array(
                                    'path' => '/app/themes/' . $match[1] . $map['path'],
                                    'checksum' => $map['checksum']
                                );
                            }
                        }
