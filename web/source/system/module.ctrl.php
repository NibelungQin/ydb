        ...
        if (empty($manifest)) {
            if (in_array($module['name'], array_keys($cloud_m_query_module))) {
                ...
                if (version_compare($module['version'],
                        $cloud_branch_version) == -1 || ($cloud_m_info['displayorder'] < $best_branch['displayorder'] && !empty($cloud_m_info['version']))) {
                    $module['upgrade'] = true;
                } else {
                    $module['upgrade'] = false;
                }
...
if ($do == 'install') {
    ...
    if (!empty($manifest)) {
        ...
        if (is_error($result)) {
            itoast($result['message'], url('system/module/not_installed', array('account_type' => ACCOUNT_TYPE)),
                'error');
        }
    } else {
    ...
    if (empty($manifest)) {
        itoast('模块安装配置文件不存在或是格式不正确，请刷新重试！', url('system/module/not_installed', array('account_type' => ACCOUNT_TYPE)),
            'error');
    }
    ...
    if (pdo_insert('modules', $module)) {
        ...
        if (empty($module_subscribe_success)) {
            itoast('模块安装成功！模块订阅消息有错误，系统已禁用该模块的订阅消息，详细信息请查看',
                url('system/module/module_detail', array('name' => $module['name'])), 'tips');
        } else {
...
if ($do == 'uninstall') {
    ...
    if (!isset($_GPC['confirm'])) {
        if ($module['isrulefields']) {
            $message .= '是否删除相关规则和统计分析数据<div><a class="btn btn-primary" style="width:80px;" href="' . url('system/module/uninstall',
                    array(
                        'name' => $name,
                        'confirm' => 1
                    )) . '">是</a> &nbsp;&nbsp;<a class="btn btn-default" style="width:80px;" href="' . url('system/module/uninstall',
                    array('account_type' => ACCOUNT_TYPE, 'name' => $name, 'confirm' => 0)) . '">否</a></div>';
        } elseif (!empty($plugin_list)) {
            $message .= "<a href=" . url('system/module/uninstall',
