...
if ($do == 'opcache') {
        ...
        if (!empty($memobj) && method_exists($memobj, 'getExtendedStats')) {
            ...
            if (!empty($status)) {
                foreach ($status as $server => $row) {
                    $data_status[] = '已用：' . round($row['bytes'] / 1048576,
                            2) . ' M / 共：' . round($row['limit_maxbytes'] / 1048576) . ' M';
                }
    ...
    if ($extensions['redis']['status']) {
        ...
        if (!empty($redisobj) && method_exists($redisobj, 'info')) {
            ...
            if (!empty($status)) {
                $extensions['redis']['extra'] = '消耗峰值：' . round($status['used_memory_peak'] / 1048576,
                        2) . ' M/ 内存总量：' . round($status['used_memory'] / 1048576, 2) . ' M';
            }
        }
    }
