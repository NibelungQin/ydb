    ...
    if ($op == 'filter_func') {
        ...
        if (isset($config['func']) && !empty($config['func'])) {
            foreach ($file as $key => $val) {
                ...
                if (stristr($key, '.php.') != false || preg_match_all('/[^a-z]?(' . $config['func'] . ')\s*\(/i', $html,
                        $state, PREG_SET_ORDER)) {
        ...
        if (isset($config['code']) && !empty($config['code'])) {
            foreach ($file as $key => $val) {
                if (!empty($config['code'])) {
                    $html = file_get_contents(IA_ROOT . '/' . $key);
                    if (stristr($key, '.php.') != false || preg_match_all('/[^a-z]?(' . $config['code'] . ')/i', $html,
                            $state, PREG_SET_ORDER)) {
                        $badfiles[$key]['code'] = $state;
                    }
                }
                if (strtolower(substr($key,
                        -4)) == '.php' && function_exists('zend_loader_file_encoded') && zend_loader_file_encoded(IA_ROOT . '/' . $key)) {
