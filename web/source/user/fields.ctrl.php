...
if ($do == 'post') {
    ...
    if (checksubmit('submit')) {
        if (empty($_GPC['title'])) {
            web_itoast('抱歉，请填写资料名称！', '', '');
        }
        if (empty($_GPC['field'])) {
            web_itoast('请填写字段名!', '', '');
        }
        if (!preg_match('/^[A-Za-z0-9_]*$/', $_GPC['field'])) {
            web_itoast('请使用字母或数字或下划线组合字段名!', '', '');
        }
        $data = array(
            'title' => $_GPC['title'],
