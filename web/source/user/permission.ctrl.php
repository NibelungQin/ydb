...
if ($do == 'edit') {
    ...
    $weids = pdo_fetchall("SELECT uniacid, role FROM " . tablename('uni_account_users') . " WHERE uid = '$uid'",
        array(), 'uniacid');
    if (!empty($weids)) {
        $wechats = pdo_fetchall("SELECT * FROM " . tablename('uni_account') . " WHERE uniacid IN (" . implode(',',
                array_keys($weids)) . ")");
    }
...
if ($do == 'select') {
    ...
    $permission = pdo_fetchall("SELECT uniacid FROM " . tablename('uni_account_users') . " WHERE uid = '$uid'", array(),
        'uniacid');
...
if ($do == 'module') {
    if ($_W['isajax']) {
        ...
        $purview = pdo_fetch('SELECT * FROM ' . tablename('users_permission') . ' WHERE uniacid = :aid AND uid = :uid AND type = :type',
            array(':aid' => $uniacid, ':uid' => $uid, ':type' => $m));
...
if ($do == 'menu') {
    ...
    if (!pdo_fetchcolumn("SELECT id FROM " . tablename('uni_account_users') . " WHERE uid = :uid AND uniacid = :uniacid",
        array(':uid' => $uid, ':uniacid' => $uniacid))) {
        message('此用户没有操作该统一公众号的权限，请选指派“管理者”权限！');
    }
    $system_permission = pdo_fetch('SELECT * FROM ' . tablename('users_permission') . ' WHERE uniacid = :aid AND uid = :uid AND type = :type',
        array(':aid' => $uniacid, ':uid' => $uid, ':type' => 'system'));
    ...
    $mods = pdo_fetchall('SELECT * FROM ' . tablename('users_permission') . ' WHERE uniacid = :aid AND uid = :uid AND type != :type',
        array(':aid' => $uniacid, ':uid' => $uid, ':type' => 'system'), 'type');
    ...
    if (checksubmit('submit')) {
        ...
        pdo_query('DELETE FROM ' . tablename('users_permission') . ' WHERE uniacid = :uniacid AND uid = :uid AND type != :type',
            array(':uniacid' => $uniacid, ':uid' => $uid, ':type' => 'system'));
...
if ($do == 'auth') {
    ...
    $isexists = pdo_fetch("SELECT * FROM " . tablename('uni_account_users') . " WHERE uid = :uid AND uniacid = :uniacid",
        array(':uid' => $uid, ':uniacid' => $uniacid));
    if (empty($isexists)) {
