...
if ($do == 'post' && $_W['isajax'] && $_W['ispost']) {
    ...
    if ($type == 'birth') {
        ...
        if (in_array($type, array('username', 'password'))) {
            if ($user[$type] == $_GPC[$type] && $type != 'password') {
                iajax(0, '未做修改！', '');
            }
        } else {
            if ($users_profile_exist[$type] == $_GPC[$type]) {
                iajax(0, '未作修改！', '');
            }
        }
    }
    switch ($type) {
        ...
        case 'birth':
            if ($users_profile_exist) {
                $result = pdo_update('users_profile', array(
...
if ($do == 'bind') {
    ...
    if (!empty($user['openid']) && !in_array($user['openid'], $signs)) {
        pdo_insert('users_bind', array(
            'uid' => $user['uid'],
            'bind_sign' => $user['openid'],
            'third_type' => $user['register_type'],
            'third_nickname' => $user_profile['nickname']
        ));
    }
    ...
    if (!empty($user_profile['mobile']) && !in_array($user_profile['mobile'], $signs)) {
        pdo_insert('users_bind', array(
            ...
            'bind_sign' => $user_profile['mobile'],
            'third_type' => USER_REGISTER_TYPE_MOBILE,
            'third_nickname' => $user_profile['mobile']
        ));
    }
    ...
    web_template('user/bind');
}

if (in_array($do, array(
        'validate_mobile',
        'bind_mobile'
    )) || $_GPC['bind_type'] == USER_REGISTER_TYPE_MOBILE && $do == 'unbind') {
    $user_table = table('users');
    $user_profile = $user_table->userProfile($_W['uid']);
...
if ($do == 'unbind') {
    ...
    if (!in_array($_GPC['bind_type'],
        array(USER_REGISTER_TYPE_QQ, USER_REGISTER_TYPE_WECHAT, USER_REGISTER_TYPE_MOBILE))) {
        iajax(-1, '类型错误');
    }
    $bind_type = $types[$_GPC['bind_type']];
    if ($_W['isajax'] && $_W['ispost']) {
        $unbind_info = OAuth2Client::create($bind_type, $_W['setting']['thirdlogin'][$bind_type]['appid'],
            $_W['setting']['thirdlogin'][$bind_type]['appsecret'])->unbind();
