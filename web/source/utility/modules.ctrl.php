...
if ($do == 'list') {
    ...
    if (!empty($_COOKIE['special_reply_type'])) {
        ...
        foreach ($_W['account']['modules'] as $m) {
            if (is_array($_W['account']['modules'][$m['name']]['handles']) && in_array($_COOKIE['special_reply_type'],
        ...
        foreach ($installedmodulelist as $k => $value) {
            $installedmodulelist[$k]['official'] = empty($value['issystem']) && (strexists($value['author'],
                        'WeEngine Team') || strexists($value['author'], '一道宝团队'));
        }
    ...
    $result = array(
        ...
        'pager' => pagination(count($enable_modules), $pindex, $psize, '',
