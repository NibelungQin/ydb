...
if ($do == 'browser') {
    ...
    iajax(0, array(
        'page' => pagination($total, $page, $size, '',
...
if ($do == 'delete') {
    $id = intval($_GPC['id']);
    $acid = $_W['acid'];
    $data = pdo_fetch('SELECT * FROM ' . tablename('wechat_attachment') . ' WHERE acid = :acid AND id = :id',
...
{
    pdo_query('DELETE FROM ' . tablename('wechat_attachment') . ' WHERE createtime + 259200 < :time',
        array(':time' => time()));
}