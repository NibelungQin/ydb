...
if ($do == 'location') {
    $location = pdo_fetchall('SELECT id,location_id, business_name, branch_name, address FROM ' . tablename('activity_stores') . " WHERE uniacid = :uniacid AND status = :status AND location_id != ''",
...
$setting = pdo_fetch('SELECT * FROM  ' . tablename('coupon_setting') . ' WHERE uniacid = :aid AND acid = :cid',
    array(':aid' => $_W['uniacid'], ':cid' => $acid));
...
if ($do == 'display') {
    ...
    $data = pdo_fetchall('SELECT * FROM ' . tablename('coupon') . $condition . ' ORDER BY id DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize,
        $parma);
    if (!empty($data)) {
        foreach ($data as &$da) {
...
if ($do == 'sync') {
    $id = intval($_GPC['cid']);
    $card = pdo_fetch('SELECT id,status,card_id,acid FROM ' . tablename('coupon') . ' WHERE acid = :acid AND id = :id',
        array(':acid' => $acid, ':id' => $id));
...
if ($do == 'del') {
    $id = intval($_GPC['id']);
    $card_id = pdo_fetchcolumn('SELECT card_id FROM ' . tablename('coupon') . ' WHERE acid = :acid AND id = :id',
        array(':acid' => $acid, ':id' => $id));
...
if ($do == 'qr') {
    if ($op == 'post') {
        $cid = intval($_GPC['cid']);
        $id = intval($_GPC['id']);
        $title = pdo_fetchcolumn('SELECT title FROM ' . tablename('coupon') . ' WHERE uniacid = :aid AND id = :id',
            array(':aid' => $_W['uniacid'], ':id' => $cid));
        if (empty($title)) {
            message('卡券不存在或已经删除', url('wechat/card/display'), 'error');
        }
        $row = pdo_fetch('SELECT * FROM ' . tablename('qrcode') . ' WHERE uniacid = :aid AND id = :id',
            array(':aid' => $_W['uniacid'], ':id' => $id));
        if (checksubmit('submit')) {
            ...
            if ($qrctype == 1) {
                $qrcid = pdo_fetchcolumn("SELECT qrcid FROM " . tablename('qrcode') . " WHERE acid = :acid AND model = '1' ORDER BY qrcid DESC",
                    array(':acid' => $acid));
                $data['outer_id'] = !empty($qrcid) ? ($qrcid + 1) : 100001;
                $data['expire_seconds'] = intval($_GPC['expire-seconds']) ? min(array(
                    intval($_GPC['expire-seconds']),
                    1800
                )) : 1800;
                $result = coupon_qr($data);
            } else {
                if ($qrctype == 2) {
                    $qrcid = pdo_fetchcolumn("SELECT qrcid FROM " . tablename('qrcode') . " WHERE acid = :acid AND model = '2' ORDER BY qrcid DESC",
                        array(':acid' => $acid));
    ...
    if ($op == 'list') {
        ...
        $title = pdo_fetchcolumn('SELECT title FROM ' . tablename('coupon') . ' WHERE uniacid = :aid AND id = :id',
            array(':aid' => $_W['uniacid'], ':id' => $cid));
        $data = pdo_fetchall('SELECT * FROM ' . tablename('qrcode') . ' WHERE uniacid = :aid AND type = :type AND extra = :cid',
            array(':aid' => $_W['uniacid'], ':type' => 'card', ':cid' => $cid));
    ...
    if ($op == 'del') {
        $cid = intval($_GPC['cid']);
        if ($_GPC['scgq']) {
            $list = pdo_fetchall("SELECT id FROM " . tablename('qrcode') . " WHERE uniacid = '{$_W['uniacid']}' AND extra = {$cid} AND status = '0' AND type='card'",
                array(), 'id');
            if (!empty($list)) {
                pdo_query("DELETE FROM " . tablename('qrcode') . " WHERE id IN (" . implode(',',
                        array_keys($list)) . ")");
            }
            message('执行成功<br />删除二维码：' . count($list), url('wechat/card/qr', array('op' => 'list', 'cid' => $cid)),
                'success');
        } else {
            $id = $_GPC['id'];
...
if ($do == 'record') {
    ...
    if ($op == 'list') {
        ...
        if ($cid > 0) {
            $coupon = pdo_fetch('SELECT title,card_id FROM ' . tablename('coupon') . ' WHERE acid = :acid AND id = :id',
                array(':acid' => $acid, ':id' => $cid));
            $card_id = $coupon['card_id'];
        } else {
            $coupon = pdo_fetch('SELECT title FROM ' . tablename('coupon') . ' WHERE acid = :acid AND card_id = :card_id',
                array(':acid' => $acid, ':card_id' => $card_id));
        }
        if (!empty($card_id)) {
        ...
        $data = pdo_fetchall('SELECT * FROM ' . tablename('coupon_record') . $condition . ' ORDER BY id DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize,
            $parma);
        if (!empty($data)) {
            foreach ($data as &$da) {
                if (!empty($da['openid'])) {
            ...
            if (!empty($openids)) {
                ...
                $nicknames = pdo_fetchall('SELECT nickname,openid FROM ' . tablename('mc_mapping_fans') . "WHERE acid = {$acid} AND openid IN ({$openids_str})",
                    array(), 'openid');
            }
            if (!empty($outer_ids)) {
                $outer_str = implode(',', $outer_ids);
                $outers = pdo_fetchall('SELECT name,qrcid FROM ' . tablename('qrcode') . "WHERE acid = {$acid} AND type = 'card' AND qrcid IN ({$outer_str})",
                    array(), 'qrcid');
            }
    ...
    if ($op == 'unavailable') {
        $id = intval($_GPC['id']);
        $del = intval($_GPC['del']);
        $record = pdo_fetch('SELECT code,status FROM ' . tablename('coupon_record') . ' WHERE acid = :acid AND id = :id',
            array(':acid' => $acid, ':id' => $id));
        if (empty($record)) {
        ...
        if ($record['status'] == 1) {
            ...
            if (is_error($status)) {
                ...
                pdo_update('coupon_record', array(
                    ...
                    'usetime' => TIMESTAMP
                ), array('acid' => $acid, 'code' => $record['code'], 'id' => $id));
            }
        }
        if ($del == 1) {
...
if ($do == 'toggle') {
    $id = intval($_GPC['id']);
    if ($op == 'is_display') {
        $display = pdo_fetchcolumn('SELECT is_display FROM ' . tablename('coupon') . ' WHERE acid = :acid AND id = :id',
            array(':acid' => $acid, ':id' => $id));
        if ($display == 1) {
...
if ($do == 'selfconsume') {
    ...
    load()->classs('coupon');
    $coupon = new coupon($acid);
    $card_info = pdo_get('coupon', array('acid' => $acid, 'id' => $id),
        array('is_selfconsume', 'card_id', 'location_id_list'));
...
if ($do == 'discount') {
    if ($op == 'post') {
        ...
        $is_ok = pdo_fetchcolumn('SELECT id FROM ' . tablename('coupon') . ' WHERE acid = :acid AND card_id = :card_id',
            array(':acid' => $acid, ':card_id' => $post['card_id']));
        if (empty($is_ok)) {
...
if ($do == 'cash') {
    if ($op == 'post') {
        ...
        $post['acid'] = $acid;
        $post['type'] = 'cash';
        $post['extra'] = iserializer(array(
            'least_cost' => $post['least_cost'],
            'reduce_cost' => $post['reduce_cost'],
        ));
        $post['is_display'] = intval($post['is_display']);
        empty($post['code_type']) && $post['code_type'] = 1;
        $post['status'] = 1;
