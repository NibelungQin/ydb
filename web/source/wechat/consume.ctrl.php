    ...
    if ($op == 'list') {
        ...
        if (!empty($data)) {
            ...
            if (!empty($card_ids)) {
                ...
                $card_str = "'" . $card_str . "'";
                $cards = pdo_fetchall('SELECT card_id,title FROM ' . tablename('coupon') . "WHERE acid = {$acid} AND card_id IN ({$card_str})",
                    array(), 'card_id');
            }
        }
    ...
    if ($op == 'unavailable') {
        ...
        if ($record['status'] == 1) {
            ...
            if (is_error($status) && !$_GPC['force']) {
                ...
                $url_2 = url('wechat/consume/record',
                    array('op' => 'unavailable', 'del' => 1, 'id' => $id, 'force' => 1));
