...
if ($do == 'logo') {
    $coupon_setting = pdo_fetch('SELECT * FROM ' . tablename('coupon_setting') . ' WHERE uniacid = :aid AND acid = :acid',
        array(':aid' => $_W['uniacid'], ':acid' => $acid));
...
if ($do == 'edit') {
    $id = intval($_GPC['id']);
    $location = pdo_fetch('SELECT * FROM ' . tablename('activity_stores') . ' WHERE uniacid = :aid AND id = :id',
...
if ($do == 'list') {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('activity_stores') . $condition . " ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,
        $pars);
    if (!empty($list)) {
        foreach ($list as &$li) {
...
if ($do == 'del') {
    $id = intval($_GPC['id']);
    $count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('activity_clerks') . ' WHERE uniacid = :uniacid AND storeid = :id',
        array(':id' => $_GPC['id'], ':uniacid' => $_W['uniacid']));
    $count = intval($count);
    if ($count > 0) {
        message("该门店下有{$count}名店员.请将店员变更到其他门店后,再进行删除操作", referer(), 'error');
    }
    $location = pdo_fetch('SELECT status,location_id FROM ' . tablename('activity_stores') . ' WHERE uniacid = :aid AND id = :id',
        array(':aid' => $_W['uniacid'], ':id' => $id));
    if (!empty($location['location_id'])) {
        load()->classs('coupon');
        $acc = new coupon($acid);
    ...
    if (is_error($status)) {
        message("删除本地门店数据成功<br>通过微信接口删除微信门店数据失败,请登陆微信公众平台手动删除门店<br>错误原因：{$status['message']}",
            url('wechat/location/list'), 'error');
    }
    ...
    if (!empty($location)) {
        foreach ($location as $row) {
            ...
            if ($li['available_state'] == 3) {
                $isexist = pdo_fetchcolumn('SELECT id FROM ' . tablename('activity_stores') . ' WHERE uniacid = :uniacid AND location_id = :location_id',
                    array(':uniacid' => $_W['uniacid'], ':location_id' => $li['poi_id']));
                if (empty($isexist)) {
                    $li['uniacid'] = $_W['uniacid'];
                    $li['status'] = 1;
                    $li['location_id'] = $li['poi_id'];
                    $category_temp = explode(',', $li['categories'][0]);
                    $li['category'] = iserializer(array(
                        'cate' => $category_temp[0],
                        'sub' => $category_temp[1],
                        'clas' => $category_temp[2]
                    ));
                ...
                $select_type == 'sid';
                if (!empty($li['sid'])) {
                    $isexist = pdo_fetch('SELECT id,sid FROM ' . tablename('activity_stores') . ' WHERE uniacid = :uniacid AND (sid = :sid OR id = :sid)',
                        array(':uniacid' => $_W['uniacid'], ':sid' => $li['sid']));
                }
                if (empty($isexist)) {
                    $select_type = 'location';
                    $isexist = pdo_fetch('SELECT id,sid FROM ' . tablename('activity_stores') . ' WHERE uniacid = :uniacid AND location_id = :location_id',
                        array(':uniacid' => $_W['uniacid'], ':location_id' => $li['poi_id']));
                }
                $li['uniacid'] = $_W['uniacid'];
