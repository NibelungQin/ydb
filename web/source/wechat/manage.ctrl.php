...
if ($do == 'location_edit') {
    $id = intval($_GPC['id']);
    $location = pdo_fetch('SELECT * FROM ' . tablename('coupon_location') . ' WHERE uniacid = :aid AND id = :id',
...
if ($do == 'location_list') {
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('coupon_location') . $condition . " ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize,
        $pars);
    if (!empty($list)) {
        foreach ($list as &$li) {
...
if ($do == 'location_del') {
    $id = intval($_GPC['id']);
    $location = pdo_fetch('SELECT status,location_id FROM ' . tablename('coupon_location') . ' WHERE uniacid = :aid AND id = :id',
        array(':aid' => $_W['uniacid'], ':id' => $id));
    if (!empty($location['location_id'])) {
        load()->classs('coupon');
        $acc = new coupon($acid);
    ...
    if (is_error($status)) {
        message("删除本地门店数据成功<br>通过微信接口删除微信门店数据失败,请登陆微信公众平台手动删除门店<br>错误原因：{$status['message']}",
            url('wechat/manage/location_list'), 'error');
    }
    ...
    if (!empty($location)) {
        foreach ($location as $row) {
            ...
            if ($li['available_state'] == 3 && !empty($li['poi_id']) && empty($li['sid'])) {
                $isexist = pdo_fetchcolumn('SELECT id FROM ' . tablename('coupon_location') . ' WHERE uniacid = :uniacid AND acid = :acid AND location_id = :location_id',
                    array(':uniacid' => $_W['uniacid'], ':acid' => $acid, ':location_id' => $li['poi_id']));
                if (empty($isexist)) {
                ...
                $data = pdo_fetch('SELECT id,sid FROM ' . tablename('coupon_location') . ' WHERE uniacid = :uniacid AND acid = :acid AND (sid = :sid OR id = :sid)',
                    array(':uniacid' => $_W['uniacid'], ':acid' => $acid, ':sid' => $li['sid']));
                $li['uniacid'] = $_W['uniacid'];
                $li['acid'] = $acid;
...
if ($do == 'whitelist') {
    $whitelist = pdo_fetchcolumn('SELECT whitelist FROM ' . tablename('coupon_setting') . ' WHERE uniacid = :aid AND acid = :acid',
        array(':aid' => $_W['uniacid'], ':acid' => $acid));
    if (!empty($whitelist)) {
    ...
    if (checksubmit('submit')) {
        ...
        if (is_error($status)) {
            ...
            pdo_update('coupon_setting', array('whitelist' => $data),
                array('uniacid' => $_W['uniacid'], 'acid' => $acid));
        }
